...
implemented as :ref:`hashJoinV3 <cid-xf::database::hashJoinV3>` and :ref:`hashJoinV3 <cid-xf::database::hashJoinV3>` function.
...
The Hash-Join-v3 and Hash-Build-Probe-v3 primitive are general primitive to accerlerate Hash-Join algorithm with utilizing the advantage of high memory bandwidth in Xilinx FPGA. 
Hash-Join-v3 perform Hash-Join in single-call mode which means the small table and the big table should be scaned one after another. 
Hash-Build-Probe-v3 provide a separative solution for build and probe in Hash-Join. In Hash-Build-Probe, incremental build is supported, and the workflow can be scheduled as build0->build1->probe0->build2->probe2...
...
Current design uses maximum of PUs, served by 4 input channels though each of which a pair of key and payload can be passed in each cycle. 
...
The Number of hash entry is limitied by the number of URAM in a single SLR. For example, there are 320 URAMs in a SLR of U280, and 1M hash entry will take 192 URAMs (96 URAMs for base hash counter + 96 URAMs for overflow hash counter). Because the number of hash entry must be the power of 2, 1M hash entry is the maximum for U280 to avoid accrossing SLR logic.
