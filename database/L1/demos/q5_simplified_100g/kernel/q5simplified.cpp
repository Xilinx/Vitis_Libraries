...
#pragma HLS resource variable = o_odate_1 core = FIFO_SRL
...
#pragma HLS resource variable = o_okey_2 core = FIFO_SRL
...
#pragma HLS INTERFACE m_axi port=buf0 bundle=gmem1_0 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf1 bundle=gmem1_1 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf2 bundle=gmem1_2 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf3 bundle=gmem1_3 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf4 bundle=gmem1_4 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf5 bundle=gmem1_5 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf6 bundle=gmem1_6 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
#pragma HLS INTERFACE m_axi port=buf7 bundle=gmem1_7 num_write_outstanding=32 \
  num_read_outstanding=32 max_read_burst_length=8 latency=125
...
#pragma HLS dataflow
    ...
    hls::stream<ap_uint<8 * KEY_SZ> > k0_strm_arry[Q5E2_HJ_CH_NM];
...
#pragma HLS resource variable = k0_strm_arry core = FIFO_SRL
    hls::stream<ap_uint<8 * (MONEY_SZ + MONEY_SZ)> > p0_strm_arry[Q5E2_HJ_CH_NM];
...
#pragma HLS array_partition variable = p0_strm_arry dim = 0
    ...
    hls::stream<bool> e0_strm_arry[Q5E2_HJ_CH_NM];
#pragma HLS stream variable = e0_strm_arry depth = 8
#pragma HLS array_partition variable = e0_strm_arry dim = 0

    scan_wrapper<Q5E2_HJ_CH_NM>(buf_o_orderkey, buf_o_orderdate, o_nrow, buf_l_orderkey, buf_l_extendedprice,
                                buf_l_discount, l_nrow, k0_strm_arry, p0_strm_arry, e0_strm_arry);
...
#pragma HLS stream variable = e5_strm depth = 8
    ...
    xf::database::hashJoinMPU<Q5E2_HJ_MODE,              // hash algorithm
                              8 * KEY_SZ,                // key width
                              8 * (MONEY_SZ + MONEY_SZ), // payload width max
                              0,                         // payload width small
                              8 * (MONEY_SZ + MONEY_SZ), // payload width big
                              Q5E2_HJ_HW_P,              // log2(number of PU)
                              Q5E2_HJ_HW_J,              // hash width for join
                              Q5E2_HJ_AW,                // address width
                              8 * KEY_SZ,                // buffer width
                              Q5E2_HJ_CH_NM,             // input channel number
                              Q5E2_HJ_BVW,               // bloom-filter width
                              0>                         // enable bloom-filter
