...
void columnize_o(orders_t* buf_o,
    ...
    size_t j = 0;
    for (size_t i = 0; i < nrow; ++i) {
        TPCH_INT date = buf_o[i].orderdate;
        if (date >= 19940101L && date < 19950101L) {
            col_o_orderkey[j++] = buf_o[i].orderkey;
        }
    }
    *nrow_loaded = j;
}
...
int main(int argc, const char* argv[]) {
    ...
    if (flag_o) {
        o_thread = std::thread(columnize_o, o_vec.data(), o_nrow, col_o_orderkey.data(), &o_nrow_filtered);
    }
    ...
    printf("After filtering, orders table have %ld rows.\n", (unsigned long)o_nrow_filtered);

    col_o_orderkey.resize(o_nrow_filtered);
