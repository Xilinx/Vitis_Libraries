...
static void scan_table(
    ...
    for (int r = 0; r < 2; ++r) {
        xf::database::scanCol<64, VEC_LEN, CH_NM, KEY_SZ>( //
...
#pragma HLS pipeline
            ...
            ap_uint<8 * (MONEY_SZ + MONEY_SZ)> p = k; // XXX no payload.
            oe = o_1e.read();
...
static void scan_wrapper(
    ...
    ap_uint<8 * MONEY_SZ * VEC_LEN> buf_l_discount[L_DEPTH],
    const int l_nrow,
    hls::stream<ap_uint<8 * KEY_SZ> > key_ostrm[CH_NM],
    hls::stream<ap_uint<8 * 2 * MONEY_SZ> > pld_ostrm[CH_NM],
...
#pragma HLS stream variable = e5_strm depth = 8
    ...
    xf::database::hashJoinMPU<HJ_MODE,                   // hash algorithm
                              ...
                              0,                         // payload small width
                              8 * (MONEY_SZ + MONEY_SZ), // payload big width
