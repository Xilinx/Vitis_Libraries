...
#pragma HLS STREAM variable = hash_strm depth = 32
...
#pragma HLS pipeline
        ...
        ap_uint<PW> pld = i_pld_strm.read();
#ifdef USER_DEBUG
        std::cout << "key0: " << key.range(63, 0) << std::endl;
        std::cout << "key1: " << key.range(127, 64) << std::endl;
        std::cout << "pld: " << pld << std::endl;
#endif
        ...
        // std::cout << "hashInfo, waddr: " << waddr << std::endl;
...
void readWriteUram(hls::stream<int>& k_depth_strm, // bucket depth
    ...
    hls::stream<ap_uint<HASHWL + 1> > wlist_strm;
...
#pragma HLS STREAM variable = mid_e_strm depth = 32
    ...
    hashInfo<HASHWH, HASHWL, KEYW, PW, ARW>(k_depth_strm, bit_num_strm, i_hash_strm, i_key_strm, i_pld_strm, i_e_strm,
                                            mid_depth_strm, mid_bit_num_strm, mid_bucket_idx_strm, mid_waddr_strm,
                                            mid_wdata_strm, mid_bucket_idx_cnt_strm, mid_e_strm);
    readWriteUram<HASHWH, HASHWL, KEYW, PW, ARW>(mid_depth_strm, mid_bit_num_strm, mid_bucket_idx_strm, mid_waddr_strm,
                                                 mid_wdata_strm, mid_bucket_idx_cnt_strm, mid_e_strm, o_kpld_strm,
                                                 o_nm_strm, o_bk_nm_strm);
}
...
#pragma HLS unroll
        ...
        for (int p = 0; p < PU; p++) {
#pragma HLS unroll
            details::merge4_1<KEYW, PW, HASHWL>(
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[0], mid_bit_num_strms[0],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[1], mid_bit_num_strms[1],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[2], mid_bit_num_strms[2],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[3], mid_bit_num_strms[3],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[4], mid_bit_num_strms[4],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[5], mid_bit_num_strms[5],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[6], mid_bit_num_strms[6],
...
#endif
        details::build_unit<HASHWH, HASHWL, KEYW, PW, ARW>(
            mid_k_depth_strms[7], mid_bit_num_strms[7],
    ...
    for (int pp = 0; pp < PU; pp++) {
        std::cout << "size after build_unit: " << (int)t_kpld_strm_arry[pp].size() << std::endl;
    }
