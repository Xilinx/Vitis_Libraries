...
static unsigned long read_binary_file(const char* fname, void** buffer) {
    ...
    *buffer = (void*)malloc(size);
    ...
    assert((long unsigned int)read_size == size);
    fclose(fp);
    return size;
}
...
cl_int init_hardware(cl_context* context,
    ...
    for (pid = 0; pid < platform_count; ++pid) {
        ...
        if (user_setting) {
            if (device_id_user >= (cl_int)device_count || device_id_user < 0) {
        ...
        if (user_setting) {
            ...
            for (did = 0; did < device_count; ++did) {
                ...
                if (device_name_str == "xilinx_u200_xdma_201830_2" || device_name_str == "xilinx_u250_xdma_201830_2" ||
                    device_name_str == "xilinx_u50_gen3x16_xdma_201920_3" ||
                    device_name_str == "xilinx_u200_gen3x16_xdma_1_202110_1" ||
                    device_name_str == "xilinx_u200_gen3x16_xdma_2_202110_1" ||
                    device_name_str == "xilinx_u200_gen3x16_xdma_base_2") {
                    printf("DEBUG: select device %d: %s\n", did, device_name);
                    dsa_name = device_name;
