...
void q1GroupBy(Table& tin, Table& tout) {
    ...
    for (auto& it : m) {
        ...
        tout.setInt64(r, 0, it.first.k1);
        tout.setInt64(r, 1, it.first.k2);
...
void q1Sort(Table& tin, Table& tout) {
    ...
    for (int i = 0; i < nrow; i++) {
        ...
        if(i<10){
          for(int j=0;j<16;j++)
            std::cout<<std::dec<<tin.getUint32(i,j)<<" ";
          std::cout<<std::endl;
        }
        */
    }
    ...
    for (auto& it : rows) {
        ...
        if(r<32){
            for(int j=0;j<10;j++)
              std::cout<<std::dec<<tout.getInt64(r,j)<<" ";
            std::cout<<std::endl;
        }
        */
...
int check_result(Table& tout, Table& golden) {
    int nerror = 0;
    unsigned nrow = tout.getNumRow();
    unsigned nrow_g = golden.getNumRow();
    printf("kernel result row:%d\n", nrow);
    printf("ref result row:%d\n", nrow_g);
    for (int i = 0; i < nrow_g; ++i) {
        if (tout.getInt64(i, 4) != golden.getInt64(i, 4)) nerror++;
    }
    return nerror;
}

void q1SortSW(Table& tin, Table& tout) {
    struct Q1SortRow {
        int64_t l_returnflag;
        int64_t l_linestatus;
        int64_t sum_qty;
    ...
    std::vector<Q1SortRow> rows;
    for (int i = 0; i < nrow; i++) {
        Q1SortRow t = {tin.getInt64(i, 0), tin.getInt64(i, 1), tin.getInt64(i, 2), tin.getInt64(i, 3),
    ...
    for (auto& it : rows) {
        ...
        /*
        if (r < 32)
            std::cout << std::dec << tout.getInt64(r, 0) << " " << tout.getInt64(r, 1) << " " << tout.getInt64(r, 2)
...
void q1PrintSW(Table& tout) {
    int nrow = tout.getNumRow();
    printf("after sort:%d\n", nrow);
    int r = nrow > 32 ? 32 : nrow;
    std::cout << "Query result:" << nrow << std::endl;
    for (int i = 0; i < r; ++i) {
        int64_t l_returnflag = tout.getInt64(i, 0);
        int64_t l_linestatus = tout.getInt64(i, 1);
        int64_t sum_qty = tout.getInt64(i, 2);
        int64_t sum_price = tout.getInt64(i, 3);
        int64_t sum_disc_price = tout.getInt64(i, 4);
        int64_t sum_charge = tout.getInt64(i, 5);
        int64_t avg_qty = tout.getInt64(i, 6);
        int64_t avg_price = tout.getInt64(i, 7);
        int64_t avg_disc = tout.getInt64(i, 8);
        int64_t sum_count = tout.getInt64(i, 9);
        printf(
            "l_returnflag:%d, "
