...
static void gen_fcfg_pass(uint32_t cfg[]) {
...
void get_cfg_dat_1(ap_uint<512>* hbuf) {
    ...
    signed char id_a[] = {1, 2, 0, -1, -1, -1, -1, -1}; // Orders, 2col
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }

    signed char id_b[] = {1, 2, 3, 0, -1, -1, -1, -1}; // Lineitem, 3col
    for (int c = 0; c < 8; ++c) {
...
void get_cfg_dat_2(ap_uint<512>* hbuf) {
    ...
    signed char id_a[] = {0, 1, 2, -1, -1, -1, -1, -1, -1}; // Orders, 2col
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }

    signed char id_b[] = {0, 1, -1, -1, 0, -1, -1, -1, -1}; // Lineitem, 3col
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
...
void get_cfg_dat_3(ap_uint<512>* hbuf) {
    ...
    signed char id_a[] = {0, 1, -1, -1, -1, -1, -1, -1, -1}; // Orders, 2col
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }

    signed char id_b[] = {0, 1, 2, 3, -1, -1, -1, -1, -1}; // Lineitem, 3col
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
