...
void customerFilter_c_acctbal(Table& tin, Table& tout, int32_t avg_acct) {
    ...
    for (int i = 0; i < nrow; i++) {
        ...
        int32_t c_phone_int = tin.getInt32(i, 1);
        int32_t c_acctbal = tin.getInt32(i, 2);
        if (c_acctbal > avg_acct && c_phone_int_in(c_phone_int)) {
...
void q22GroupBy(Table* tin, int hjTimes, Table& tout) {
    struct q22Groupload {
        int32_t numcust;
        int64_t sumacct;
    };
    ...
    for (int p = 0; p < hjTimes; p++) {
        unsigned nrow = tin[p].getNumRow();
        for (size_t i = 0; i < nrow; ++i) {
            int32_t c_phone_int = tin[p].getInt32(i, 0);
            int32_t c_acctbal = tin[p].getInt32(i, 1);
            auto it = m.find(c_phone_int);
            if (it != m.end()) {
                int32_t s0 = it->second.numcust + 1;
                int64_t s1 = it->second.sumacct + c_acctbal;
