...
int main(int argc, const char* argv[]) {
    ...
    struct timeval tv_r_s, tv_r_0, tv_r_1, tv_r_2, tv_r_e, tv_r_3, tv_r_4;
    gettimeofday(&tv_r_s, 0);
    // step1 :lineiteme groupy -> t1
    q18GroupBy(tbs[0], th0);

    gettimeofday(&tv_r_0, 0);

    // step2 : t1 orders -> t2
    q18Join_t1_o(th0, tbs[1], tk0);

    gettimeofday(&tv_r_1, 0);
    // step3 :t2 customer->t3
    q18Join_t2_c(tk0, tbs[2], tk1);

    gettimeofday(&tv_r_2, 0);
    // step4 : t3 lineitem->t4
    q18Join_t3_l(tk1, tbs[0], th0);
    ...
    std::cout << "Groupby CPU time of Host " << tvdiff(&tv_r_s, &tv_r_0) / 1000 << " ms" << std::endl;
    std::cout << "Join CPU time of Host " << tvdiff(&tv_r_0, &tv_r_1) / 1000 << " ms" << std::endl;
    std::cout << "Join CPU time of Host " << tvdiff(&tv_r_1, &tv_r_2) / 1000 << " ms" << std::endl;
