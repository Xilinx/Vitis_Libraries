...
void q18Join_t1_o(Table& tin1, Table& tin2, Table& tout) {
    ...
    std::cout << std::dec << nrow1 << " After q18GroupBy" << std::endl;

    std::unordered_map<int32_t, int64_t> ht1;
    for (int i = 0; i < nrow1; i++) {
        int32_t l_orderkey = tin1.getInt32(i, 15);
        int64_t sum = tin1.combineInt64(i, 8, 0);
        /*
                if(i<10)
                    printf("sum%d:%lld\n",i,sum);
        */
...
void q18Print(Table& tin) {
    ...
    for (int i = 0; i < r; ++i) {
        for (int j = 0; j < 16; j++) {
            int32_t col = tin.getInt32(i, j);
            printf("col%d=%d ", j, col);
        }
        printf("\n");
    }
}
