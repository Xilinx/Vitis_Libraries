...
static void gen_fcfg_1(uint32_t cfg[]) {
    ...
    cfg[n++] = 0UL | (FOP_DC << FilterOpWidth) | (FOP_DC);
    // cond_2
    cfg[n++] = (uint32_t)300UL;
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = 0UL | (FOP_GTU << FilterOpWidth) | (FOP_DC);
    // cond_3
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = (uint32_t)0UL;
...
void get_cfg_dat_1(ap_uint<512>* hbuf) {
    ...
    signed char id_a[] = {15, 0, 8, -1, -1, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
...
void get_cfg_dat_2(ap_uint<512>* hbuf) {
    ...
    t.range(5, 3) = 0;  // hash join flag = 0 for normal, 1 for semi, 2 for anti
    t.range(39, 8) = 32;
    signed char id_a[] = {1, 0, 2, 3, -1, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }
    ...
    shuffle2_cfg(7, 0) = 6;
    shuffle2_cfg(15, 8) = 7;
    shuffle2_cfg(23, 16) = 8;
    shuffle2_cfg(31, 24) = 0;
    shuffle2_cfg(39, 32) = 12;
    shuffle2_cfg(47, 40) = -1;
    shuffle2_cfg(55, 48) = -1;
    shuffle2_cfg(63, 56) = -1;
...
void get_cfg_dat_3(ap_uint<512>* hbuf) {
    ...
    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 + 16 + 8 + 4 + 2 + 1);
