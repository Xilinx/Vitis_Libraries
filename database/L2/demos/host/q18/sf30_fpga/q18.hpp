...
void q18Join_t1_o(Table* tin1, Table& tin2, Table& tout, int hjTimes) {
    int nrow = 0;
    ...
    for (int j = 0; j < hjTimes; j++) {
        ...
        for (int i = 0; i < nrow1; i++) {
            int32_t l_orderkey = tin1[j].getInt32(i, 15);
            int64_t sum = tin1[j].combineInt64(i, 8, 0);
            // int32_t l_orderkey = tin1[j].getInt32(i,0);
            // int64_t sum = tin1[j].combineInt64(i,2,1);
            /*
                    if(i<10)
                        printf("sum%d:%lld\n",i,sum);
            */
    ...
    std::cout << std::dec << nrow << " After q18GroupBy" << std::endl;
...
// t1 order
void q18Join_t1_o_(Table& tin1, Table& tin2, Table& tout) {
