...
void Tb_Aggr_Hash(Table& tin, Table& tout) {
    int nrow1 = tin.getNumRow();
    int nrow2 = tout.getNumRow();
    if (nrow1 > nrow2) {
        std::cout << "ERR" << std::endl;
        std::cout << "input nrow should not less than output nrow!" << std::endl;
        exit(0);
    }
    for (int i = 0; i < nrow1; i++) {
        int32_t l_partkey = tin.getInt32(i, 7);
        int32_t l_suppkey = tin.getInt32(i, 6);
        int32_t sum_l = tin.getInt32(i, 0);
        int32_t sum_h = tin.getInt32(i, 8);
        tout.setInt32(i, 0, l_partkey);
        tout.setInt32(i, 1, l_suppkey);
        tout.setInt32(i, 2, sum_l);
        tout.setInt32(i, 3, sum_h);
    }
    tout.setNumRow(nrow1);
    std::cout << std::dec << nrow1 << " Tb_Aggr_Hash" << std::endl;
}
