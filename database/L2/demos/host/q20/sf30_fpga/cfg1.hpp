...
void get_cfg_dat_1(ap_uint<512>* hbuf) {
    ...
    t.range(39, 8) = 32; // hash join flag = 0 for normal, 1 for semi, 2 for anti
...
void get_cfg_dat_2(ap_uint<512>* hbuf) {
    ...
    signed char id_b[] = {7, 6, 0, 8, -1, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
    ...
    xf::database::dynamicALUOPCompiler<uint32_t, uint32_t, uint32_t, uint32_t>(
        "strm1*c1", 10, 0, 0, 0,
        // xf::database::dynamicALUOPCompiler<uint32_t, uint32_t, uint32_t, uint32_t>("strm1*c1", 10, 0, 0, 0,
