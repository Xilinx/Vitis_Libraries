...
#include <regex>
// 2388
void GetTable(Table& tin) {
    int nrow = tin.getNumRow();
    for (int i = 0; i < nrow; i++) {
        int32_t odatal = tin.getInt32(i, 0);
        int32_t odatah = tin.getInt32(i, 8);
        tin.setInt32(i, 0, odatal * 5);
        tin.setInt32(i, 8, odatah * 5);
    }
}
...
void q20Join_t2_t3_(Table& tin1, Table& tin2, Table& tout) {
    int nrow1 = tin1.getNumRow();
    int nrow2 = tin2.getNumRow();
...
void q20Join_t2_t3(Table& tin1, Table* tin2, Table& tout, int hjTimes) {
    int nrow1 = tin1.getNumRow();
    std::unordered_map<Q20GroupKey, int32_t> ht1;
    for (int i = 0; i < nrow1; i++) {
        int32_t p_partkey = tin1.getInt32(i, 0);
    ...
    int r = 0;
    for (int j = 0; j < hjTimes; j++) {
        int nrow2 = tin2[j].getNumRow();
        for (int i = 0; i < nrow2; i++) {
            int32_t l_partkey = tin2[j].getInt32(i, 7);
            int32_t l_suppkey = tin2[j].getInt32(i, 6);
            Q20GroupKey k = {l_partkey, l_suppkey};
            auto it = ht1.find(k);
            if (it != ht1.end()) {
                int32_t ps_availqty = it->second;
                ...
                // int64_t sum_t1 = tin2.getInt64(i,2);
                int32_t sum_l = tin2[j].getInt32(i, 0);
                int32_t sum_h = tin2[j].getInt32(i, 8);
                /*tout.setInt32(r,0,ps_suppkey);
                tout.setInt32(r,1,ps_availqty);
                tout.setInt32(r,2,sum_l>>1);
                tout.setInt32(r,3,sum_h>>1);*/
                tout.setInt32(r, 1, 10 * ps_availqty);
                tout.setInt32(r, 2, sum_l);
                tout.setInt32(r, 3, sum_h);
                r++;
...
void q20_Sort(Table& tin, Table& suppliertb, Table& tout) {
    ...
    for (auto& it : rows) {
        ...
        //        std::cout << s_name.data() << " " << s_address.data() << " " << std::endl;
        ++r;
    }
    tout.setNumRow(r);
