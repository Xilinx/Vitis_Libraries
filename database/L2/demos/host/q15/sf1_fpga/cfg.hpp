...
void gen_filter_cfg(uint32_t cfg[]) {
    ...
    cfg[n++] = 0UL | (FOP_DC << FilterOpWidth) | (FOP_DC);
    // cond_4
    cfg[n++] = (uint32_t)19960101;
    cfg[n++] = (uint32_t)19960401;
...
void get_aggr_cfg(ap_uint<32>* buf, int i) {
    ...
    ap_uint<32> t;
    signed char id[] = {1, 2, 0, 3, -1, -1, -1, -1}; // Lineitem, 6col
    for (int c = 0; c < 4; ++c) {
    ...
    shuffle1_cfg(7, 0) = 0;   // price
    shuffle1_cfg(15, 8) = 1;  // discount
    shuffle1_cfg(23, 16) = 2; // key
    ...
    shuffle2_cfg(7, 0) = 0;   // price
    shuffle2_cfg(15, 8) = 1;  // discount
    shuffle2_cfg(23, 16) = 2; // key
    shuffle2_cfg(31, 24) = 3; // shipdate
    shuffle2_cfg(39, 32) = 8; // eval1
    ...
    // write out
    config[82] = 0xffff; // 0000_0001_1000_0001
