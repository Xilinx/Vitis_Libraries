...
// t5
void q9Join_o_t4(Table* tin1, Table& tin2, Table& tout, int hjTimes) {
    std::unordered_multimap<int32_t, int32_t> ht1;
    for (int j = 0; j < hjTimes; j++) {
        int nrow1 = tin1[j].getNumRow();
        for (int i = 0; i < nrow1; i++) {
            int32_t o_orderkey = tin1[j].getInt32(i, 0);
            int32_t o_orderdate = tin1[j].getInt32(i, 1);
            ht1.insert(std::make_pair(o_orderkey, o_orderdate));
        }
    }
    ...
    for (int i = 0; i < nrow2; i++) {
        ...
        while (it != its.second) {
            ...
            int32_t o_orderdate = it->second;
            tout.setInt32(r, 0, s_nationkey);
            if ((o_orderdate / 10000) == 0) std::cout << "###" << std::endl;
            tout.setInt32(r, 1, o_orderdate);
            if (tout.getInt32(r, 1) != o_orderdate) std::cout << "***" << std::endl;
            tout.setInt32(r, 2, eval0);
            it++;
            r++;
        }
...
void q9Join_n_t5(Table& tin1, Table& tin2, Table& tout, int hjTimes) {
    int nrow1 = tin1.getNumRow();
    std::unordered_multimap<int32_t, int32_t> ht1;
    for (int i = 0; i < nrow1; i++) {
        int32_t n_nationkey = tin1.getInt32(i, 0);
...
void q9GroupBy(Table& tin, Table& origin, Table& tout) {
    ...
    std::cout << "row number: " << tin.getNumRow() << std::endl;
