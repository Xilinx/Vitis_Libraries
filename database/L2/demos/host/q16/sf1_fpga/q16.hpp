...
void q16Join_t4_t2(Table& tin1, Table& tin2, Table& tout) {
    std::cout << tin2.getInt32(0, 2) << std::endl;
    std::cout << tin2.getInt32(1, 2) << std::endl;
    std::cout << tin2.getInt32(1, 2) << std::endl;
    int nrow1 = tin1.getNumRow();
    int nrow2 = tin2.getNumRow();
...
void q16Sort(Table& tin, Table& tout) {
    ...
    for (auto& it : rows) {
        ...
        if (r < 10)
            std::cout << std::dec << "supplier_cnt: " << tout.getInt64(r, 3)
                      << " p_brand_s: " << tout.getcharN<char, TPCH_READ_P_BRND_LEN + 1>(r, 0).data()
                      << " p_type_s: " << tout.getcharN<char, TPCH_READ_P_TYPE_LEN + 1>(r, 1).data()
                      << " p_size: " << tout.getInt32(r, 2) << std::endl;
        ++r;
    }
    tout.setNumRow(r);
}
