...
void LineFilter(Table& tin, Table& tout) {
    ...
    for (int i = 0; i < nrow; i++) {
        ...
        if ((!strcmp(l_shipmode.data(), "MAIL")) || (!strcmp(l_shipmode.data(), "SHIP"))) {
            ...
            int32_t l_rowid = i;
            if (r < 50 && l_orderkey > 174576000) std::cout << l_orderkey << "  " << l_shipmode.data() << std::endl;
