...
void q11Filter_Sort(int64_t filterv, Table& tin, Table& tout) {
    ...
    for (int i = 0; i < nrow; i++) {
        ...
        if (sum > filterv) {
            int32_t ps_partkey = tin.getInt32(i, 7);
...
// t4
void q11Print(Table& tin) {
    unsigned nrow = tin.getNumRow();
    printf("after sort:%d\n", nrow);
    int r = nrow > 32 ? 32 : nrow;
    std::cout << "Query result:" << nrow << std::endl;
    for (int i = 0; i < r; ++i) {
        int32_t col0 = tin.getInt32(i, 0); // index much patMatch YAML
        int32_t col1 = tin.getInt32(i, 1);
        int32_t col2 = tin.getInt32(i, 2);
        int32_t col3 = tin.getInt64(i, 3);
        printf("col0:%d, col1:%d, col2:%d, col3:%d\n", col0, col1, col2, col3);
    }
}
