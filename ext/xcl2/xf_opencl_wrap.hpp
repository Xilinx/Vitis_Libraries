...
template <typename T> class XclIn;
class XclIn2;

template <typename T> class XclOut;
class XclOut2;
...
struct cl_buffer_wrapper {
    ...
    cl_buffer_wrapper(cl::Context& context,
                      void* data,
                      size_t size,
                      cl_mem_flags flag): mData(data),
                                          mSize(size),
                                          mFlag(flag) {
        if(size > 0) {
...
class cl_kernel_wrapper {
    private:
        ...
        void sanitizeRW(cl_buffer_wrapper* buff, const cl_mem_flags flag) {
            /* In case a cl::Buffer was created as output of a kernel function and later is modified as input to another kernel function then the flag needs to be modified to RW */
    ...
    public:
        cl_kernel_wrapper(const cl::Context& context,
                          ...
                          std::vector<cl::Device>& devices) : mContext(context),
                                                              mFuncName(func_name),
                                                              mBinName(bin_name),
                                                              mQueue(Queue) {
...
class cl_kernel_mgr {
    private:
        ...
        cl_kernel_mgr() {
            ...
            OCL_CHECK(err, cl::CommandQueue queue(mContext, mDevice, (CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE), &err));
    ...
    public:
        static cl_kernel_wrapper* registerKernel(std::string& func_name, std::string& bin_name) {
            ...
            if(it != mRegistry->mMap.end()) {
                ...
                kernel = new cl_kernel_wrapper(mRegistry->mContext, func_name, bin_name, mRegistry->mDeviceName, mRegistry->mCurrQueue, mRegistry->mDevices);
        ...
        static void createNewQueue() {
            ...
            OCL_CHECK(err, cl::CommandQueue queue(mRegistry->mContext, mRegistry->mDevice, (CL_QUEUE_PROFILING_ENABLE | CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE), &err));
...
class XclIn {
    public:
        T* mI;
        XclIn(T& I) : mI(&I) {}
        void addArg(cl_kernel_wrapper* kl) {
            kl->registerInput(*mI);
        }
};
