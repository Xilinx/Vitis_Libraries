...
class cholesky_cascade_recur {
   public:
    ...
    using cholesky_template =
        cholesky<TT_DATA, TP_DIM, TP_NUM_FRAMES, TP_GRID_DIM, TP_DIAG_INV, TP_X, TP_Y, kDiagStart, kDiagEnd>;
    using cholesky_cascade_recur_template = cholesky_cascade_recur<TT_DATA,
                                                                   TP_DIM,
                                                                   TP_NUM_FRAMES,
                                                                   TP_GRID_DIM,
                                                                   TP_CASC_LEN,
    ...
    static void create(kernel (&m_kernels)[TP_GRID_DIM][TP_GRID_DIM][TP_CASC_LEN],
        ...
        if
            constexpr(kDiagEnd < kNumDiagsSeenByXY) {
...
class cholesky_grid_recur {
   public:
    using cholesky_cascade_recur_entry =
        cholesky_cascade_recur<TT_DATA, TP_DIM, TP_NUM_FRAMES, TP_GRID_DIM, TP_CASC_LEN, TP_DIAG_INV, TP_X, TP_Y, 0, 0>;
    using cholesky_recur_right_template =
        cholesky_grid_recur<TT_DATA, TP_DIM, TP_NUM_FRAMES, TP_GRID_DIM, TP_CASC_LEN, TP_DIAG_INV, TP_X + 1, TP_Y>;
    using cholesky_recur_next_row_template =
        cholesky_grid_recur<TT_DATA, TP_DIM, TP_NUM_FRAMES, TP_GRID_DIM, TP_CASC_LEN, TP_DIAG_INV, 0, TP_Y + 1>;
    ...
    static void create(kernel (&m_kernels)[TP_GRID_DIM][TP_GRID_DIM][TP_CASC_LEN],
        ...
        if
            constexpr(TP_X < TP_Y) { cholesky_recur_right_template::create(m_kernels, depthMap, activeGridOffsets); }
        else if
...
class cholesky_graph : public graph {
    ...
    */
    ...
    static_assert(TP_DIM % TP_GRID_DIM == 0, "ERROR: TP_DIM must exactly divide into TP_GRID_DIM.");
    static_assert(TP_NUM_FRAMES > 0, "ERROR: TP_NUM_FRAMES must be greater than 0.");
    static_assert(TP_CASC_LEN > 0, "ERROR: TP_CASC_LEN must be greater than 0.");
    static_assert(TP_CASC_LEN <= TP_DIM, "ERROR: TP_CASC_LEN must be less than or equal to TP_DIM.");
    ...
    **/
     ...
     **/
    cholesky_graph() {
        cholesky_grid_recur<TT_DATA, TP_DIM, TP_NUM_FRAMES, TP_GRID_DIM, TP_CASC_LEN, TP_DIAG_INV, 0, 0>::create(
            m_kernels, depthMap, activeGridOffsets);
