...
#ifndef _XF_SOLVER_BackSubstitution_GRAPH_HPP_
#define _XF_SOLVER_BackSubstitution_GRAPH_HPP_
...
/**
 * @class BackSubstitution_Graph
 * @brief To solve vector x via Rx=c, R is upper triangular matrix, c is input vector, x is output vector.
 *
...
class BackSubstitution_Graph : public adf::graph {
   ...
   public:
    ...
    /**
     * @brief This is the constructor function for the BackSubstitution_Graph.
     **/
    BackSubstitution_Graph() {
        for (int i = 0; i < colA_num; i++) {
            k0[i] = kernel::create(backSubstitution<colA_num, colA_num, colB_num>);
        ...
        for (int i = 0; i < colA_num - 1; i++) {
            connect<stream> net02(k0[i].out[0], k0[i + 1].in[0]);
            connect<stream> net03(k0[i].out[1], k0[i + 1].in[1]);
