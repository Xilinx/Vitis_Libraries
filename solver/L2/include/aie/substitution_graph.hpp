...
class substitution_recur {
   public:
    ...
    // However, on AIE1, the buffer descriptor can only handle 2 dimensions, but one of those dimensions is used for
    // cfloat
    //(real and imag are considered a dimension). Therefore, for cfloat on AIE1, the data type in the kernel iobuffer,
    // and therefore
    // inferred by the buffer descriptor, is set to float. This results in the transform being incorrect, but in a known
    // incorrect pattern
    ...
    static constexpr bool kUsingSpoofTranspose = (__MAX_BD_DSIZE_TPOSE__ == 4) &&          // i.e. AIE1
                                                 ...
                                                 ((TP_L_LEADING == 0 && TP_SUBST_TYPE == 0) ||
                                                  (TP_L_LEADING == 1 && TP_SUBST_TYPE == 1)); // and transpose required
    ...
    using substitution_template = substitution<TT_DATA,
                                               T_KERNEL_DIM,
                                               TP_SUBST_TYPE,
                                               TP_L_LEADING,
                                               TP_X,
                                               ...
                                               TP_DIAG_INV,
                                               TT_IO_DATA>;
    using substitution_recur_right_template = substitution_recur<TT_DATA,
                                                                 T_KERNEL_DIM,
                                                                 TP_SUBST_TYPE,
                                                                 TP_L_LEADING,
                                                                 T_NUM_KERNELS,
                                                                 ...
                                                                 TP_NUM_FRAMES,
                                                                 TP_DIAG_INV>;
    using substitution_recur_next_row_template = substitution_recur<TT_DATA,
                                                                    T_KERNEL_DIM,
                                                                    TP_SUBST_TYPE,
                                                                    TP_L_LEADING,
                                                                    T_NUM_KERNELS,
    ...
    static void create(kernel (&m_kernels)[T_NUM_KERNELS], kernel (&m_castKernel_kernels)[T_NUM_KERNELS]) {
        ...
        if
            constexpr(TP_X < TP_Y) { substitution_recur_right_template::create(m_kernels, m_castKernel_kernels); }
        else if
            constexpr(TP_Y < TP_GRID_DIM - 1) {
...
class substitution_graph : public adf::graph {
    ...
    */
    ...
    static constexpr bool kUsingSpoofTranspose =
        (__MAX_BD_DSIZE_TPOSE__ == 4) && (std::is_same<TT_DATA, cfloat>::value) &&
    ...
    static_assert(
        kKernelMatrixSize* TP_NUM_FRAMES * sizeof(TT_DATA) <= __DATA_MEM_BYTES__,
        "ERROR: kKernelMatrixSize * TP_NUM_FRAMES * sizeof(TT_DATA) must be less than or equal to __DATA_MEM_BYTES__.");
    static_assert(TP_DIM_SIZE % TP_GRID_DIM == 0, "ERROR: TP_DIM_SIZE must exactly divide into TP_GRID_DIM.");
    static_assert(TP_NUM_FRAMES > 0, "ERROR: TP_NUM_FRAMES must be greater than 0.");
    static_assert(TP_NUM_FRAMES <= 4,
                  "ERROR: TP_NUM_FRAMES must be less than or equal to 4."); // max num frames due to transpose hardware
                                                                            // constraints.
    static_assert(TP_DIAG_INV == 0 || TP_DIAG_INV == 1, "ERROR: TP_DIAG_INV must be either 0 or 1.");
    ...
    **/
    ...
    substitution_graph() {
        // create the kernels
        substitution_recur<TT_DATA, kKernelDim, TP_SUBST_TYPE, TP_L_LEADING, kNumKernels, 0 /*T_X*/, 0 /*T_Y*/,
                           TP_GRID_DIM, 0 /*tileIdx*/, TP_NUM_FRAMES, TP_DIAG_INV>::create(m_kernels,
                                                                                           m_castKernel_kernels);
        ...
        if
            constexpr(TP_SUBST_TYPE == 0) { // forward
        ...
        for (int y = 0; y < TP_GRID_DIM; y++) {
            for (int x = 0; x < y + 1; x++) {
                ...
                // Kernel outputs are : x (index 0 - but only for diagonal kernels) and, for internal connections pp (0
                // or 1)
                ...
                if ((TP_L_LEADING == 0 && TP_SUBST_TYPE == 0) || (TP_L_LEADING == 1 && TP_SUBST_TYPE == 1)) {
                    // TP_L_LEADING == 0 means row-major, but the forward kernel algorithm is column-major, so a
                    // transpose is required here, but not when TP_L_LEADING == 1
                    // insert buffer descriptor here for transpose.
                    // For backward substitution, the matrix expected is L, but the algo works on L^H so transpose is
                    // required before the kernel see it.

                    // This next line takes some explanation
                    // For AIE1 cfloat transpose is not supported, so the iobuffer is defined as float.
                    // This makes each frame in the iobuffer an TP_DIM_SIZE*2,TP_DIM_SIZE array rather than
                    // TP_DIM_SIZE*TP_DIM_SIZE,
                    ...
                    write_access(m_kernels[currTileIdx].in[0]) =
                        ...
                        adf::tiling(
                            {.buffer_dimension = {kKernelDim * kSpoofFactor, kKernelDim, TP_NUM_FRAMES},
                             ...
                             .offset = {0, 0, 0},
                             .tile_traversal = {{.dimension = 1, .stride = 1, .wrap = kKernelDim},
                                                {.dimension = 0, .stride = 1, .wrap = kKernelDim * kSpoofFactor},
                                                {.dimension = 2, .stride = 1, .wrap = TP_NUM_FRAMES}}});

                    if
                        constexpr(kUsingSpoofTranspose) {
                            connect(L_in[L_port_map[currTileIdx]], m_castKernel_kernels[currTileIdx].in[0]);
                            dimensions(m_castKernel_kernels[currTileIdx].in[0]) = {TP_NUM_FRAMES * kKernelDim *
                                                                                   kKernelDim};
                            connect(m_castKernel_kernels[currTileIdx].out[0], m_kernels[currTileIdx].in[0]);
                            dimensions(m_castKernel_kernels[currTileIdx].out[0]) = {TP_NUM_FRAMES * kKernelDim *
                                                                                    kKernelDim * kSpoofFactor};
                        }
                    else {
                        connect(L_in[L_port_map[currTileIdx]], m_kernels[currTileIdx].in[0]);
                    }
                    dimensions(m_kernels[currTileIdx].in[0]) = {TP_NUM_FRAMES * kKernelDim * kKernelDim *
                                                                kSpoofFactor}; // Matrix L_in
                ...
                stack_size(m_kernels[currTileIdx]) =
                    900; // no specific justification, simply enough to accommodate minimal stack operations
                if (kUsingSpoofTranspose) {
