...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_polinearsolver_0(program, "kernel_polinearsolver_0", &err);
    ...
    const int MAXN = 4, LDA = 4, NA = 4, LDB = 2, NB = 2;
    int in_size = MAXN * MAXN;
    int inout_size = NB * MAXN;
    double* dataA;
    double* dataB;
    dataA = aligned_alloc<double>(in_size);
    dataB = aligned_alloc<double>(inout_size);
    ...
    triLowerMatGenSPD<double>(dataAN, seed, dataC);
    ...
    // mext_io[0].obj = dataB;
    // mext_io[0].param = 0;
    mext_i[0] = {2, dataA, kernel_polinearsolver_0()};
    mext_io[0] = {3, dataB, kernel_polinearsolver_0()};
    ...
    output_buffer[0] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
    ...
    ob_in.push_back(output_buffer[0]);
    ...
    // Setup kernel
    kernel_polinearsolver_0.setArg(0, NA);
    kernel_polinearsolver_0.setArg(1, NB);
    kernel_polinearsolver_0.setArg(2, input_buffer[0]);
    kernel_polinearsolver_0.setArg(3, output_buffer[0]);
    q.finish();
    ...
    for (int i = 0; i < num_runs; ++i) {
        q.enqueueTask(kernel_polinearsolver_0, nullptr, nullptr);
    }
    q.finish();
    gettimeofday(&tend, 0);
    ...
    for (int p = 0; p < NB; p++) {
        ...
        for (int i = 0; i < dataAM; i++) {
            for (int j = 0; j < dataAN; j++) {
                res += dataA[i * LDA + j] * dataB[j * LDB + p];
            }
            res -= i;
        }
        errA += res * res;
