...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_gematrixinverse_0(program, "kernel_gematrixinverse_0", &err);
    ...
    // mext_io[0].obj = dataA;
    // mext_io[0].param = 0;
    mext_io[0] = {1, dataA, kernel_gematrixinverse_0()};
    ...
    // Setup kernel
    kernel_gematrixinverse_0.setArg(0, dataAN);
    kernel_gematrixinverse_0.setArg(1, buffer[0]);
    q.finish();
    ...
    for (int i = 0; i < num_runs; ++i) {
        q.enqueueTask(kernel_gematrixinverse_0, nullptr, nullptr);
    }
    q.finish();
    gettimeofday(&tend, 0);
    ...
    // Calculate err between dataA and dataE
    double errA = 0;
    for (int i = 0; i < dataAM; i++) {
        for (int j = 0; j < dataAN; j++) {
            for (int m = 0; m < dataAN; m++) {
                errA += (dataA[i * dataAN + m] * dataE[m][j]);
            }
            errA -= 1;
        }
    }
