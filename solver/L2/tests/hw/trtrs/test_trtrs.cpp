...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_trtrs_0(program, "kernel_trtrs_0", &err);
    ...
    const int MAXN = dataAM, LDA = dataAN, NA = dataAN, LDB = 2;
    const int NB = LDB;
    ...
    // mext_io[0].obj = dataB;
    // mext_io[0].param = 0;
    mext_i[0] = {2, dataA, kernel_trtrs_0()};
    mext_io[0] = {3, dataB, kernel_trtrs_0()};
    ...
    // Setup kernel
    kernel_trtrs_0.setArg(0, NA);
    kernel_trtrs_0.setArg(1, NB);
    kernel_trtrs_0.setArg(2, input_buffer[0]);
    kernel_trtrs_0.setArg(3, output_buffer[0]);
    q.finish();
    ...
    for (int i = 0; i < num_runs; ++i) {
        q.enqueueTask(kernel_trtrs_0, nullptr, nullptr);
    }
    q.finish();
    gettimeofday(&tend, 0);
