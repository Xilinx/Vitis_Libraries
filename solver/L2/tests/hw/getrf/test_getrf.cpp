...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_getrf(program, "kernel_getrf_0", &err);
    ...
    mext_io[0] = {0, dataA, kernel_getrf()};
    mext_io[1] = {1, dataP, kernel_getrf()};
    ...
    for (int i = 0; i < num_runs; ++i) {
        q.enqueueTask(kernel_getrf, nullptr, nullptr);
    }
    q.finish();
    gettimeofday(&tend, 0);
