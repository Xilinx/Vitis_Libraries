...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_getrf_nopivot(program, "kernel_getrf_nopivot_0", &err);
    ...
    // Setup kernel
    kernel_getrf_nopivot.setArg(0, buffer[0]);
    q.finish();
    ...
    for (int i = 0; i < num_runs; ++i) {
        q.enqueueTask(kernel_getrf_nopivot, nullptr, nullptr);
    }
    q.finish();
    gettimeofday(&tend, 0);
