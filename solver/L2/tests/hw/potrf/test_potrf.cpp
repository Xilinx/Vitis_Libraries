...
int main(int argc, const char* argv[]) {
    ...
    cl::Kernel kernel_potrf_0(program, "kernel_potrf_0", &err);
    ...
    DT* dataA;
    dataA = aligned_alloc<DT>(inout_size);

    // Generate general matrix dataAM x dataAN
    DT** dataC = new DT*[dataAM];
    DT** dataD = new DT*[dataAM];
    DT** dataE = new DT*[dataAM];
    for (int i = 0; i < dataAM; ++i) {
        dataC[i] = new DT[dataAN];
        dataD[i] = new DT[dataAN];
        dataE[i] = new DT[dataAN];
    }
    triLowerMatGen<DT>(dataAN, seed, dataC);
    transposeMat<DT>(dataAN, dataC, dataD);
    MulMat<DT>(dataAM, dataAN, dataAN, dataC, dataD, dataE);
    ...
    buffer[0] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                           sizeof(DT) * inout_size, &mext_io[0]);
