...
int golden_check(std::string output_file, std::string golden_file, int num) {
    ...
    for (int j = 0; j < num; j++) {
        ...
        // std::cout << "Golden check, error_num=" << checked << ", element id=" << j << ", out=" << out << ", gld=" <<
        // gld << std::endl;
...
int main(void) {
    ...
    HermitianMatrix<std::complex<double> > A(DIM);
    HermitianMatrix<std::complex<double> > L(DIM);
    HermitianMatrix<std::complex<double> > P(DIM);
    L.gen_lower_triangular_randf(1, 4, 0);
    L.gen_file_matrixL_by_column(L, 0, DIM-1, matL_real_file, matL_imag_file);
    std::cout << "matrix L: \n";
    //L.print();
    A.gen_hermitianMatrix(L);
    A.gen_file_matrixA_by_column(A, matA_real_file, matA_imag_file);
    std::cout << "matrix A: \n";
    //A.print();

    P.cholesky_opt_1(A);
    P.gen_file_matrixL_by_column(P, 0, DIM-1, gldMatL_real_file, gldMatL_imag_file);
    ...
    */
    ...
    int errNum_real = 0;
    int errNum_imag = 0;
    errNum_real = golden_check(outMatL_real_file, gldMatL_real_file, num);
    errNum_imag = golden_check(outMatL_imag_file, gldMatL_imag_file, num);
    if (errNum_real != 0) {
        std::cout << "File: " << outMatL_real_file << " Mis-Matched! \n";
        std::cout << "Test failed! \n";
    } else if (errNum_imag != 0) {
        std::cout << "File: " << outMatL_imag_file << " Mis-Matched! \n";
        std::cout << "Test failed! \n";
    } else {
        std::cout << "Test passed! \n";
    }
    return (errNum_real + errNum_imag);
}
