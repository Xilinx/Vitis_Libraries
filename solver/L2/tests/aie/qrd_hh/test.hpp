...
// This file holds the definition of the test harness for the qrd_hh graph.
...
// Make connections
        ...
        for (int i = 0; i < CASC_LEN; i++) {
            ...
            connect<>(inA[i].out[0], qrd_hh_graph.inA[i]);
        ...
        connect<>(inA[0].out[0], qrd_hh_graph.inA);
        ...
        connect<>(qrd_hh_graph.outQ, outQ[0].in[0]);
        connect<>(qrd_hh_graph.outR, outR[0].in[0]);
