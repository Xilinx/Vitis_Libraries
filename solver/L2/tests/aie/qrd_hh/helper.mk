...
###############################################################################
# Makefile helper used for qrd_hh compilation, simulation and QoR harvest.
###############################################################################
...
create_input:
	...
	$(VITIS_PYTHON3) $(HELPER_ROOT_DIR)/L2/tests/aie/qrd_hh/gen_input_qrd_hh.py --loc_input_file $(LOC_INPUT_FILE_A) --aie_variant $(AIE_VARIANT) --data_type $(DATA_TYPE) --row_dim_size $(DIM_ROWS) --col_dim_size $(DIM_COLS) --casc_len $(CASC_LEN) --num_frames $(NUM_FRAMES) --niter $(NITER) --seed $(SEED) --pliowidth 64 --dim_a_leading $(DIM_A_LEADING)

	echo Input ready

sim_ref:
	make UUT_KERNEL=qrd_hh_ref run TARGET=x86sim TAG=REF
...
get_latency:
	...
	$(VITIS_PYTHON3) $(DSP_ROOT_DIR)/L2/tests/aie/common/scripts/get_qor.py -t_in_file T_input_A_0.txt -t_out_file uut_output_Q_0.txt -status_file_dir $(STATUS_FILE) -num_of_samples $(WINDOW_VSIZE_Q) -niter $(NITER) -use_outputs_if_no_inputs False -casc_len $(CASC_LEN) -aiesim_out_dir ./aiesimulator_output -ip qrd_hh
