...
class ComplexMatrix {
   ...
   public:
    ...
    void qrd_householder_1(ComplexMatrix<T>& Q, ComplexMatrix<T>& R) {
        ...
        //std::complex<T>* v = new std::comoplex<T>[ M ];
        ...
        for (int k = 0; k < this->N; k++) {
            ...
            if(z > tol) {
                ...
                b[k] = z;
                T w = 0;
                w += elem(k,k).real() * elem(k,k).real();
                w += elem(k,k).imag() * elem(k,k).imag();
                if(w != (T)0) {
                    q.real( elem(k,k).real()/w );
                    q.imag( elem(k,k).imag()/w );
                }//endif
                else {
                    q.real( 1 );
                    q.imag( 0 );
                }
                elem(k,k).real( q.real() * (z+w) );
                elem(k,k).imag( q.imag() * (z+w) );
                //if(k != N-1) {
                    for(int j = k+1; j<N; j++) {
                        T real=0;
                        T imag=0;
                        ...
                        for(int i=k; i<M; i++) {
                            ...
                            aij_real -= q.real() * elem(i,j).real() - q.imag() * elem(i,j).imag();
                            aij_imag -= q.real() * elem(i,j).imag() + q.imag() * elem(i,j).real();
