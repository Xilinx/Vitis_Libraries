...
INLINE_DECL constexpr bool fnCheckKernelLoadShare() {
    ...
    if  (rowchunks % TP_CASC_LEN == 0) { // rowchuncks must be divisible by cascade length to ensure each kernel can have equal load
        return true;
    }
    return false;
};

template <typename TT_DATA, unsigned int TP_DIM_ROWS, unsigned int TP_DIM_COLS, unsigned int TP_NUM_FRAMES, unsigned int TP_CASC_LEN>
...
template <int m_kColChunkNum, int m_kRowChunkNum, int m_kColChunkLeadingStart, int m_kColChunkLeadingEnd, int m_kColChunkFinalStart, int TP_KERNEL_POS, int TP_CASC_LEN>
...
constexpr KernelRoleArrays<m_kColChunkNum> det_kernel_roles_q() {
    ...
    for (int col_chunk = 0; col_chunk < m_kColChunkNum; col_chunk++) {
        if (col_chunk < m_kColChunkLeadingStart - m_kRowChunkNum) { //upper kernels are calculating the beta, keep working all the columns
            result.col_chunk_in_calc_arr[col_chunk] = 0;
            result.kernel_role_arr[col_chunk] = 0;
        } else if ((col_chunk >= m_kColChunkLeadingStart - m_kRowChunkNum) && (col_chunk < m_kColChunkLeadingStart)) { // the kernel after the leading
            result.col_chunk_in_calc_arr[col_chunk] = 0;
            result.kernel_role_arr[col_chunk] = 2;
        } else if ((col_chunk >= m_kColChunkLeadingStart) && (col_chunk <= m_kColChunkLeadingEnd)) { //kernel is the leading kernel
            result.col_chunk_in_calc_arr[col_chunk] = col_chunk - m_kColChunkLeadingStart;
            result.kernel_role_arr[col_chunk] = 1;
        } else if ((col_chunk >= m_kColChunkFinalStart) && (TP_KERNEL_POS != TP_CASC_LEN - 1)) { // final kernel is leading and this kernel is not last kernel
            result.col_chunk_in_calc_arr[col_chunk] = m_kRowChunkNum;
