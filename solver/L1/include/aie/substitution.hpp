...
class substitution_base {
   ...
   public:
    ...
    // but they may be needed for more than one lane of processing, so they are read from stream for the first lane
    // (vector of rows) so that
    // they can be retrieved for later lanes, rather than have the diagonal kernel transmit them several times which
    // could lead to stall delays.
    alignas(__ALIGN_BYTE_SIZE__) TT_DATA xStoreSamp[TP_GRID_DIM > 1 ? TP_DIM_SIZE : 0]; // This stores the X values from
                                                                                        // a kernel at the top of a
                                                                                        // column. Not required for
                                                                                        // single kernel operation.
    void substitution_main(fbsub_input_params<TT_DATA>& input_params, fbsub_output_params<TT_DATA>& output_params);
};
...
template <typename TT_DATA,
          ...
          unsigned int TP_DIAG_INV,
          typename TT_IO_DATA>
class substitution : public substitution_base<TT_DATA,
                                              TP_DIM_SIZE,
                                              TP_SUBST_TYPE,
                                              TP_L_LEADING,
                                              TP_X,
