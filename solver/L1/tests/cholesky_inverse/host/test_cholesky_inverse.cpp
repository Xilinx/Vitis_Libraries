...
// ======================================================================
// Common testbench for Cholesky Inverse function.
// ======================================================================

#include "test_cholesky_inverse.hpp"
...
int main(int argc, char* argv[]) {
    ...
    // long unsigned num_tests = (ROWSCOLSA >= 16 ? 4 : 20);    // Small default for HLS
    ...
    CHOL_INV_TYPE InverseA_cast[ROWSCOLSA][ROWSCOLSA];          // The inverse result from the DUT, cast to double
    MATRIX_OUT_T InverseA_expected[ROWSCOLSA][ROWSCOLSA];       // The inverse result from LAPACK in target format
    CHOL_INV_TYPE InverseA_expected_cast[ROWSCOLSA][ROWSCOLSA]; // The inverse result from LAPACK, cast to double

    CHOL_INV_TYPE I[ROWSCOLSA][ROWSCOLSA]; // The identity matrix to compare against

    // Test variables
    CHOL_INV_TYPE A_cast[ROWSCOLSA][ROWSCOLSA]; // Cast A back to LAPACK compatible type for analysis
    MATRIX_OUT_T InverseA_delta_vs_lapack[ROWSCOLSA]
                                         [ROWSCOLSA]; // The difference between the DUT's InverseA and LAPACK's InverseA
    CHOL_INV_TYPE I_restored[ROWSCOLSA][ROWSCOLSA];   // I recreated from InverseA_cast*A_cast
    CHOL_INV_TYPE I_restored_lapack[ROWSCOLSA][ROWSCOLSA]; // I recreated from LAPACK's InverseA_cast*A_cast

    CHOL_INV_TYPE I_delta[ROWSCOLSA][ROWSCOLSA]; // The delta values will be passed to xLANSY so need to use LAPACK
                                                 // compatible types
    CHOL_INV_TYPE I_delta_lapack[ROWSCOLSA][ROWSCOLSA]; //

    // Non-complex type
    double A_norm;
    double InverseA_norm;
    CHOL_INV_BASE_TYPE I_delta_norm;
    CHOL_INV_BASE_TYPE I_delta_lapack_norm;
    CHOL_INV_BASE_TYPE I_DUT_ratio, I_LAPACK_ratio;
    ...
    for (unsigned int imat = 0; imat < NUM_MAT_TYPES; imat++) {
        ...
        if ((mat_type == 0 || imat + 1 == mat_type) && (skip_mat_type[0] - 1 != imat && skip_mat_type[1] - 1 != imat)) {
            for (long unsigned i = 0; i < num_tests; i++) {
                ...
                // Get Actual results
                // ====================================================================
                kernel_cholesky_inverse_0(matrixAStrm, matrixInverseAStrm);
                ...
                if (anyNaN<ROWSCOLSA, ROWSCOLSA>(InverseA) == 1) {
                ...
                for (int r = 0; r < ROWSCOLSA; r++) {
                    for (int c = 0; c < ROWSCOLSA; c++) {
                        InverseA_expected_cast[r][c] = InverseA_expected[r][c];
                        InverseA_cast[r][c] = InverseA[r][c];
                    }
                }
                ...
                mmult<false, false, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA>(
                    InverseA_cast, A_cast, I_restored);
                mmult<false, false, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA, ROWSCOLSA>(
                    InverseA_expected_cast, A_cast, I_restored_lapack);
                msub<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_TYPE>(I, I_restored, I_delta);
                msub<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_TYPE>(I, I_restored_lapack, I_delta_lapack);

                // REVISIT: is A_cast the appropriate format to use here?
                // norm1 as used in SPOT01
                A_norm = norm1_dbl<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_BASE_TYPE>(A_cast);
                ...
                // norm1 as used in SPOT01
                InverseA_norm =
                    norm1_dbl<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_BASE_TYPE>(InverseA_expected_cast);
                ...
                // norm1 as used in SPOT01
                I_delta_norm = norm1<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_BASE_TYPE>(I_delta);

                // norm1 as used in SPOT01
                I_delta_lapack_norm = norm1<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, CHOL_INV_BASE_TYPE>(I_delta_lapack);
                ...
                printf("I_delta \n");
                xf::solver::print_matrix<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, xf::solver::NoTranspose>(I_delta, "   ",
                print_precision,1);
                printf("I_delta_lapack\n");
                xf::solver::print_matrix<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, xf::solver::NoTranspose>(I_delta_lapack, "
                ", print_precision,1);

                printf("I_restored \n");
                xf::solver::print_matrix<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, xf::solver::NoTranspose>(I_restored, "
                ", print_precision,1);
                printf("I_restored_lapack \n");
                xf::solver::print_matrix<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE,
                xf::solver::NoTranspose>(I_restored_lapack, "   ", print_precision,1);

                printf("A_norm %f I_delta_norm %f I_delta_lapack_norm %f InverseA_norm %f\n",A_norm, I_delta_norm ,
                I_delta_lapack_norm, InverseA_norm);
                //----------------------------------------------------------------------------------------------------------------------------------------
                */
                ...
                */
                ...
                if ((I_ratio_difference > 0 && debug > 0) || debug > 1 || I_DUT_ratio > ratio_threshold) {
                    printf("  A=\n");
                    ...
                    printf("  I_restored=\n");
                    xf::solver::print_matrix<ROWSCOLSA, ROWSCOLSA, CHOL_INV_TYPE, xf::solver::NoTranspose>(
