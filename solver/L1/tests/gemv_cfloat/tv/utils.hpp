...
template <typename T>
void writeComplextoFileByColumn(ComplexMatrix<T>& X, std::string filename0) {
    ofstream myfile0;
    ...
    for (int j = 0; j < X.N; j++) {
        for (int i = 0; i < X.M; i++) {
            myfile0 << X.elem(i, j).real() << " ";
            myfile0 << X.elem(i, j).imag() << " " << std::endl;
        }
    }
    myfile0.close();
}
template <typename T>
void writeComplextoFileByRow(ComplexMatrix<T>& X, std::string filename0) {
    ofstream myfile0;
    myfile0.open(filename0.c_str());
    if (!myfile0) {
        std::cout << "[ERROR]: file " << filename0 << "could not be opened !" << std::endl;
...
template <typename T>
void writeComplextoFileByBlock(ComplexMatrix<T>& X, std::string filename0, unsigned BR, unsigned BC) {
    ofstream myfile0;
    myfile0.open(filename0.c_str());
    if (!myfile0) {
        std::cout << "[ERROR]: file " << filename0 << "could not be opened !" << std::endl;
