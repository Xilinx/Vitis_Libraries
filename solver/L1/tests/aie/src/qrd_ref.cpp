...
void qrd_ref<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN>::qrd_main(
    ...
    input_buffer<TT_DATA>& inWindowA, output_buffer<TT_DATA>& outWindowQ, output_buffer<TT_DATA>& outWindowR) {
    ...
    for (int frame = 0; frame < TP_NUM_FRAMES; frame++) {
        for (unsigned int col = 0; col < TP_DIM_COLS; col++) {
            ...
            norm_val = arrOps_row.calc_norm_val(
                v_mat[col]); // assign the norm val of the corresponding vector to the diagonal elements of R
