...
NOINLINE_DECL void
castKernel<TP_DIM_SIZE, TP_NUM_FRAMES>::cast_main(
    input_buffer<cfloat>& __restrict in,
  ...
  typedef float TT_DATA;
    ...
    for (int row = 0; row < TP_DIM_SIZE; row++)
      chess_prepare_for_pipelining chess_loop_range(TP_DIM_SIZE,) {
        for (int colvect = 0; colvect< TP_DIM_SIZE*2/m_kvecSampleNum; colvect++) //*2 because input is interpreted as float, not cfloat
