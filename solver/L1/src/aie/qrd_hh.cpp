...
template <typename TT_DATA,
          ...
          bool TP_STREAM_EN>
INLINE_DECL void
qrd_hh_kernel<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN, TP_KERNEL_POS, TP_ROUT_EN, TP_STREAM_EN>::
    qrd_hh_r_func(T_inputIF<TP_STREAM_EN, TT_DATA>& inInterface,
    ...
    if
        constexpr(TP_ROUT_EN) { outRPtr = (vect_t*)outInterface.outWindowR.data(); }
    ...
    for (int frame = 0; frame < TP_NUM_FRAMES; frame++) chess_prepare_for_pipelining chess_loop_range(TP_NUM_FRAMES, ) {
            ...
            betavectBuffPtr =
                (vect_t*)&betavectBuff[0]; // point to the begining of the beta buffer for the current frame

            for (int col_chunk = 0; col_chunk < m_kColChunkNum; col_chunk++)
                chess_prepare_for_pipelining chess_loop_range(m_kColChunkNum, ) {
                    x_mask = ::aie::mask<vecSampleNum>::from_uint32(0x00000000);

                    for (int col = 0; col < vecSampleNum; col++)
                        chess_prepare_for_pipelining chess_loop_range(vecSampleNum, ) {
                            ...
                            inAPtr = (vect_t*)inInterface.inWindowA.data() +
                                     offset; // move to the current column, regarding row chunk
                            vvectBuffPtr = (vect_t*)&vvectBuff[0] + col_chunk + 1;

                            v_vect = ::aie::select(*inAPtr++, m_kZERO, x_mask); // x_vect is stored in the v_vect
                                                                                // variable to prevent re-reading from
                                                                                // memory -for the first chunk
                            ...
                            // this scalar opertion is to help v_norm calculation without going through the whole
                            // vvector
                            mul_scalars(x_1, x_1, x_col_sqr);

                            for (int row_chunk = col_chunk + 1; row_chunk < m_kRowChunkNum;
                                 row_chunk++) {     // remainder of the row chunks
                                x_vect = *inAPtr++; // reading current A vector
                                mac_vectors(acc_x_norm, x_vect, x_vect);
                                *vvectBuffPtr++ =
                                    x_vect; // save the xvector, vvector is the same for the rest of the chunks
                            }
                            ...
                            v_vect[col] =
                                x_1 -
                                alpha; // calculating the first element of the x_vect, v_vect is actually generated here
                            *(vvectBuffPtr - m_kRowChunkNum + col_chunk) =
                                v_vect; // save the vvector back to the cache's first chunk address
                            ...
                            for (int i = col + col_chunk * vecSampleNum; i < TP_DIM_COLS; i++)
                                chess_prepare_for_pipelining chess_loop_range(0, TP_DIM_COLS) {
                                    //  To compute the dot product as v_col * r_mat[i]
                                    vect_t* vvectPtrBase = (vect_t*)&vvectBuff[0] + col_chunk;
                                    vect_t* inAPtrBase = (vect_t*)inInterface.inWindowA.data() + frame_offset_q +
                                                         i * m_kRowChunkNum + col_chunk; // move to the current V column

                                    vvectBuffPtr = vvectPtrBase;
                                    inAPtr = inAPtrBase;

                                    mul_vectors(acc_mid_prod, *vvectBuffPtr++,
                                                *inAPtr++); // initial condition for acc_mid_prod
                            ...
                            inAPtr = (vect_t*)inInterface.inWindowA.data() + frame_offset_q +
                                     col_idx * m_kRowChunkNum; // transfer the R values for the current column
                            for (int i = 0; i < m_kColChunkNum; i++) {
                                *outRPtr++ = *inAPtr++;
                            }
                            chess_memory_fence(); // ensure R read completes before V-save overwrites A

                            // save V vector to the completed input vector buffer, also set the initial value for the
                            // output Q buffer for the current column
                            inAPtr = (vect_t*)inInterface.inWindowA.data() + frame_offset_q +
                                     col_idx * m_kRowChunkNum; // move to the current column, regarding row chunk
                            ...
                            *inAPtr++ = *vvectBuffPtr++;
                            ...
                            for (int row_chunk = col_chunk + 1; row_chunk < m_kRowChunkNum;
                                 row_chunk++) { // save the vvector to the output Q buffer (the necessary chunks)
                                *inAPtr++ = *vvectBuffPtr++;
                                *outQPtr++ = blank_vect;
                            }
            ...
            this->qrd_hh_q_func(inInterface, outInterface,
                                frame); // call the qrd_hh_q function to start the reverse sweep for Q generation
        }
...
INLINE_DECL void
qrd_hh_kernel<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN, TP_KERNEL_POS, TP_ROUT_EN, TP_STREAM_EN>::
    qrd_hh_q_func(T_inputIF<TP_STREAM_EN, TT_DATA>& inInterface,
                  T_outputIF<TP_STREAM_EN, TP_ROUT_EN, TT_DATA>& outInterface,
                  int& frame_id) {
    using acc_t = accTypeMult_t<TT_DATA, TT_DATA>;
    using vect_t = ::aie::vector<TT_DATA, vecSampleNum>;
    ...
    vect_t* betaBuffPtr;
    vect_t* outQPtr = (vect_t*)outInterface.outWindowQ.data();
    vect_t* betavectBuffPtr =
        (vect_t*)&betavectBuff[0] + m_kColChunkNum - 1; // point to the last chunk of beta vector for the reverse sweep
    vect_t* inAPtr = (vect_t*)inInterface.inWindowA.data();
    ...
    for (int col_chunk = m_kColChunkNum - 1; col_chunk >= 0; col_chunk--)
        chess_prepare_for_pipelining chess_loop_range(m_kColChunkNum, ) { // reverse sweep for v capture
            beta_vect = *betavectBuffPtr--;                               // get the beta vector for the current chunk
            for (int col = vecSampleNum - 1; col >= 0; col--)
                chess_prepare_for_pipelining chess_loop_range(vecSampleNum, ) {
                    // offset calculation for the row chunk that will be processed
                    unsigned int col_idx = col_chunk * vecSampleNum + col;
                    unsigned int offset = frame_offset_q + col_idx * m_kRowChunkNum + col_chunk;
                    ...
                    for (int i = 0; i < TP_DIM_COLS; i++)
                        chess_prepare_for_pipelining chess_loop_range(
                            TP_DIM_COLS, ) { // forward sweep for Q accumulation
                            vect_t* QPtrBase = (vect_t*)outInterface.outWindowQ.data() + frame_offset_q +
                                               i * m_kRowChunkNum +
                                               col_chunk; // set to the begining of the current vector
                            vect_t* inAPtrBase =
                                (vect_t*)inInterface.inWindowA.data() + offset; // move to the current V column
...
INLINE_DECL void
qrd_hh_kernel<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN, TP_KERNEL_POS, TP_ROUT_EN, TP_STREAM_EN>::
    qrd_hh_r_casc_func(T_inputIF<TP_STREAM_EN, TT_DATA>& inInterface,
                       T_outputIF<TP_STREAM_EN, TP_ROUT_EN, TT_DATA>& outInterface) {
    using acc_t = accTypeMult_t<TT_DATA, TT_DATA>;
    using vect_t = ::aie::vector<TT_DATA, vecSampleNum>;
    using acc_vect_t = ::aie::accum<acc_t, vecSampleNum>;
    ...
    for (int frame = 0; frame < TP_NUM_FRAMES; frame++) chess_prepare_for_pipelining chess_loop_range(TP_NUM_FRAMES, ) {
            ...
            if
                constexpr(TP_ROUT_EN) { outRPtr = (vect_t*)outInterface.outWindowR.data() + frame_offset_r; }
                ...
                for (int col = 0; col < vecSampleNum; col++)
                    chess_prepare_for_pipelining chess_loop_range(vecSampleNum, ) {
                        ...
                        if (kernel_role < 3) { // kernel is not idle
                            ...
                            x_vect = *inAPtr++;                              // reading current A vector
                            v_vect = ::aie::select(x_vect, m_kZERO, x_mask); // x_vect is stored in the v_vect variable
                                                                             // to prevent re-reading from memory -for
                                                                             // the first chunk
                            *vvectBuffPtr++ = x_vect;
                        }

                        if (kernel_role ==
                            1) { // leading kernel, needs to calculate the alpha, phase and the first element of v_vect
                            x_mask.set(col); // shift the mask down by 1 position and fill the first position with 0s
                            // alpha is to be calculated first chunk, first element-in-process
                            // get x_1 so following loop can start whilst scalar harware is calculating phase
                            x_1 = v_vect[col];
                        ...
                        if (kernel_role < 3) {
                            mul_vectors(acc_x_norm, v_vect, v_vect); // initial condition for acc_x_norm
                            for (int row_chunk = col_chunk_in_calc + 1; row_chunk < m_kRowChunkNum;
                                 row_chunk++) {     // remainder of the row chunks
                                x_vect = *inAPtr++; // reading current A vector
                                mac_vectors(acc_x_norm, x_vect, x_vect);
                                *vvectBuffPtr++ =
                            ...
                            if ((TP_KERNEL_POS != TP_CASC_LEN - 1) &&
                                (kernel_role < 3)) { // if not the last kernel read the val and update your calc
                                chess_separator_scheduler();
                                chess_memory_fence();
                                x_acc_val_stream = readincr<aie_stream_resource_in::a>(inInterface.inStream);
                                x_acc_val =
                                    x_acc_val + x_acc_val_stream; // accumulate the x_acc_val from different kernels
                            }

                            if (kernel_role != 1) { // if not the leading kernel, need to send the x_acc_val to the next
                                                    // kernel through the cascade stream
                                writeincr<aie_stream_resource_out::a, TT_DATA>(outInterface.outStream,
                                                                               x_acc_val); // send the xacc
                            }
                        }
                        ...
                        if (kernel_role == 1) {
                            ...
                            v_vect[col] =
                                x_1 -
                                alpha; // calculating the first element of the x_vect, v_vect is actually generated here
                            *(vvectBuffPtr - m_kRowChunkNum + col_chunk_in_calc) =
                                v_vect; // save the vvector back to the cache's first chunk address
                            ...
                            writeincr<aie_stream_resource_out::a, TT_DATA>(outInterface.outStream,
                                                                           beta_val); // send the beta
                        } else {
                            chess_separator_scheduler();
                            chess_memory_fence();
                            beta_val = readincr<aie_stream_resource_in::a>(
                                inInterface.inStream); // if not produced wait for the beta value
                        ...
                        for (int i = col + col_chunk * vecSampleNum; i < TP_DIM_COLS; i++)
                            chess_prepare_for_pipelining chess_loop_range(0, TP_DIM_COLS) {
                                ...
                                vect_t* inAPtrBase = (vect_t*)inInterface.inWindowA.data() + frame_offset_q +
                                                     i * m_kRowChunkNum + col_chunk_in_calc;
                                ...
                                if (kernel_role < 3) { // if not idle, calculate the mid_prod_val and update the r_mat
                                    mul_vectors(acc_mid_prod, *vvectBuffPtr++,
                                                *inAPtr++); // initial condition for acc_x_norm
                                    for (int row_chunk = col_chunk_in_calc + 1; row_chunk < m_kRowChunkNum;
                                         row_chunk++) {
                                        mac_vectors(acc_mid_prod, *vvectBuffPtr++, *inAPtr++);
                                    }
                                    ...
                                    if (TP_KERNEL_POS !=
                                        TP_CASC_LEN - 1) { // if not the last kernel, need to accumulate the x_acc_val
                                                           // from the previous kernels through the cascade stream
                                        chess_separator_scheduler();
                                        chess_memory_fence();
                                        mid_prod_val_stream = readincr<aie_stream_resource_in::a>(inInterface.inStream);
                                        mid_prod_val =
                                            mid_prod_val +
                                            mid_prod_val_stream; // accumulate the x_acc_val from different kernels
                                    }

                                    if (kernel_role != 1) { // if it is down the leading kernel
                                        writeincr<aie_stream_resource_out::a, TT_DATA>(
                                            outInterface.outStream, mid_prod_val); // send the mid_prod_val
                                    }
                                }
                                ...
                                if (kernel_role == 1) { // leading kernel calculates the pp
                                    ...
                                    chess_separator_scheduler();
                                    chess_memory_fence();
                                    mid_prod_val = readincr<aie_stream_resource_in::a>(
                                        inInterface.inStream); // if not produced wait for the mid_prod_val
                                }

                                if ((kernel_role != 2) &&
                                    ((kernel_role != 3) || (TP_KERNEL_POS != 0))) { // stop transfer
                                    writeincr<aie_stream_resource_out::a, TT_DATA>(outInterface.outStream,
                                                                                   mid_prod_val);
                                }
                        ...
                        outQPtr = (vect_t*)outInterface.outWindowQ.data() + frame_offset_q +
                                  col_idx * m_kRowChunkNum; // move to the current column for Q output
                        for (int row = 0; row < col_chunk_in_calc; row++) {
                            *outQPtr++ = blank_vect;
                        }
            ...
            qrd_hh_q_casc_func(inInterface, outInterface,
                               frame); // call the qrd_hh_q function to start the reverse sweep for Q generation
        }
};
...
INLINE_DECL void
qrd_hh_kernel<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN, TP_KERNEL_POS, TP_ROUT_EN, TP_STREAM_EN>::
    qrd_hh_q_casc_func(T_inputIF<TP_STREAM_EN, TT_DATA>& inInterface,
                       T_outputIF<TP_STREAM_EN, TP_ROUT_EN, TT_DATA>& outInterface,
                       int& frame_id) {
    using acc_t = accTypeMult_t<TT_DATA, TT_DATA>;
    using vect_t = ::aie::vector<TT_DATA, vecSampleNum>;
    ...
    for (int col_chunk = m_kColChunkNum - 1; col_chunk >= 0; col_chunk--)
        chess_prepare_for_pipelining chess_loop_range(m_kColChunkNum, ) { // reverse sweep for v capture
            ...
            int kernel_role = m_kKernelRolesQ.kernel_role_arr[col_chunk];
            ;                               // 0 not leading, 1 next leading, 2 leading, 3 idle
            beta_vect = *betavectBuffPtr--; // get the beta vector for the current chunk
            for (int col = vecSampleNum - 1; col >= 0; col--)
                chess_prepare_for_pipelining chess_loop_range(vecSampleNum, ) {
                    // offset calculation for the row chunk that will be processed
                    ...
                    for (int i = 0; i < TP_DIM_COLS; i++) { // forward sweep for Q accumulation
                        ...
                        inAPtr = inAPtrBase;
                        if (kernel_role <
                            3) { // kernel is not idle, needs to calculate the mid_prod_val and update the q_mat
                            ...
                            if (kernel_role != 1) { // if it is down the leading kernel
                                writeincr<aie_stream_resource_out::a, TT_DATA>(outInterface.outStream,
                                                                               mid_prod_val); // send the mid_prod_val
                            }
                        } // if not idle
                        ...
                        if ((kernel_role != 2) && ((kernel_role != 3) || (TP_KERNEL_POS != 0))) { // stop transfer
                            writeincr<aie_stream_resource_out::a, TT_DATA>(
                                ...
                                mid_prod_val); // send the mid_prod_val to the previous kernel for the update
                        }
...
template <typename TT_DATA,
          ...
          bool TP_STREAM_EN>
NOINLINE_DECL void
qrd_hh<TT_DATA, TP_DIM_ROWS, TP_DIM_COLS, TP_NUM_FRAMES, TP_CASC_LEN, TP_KERNEL_POS, TP_ROUT_EN, TP_STREAM_EN>::
    qrd_hh_main(input_buffer<TT_DATA>& __restrict inWindowA,
