...
INLINE_DECL void substitution_base<TT_DATA,
                                   TP_DIM_SIZE,
                                   TP_SUBST_TYPE,
                                   TP_L_LEADING,
                                   TP_X,
                                   ...
                                   TP_GRID_DIM,
                                   TP_NUM_FRAMES,
                                   TP_DIAG_INV>::substitution_main(fbsub_input_params<TT_DATA>& input_params,
    ...
    vect_t* __restrict pL =
        (vect_t*)input_params.in_L; // L values are only ever read. They may be read as samples or vectors.
    vect_t* __restrict pLlocal = (vect_t*)input_params.in_L;  // a local L pointer is used to speed up a loop, while pL
                                                              // remains the anchor to the start of the iobuffer.
    TT_DATA* __restrict pLsamp = (TT_DATA*)input_params.in_L; // used to retrieve the special diagonal values.
    vect_t* __restrict pXv =
        (vect_t*)output_params.out_x; // actually not unique to region, but chess_memory_fence protects
    ...
    TT_DATA yj,
        sumj; // The calculation of each X is ultimately a scalar equation, needing scalar members of yvect and sum
    ...
    for (int frame = 0; frame < TP_NUM_FRAMES; frame++) {
        // Only kernels on the diagonal create an X output and only these need to invert the diagonal elements coming
        // in.
        if
            constexpr(TP_X == TP_Y) {
// zero output
#pragma unroll kVecPerRow
                ...
                // or, for pre-inverted diagonal elements, collect them together so they may later be accessed as a
                // vector
                for (int i = kStartRow; i < kEndRow; i++) {
                    if
                        constexpr(TP_DIAG_INV == 0) {
        ...
        //--------------------
        if
            constexpr(TP_SUBST_TYPE == 0) { // forward substitution

                for (int i = kStartRow / m_kvecSampleNum; i < kEndRow / m_kvecSampleNum;
                     i++) { // for all vertical chunks (each chunk is m_kvecSampleNum high)
                    if
                        constexpr(kStartCol == 0) { acc = ::aie::zeros<acc_t, m_kvecSampleNum>(); }
                    else {
                    ...
                    int looplim;
                    if
                        constexpr(TP_X == TP_Y) { looplim = i; }
                    else {
                        looplim = kEndCol / m_kvecSampleNum;
                    }
                    for (int k = kStartCol / m_kvecSampleNum; k < looplim;
                         k++) { // for each column chunk in the vertical chunk
                        if (k < kEndCol / m_kvecSampleNum) {
                            pLlocal = &pL[((k - kStartCol / m_kvecSampleNum) * m_kvecSampleNum) * kVecPerRow +
                                          (i - kStartRow / m_kvecSampleNum)];
                            if ((k < kStartRow / m_kvecSampleNum) &&
                                (i == kStartRow / m_kvecSampleNum)) { // x from higher kernel
                                xVec = readincr_v<m_kvecSampleNum, aie_stream_resource_in::a>(input_params.in_x);
                                xStore[k - kStartCol / m_kvecSampleNum] = xVec;
                            } else {
                                // x from local output
                                if
                                    constexpr(TP_X == TP_Y) { xVec = pXv[k - kStartCol / m_kvecSampleNum]; }
                                else {
                                    xVec = xStore[k - kStartCol / m_kvecSampleNum];
                                }
                            }
#pragma unroll m_kvecSampleNum
                            for (int p = 0; p < m_kvecSampleNum;
                                 p++) { // row within a block. chess pipelining and chess_loop_range(m_kvecSampleNum,)
                                        // is slower than unroll here
                                lVec = *pLlocal;
                                pLlocal += kVecPerRow;
                    ...
                    if
                        constexpr(TP_X == TP_Y) {
                            ...
                            for (int j = 0; j < m_kvecSampleNum; j++) {
                                if
                                    constexpr(TP_DIM_SIZE % m_kvecSampleNum != 0) {
                                        int z = i * m_kvecSampleNum + j; // Current row/element to resolve
                                        if (z >= TP_DIM_SIZE) continue;
                                    }
                                lVec = pL[i - kStartRow / m_kvecSampleNum +
                                          (i - kStartRow / m_kvecSampleNum) * m_kvecSampleNum * kVecPerRow +
                                          j * kVecPerRow];
                            ...
                            if
                                constexpr(TP_Y < TP_GRID_DIM - 1) { // i.e. not a bottom kernel
                                    writeincr<aie_stream_resource_out::a, TT_DATA, m_kvecSampleNum>(
                                        output_params.out_x_strm, xCache);
                                }
            ...
            for (int i = (kEndRow / m_kvecSampleNum) - 1; i + 1 >= kStartRow / m_kvecSampleNum + 1;
                 i--) { // for all vertical chunks, backwards
                if
                    constexpr(TP_X == 0) { // Kernels in the first physical column are the start of a pp chain.
                        acc = ::aie::zeros<acc_t, m_kvecSampleNum>();
                    }
                else {
                    acc = readincr_v<m_kvecSampleNum, acc_t>(input_params.in_pp);
                }
                for (int k = (kEndCol / m_kvecSampleNum) - 1; k + 1 >= kStartCol / m_kvecSampleNum + 1;
                     k--) {                     // for each column chunk in the vertical chunk, backwards
                    if (TP_X < TP_Y || k > i) { // Off-diagonal kernels process the whole rectangle, diagonal kernels
                                                // process the upper triangle.
                        pLlocal = &pL[((k - kStartCol / m_kvecSampleNum) * m_kvecSampleNum) * kVecPerRow +
                                      (i - kStartRow / m_kvecSampleNum)];

                        if
                            constexpr(TP_X < TP_Y) { // This kernel is off-diagonal, it consumes x from stream
                                if (i ==
                                    (kEndRow / m_kvecSampleNum) - 1) { // First pass of the i-loop, read from stream
                                    xVec = readincr_v<m_kvecSampleNum, aie_stream_resource_in::a>(input_params.in_x);
                                    xStore[k - kStartCol / m_kvecSampleNum] = xVec;
...
#pragma unroll m_kvecSampleNum
                ...
                if
                    constexpr(TP_X < TP_Y) { // Off-diagonal kernels write to the pp-cascade.
                        writeincr<acc_t, m_kvecSampleNum>(output_params.out_pp, acc);
                    }
                ...
                if
                    constexpr(TP_X == TP_Y) {
                        ...
                        for (int j = m_kvecSampleNum - 1; j + 1 >= 0 + 1; j--) {
                            ...
                            lVec =
                                pL[i - kStartRow / m_kvecSampleNum +
                                   (i - kStartRow / m_kvecSampleNum) * m_kvecSampleNum * kVecPerRow + j * kVecPerRow];
                            myInv = myInvs.get(j);
                            yj = yVect.get(j);
                            sumj = sumv.get(j);
                            newX = (yj - sumj) * myInv;
                            xCache.set(newX, j);
                            if
                                constexpr(isComplex<TT_DATA>()) { acc = ::aie::mac(acc, ::aie::op_conj(lVec), newX); }
                            else {
                                acc = ::aie::mac(acc, lVec, newX);
                            }
                            sumv = acc.template to_vector<TT_DATA>();
...
template <typename TT_DATA,
          ...
          unsigned int TP_DIAG_INV,
          typename TT_IO_DATA>
NOINLINE_DECL void substitution<TT_DATA,
                                TP_DIM_SIZE,
                                TP_SUBST_TYPE,
                                TP_L_LEADING,
                                TP_X,
                                ...
                                TP_NUM_FRAMES,
                                TP_DIAG_INV,
                                TT_IO_DATA>::substitution_single(input_buffer<TT_IO_DATA>& __restrict in_L,
...
NOINLINE_DECL void substitution<TT_DATA,
                                ...
                                TP_NUM_FRAMES,
                                TP_DIAG_INV,
                                TT_IO_DATA>::substitution_top(input_buffer<TT_IO_DATA>& __restrict in_L,
                                                              input_buffer<TT_DATA>& __restrict in_y,
...
substitution<TT_DATA,
             ...
             TP_NUM_FRAMES,
             TP_DIAG_INV,
             TT_IO_DATA>::substitution_first(input_buffer<TT_IO_DATA>& __restrict in_L,
                                             input_buffer<TT_DATA>& __restrict in_y,
...
substitution<TT_DATA,
             ...
             TP_NUM_FRAMES,
             TP_DIAG_INV,
             TT_IO_DATA>::substitution_mid(input_buffer<TT_IO_DATA>& __restrict in_L,
                                           input_buffer<TT_DATA>& __restrict in_y,
                                           input_stream<TT_DATA>* __restrict in_x,
...
NOINLINE_DECL void substitution<TT_DATA,
                                ...
                                TP_NUM_FRAMES,
                                TP_DIAG_INV,
                                TT_IO_DATA>::substitution_last(input_buffer<TT_IO_DATA>& __restrict in_L,
                                                               input_buffer<TT_DATA>& __restrict in_y,
                                                               input_cascade<accTypeFbsub<TT_DATA> >* __restrict in_pp,
                                                               output_buffer<TT_DATA>& __restrict out_x,
                                                               output_stream<TT_DATA>* __restrict out_x_strm) {
...
substitution<TT_DATA,
             ...
             TP_NUM_FRAMES,
             TP_DIAG_INV,
             TT_IO_DATA>::substitution_last_bottom(input_buffer<TT_IO_DATA>& __restrict in_L,
                                                   input_buffer<TT_DATA>& __restrict in_y,
                                                   input_cascade<accTypeFbsub<TT_DATA> >* __restrict in_pp,
                                                   output_buffer<TT_DATA>& __restrict out_x) {
    fbsub_input_params<TT_DATA> input_params = {(TT_DATA*)in_L.data(), in_y.data(), nullptr, in_pp};
