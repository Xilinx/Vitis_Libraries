...
void GramSchmidtKernelComplexFloat<ROW, COL, KN>::process(input_stream_cfloat* __restrict in_0,
...
template <int KN>
void GramSchmidtKernelComplexFloat_Start<KN>::process(input_stream_cfloat* in_0,
                                                      input_stream_cfloat* in_1,
                                                      output_stream_caccfloat* out) {
    v4cfloat Q[(1024 + 256) / 4];
    v4cfloat M[(1024 + 256) / 4];
    for (int kk = 0; kk < KN; kk++) {
        // 1. pass processed                        :load id x (r+c),   :write id x (r + c)
        if (column_id > 0) {
            v4cfloat* ptr_q = Q;
            for (int i = 0; i < (row_num + column_num) / 4; i++) chess_prepare_for_pipelining {
                    v4cfloat n0;
                    n0 = upd_v(n0, 0, as_v2cfloat(READINCRW(WSS_rsrc1, in_0)));
                    n0 = upd_v(n0, 1, as_v2cfloat(READINCRW(WSS_rsrc2, in_1)));
                    *ptr_q++ = n0;
        ...
        cfloat norm2, norm2_inv, norm_inv;
        {
            ...
            norm_inv.imag = 0;
            norm2.real = aie::sqrt(norm2.real);
            update(Q, norm2, row_num + column_id);
        }
        ...
        for (int j = column_id + 2; j < column_num; j++) {
            ...
            cfloat res_r = ext_elem(scale, 1);
            update(M, res_r, row_num + column_id);
            for (int i = 0; i < row_num / 4; i++) chess_prepare_for_pipelining {
                    v4cfloat u0 = *ptr_q++;
                    v4cfloat new_a0;
                    new_a0 = upd_v(new_a0, 0, as_v2cfloat(READINCRW(WSS_rsrc1, in_0)));
...
template <int KN>
void GramSchmidtKernelComplexFloat_Mid<KN>::process(input_stream_caccfloat* in, output_stream_caccfloat* out) {
    v4cfloat Q[(1024 + 256) / 4];
    v4cfloat M[(1024 + 256) / 4];
    for (int kk = 0; kk < KN; kk++) {
        // 1. pass processed                        :load id x (r+c),   :write id x (r + c)
        ...
        for (int j = column_id + 2; j < column_num; j++) {
            ...
            for (int i = 0; i < row_num / 4; i++) chess_prepare_for_pipelining {
                    v4cfloat u0 = *ptr_q++;
                    v4cfloat new_a0 = get_scd_v4cfloat();
                    v4cfloat old_a0 = *ptr_m;
                    *ptr_m++ = new_a0;
                    ac0 = fpmac_cn(ac0, u0, new_a0);
                    v4cfloat update_old_a0 = fpmsc(old_a0, scale, 0, 0, u0, 0, 0x3210);
            ...
            for (int i = 0; i < column_num / 4; i++) chess_prepare_for_pipelining {
                    v4cfloat old_r0 = *ptr_m;
                    put_mcd(old_r0);
                    v4cfloat new_r0 = get_scd_v4cfloat();
                    *ptr_m++ = new_r0;
                }
...
template <int KN>
void GramSchmidtKernelComplexFloat_End<KN>::process(input_stream_caccfloat* in,
