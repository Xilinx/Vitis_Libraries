...
.. meta::
   ...
   :description: The singular value decomposition (SVD) is a very useful technique to deal with general dense matrix problems.
...
The `singular value decomposition` (SVD) is a useful technique to deal with general dense matrix problems. In recent years, SVD has become a computationally viable tool to solve a wide variety of problems raised in many practical applications, such as least squares data fitting, image compression, facial recognition, principal component analysis, latent semantic analysis, and computing the 2-norm, condition number, and numerical rank of a matrix. 

For more information, refer to **"Jack Dongarra, Mark Gates, Azzam Haidar. The Singular Value Decomposition: Anatomy of Optimizing an Algorithm for Extreme Scale. 2018 SIAM Review, vol.60, No.4, pp.808-865"**
...
Each plane rotation, :math:`J_{k} = J_{k}(i, j, \theta)`, is now called a Jacobi or Givens rotation
...
where :math:`\hat{A}` is a 2X2 submatrix of matrix A. After the Givens rotations of the whole matrix A, the off-diagonal value of A is reduced to zero-like value after 3-15 times iteration of the process.
