...
This feature is available when the template parameter ``TP_DYN_PT_SIZE`` is set to 1.
...
When Super Sample Rate operation is used, data is input and output using multiple ports.
These multiple ports on input or output act as one channel.
The mapping of samples to ports is that each successive sample should be passed to a different port in a round-robin fashion, e.g. with ``TP_SSR`` set to 4, samples 0, 4, 8, ... should be sent to input port 0, samples 1, 5, 9, ... to input port 1, samples 2, 6, 10, ... to input port 2, sample 3, 7, 11, ... to input port 3 and so on.
