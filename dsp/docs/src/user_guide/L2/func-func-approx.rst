...
The Function Approximation library element provides a vectorized linear approximation of a function, f(x), for a given input data, x, using a configured lookup table of slope and offset values that describe the a function. 
...
.. code-block:: c  
    ...
    output = offset[index] + slope[index] * input[TP_FINE_BITS:0]  
...
There will be `2^TP_COARSE_BITS` in the lookup table. Each section will contain a slope value and an offset value which represent the linear approximation of the function at the corresponding section of the domain. 
...
|                |                    |                                                           |                         | sections in the lookup tables is half that of other TP_DOMAIN_MODES. This applies to floating-point values where float values    |  
|                |                    |                                                           |                         | 0 to 1 will correspond to a domain of 1 to 2 for the function.                                                                   |  
+----------------+--------------------+-----------------------------------------------------------+-------------------------+----------------------------------------------------------------------------------------------------------------------------------+  
| 2              | 1 <= x < 4         | 2 ^ (TP_COARSE_BITS + TP_FINE_BITS - 2) <= int(x) < 2 ^   | 1 <= float(x) < 4       | Lookup tables values for the first quadrant of sections are created but are ignored. As such, the latter three quadrants of the  |  
...
The functions, as well as their recommended domain modes, are documented in :ref:`LINK_TO_API_REFERENCE`.  
