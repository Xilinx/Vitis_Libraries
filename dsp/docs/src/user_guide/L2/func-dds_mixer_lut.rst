...
AIE1: On AIE1, the dds_mixer_lut supports cint16, cint32 and cfloat as TT_DATA type that specifies the type of input and output data. Input is only required when TP_MIXER_MODE is
set to 1 (simple mixer) or 2 (dual conjugate mixer).  
AIE2: The AIE2 hardware does not support cfloat data type, so the dds_mixer_lut also does not support this data type. Only cint16 and cint32 are supported.
...
Please note that on AIE2 devices, outputs of the streaming and windowed modes are not bitwise identical due to some implementation choices made for higher performance.
...
When configured as a DDS (TP_MIXER_MODE=0) the output of the DDS is intended to be the components of a unit vector. For TT_DATA = cfloat, this means that the outputs will be in the range -1.0 to +1.0. For TT_DATA=cint16 and cint32 the output is scaled by 2 to the power 15 and 31 respectively such that the binary point follows the most significant bit of the output. Therefore, if the DDS output is used to multiply/mix, you must account for this bit shift.
