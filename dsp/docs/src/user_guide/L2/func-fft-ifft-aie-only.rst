...
Super Sample Rate operation (``TP_PARALLEL_POWER``>0) supports both  iobuffer IO (``TP_API`` == 0) or stream input (``TP_API`` == 1). For iobuffer IO, there is one port per lane of processing (number of lanes= 2^ ``TP_PARALLEL_POWER``). For stream IO the number of ports is the number of lanes multiplied by the number of streams per tile (2 for AIE, 1 for AIE-ML).
