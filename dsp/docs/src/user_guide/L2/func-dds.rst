...
The DSPLib contains two different DDS/Mixer implementations. These will be referred to as the dds_mixer and dds_mixer_lut. They both differ in their implementations of the DDS and hence do not have identical outputs. The dds_mixer solution is available only on AI Engine (AIE), while the dds_mixer_lut is available on both AIE and AIE-ML. The dds_mixer_lut has modes that enable higher SFDR figures for the signal generator than the dds_mixer. Both implementations have three different modes of operation. The mode of the component is driven by template parameter ``TP_MIXER_MODE``.
