...
The AMD Vitis |trade| DSP Library offers a fully synthesizable Super Sample Rate (SSR) FFT with a systolic architecture to process multiple input samples every clock cycle. The number of samples
...
2. Feed data to the input FIFO and read from the output FIFO:
