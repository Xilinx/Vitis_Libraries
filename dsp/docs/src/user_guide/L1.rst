...
acceptable SNR/RMS error at the output, switch to **SSR_FFT_GROW_TO_MAX_WIDTHi**. If there is still an acceptable SNR/RMS error, switch to **SSR_FFT_SCALE**, and observe the SNR/RMS again if it
...
to the DSP blocks, it allows the bit growth. When the bit width grows beyond what can be mapped to zthe DSP blocks, it will start scaling the output.
