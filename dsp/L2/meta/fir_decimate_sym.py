...
def update_TP_FIR_LEN(args):
    ...
    return fn_update_TP_FIR_LEN(
        TT_DATA, TT_COEFF, TP_FIR_LEN, TP_API, TP_USE_COEFF_RELOAD
    )


def fn_update_TP_FIR_LEN(TT_DATA, TT_COEFF, TP_FIR_LEN, TP_API, TP_USE_COEFF_RELOAD):
    symFactor = 2
    TP_FIR_LEN_max_temp = comFirUpd.fn_max_fir_len_each_kernel_update(
...
def fn_validate_TP_FIR_LEN(TT_DATA, TT_COEFF, TP_FIR_LEN, TP_API, TP_USE_COEFF_RELOAD):
    ...
    return isError(
        f"TP_FIR_LEN should be a multiple of one of the possible TP_DECIMATE_FACTOR legal set values : {legal_set_dec_fact}"
    )
...
def fn_update_TP_DECIMATE_FACTOR(TT_DATA, TT_COEFF, TP_FIR_LEN):
    ...
    param_dict = {"name": "TP_DECIMATE_FACTOR", "enum": legal_set_deci_factor}
    return param_dict
...
def validate_TP_DECIMATE_FACTOR(args):
    ...
    return fn_validate_TP_DECIMATE_FACTOR(
        TT_DATA, TT_COEFF, TP_FIR_LEN, TP_DECIMATE_FACTOR
    )
...
def fn_update_TP_DUAL_IP(AIE_VARIANT):
    ...
    if AIE_VARIANT == AIE_ML or AIE_VARIANT == AIE_MLv2:
        legal_set_dual_ip = [0]
    param_dict = {"name": "TP_DUAL_IP", "enum": legal_set_dual_ip}
    return param_dict
...
def fn_update_TP_CASC_LEN(
    ...
    symFactor = 2
    symFactorSSR = (
        1 if (TP_SSR != 1) else symFactor
    )  # SSR mode will discard the symmetry
    symApiSSR = (
    ...
    if TP_SSR > 1 or AIE_VARIANT == AIE_ML or AIE_VARIANT == AIE_MLv2:
        # AIE-ML doesn't support symmtery.
        # Also, can't decompose a symmetric FIR into SSR structure.
        legal_set_casc3 = dec_asym.fn_eliminate_casc_len_data_needed_within_buffer_size(
            TT_DATA,
            TT_COEFF,
            TP_FIR_LEN,
            TP_API,
...
def fn_update_input_window_size(
    ...
    if TP_API == API_BUFFER:
        TP_INPUT_WINDOW_VSIZE_max = int(
            comFirUpd.fn_max_windowsize_for_buffer_update(
                TT_DATA,
                TP_FIR_LEN,
                TP_SSR,
    ...
    factor_ws = calc_factor_ws(
        TT_DATA, TT_COEFF, AIE_VARIANT, TP_API, TP_DECIMATE_FACTOR, TP_SSR
    )
...
def calc_factor_ws(TT_DATA, TT_COEFF, AIE_VARIANT, TP_API, TP_DECIMATE_FACTOR, TP_SSR):
    ...
    windowSizeMultiplier = (
        fn_decimate_asym_lanes(TT_DATA, TT_COEFF, AIE_VARIANT)
        * TP_DECIMATE_FACTOR
        * repeatFactor
    )  # if TP_API == API_BUFFER else (fn_decimate_asym_lanes(TT_DATA, TT_COEFF)
...
def info_ports(args):
    ...
    rtp_port_length = (
        TP_FIR_LEN
        if AIE_VARIANT == AIE_ML or AIE_VARIANT == AIE_MLv2 or TP_SSR > 1
        else (TP_FIR_LEN + 1) / 2
    )
    ...
    coeff_ports = (
        get_parameter_port_info(
            "coeff", "in", TT_COEFF, TP_SSR, rtp_port_length, "async"
        )
    ...
    out_ports = get_port_info(
        "out", "out", TT_DATA, out_win_size, num_out_ports, TP_API=TP_API
    )
    out2_ports = (
        get_port_info(
            "out2", "out", TT_DATA, out_win_size, num_out_ports, TP_API=TP_API
        )
