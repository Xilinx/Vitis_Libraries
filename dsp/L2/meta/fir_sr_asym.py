...
def fn_fir_len_divisible_ssr(TP_FIR_LEN, TP_SSR): # when rate changers get SSR do scaling with TP_INTERPOLATE_FACTOR=1, TP_DECIMATE_FACTOR=1
  ...
  if TP_FIR_LEN % TP_SSR != 0 :
    return isError(f"TP_FIR_LEN ({TP_FIR_LEN}) needs to be divisible by TP_SSR ({TP_SSR}).")
  return isValid
...
def fn_get_data_needed( TT_DATA,  TT_COEF, TP_FIR_LEN, TP_CASC_LEN, TP_KERNEL_POSITION, TP_SSR, TP_API):
  ...
  m_kInitDataNeeded = ((m_kArchFirLen + m_kLanes* m_kDataLoadVsize // m_kVOutSize) - 1);
...
def fn_validate_num_outputs(TP_NUM_OUTPUTS, TP_API, TP_SSR):
  ...
  if (TP_SSR > 1 and TP_NUM_OUTPUTS > 1 and TP_API == 0 ) :
    return isError("Dual output ports not supported when port API is a window and SSR > 1.")
...
def info_ports(args):
    ...
    out_ports = get_port_info("out", "out", TT_DATA, TP_INPUT_WINDOW_VSIZE, TP_SSR, TP_API=args["TP_API"])
    out2_ports = (get_port_info("out2", "out", TT_DATA, TP_INPUT_WINDOW_VSIZE, TP_SSR, TP_API=args["TP_API"]) if (args["TP_NUM_OUTPUTS"] == 2) else [])
