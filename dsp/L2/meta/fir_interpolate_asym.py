...
def fn_validate_fir_len(TT_DATA, TT_COEF, TP_FIR_LEN, TP_INTERPOLATE_FACTOR, TP_CASC_LEN, TP_SSR, TP_API, TP_USE_COEF_RELOAD, TP_DUAL_IP):
    ...
    maxLenCheck = fn_max_fir_len_each_kernel(TT_DATA, TP_FIR_LEN, TP_CASC_LEN, TP_USE_COEF_RELOAD, TP_SSR, TP_API, coeffSizeMult)
...
def fn_validate_para_interp_poly(TP_INTERPOLATE_FACTOR, TP_PARA_INTERP_POLY):
    ...
    if TP_INTERPOLATE_FACTOR % TP_PARA_INTERP_POLY != 0:
        return isError(
            f"TP_PARA_INTERP_POLY must be a number that is exactly divisible by TP_INTERPOLATE_FACTOR. Please update TP_PARA_INTERP_POLY."
        )
    return isValid
