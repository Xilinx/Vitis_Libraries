    ...
    while True:
      ...
      if self.type != "string":
        if ((user_input.isnumeric()) or (user_input== "")):
          ...
          self.value  = int(self.value) #wooow python:D str assigned to itself as integer 
          break          
        else:
          print_message("ERROR: Input should be numerical!", should_print_message)
      else:
        self.value  = user_input or self.default
        break
...
class IP:
  ...
  def validate_all(self, print_err=1):
    for param in self.params:
      ...
      self.params_valid_results_dict.update({param:self.param_obj_dict[param].valid})
      self.params_valid_errors_dict.update({param:self.param_obj_dict[param].err_msg})
  ...
  def update_all(self):
    for param in self.params:
      ...
      self.param_obj_dict[param].param_validate(self.args, self.module, print_err=0)
      if (self.param_obj_dict[param].valid=="False"):
        break
      del self.param_obj_dict[param].update_result["name"]
      self.params_update_results_dict.update({param:self.param_obj_dict[param].update_result}) 

  def generate_graph(self, graph_name): #all parameters should be valid to run generate_graph
