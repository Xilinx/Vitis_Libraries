...
def validate_json(data, schema, error_messages):  
    ...
    if not schema.get("additionalProperties", True):  
        ...
        if extra_fields:  
            error_messages.update({"schema check error": f"Extra fields found in JSON data: {extra_fields}"})  
            return False  
  
    # Check required fields  
    for field in schema.get("required", []):  
        if field not in data:  
            error_messages.update({"schema check error": f"Missing required field: {field}"})  
            return False  
    ...
    for field, field_schema in schema.get("properties", {}).items():  
        if field in data:  
            if not validate_field(data[field], field_schema, error_messages):  
                error_messages.update({"schema check error": f"Field '{field}' is invalid."})  
                return False  
...
def validate_field(value, field_schema, error_messages):  
    ...
    if expected_type == "string":  
        if not isinstance(value, str):  
            error_messages.update({"schema check error": f"Expected string for value: {value}"})  
            return False  
    elif expected_type == "integer":  
        if not isinstance(value, int):  
            error_messages.update({"schema check error": f"Expected integer for value: {value}"})  
            return False  
    elif expected_type == "array":  
        if not isinstance(value, list):  
            error_messages.update({"schema check error": f"Expected array for value: {value}"})  
            return False  
        ...
        if not isinstance(value, dict):  
            error_messages.update({"schema check error": f"Expected object for value: {value}"})  
            return False  
...
def check_parameters(json_data, config_params, module, error_messages):  
    ...
    if "parameters" not in json_data:  
        error_messages.update({"dependency check error": "No parameters found in JSON data."}) 
        return False  
  
    parameters = json_data["parameters"] 
  
    for index, param in enumerate(parameters):  #sweep parameters for args, validator and updater checks

        # Check for validator and updater  
        if "validator" not in param or "updater" not in param:  
            error_messages.update({"dependency check error": f"Parameter '{param['name']}' is missing a validator or updater."}) 
            return False  
        ...
        if param["name"] in validator_args:  
            error_messages.update({"dependency check error": f"Parameter '{param['name']}' should not include itself in its validator args."})   
            return False  
        ...
        if not all(arg in previous_parameter_names for arg in validator_args):  
            ...
            for param_name in validator_args:
                ...
                error_messages.update({"dependency check error": f"Validator args for parameter '{param['name']}' include invalid names: {prior_args}"})   
            return False  
        ...
        if param["name"] in updater_args:  
            error_messages.update({"dependency check error": f"Parameter '{param['name']}' should not include itself in its updater args."})   
            return False   
        # Check args in validator
        if not all(arg in previous_parameter_names for arg in updater_args):  
            ...
            for param_name in updater_args:
                if param_name not in previous_parameter_names:
                    prior_args.append(param_name)
                error_messages.update({"dependency check error": f"Updater args for parameter '{param['name']}' include invalid names: {prior_args}"})  
            return False  
        ...
        update_args_dict.update({param["name"]: config_params[param["name"]]})#add the parameter itself in case needed
        param_under_test.param_update(update_args_dict, module) ##will throw a python error if there is missing args
        
        for arg in validator_args:
            validate_args_dict.update({arg: config_params[arg]})
        validate_args_dict.update({param["name"]: config_params[param["name"]]}) #add the parameter itself in case needed
        param_under_test.param_validate(validate_args_dict, module, print_err="False")  ##will throw a python error if there is missing args

        #Validate the configuration
        param_under_test.param_validate(config_params, module, print_err="False")  ##will throw a python error if there is missing args
        if param_under_test.valid == "False":
            error_messages.update({"configuration_validation_error" :f"{param_under_test.err_msg}"}) 
            return False 
...
def compare_parameter_names(ip_name, metadata_json_data, config_json_data, error_messages): 
    print("Starting config.json checks...") 
    metadata_params = get_parameter_names_from_metadata_json(ip_name, metadata_json_data)  
    ...
    if missing_in_metadata:
        error_messages.update({"dependency check error": f"Parameter Mismatch! {missing_in_metadata} found in config.json but not in {ip_name}.json."})   
    ...
    if unexpected_in_metadata:  
        error_messages.update({"dependency check error": f"Parameter Mismatch! {unexpected_in_metadata} found in {ip_name}.json but not in config.json."})   
...
def get_canary_test(ip_name):
    ...
    if default_params_load==None and multi_params_load==None:
        raise FileNotFoundError(f"Both {default_params_path} and {multi_params_path} files do not exist!")
...
def extract_canary_test_args(ip_name, config_params):
    ...
    canary_args={}
    for param in canary_test: #Translate graph level parameters to kernel level for update and validate
        new_config=config_translate(param, configTranslatePath, back_translate=0, canary_test=canary_test)
        if new_config in config_params:
            if isinstance(canary_test[param], str) and is_valid_integer(canary_test[param]):
                canary_args.update({new_config:int(canary_test[param])})
            else: canary_args.update({new_config:canary_test[param]})
    return canary_args
...
def test_config_helper(ip_name):
    ...
    config_helper_test_result = subprocess.run(  
        [sys.executable, os.path.join(L2_dir, "meta/scripts/config_helper.py"), "--ip", ip_name, "--test_config_helper", config_test_json_path])
...
def gen_out_json_error(ip_name, error_messages):
    ...
    with open(out_json_path, 'w') as out_json_error: 
        json.dump(error_messages, out_json_error, indent=4)  # indent for pretty printing
...
def port_info_map(ports_info):
    ...
    for port_info in ports_info:
        port_info_mapped={"name"        : port_info["name"],
        ...
        if port_info["type"] == "window":
            ...
            port_info_mapped.update({
                "numElements"  : int(port_info["window_size"] / get_size_byte(port_info["data_type"])),
                "windowsize"   : port_info["window_size"],
                "isArray"      : True,
                "synchronicity": "sync"
            })
...
def get_size_byte(port_type):
    if   port_type == "uint8"     : return 1
    if   port_type == "int8"      : return 1
    elif port_type == "int16"     : return 2
    elif port_type == "int32"     : return 4
    elif port_type == "cint16"    : return 4
    elif port_type == "cint32"    : return 8
    elif port_type == "float"     : return 4
    elif port_type == "cfloat"    : return 8
    elif port_type == "bfloat16"  : return 2
    else: return 0

def isComplexType(typeStr):
    if (typeStr == "cfloat" or typeStr == "cint16" or typeStr == "cint32" or typeStr == "cint64" or typeStr == "cacc48"):
           return True
    return False
...
if __name__ == "__main__":  
    ...
    schema_file_path = f"{XILINX_VITIS}/data/ipmetadata/vitis_library_api_spec_schema.json"
    ...
    error_messages={}
    check_exit(validate_json(json_data, schema, error_messages), error_messages, ip_name) 
    check_exit(compare_parameter_names(ip_name,json_data, config_json_data, error_messages), error_messages, ip_name)
    check_exit(check_parameters(json_data, config_json_data["parameters"], module, error_messages), error_messages, ip_name)
