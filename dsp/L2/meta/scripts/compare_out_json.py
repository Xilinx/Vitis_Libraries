...
def compare_json(json1, json2, path=""):  
    ...
    if isinstance(json1, dict) and isinstance(json2, dict):  
        ...
        if len(json1) != len(json2):  
            differences.append(f"Different lengths at {path}: {len(json1)} != {len(json2)}")  
        else:  
            # Sort lists for comparison  
            sorted_json1 = sorted(json1, key=lambda x: json.dumps(x, sort_keys=True) if isinstance(x, dict) else x)  
            sorted_json2 = sorted(json2, key=lambda x: json.dumps(x, sort_keys=True) if isinstance(x, dict) else x)  
