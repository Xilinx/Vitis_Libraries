...
def fn_update_TP_CASC_LEN(
    ...
    symApiSSR = (
        0 if (TP_SSR == 1 and TP_PARA_INTERP_POLY == 1) else TP_API
    ...
    if (AIE_VARIANT == AIE_ML or AIE_VARIANT == AIE_MLv2) and TP_API == API_STREAM:
        legal_set_casc3 = fn_eliminate_casc_len_data_needed_within_buffer_size_ml(
            TT_DATA, TP_FIR_LEN, TP_SSR, legal_set_casc3.copy()
        )
...
def fn_eliminate_casc_len_data_needed_within_buffer_size_ml(
    TT_DATA, TP_FIR_LEN, TP_SSR, legal_set_casc_len
):
    ...
    for casc_len in legal_set_casc_len.copy():
        for TP_KP in range(casc_len):
            m_kFirRangeLen = (
                fnFirRangeRem(firLenPerSSR, casc_len, TP_KP, 2)
                if (TP_KP + 1 == casc_len)
                else fnFirRange(firLenPerSSR, casc_len, TP_KP, 2)
            )
            m_kNumTaps = (
                ((m_kFirRangeLen + 1) / 2)
                if (TP_KP == casc_len - 1)
                else (m_kFirRangeLen / 2)
            )
            m_kWinAccessByteSize = 32
            m_kFirRangeOffset = (
                sr_asym.fnFirRangeOffset(
                    firLenPerSSR, casc_len, TP_KP, kInterpolateFactor
                )
                / kInterpolateFactor
            )
            m_kFirAsymTaps = (firLenPerSSR + 1) / kInterpolateFactor
            m_kFirMarginOffset = (
                sr_asym.fnFirMargin(m_kFirAsymTaps, TT_DATA) - (m_kFirAsymTaps) + 1
            )
...
def update_TP_NUM_OUTPUTS(args):
    ...
    return comFirUpd.fn_update_hb_num_outputs(
        TP_PARA_INTERP_POLY, TP_DUAL_IP, "TP_NUM_OUTPUTS"
    )
...
def fn_update_input_window_size(
    ...
    if TP_API == API_BUFFER:
        TP_INPUT_WINDOW_VSIZE_max_int = comFirUpd.fn_max_windowsize_for_buffer_update(
            TT_DATA, TP_FIR_LEN, TP_SSR, TP_INTERPOLATE_FACTOR=2, TP_DECIMATE_FACTOR=1
        )
...
def info_ports(args):
    ...
    in_ports = get_port_info(
        "in", "in", TT_DATA, in_win_size, num_in_ports, margin_size, TP_API=TP_API
    )
    in2_ports = (
        get_port_info(
            "in2", "in", TT_DATA, in_win_size, num_in_ports, margin_size, TP_API=TP_API
        )
        if (TP_DUAL_IP == 1)
        else []
    )
    ...
    coeffCT_ports = (
        get_parameter_port_info("coeffCT", "in", TT_COEFF, TP_SSR, 1, "async")
        if (TP_USE_COEFF_RELOAD == 1 and TP_PARA_INTERP_POLY > 1)
        else []
    )

    # interp by 2 for halfband
    out_ports = get_port_info(
        "out", "out", TT_DATA, out_win_size, num_out_ports_per_poly, TP_API=TP_API
    )
    out2_ports = (
        get_port_info(
            "out2", "out", TT_DATA, out_win_size, num_out_ports_per_poly, TP_API=TP_API
        )
        if (TP_NUM_OUTPUTS == 2)
        else []
    )
    out3_ports = (
        get_port_info(
            "out3", "out", TT_DATA, out_win_size, num_out_ports_per_poly, TP_API=TP_API
        )
        if (TP_PARA_INTERP_POLY > 1)
        else []
    )
    out4_ports = (
        get_port_info(
            "out4", "out", TT_DATA, out_win_size, num_out_ports_per_poly, TP_API=TP_API
        )
        if (TP_PARA_INTERP_POLY > 1 and TP_NUM_OUTPUTS == 2)
        else []
    )
    return (
        in_ports
...
def generate_graph(graphname, args):
    ...
    coeffCT_ip_declare_str = (
        f"ssr_port_array<input> coeffCT;"
        if (TP_USE_COEFF_RELOAD == 1 and TP_PARA_INTERP_POLY > 1)
        else "// No coeffCT port"
    )
    coeffCT_ip_connect_str = (
        f"adf::connect<> net_coeffCT(coeffCT[i], filter.coeffCT[i]);"
        if (TP_USE_COEFF_RELOAD == 1 and TP_PARA_INTERP_POLY > 1) == 1
        else "// No coeffCT port"
    )
    dual_op_declare_str = (
    ...
    op3_connect_str = (
        f"adf::connect<> net_out3(filter.out3[i], out3[i]);"
        if TP_PARA_INTERP_POLY > 1
        else ""
    )
    op4_declare_str = (
        f"ssr_port_array<output> out4;"
        if (TP_PARA_INTERP_POLY > 1 and TP_NUM_OUTPUTS == 2)
        else ""
    )
    op4_connect_str = (
        f"adf::connect<> net_out4(filter.out4[i], out4[i]);"
        if (TP_PARA_INTERP_POLY > 1 and TP_NUM_OUTPUTS == 2)
        else ""
    )
