...
def validate_TP_MAX_DELAY(args):
  ...
  if (TP_API): # stream interface
    if (TP_MAX_DELAY % 128):
      return com.isError(f"Min value of TP_MAX_DELAY for TP_API = {TP_API} is 128. The legal values of TP_MAX_DELAY are 2^n where n = 7, 8, 9 ...")
    return com.isValid
  else: # iobuff (window) interface
    if (TP_MAX_DELAY % VEC_SIZE) :
      return com.isError(f"TP_MAX_DELAY for TP_API = {TP_API} should be an integer multiple of vector size. Vector size for the data type {TT_DATA} is {VEC_SIZE}")
  return com.isValid
...
def validate_TP_WINDOW_VSIZE(args):
  ...
  if (TP_API): # stream interface
    if (TP_WINDOW_VSIZE % TP_MAX_DELAY):
      return com.isError(f"TP_WINDOW_VSIZE for TP_API = {TP_API} should be an integer multiple of TP_MAX_DELAY")
    return com.isValid
  else: # iobuff (window) interface
    if (TP_WINDOW_VSIZE % VEC_SIZE):
      return com.isError(f"TP_WINDOW_VSIZE for TP_API = {TP_API} should be an integer multiple of vector size. Vector size for the data type {TT_DATA} is {VEC_SIZE}")
  return com.isValid
