...
int main(int argc, char** argv) {
    ...
    ap_uint<k_memWidthBits>* inData = aligned_alloc<ap_uint<512> >(k_numRows * k_numCols * k_numImages / k_memWidth);
    ap_uint<k_memWidthBits>* outData = aligned_alloc<ap_uint<512> >(k_numRows * k_numCols * k_numImages / k_memWidth);
        ...
        for (int r = 0; r < k_numRows; r++) {
            for (int c = 0; c < k_numCols / k_memWidth; c++) {
                ...
                for (int w = 0; w < k_memWidth; w++) {
                    if (r == 0 && c == 0 && w == 0) {
                        inData[n * k_numRows * k_numCols / k_memWidth + r * k_numCols / k_memWidth + c].range(31, 0) =
                            1;
                        inData[n * k_numRows * k_numCols / k_memWidth + r * k_numCols / k_memWidth + c].range(63, 32) =
                            1;
                    }
                }
            }
