   ...
   public:
    ...
    test_graph() {
...
#ifdef USING_UUT
        static constexpr int kMaxTaps = uut_g<INPUT_SAMPLES>::fnGetMaxTapsPerKernel<PORT_API, DATA_TYPE>();
        printf("For this config the Maximum Taps Per Kernel is %d\n", kMaxTaps);
        static constexpr int kMinLen = uut_g<INPUT_SAMPLES>::fnGetMinCascLen<FIR_LEN, PORT_API, DATA_TYPE, COEFF_TYPE,
        ...
        static constexpr int kOptLen = uut_g<INPUT_SAMPLES>::fnGetOptCascLen<FIR_LEN, DATA_TYPE, COEFF_TYPE, PORT_API,
                                                                             NUM_OUTPUTS, INTERPOLATE_FACTOR>();
