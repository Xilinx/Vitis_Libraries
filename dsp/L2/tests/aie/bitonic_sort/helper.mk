...
PARAM_MAP = AIE_VARIANT $(AIE_VARIANT) DATA_TYPE $(DATA_TYPE) DIM_SIZE $(DIM_SIZE) NUM_FRAMES $(NUM_FRAMES) ASCENDING $(ASCENDING) CASC_LEN $(CASC_LEN)
...
sim_ref:
	make UUT_KERNEL=bitonic_sort_ref UUT_SIM_FILE=./data/ref_output.txt run TARGET=x86sim TAG=REF |& tee -a ref_log.txt
...
get_stats:
	tclsh $(HELPER_ROOT_DIR)/L2/tests/aie/common/scripts/get_stats.tcl $(DIM_SIZE) 1 $(STATUS_FILE) ./aiesimulator_output "bitonic_sort_main" $(NITER)
