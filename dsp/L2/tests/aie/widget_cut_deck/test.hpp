...
class test_graph : public graph {
   public:
    ...
    test_graph() {
        printf("========================\n");
        printf("== Widget Cut Deck test.hpp parameters: ");
        ...
        dsplib::widget::deck_cut::UUT_GRAPH<DATA_TYPE, WINDOW_VSIZE, IDX> widget_cut_deckGraph;

        // Make connections
        std::string filenameIn = QUOTE(INPUT_FILE);
        in[0] = input_plio::create("PLIO_in_" + std::to_string(0), adf::plio_64_bits, filenameIn);
        connect<>(in[0].out[0], widget_cut_deckGraph.in[0]);
        ...
        connect<>(widget_cut_deckGraph.out[0], out[0].in[0]);
#else
        // widget_2ch_real_fft_graph sub-graph
        dsplib::widget::deck_cut::UUT_GRAPH<DATA_TYPE, WINDOW_VSIZE, IDX> widget_cut_deckGraph;
        ...
        std::string filenameIn = QUOTE(INPUT_FILE);
        in[0] = input_plio::create("PLIO_in_" + std::to_string(0), adf::plio_64_bits, filenameIn);
        connect<>(in[0].out[0], widget_cut_deckGraph.in);

        std::string filenameOut = QUOTE(OUTPUT_FILE);
        out[0] = output_plio::create("PLIO_out_" + std::to_string(0), adf::plio_64_bits, filenameOut);
        connect<>(widget_cut_deckGraph.out, out[0].in[0]);
#endif
    };
};
}
