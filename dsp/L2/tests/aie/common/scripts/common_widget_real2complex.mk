...
UUT_PARAMS_CONCAT   = $(DATA_TYPE)_$(DATA_OUT_TYPE)_$(WINDOW_VSIZE)
...
PREPROC_ARGS       = -DINPUT_FILE=$(LOC_INPUT_FILE) -DDATA_TYPE=$(DATA_TYPE) -DDATA_OUT_TYPE=$(DATA_OUT_TYPE) -DWINDOW_VSIZE=$(WINDOW_VSIZE) -DSTIM_TYPE=$(STIM_TYPE) -DUUT_TARGET_ENUM=$(UUT_TARGET_ENUM)
REF_PREPROC_ARGS   = "-DUUT_GRAPH=$(REF_GRAPH) -DOUTPUT_FILE=$(REF_SIM_FILE) $(PREPROC_ARGS)  -DWINDOW_VSIZE=$(REF_WINDOW_VSIZE) -DNITER=$(NITER_REF)"
UUT_PREPROC_ARGS   = "-DUUT_GRAPH=$(UUT_GRAPH) -DOUTPUT_FILE=$(UUT_SIM_FILE) $(PREPROC_ARGS)  -DWINDOW_VSIZE=$(WINDOW_VSIZE) -DNITER=$(NITER_UUT) -DUSING_UUT=$(USING_UUT_YES)"
...
create:
	@echo Start testing|& tee $(LOG_FILE);\
	 ...
	 echo "Configuration in common_widget.mk:"               >> $(STATUS_FILE);\
	 echo "    UUT_KERNEL:           " $(UUT_KERNEL)         >> $(STATUS_FILE);\
	 echo "    DATA_TYPE:            " $(DATA_TYPE)          >> $(STATUS_FILE);\
	 echo "    DATA_OUT_TYPE:        " $(DATA_OUT_TYPE)      >> $(STATUS_FILE);\
...
get_status_ref:
	...
	if [ $(GET_STATS) = true -a `cat compile_exit` -eq 0 ]; then \
		tclsh $(DSPLIB_REPO)/L2/scripts/get_stats.tcl $(DATA_TYPE) cint16 1 $(WINDOW_VSIZE) 1 1 1 1 0 0 1 $(STATUS_FILE) $(UUT_OUT_DIR) "convertData" $(NITER_UUT);\
