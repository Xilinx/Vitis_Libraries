...
# if NITER = 16 then we run for 8 iteractions 
NITER 			   = 16
NITER_UUT          = $(NITER)
...
TEST_PARAMS_CONCAT  = $(UUT_PARAMS_CONCAT)$(MAKE_PARAMS_CONCAT)
...
ifeq ($(USE_AIE_API), 1)
	PREPROC_ARGS += -D_DSPLIB_FIR_AIE_API_DEBUG_=1
endif
...
get_status_ref:
	...
	if [ $(GET_STATS) = true -a `cat compile_exit` -eq 0 ] ; then \
		tclsh $(DSPLIB_REPO)/L2/scripts/get_stats.tcl $(DATA_TYPE) $(INPUT_WINDOW_VSIZE) $(STATUS_FILE) $(UUT_OUT_DIR) "filter" $(NITER_UUT);\
