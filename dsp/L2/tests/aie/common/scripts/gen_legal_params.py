...
for param in param_list:
    param_new=config_translate(param, script_path, back_translate=1, canary_test=canary_test)  
...
if database_wr:    
    ...
    if len_list_1k !=0:
        ...
        if len_list_remainder != 0: 
            param_config_dict.update({k+1:param_config_list[k*1000 : k * 1000 + len_list_remainder]})
    else:
    ...
    for i in range(1,len(param_config_dict)+1,1 ):
        ...
        with open(f"{out_file_dir}/{IP_name}_test_arr_database_{i}.txt", "w") as file:
            ...
            print(f"Database file is generated: {out_file_dir}/{IP_name}_test_arr_database_{i}.txt")
else:
    if count_illegal != 0:
