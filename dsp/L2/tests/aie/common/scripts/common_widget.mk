###############################################################################
#  Common Makefile used for AIE DSPLIB widget functions compilation, simulation and QoR harvest.
...
REF_WINDOW_VSIZE   = $$(( $(WINDOW_VSIZE) ))

UUT_PARAMS_CONCAT   = $(DATA_TYPE)_$(IN_API)_$(OUT_API)_$(NUM_INPUTS)_$(WINDOW_VSIZE)_$(NUM_OUTPUT_CLONES)
...
PREPROC_ARGS       = -DINPUT_FILE=$(LOC_INPUT_FILE) -DINPUT_FILE2=$(LOC_INPUT_FILE2) -DDATA_TYPE=$(DATA_TYPE) -DIN_API=$(IN_API) -DOUT_API=$(OUT_API) -DNUM_INPUTS=$(NUM_INPUTS) -DWINDOW_VSIZE=$(WINDOW_VSIZE) -DNUM_OUTPUT_CLONES=$(NUM_OUTPUT_CLONES) -DSTIM_TYPE=$(STIM_TYPE)
REF_PREPROC_ARGS   = "-DUUT_GRAPH=$(REF_GRAPH) -DOUTPUT_FILE=$(REF_SIM_FILE) -DOUTPUT_FILE2=$(REF_SIM_FILE2) -DOUTPUT_FILE3=$(REF_SIM_FILE3) -DOUTPUT_FILE4=$(REF_SIM_FILE4) $(PREPROC_ARGS)  -DWINDOW_VSIZE=$(REF_WINDOW_VSIZE) -DNITER=$(NITER_REF)"
UUT_PREPROC_ARGS   = "-DUUT_GRAPH=$(UUT_GRAPH) -DOUTPUT_FILE=$(UUT_SIM_FILE) -DOUTPUT_FILE2=$(UUT_SIM_FILE2) -DOUTPUT_FILE3=$(UUT_SIM_FILE3) -DOUTPUT_FILE4=$(UUT_SIM_FILE4) $(PREPROC_ARGS)  -DWINDOW_VSIZE=$(WINDOW_VSIZE) -DNITER=$(NITER_UUT) -DUSING_UUT=$(USING_UUT_YES)"
...
UUT_DATA_FILE3     = ./data/uut_out_$(UUT_FILE_SUFFIX)_3.txt
REF_DATA_FILE3     = ./data/ref_out_$(REF_FILE_SUFFIX)_3.txt
UUT_SIM_FILE3      = data/uut_output_3.txt
REF_SIM_FILE3      = data/ref_output_3.txt
UUT_OUT_FILE3      = $(UUT_OUT_DIR)/$(UUT_SIM_FILE3)
REF_OUT_FILE3      = $(REF_OUT_DIR_X86)/$(REF_SIM_FILE3)

UUT_DATA_FILE4     = ./data/uut_out_$(UUT_FILE_SUFFIX)_4.txt
REF_DATA_FILE4     = ./data/ref_out_$(REF_FILE_SUFFIX)_4.txt
UUT_SIM_FILE4      = data/uut_output_4.txt
REF_SIM_FILE4      = data/ref_output_4.txt
UUT_OUT_FILE4      = $(UUT_OUT_DIR)/$(UUT_SIM_FILE4)
REF_OUT_FILE4      = $(REF_OUT_DIR_X86)/$(REF_SIM_FILE4)
...
ifeq ($(USE_AIE_API), 1)
	PREPROC_ARGS += -D_DSPLIB_WIDGET_API_CAST_HPP_AIE_API_ 
endif
...
ifeq ($(UUT_TARGET), hw)
    UUT_TARGET_COMPILE_ARGS:= -heapsize=$(HEAPSIZE_VAL) -stacksize=$(STACKSIZE_VAL) -Xchess=llvm.xargs="-std=c++17 "
#    UUT_TARGET_COMPILE_ARGS:= -xlopt=1 -stacksize=$(STACKSIZE_VAL) -Xchess=llvm.xargs="-std=c++17"
...
create_r:
	...
	@if [ $(GEN_INPUT_DATA) = false ]; then \
		...
		cp -f $(INPUT_FILE2) $(RESULTS_DIR)/$(LOC_INPUT_FILE2) ;\
	fi
...
create:
	@echo Start testing|& tee $(LOG_FILE);\
	 ...
	 echo "    IN_API:               " $(IN_API)             >> $(STATUS_FILE);\
	 echo "    OUT_API:              " $(OUT_API)            >> $(STATUS_FILE);\
	 echo "    NUM_INPUTS:           " $(NUM_INPUTS)         >> $(STATUS_FILE);\
	 echo "    WINDOW_VSIZE:         " $(WINDOW_VSIZE)       >> $(STATUS_FILE);\
	 echo "    NUM_OUTPUT_CLONES:    " $(NUM_OUTPUT_CLONES)  >> $(STATUS_FILE);\
	 echo "Results:"                                         >> $(STATUS_FILE)

create_input:
	@if [ $(GEN_INPUT_DATA) = true ]; then \
		tclsh $(DSPLIB_REPO)/L2/scripts/gen_input.tcl $(LOC_INPUT_FILE) $(WINDOW_VSIZE) $(NITER_UUT) $(SEED) $(STIM_TYPE) 0 0 $(DATA_TYPE);\
		tclsh $(DSPLIB_REPO)/L2/scripts/gen_input.tcl $(LOC_INPUT_FILE2) $(WINDOW_VSIZE) $(NITER_UUT) $(SEED2) $(STIM_TYPE) 0 0 $(DATA_TYPE);\
	fi
	@echo Input ready;\
	rm -f $(LOCK_FILE)
...
ifeq ($(NUM_OUTPUT_CLONES), 3)
	@grep -ve '[XT]' $(UUT_OUT_FILE2) > $(UUT_DATA_FILE2);\
	grep -ve '[XT]' $(REF_OUT_FILE2) > $(REF_DATA_FILE2);\
	echo "DIFF_START" >> $(LOG_FILE)
	tclsh $(DSPLIB_REPO)/L2/scripts/diff.tcl $(UUT_DATA_FILE2) $(REF_DATA_FILE2) $(DIFF_OUT_FILE)  $(DIFF_TOLERANCE) >> $(LOG_FILE)
	@grep -ve '[XT]' $(UUT_OUT_FILE3) > $(UUT_DATA_FILE3);\
	grep -ve '[XT]' $(REF_OUT_FILE3) > $(REF_DATA_FILE3);\
	echo "DIFF_START" >> $(LOG_FILE)
	tclsh $(DSPLIB_REPO)/L2/scripts/diff.tcl $(UUT_DATA_FILE3) $(REF_DATA_FILE3) $(DIFF_OUT_FILE)  $(DIFF_TOLERANCE) >> $(LOG_FILE)
endif
ifeq ($(NUM_OUTPUT_CLONES), 4)
	@grep -ve '[XT]' $(UUT_OUT_FILE2) > $(UUT_DATA_FILE2);\
	grep -ve '[XT]' $(REF_OUT_FILE2) > $(REF_DATA_FILE2);\
	echo "DIFF_START" >> $(LOG_FILE)
	tclsh $(DSPLIB_REPO)/L2/scripts/diff.tcl $(UUT_DATA_FILE2) $(REF_DATA_FILE2) $(DIFF_OUT_FILE)  $(DIFF_TOLERANCE) >> $(LOG_FILE)
	...
	echo "DIFF_START" >> $(LOG_FILE)
	tclsh $(DSPLIB_REPO)/L2/scripts/diff.tcl $(UUT_DATA_FILE3) $(REF_DATA_FILE3) $(DIFF_OUT_FILE)  $(DIFF_TOLERANCE) >> $(LOG_FILE)
	@grep -ve '[XT]' $(UUT_OUT_FILE4) > $(UUT_DATA_FILE4);\
	grep -ve '[XT]' $(REF_OUT_FILE4) > $(REF_DATA_FILE4);\
	echo "DIFF_START" >> $(LOG_FILE)
	tclsh $(DSPLIB_REPO)/L2/scripts/diff.tcl $(UUT_DATA_FILE4) $(REF_DATA_FILE4) $(DIFF_OUT_FILE)  $(DIFF_TOLERANCE) >> $(LOG_FILE)
endif
...
get_status_ref:
	...
	if [ $(GET_STATS) = true -a `cat compile_exit` -eq 0 ]; then \
		tclsh $(DSPLIB_REPO)/L2/scripts/get_stats.tcl $(DATA_TYPE) cint16 1 $(WINDOW_VSIZE) 1 1 1 1 0 0 1 $(STATUS_FILE) $(UUT_OUT_DIR) "transferData" $(NITER_UUT);\
