...
def generate_combinations(keys, current_index, current_combination, all_combinations, params):    
    ...
    for value in params[current_key]:    
        ...
        generate_combinations(keys, current_index + 1, current_combination, all_combinations, params)    
    
    return all_combinations    
    
def parse_test_case(line, defaults, param_list, module, updater_dict, validator_dict, enable_updater_padding):    
        ...
        if enable_updater_padding:
            for upd_param in updater_dict: 
                ...
                if upd_param in params_mapped:
                    param_under_test.param_validate(params_mapped, module, print_err="False")
                    ...
                    if upd_param not in no_validation_params: 
                        try:
                            ...
                            print(params_mapped)
                            print(f"ERROR: There are no legal values for {upd_param}. Likely bug in metadata as all previous params have been validated. ")
                            exit(1)
                            break
...
def remove_duplicate_test_cases(test_cases):    
    ...
    for test_name, test_case in test_cases.items():    
        ...
        if case_signature not in seen_cases:    
            ...
            print(f"Info: {test_name} is a duplicate and will be removed from multi_params")    
    
    return unique_cases    
    
def convert_to_json(test_suite_file, default_params_file, param_list, updater_dict, validator_dict, no_verify, module, enable_updater_padding):    
            ...
            with open(suite, 'r') as file:    
                for line_number, line in enumerate(file, start=1):    
                    ...
                    if line and not line.startswith('#'):    
                        if "#" in line:    
                            line = line.split("#")[0]    
                        test_case_list = parse_test_case(line, defaults, param_list, module, updater_dict, validator_dict, enable_updater_padding) 
                        for test_case in test_case_list:    
                            test_valid = "True"   
                            if not no_verify:    
                                test_valid = validate_test_case(test_suite_file, test_case, line_number, param_list, updater_dict, validator_dict, module)    
                            if test_valid == "False":    
                                print(f"Warning: ILLEGAL TEST CASE ARGS: {test_case}")    
                                count_illegal += 1    
                            elif test_valid == "True":    
                                count_legal += 1    
                                test_unmapped = param_map(test_case, defaults, back_translate=1)    
                                test_name = create_test_name(suite_name, line_number, test_unmapped)    
    ...
    if not no_verify:    
        print(f"Info: {count_legal + count_illegal} tests are verified, {count_legal} tests passed, {count_illegal} tests failed")    
    
    final_cases = remove_duplicate_test_cases(test_cases)    
    return final_cases, count_illegal    
    
def validate_test_case(test_suite_file, test_case, line_number, param_list, updater_dict, validator_dict, module):    
...
def param_map(test_case, defaults, back_translate):    
    ...
    for param in test_case:    
        param_mapped = config_translate(param, configTranslatePath, back_translate=back_translate, canary_test=defaults)    
        if back_translate == 0:    
            try:  
...
def parse_arguments():    
    parser = argparse.ArgumentParser(description="Generate test parameters from a test suite.")    
    parser.add_argument("test_suite", help="Name or path of the test suite file.")    
    parser.add_argument("--updater_pad", action="store_true", help="Enable updater padding.")    
    parser.add_argument("--ignore_fails", action="store_true", help="Ignore test case validation failures.")    
    parser.add_argument("--no_test_verify", action="store_true", help="Disable test case verification.")    
...
def main():    
    ...
    if os.path.isfile(test_suite) and test_suite.endswith('.txt'):    
        ...
        test_suite_files = [f"{called_dir}/test_suites/checkin.txt", f"{called_dir}/test_suites/qor.txt"]    
    else:    
    ...
    json_tests, fail_count = convert_to_json(test_suite_files, default_params_file, param_list, updater_dict, validator_dict, no_test_verify, module, enable_updater_padding)    
