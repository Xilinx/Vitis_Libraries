...
for test_item in modified_args_list:
    ...
    for param in test_item:
        param_new=config_translate(param, script_path, back_translate=0, canary_test=canary_test)
...
result=subprocess.run(["python3", f"{script_directory}/gen_legal_params.py", "--IP", IP_name, "--inp_dir", f"{ip_test_dir}/{args_file}", "--test_lines", f"{test_lines}", "--test_suite", f"{test_suite}"])
