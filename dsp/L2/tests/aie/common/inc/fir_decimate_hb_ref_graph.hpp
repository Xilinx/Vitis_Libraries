   ...
   public:
    ...
    void create_connections() {
        ...
        if
            constexpr(TP_NUM_OUTPUTS == 2) {
                ...
                m_widgetKernelOut = kernel::create_object<
                    widget_api_cast_ref<TT_DATA, USE_WINDOW_API, TP_API, kNumInputs,
                                        TP_INPUT_WINDOW_VSIZE / kDecimateFactor, kNumOutputs, 0> >();
                ...
                if
                    constexpr(TP_API == USE_WINDOW_API) {
                        ...
                        dimensions(m_widgetKernelOut.out[0]) = {TP_INPUT_WINDOW_VSIZE / kDecimateFactor};
                        dimensions(m_widgetKernelOut.out[1]) = {TP_INPUT_WINDOW_VSIZE / kDecimateFactor};
                    }
                else {
            ...
            dimensions(m_firKernel.out[0]) = {TP_INPUT_WINDOW_VSIZE / kDecimateFactor / kOutputWindowReductionFactor};
        }
        if
