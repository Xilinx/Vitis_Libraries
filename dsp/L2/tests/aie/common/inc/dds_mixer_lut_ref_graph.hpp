...
class dds_mixer_lut_ref_graph<TT_DATA, TP_INPUT_WINDOW_VSIZE, 1, TP_API, TP_SSR, TP_SFDR> : public graph {
    public:
        ...
        dds_mixer_lut_ref_graph(uint32_t phaseInc, uint32_t initialPhaseOffset = 0) {
            ...
            // IO_API is ignored because it's basically just a implementation detail
            static constexpr unsigned int tp_num_luts = TP_SFDR <= 60 ? 1 : TP_SFDR <= 120 ? 2 : 3;
            m_ddsKernel = kernel::create_object<dds_mixer_ref<TT_DATA, TP_INPUT_WINDOW_VSIZE, 1, USE_LUT_SINCOS,
tp_num_luts>>(phaseInc, initialPhaseOffset);
...
class dds_mixer_lut_ref_graph<TT_DATA, TP_INPUT_WINDOW_VSIZE, 0, TP_API, TP_SSR, TP_SFDR> : public graph {
    public:
        std::array<port<output>, 1> out;
        ...
        dds_mixer_lut_ref_graph(uint32_t phaseInc, uint32_t initialPhaseOffset = 0) {
            ...
            // IO_API is ignored because it's basically just a implementation detail
            static constexpr unsigned int tp_num_luts = TP_SFDR <= 60 ? 1 : TP_SFDR <= 120 ? 2 : 3;
            m_ddsKernel = kernel::create_object<dds_mixer_ref<TT_DATA, TP_INPUT_WINDOW_VSIZE, 0, USE_LUT_SINCOS,
tp_num_luts>>(phaseInc, initialPhaseOffset);
