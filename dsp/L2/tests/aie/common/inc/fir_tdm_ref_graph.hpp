 ...
 */
#ifndef _DSPLIB_fir_tdm_REF_GRAPH_HPP_
#define _DSPLIB_fir_tdm_REF_GRAPH_HPP_
...
class fir_tdm_ref_graph : public graph {
   ...
   public:
    ...
    // Constructor with reloadable coeffs (RTP port)
    // fir_tdm_ref_graph() {
    //     m_firKernel = kernel::create_object<ref_kernel_class>();
    //     create_connections();
    // };

    // Constructor with static coeffs
    fir_tdm_ref_graph(const std::vector<TT_COEFF>& taps) {
    ...
    void create_connections() {
        ...
        // Source files
        source(m_firKernel) = "fir_tdm_ref.cpp";
    };
...
#endif // _DSPLIB_fir_tdm_REF_GRAPH_HPP_
