...
void createLut(T_L* lut_values,
    ...
    float slopes[sectionTotal];
    float offsets[sectionTotal];
    ...
    for (int i = 0; i < (sectionTotal); i++) {
        lut_values[2 * i] = (T_L)(((float)sectionTotal * sectionWidth * slopes[i]) / scaleDomain);
        offsetAccum = (int64_t)(((float)sectionTotal * sectionWidth * offsets[i]) / scaleDomain);
...
// If TP_DOMAIN_MODE = 0, the input domain is 0 to 1. The number of sections in the lookup table will be 1 <<
// (TP_COARSE_BITS).
...
// To improve performance within the func_approx kernel, the MSB of TP_COASE_BITS for each input data sample should be
// set to zero before
// it is input to the func_approx graph. This allows the corrsending LUT address to be found with a simple downshift by
// TP_FINE_BITS.
// It will half the size of the required lookup table to be provived.
// Number of sections in LUT will be 1 << (TP_COARSE_BITS - 1), as the MSB of TP_COARSE_BITS is no longer used.
// If TP_DOMAIN_MODE = 2, the input domain is 1 to 4. The number of sections in the lookup table will be 1 <<
// (TP_COARSE_BITS).
// However, the first quadrant in the lookup (LUT sections 0 to (1 << TP_COARSE_BITS) / 4) - 1 will not be used as this
 ...
 * @brief getSqrt is a utility function that creates a lookup table for approximating the square-root function of x.
 ...
 * provided lookup table. It determines the total number of sections in the lookup table.
 ...
 * value. \n
 * For domainMode values equal to 0 or 2, there are 2 ^ (coarseBits) sections, and when domainMode is equal to 1, there
 * are 2 ^ (coarseBits - 1) sections.
 * @param[in] shift The downward shift value that will be applied to each offset of a section. This is only applicable
...
void getSqrt(T_L* lut_values, const int coarseBits, const int fineBits, const int domainMode, const int shift) {
    ...
    // Total number of sections in LUT. A section contains a slope value and an office value.
    unsigned int sectionTotal = (1 << (coarseBits - biasDomain));
    // Width of a section (distance in x domain between each section) is determinde by the number of fine interpolation
    // bits.
    unsigned int sectionWidth = 1 << fineBits;
    // float arrays for f(x) and x.
    float x_points[sectionTotal + 1];
    float y_points[sectionTotal + 1];
    // y_points and x_points require an additional point in order to calculate the slope of the final section.
    for (int i = 0; i < (sectionTotal + 1); i++) {
        // create x between 0 and scaleDomain
        x_points[i] = (scaleDomain) * ((float)i / sectionTotal) + biasDomain;
        // y_points = func(x_points)
        y_points[i] = sqrt(x_points[i]);
    }
    // create slope/offset lookup using x_points and y_points
    createLut<T_D, T_L>((T_L*)&lut_values[0], &x_points[0], &y_points[0], sectionTotal, sectionWidth, domainMode,
                        shift);
}
/**
 * @ingroup func_approx_utils
 * @brief getInvSqrt is a utility function that will create a lookup table to be used for the approximation of the
 * inverse square-root function of x.
 * @tparam T_D Describes the type of data that will be input to the func_approx.
 * @tparam T_L Describes the type of the values stored in the lookup table. This should be equal to T_D unless T_D is
 * bfloat16, in which case T_L should be float.
 * @param[out] lut_values A pointer to the area where the lookup table of slope and offset values will be created.
...
/**
 * @ingroup func_approx_utils
 * @brief getSqrt is a utility function that will create a lookup table to be used for the approximation of the inverse
 * of x
 * @tparam T_D Describes the type of data that will be input to the func_approx.
 * @tparam T_L Describes the type of the values stored in the lookup table. This should be equal to T_D unless T_D is
 * bfloat16, in which case T_L should be float.
