...
namespace fir {
namespace tdm {
enum eArchType { kArchBasic = 0, kArchIncLoads, kArchInternalMargin, kArchStream };
...
INLINE_DECL constexpr unsigned int fnVOutSizeTdm() {
    if
        constexpr(std::is_same<TT_DATA, cint16>::value && std::is_same<TT_COEFF, int32>::value) {
            ...
            return (__FIR_TDM_USE_48BIT_ACC__ == 1) ? 8 : fnNumLanesTdm<TT_DATA, TT_COEFF, TP_API>();
        }
    else if
        constexpr(std::is_same<TT_DATA, cint16>::value && std::is_same<TT_COEFF, cint32>::value) {
            // use 4x1 VMAC with 48-bit accs.
            return (__FIR_TDM_USE_48BIT_ACC__ == 1) ? 4 : fnNumLanesTdm<TT_DATA, TT_COEFF, TP_API>();
        }
    else if
        constexpr(std::is_same<TT_DATA, int16>::value && std::is_same<TT_COEFF, int32>::value) {
            // use 16x1 VMAC with 48-bit accs.
            return (__FIR_TDM_USE_48BIT_ACC__ == 1) ? 16 : fnNumLanesTdm<TT_DATA, TT_COEFF, TP_API>();
        }
    else {
        //
        return fnNumLanesTdm<TT_DATA, TT_COEFF, TP_API>();
    }
};
}
}
