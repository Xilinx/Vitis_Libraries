...
class kernelDdsMixerClass {
   protected:
    static constexpr unsigned int numLanes = 8; // todo - calculate for other types
    // todo, make these parameters in future release.
    static constexpr unsigned int dds_shift = 15;
    static constexpr unsigned int mixer_shift = 16;
    // Keeps track of where we are in sincos curve, incremented by phaseIncr
    unsigned int phaseIndex = 0;
    static constexpr unsigned int loopCount = (TP_INPUT_WINDOW_VSIZE / numLanes);
    static constexpr unsigned int numMixerInputs =
        (TP_MIXER_MODE == MIXER_MODE_2) ? 2 : (TP_MIXER_MODE == MIXER_MODE_1) ? 1 : 0;
    unsigned int perCyclePhaseInc;

    // Access function for AIE Synthesizer
    TT_DATA chess_storage(% chess_alignof(v8cint16)) phRot[numLanes];
    // alternative vector alignment method to be added later  (instead of chess_storage directive)
    // alignas(aie::vector_decl_align) cint16_t phRot[numLanes];
   ...
   private:
    ...
    static_assert((TP_INPUT_WINDOW_VSIZE % numLanes) == 0,
                  "ERROR: DDS Mixer TP_INPUT_WINDOW_VSIZE must be a multiple of numLanes (8)");
   ...
   public:
    ...
    using T_inIF = T_inputIF<TT_DATA, T_inType, numMixerInputs>;
    using T_outIF = T_outputIF<TT_DATA, T_outType>;
    using T_acc = cacc48; // todo for other types
...
class dds_mixer : public kernelDdsMixerClass<TT_DATA, TP_INPUT_WINDOW_VSIZE, TP_MIXER_MODE, TP_API> {
   public:
    ...
    // dds
    void ddsMix(T_inType* restrict inWindowA, T_inType* restrict inWindowB, T_outType* restrict outWindow);
};
...
class dds_mixer<TT_DATA, TP_INPUT_WINDOW_VSIZE, MIXER_MODE_1, TP_API>
   ...
   public:
    ...
    // dds
    void ddsMix(T_inType* restrict inWindowA, T_outType* restrict outWindow);
};
...
class dds_mixer<TT_DATA, TP_INPUT_WINDOW_VSIZE, MIXER_MODE_0, TP_API>
   ...
   public:
    ...
    // dds
    void ddsMix(T_outType* restrict outWindow);
};
}
}
}
