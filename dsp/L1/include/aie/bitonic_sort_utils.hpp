...
INLINE_DECL void intrasort(::aie::vector<TT_DATA, vecSize>* __restrict(&readPtr),
    ...
    if
        constexpr((sizeof(TT_DATA) == 4) && (__SUPPORTS_COMPREHENSIVE_SHUFFLES__ == 1)) {
            ...
            for (int k = 0; k < kNumVecs / 2; k++) chess_prepare_for_pipelining chess_loop_count(kNumVecs / 2) {
        ...
        ::aie::mask<vecSize> myMask = ::aie::mask<vecSize>::from_uint32(mask_final);
        ...
        for (int k = 0; k < kNumVecs; k++) chess_prepare_for_pipelining chess_loop_count(kNumVecs) {
                ...
                pairVect_t evenOdd = ::aie::interleave_zip(
                    vectBuff, vectBuff, fnPwr2<i - j>()); // ! Necessary to throw away half the vector with select() as
                                                          // the patterns are not interleave zippable (e.g. 00111100)
...
template <typename TT_DATA, unsigned int TP_DIM, unsigned int TP_ASCEND, unsigned int vecSize>
INLINE_DECL void intersort(::aie::vector<TT_DATA, vecSize>* __restrict(&scratchPtr), int& i, int& j) {
    using dataVect_t = ::aie::vector<TT_DATA, vecSize>;
    constexpr unsigned int kNumVecs = TP_DIM / vecSize;
    ...
    for (int k = 0; k < kNumVecs / 2; k++) chess_prepare_for_pipelining chess_loop_count(kNumVecs / 2) {
            ...
            dataVect_t vectBuff0 = scratchPtr[minIdx];
            dataVect_t vectBuff1 = scratchPtr[maxIdx];
            scratchPtr[minIdx] = ::aie::min(vectBuff0, vectBuff1);
            scratchPtr[maxIdx] = ::aie::max(vectBuff0, vectBuff1);
        }
};

template <typename TT_DATA, unsigned int TP_DIM, unsigned int TP_ASCEND, unsigned int vecSize>
INLINE_DECL void intersort_handover(::aie::vector<TT_DATA, vecSize>* __restrict(&scratchPtr), int& i) {
    using dataVect_t = ::aie::vector<TT_DATA, vecSize>;
    constexpr unsigned int kNumVecs = TP_DIM / vecSize;
    ...
    for (int k = 0; k < kNumVecs / 2; k++) chess_prepare_for_pipelining chess_loop_count(kNumVecs / 2) {
            ...
            dataVect_t vectBuff0 = scratchPtr[minIdx];
            dataVect_t vectBuff1 = ::aie::reverse(scratchPtr[maxIdx]);
            scratchPtr[minIdx] = ::aie::min(vectBuff0, vectBuff1);
            scratchPtr[maxIdx] = ::aie::max(vectBuff0, vectBuff1);
        }
};
