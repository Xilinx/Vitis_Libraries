...
template <int t_L, int t_R, int t_forkNumber, int t_forkingFactor,
          typename T_dtype>
void copyToLocalBuff(T_dtype p_in[t_R][t_L / t_R],
                     T_dtype p_out[t_R / t_forkingFactor][t_L / t_R]) {
...
template <int t_L, int t_R, int t_forkNumber, int t_forkingFactor,
          typename T_dtype>
void copyFromLocalBuffToOuput(T_dtype p_in[t_R / t_forkingFactor][t_L / t_R],
...
#pragma HLS UNROLL
      ...
      p_out[(r << (ssrFFTLog2<t_forkingFactor>::val)) + t_forkNumber - 1][t] =
          p_in[r][t];
    }
  }
}
...
template <int t_L, int t_R, int t_forkNumber, int t_forkingFactor,
          typename T_dtype>
void copyBuffToOutNonInvert(T_dtype p_in[t_R / t_forkingFactor][t_L / t_R],
                            T_dtype p_out[t_R][t_L / t_R]) {
#pragma HLS INLINE off
  const int fork_width = t_R / t_forkingFactor;
  const int offset = (t_forkNumber - 1) * fork_width;
...
void convertArrayToSuperStream(
    T_dtype p_inDataArray[t_R][t_L / t_R],
...
template <int t_stage, int t_id, int t_L, int t_R, typename T_dtype>
void convertArrayToSuperStream(
    hls::stream<T_dtype> p_inDataArray[t_R],
    hls::stream<SuperSampleContainer<t_R, T_dtype>> &p_out) {
#pragma HLS INLINE off
  //#pragma HLS STREAM variable=p_inDataArray dim=2
  //#pragma HLS ARRAY_PARTITION variable = p_inDataArray complete dim = 1
...
template <int t_stage, int t_id, int t_L, int t_R, typename T_dtype,
          typename T_dtype1>
void convertSuperStreamToArray(
...
template <int t_stage, transform_direction_enum transform_direction, int t_id,
          int t_L, int t_R, typename T_dtype, typename T_dtype1>
void convertSuperStreamToArrayNScale(
    hls::stream<SuperSampleContainer<t_R, T_dtype>> &p_in,
    T_dtype1 p_outDataArray[t_R][t_L / t_R]) {
#pragma HLS INLINE off

  static const int SCALING_FACTOR =
      (transform_direction == REVERSE_TRANSFORM) ? t_L : 1;
  SuperSampleContainer<t_R, T_dtype> temp;
#pragma HLS ARRAY_PARTITION variable = temp.superSample complete dim = 1
  //#pragma HLS ARRAY_PARTITION variable = p_outDataArray complete dim = 1
...
#pragma HLS UNROLL
      p_outDataArray[r][i] =
          T_dtype1(temp.superSample[r].real() / SCALING_FACTOR,
                   temp.superSample[r].imag() / SCALING_FACTOR);
    }
  }
}
...
void streamJoinUtility(T_dtype p_in[t_R][t_L / t_R],
                       T_dtype p_out[t_R][t_L / t_R]) {
#pragma HLS INLINE off
  for (int t = 0; t < t_L / t_R; t++) {
#pragma HLS PIPELINE II = 1 rewind
...
void mergeSuperSampleStreamNonInvertOut(
    hls::stream<SuperSampleContainer<t_R / t_forkingFactor, T_dtype>>
        p_in[t_forkingFactor],
...
void forkSuperSampleStream(
    hls::stream<SuperSampleContainer<t_R, T_dtype>> &p_in,
    hls::stream<SuperSampleContainer<t_R / t_forkingFactor, T_dtype>>
        p_outStreamArray[t_forkingFactor]) {
#pragma HLS INLINE off
...
#pragma HLS UNROLL
        ...
        super_sample_sample_out
            .superSample[(sample_no << (ssrFFTLog2<t_forkingFactor>::val)) +
                         fork_no] = sub_sample_in.superSample[sample_no];
      }
    }
    p_outSuperStream.write(super_sample_sample_out);
  }
