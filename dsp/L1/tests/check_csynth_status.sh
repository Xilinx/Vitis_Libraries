...
do
    ...
    if [ -f "vitis_hls.log" ]; then
        ...
        if [ -z "$res" ]
        then
            echo "Csynthesis Failed..."
            failed_test=$((failed_test+1))
            failed_list=(${failed_list[@]} $tdir) 
        else
	   echo "Csynthesis Passed ..."
...
echo "======================================================"
echo "======================================================"
echo "----------------- **C-SYNTHEIS ** Test Result Summary------------------"
echo "Total Number of tests: $total_test"
echo "Number of test finished: $test_finished"
echo "Number of tests PASSED C-SYNTHESIS: $passed_test"
echo "Number of tests FAILED C-SYNTHESIS: $failed_test"
echo "  "
...
## run and pass status 1 to jenkins
	echo "C-SYNTHESIS has FAILED !"
	exit 1 
fi

## Check if All tests were ran
if [ $total_test -ne $passed_test ]; then
	echo "Some tests did not run CSYNTH , CSYNTH TEST RUN FAILURE !"
	exit 1 
fi
