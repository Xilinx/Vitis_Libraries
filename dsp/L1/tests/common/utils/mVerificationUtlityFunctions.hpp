...
T_dType abs(std::complex<T_dType> inSample) {
...
int exportArrayToFile(std::complex<T_type> dataArray[L], std::string fileName) {
    int flag = 0;
...
int export2DArrayToFile(std::complex<T_type> dataArray[rows][cols], std::string fileName) {
    int flag = 0;
    std::ofstream outFile;
    const char* tempFileName = fileName.c_str();
    outFile.open(tempFileName, std::ios::out);
...
template <typename T_type, int rows, int cols>
int exportArrayToFile(std::complex<T_type> dataArray[rows][cols], std::string fileName) {
    std::ofstream outFile;
    const char* tempFileName = fileName.c_str();
    outFile.open(tempFileName, std::ios::out);
    if (outFile.is_open() == false) {
            ...
            } else // d
            {
                for (int i = 0; i < dataLength; i++) {
                    ...
                    } else // File opened and verification data available
                    {
                        ...
                        // goldenSample = std::complex<T_dType>(temp_real,temp_imag);
...
int readComplexArrayFromFile(std::ofstream& logStream,
    ...
    // std::cout << "####################################---" << strArrayName <<
    // "---#######################################" << std::endl;
    // logStream << "fiofVerificationReadInputData:" << strArrayName << std::endl << "Reading Array:" << strArrayName <<
    // std::endl;
            ...
            } else // Array Length found appropriate now READ ALL the dat from FILE
            {
                for (int i = 0; i < dataLength; i++) {
                    ...
                    } else // File opened and verification data available
                    {
                        ...
                        outData[i] = std::complex<T_dType>(temp_real, temp_imag);
                        if ((i % 100) == 0) {
...
template <typename T_dType>
int readComplexArrayFromFile(std::string fiofInputFileName, std::complex<T_dType> outData[], int dataLength) {
    print_function_header();
    std::cout << currentDateTime() << std::endl;
    double max_real, max_imag;
    double min_real, min_imag;
