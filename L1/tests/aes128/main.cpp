...
int main() {
    ...
    // std::cout << "Key ";
    xf::security::printVar<128>("Key", *key);
    ...
    // std::cout << "Plain text ";
    xf::security::printVar<128>("Plain text", *input);

    ap_uint<128> cipher;
    testEnc(*input, *key, cipher);
    // std::cout << "Cipher text ";
    xf::security::printVar<128>("Cipher", cipher);
    ...
    xf::security::printVar<128>("golden cipher", *golden);

    ap_uint<128> block;
    testDec(*golden, *key, block);
    xf::security::printVar<128>("block", block);

    if (cipher == *golden && block == *input) {
        std::cout << "\nAES128 test PASS\n" << std::endl;
    } else {
        std::cout << "\nAES128 test FAIL\n" << std::endl;
    }
