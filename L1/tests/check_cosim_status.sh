...
do
    ...
    if [ -f "vitis_hls.log" ]; then
        ...
        if [ -z "$res" ]
        then
            echo "C/RTL co-simulation Failed..."
            failed_test=$((failed_test+1))
            failed_list=(${failed_list[@]} $tdir) 
        else
	   echo "C/RTL co-simulation Passed ..."
...
echo "======================================================"
echo "======================================================"
echo "----------------- **CO-SIM ** Test Result Summary------------------"
echo "Total Number of tests: $total_test"
echo "Number of test finished: $test_finished"
echo "Number of tests PASSED CO-SIM: $passed_test"
echo "Number of tests FAILED CO-SIM: $failed_test"
echo "  "
...
## run and pass status 1 to jenkins
	echo "Co-Simulation has FAILED !"
	exit 1 
fi

## Check if All tests were ran
if [ $total_test -ne $passed_test ]; then
	echo "Some tests did not run CO-SIM , CO-SIM TEST RUN FAILURE !"
	exit 1 
fi
