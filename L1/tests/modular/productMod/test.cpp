...
void test(ap_uint<2048> a, ap_uint<2048> b, ap_uint<2048> m, ap_uint<2048>& res) {
    res = xf::security::internal::productMod<2048>(a, b, m);
}
...
int main() {
    ...
    unsigned char rawm[256] =
        ...
        "ijklmnopqrstuvwxyz.";
    for (int i = 0; i < 256; i++) {
        op.range(i * 8 + 7, i * 8) = (unsigned int)rawm[255 - i];
    }

    ap_uint<2048> result;
    ap_uint<4096> tmp = op * op;
    tmp %= modulus;
    ap_uint<2048> golden = tmp;

    test(op, op, modulus, result);
