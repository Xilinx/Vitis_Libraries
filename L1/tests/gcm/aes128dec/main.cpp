...
int main() {
    ...
    for (vector<Test>::const_iterator singletest = tests.begin(); singletest != tests.end(); singletest++) {
        ...
        // call fpga module
        test(ciphertextStrm, endCiphertextStrm, cipherkeyStrm, IVStrm, AADStrm, plaintextStrm, endPlaintextStrm,
             tagStrm);
        ...
        while (!end) {
            ...
            for (unsigned int i = 0; i < BLK_SIZE; i++) {
                plaintext_golden.range(7 + 8 * i, 8 * i) = (unsigned char)((*singletest).data[i + index]);
            }
            if (plaintext_golden != plaintext) {
                checked = false;
            }
        ...
        if (!checked) {
            ++nerror;
            cout << "fpga_plaintext   : " << hex << plaintext << endl;
            cout << "golden_plaintext : " << hex << plaintext_golden << endl;
