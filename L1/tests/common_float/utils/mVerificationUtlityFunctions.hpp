...
tt_dType abs(complex_wrapper<tt_dType> inSample) {
...
int exportArrayToFile(complex_wrapper<tt_type> dataArray[L], std::string fileName) {
    int flag = 0;
...
int export2DArrayToFile(complex_wrapper<tt_type> dataArray[rows][cols], std::string fileName) {
    int flag = 0;
    std::ofstream outFile;
    const char* tempFileName = fileName.c_str();
    outFile.open(tempFileName, std::ios::out);
...
template <typename tt_type, int rows, int cols>
int exportArrayToFile(complex_wrapper<tt_type> dataArray[rows][cols], std::string fileName) {
    std::ofstream outFile;
    const char* tempFileName = fileName.c_str();
    outFile.open(tempFileName, std::ios::out);
    if (outFile.is_open() == false) {
...
int verifyArrayOutput(std::ofstream& logStream,
                      ...
                      std::string fiofLog,
                      complex_wrapper<tt_dType> inData[],
            ...
            } else // d
            {
                for (int i = 0; i < dataLength; i++) {
                    ...
                    } else // File opened and verification data available
                    {
                        ...
                        // goldenSample = complex_wrapper<tt_dType>(temp_real,temp_imag);
...
int readComplexArrayFromFile(std::ofstream& logStream,
                             ...
                             complex_wrapper<tt_dType> outData[],
                             int dataLength) {
    logStream << currentDateTime() << std::endl;
    std::cout << currentDateTime() << std::endl;
            ...
            } else // Array Length found appropriate now READ ALL the dat from FILE
            {
                for (int i = 0; i < dataLength; i++) {
                    ...
                    } else // File opened and verification data available
                    {
                        ...
                        outData[i] = complex_wrapper<tt_dType>(temp_real, temp_imag);
                    }
                }
            }
        }
