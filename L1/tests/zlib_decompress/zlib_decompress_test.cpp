...
void zlibDecompressEngineRun(hls::stream<in_t>& inStream,
...
{
    xf::compression::inflate(inStream, outStream, outStreamEoS, outSizeStream, input_size);
}
...
void validateFile(std::string& fileName, std::string& originalFileName) {
    ...
    // DECOMPRESSION CALL
    zlibDecompressEngineRun(inStream, outStream, outStreamEoS, outStreamSize, comp_length);
