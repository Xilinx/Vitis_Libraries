...
template<typename T_type,int L>
int exportArrayToFile(complexNum < T_type> dataArray[L], std::string fileName)
{
	int flag=0;
	std::ofstream outFile;
	const char *tempFileName = fileName.c_str();
...
template<typename T_type, int L,int R>
int exportArrayToFile(complexNum< T_type> dataArray[][R], std::string fileName)
{
	std::ofstream outFile;
	const char *tempFileName = fileName.c_str();
	outFile.open(tempFileName, std::ios::out);
...
template < typename T_dType >
int verifyArrayOutput(std::ofstream& logStream, std::string strArrayName, std::string fiofLog, complexNum<T_dType> inData[], T_dType maxAllowedABSError,int dataLength)
{
	logStream << currentDateTime() << std::endl;
	std::cout<< currentDateTime() << std::endl;
	...
	if (fiofFile.is_open()) /// Try to open the file for data verification
	{
		...
		std::cout << "Opened verfication data file :  " << fiofLog << " <----Successfully----->..." << std::endl;
		// Variables to collect absolute error statistics /////////////////////////////////////////START
		T_dType currentABSError;
		T_dType maxABSError;
		T_dType accumABSError = 0;
		// Variables to collect absolute error statistics /////////////////////////////////////////END

		// Variables to collect  error statistics of REAL PART ////////////////////////////////////START
		T_dType currentREALError = 0;
		T_dType maxREALError;
		T_dType minREALError;
		T_dType accumREALError = 0;
		// Variables to collect  error statistics of REAL PART //////////////////////////////////////END

		// Variables to collect  error statistics of IMAG PART ////////////////////////////////////START
		T_dType currentIMAGError;
		T_dType maxIMAGError;
		T_dType minIMAGError;
		T_dType accumIMAGError = 0;
		// Variables to collect  error statistics of IMAG PART //////////////////////////////////////END
		complexNum<T_dType> goldenSample;
		int dataLengthFromFile;
...
template <typename T_dType>
int readComplexArrayFromFile(std::ofstream& logStream, std::string strArrayName, std::string fiofInputFileName, complexNum<T_dType> outData[], int dataLength)
{
	logStream << currentDateTime() << std::endl;
	std::cout << currentDateTime() << std::endl;
	logStream << "####################################---" << strArrayName << "---#######################################" << std::endl;
