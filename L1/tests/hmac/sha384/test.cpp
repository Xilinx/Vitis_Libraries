...
struct sha384_wrapper {
    // lW=128 bits, BLOCK_SIZE=128 bytes, hshW = 384 bits
    static void hash(hls::stream<ap_uint<msgW> >& msgStrm,
                     hls::stream<u128>& lenStrm,
                     hls::stream<bool>& eLenStrm,
                     hls::stream<ap_uint<384> >& hshStrm,
                     hls::stream<bool>& eHshStrm) {
        xf::security::sha384<msgW>(msgStrm, lenStrm, eLenStrm, hshStrm, eHshStrm);
    }
};

void test_hmac_sha384(hls::stream<ap_uint<KEYW> >& keyStrm,
                      ...
                      hls::stream<bool>& eHshStrm) {
    xf::security::hmac<KEYW, MSGW, LENW, HSHW, BLOCK_SIZE, sha384_wrapper>(keyStrm, lenKeyStrm, msgStrm, lenStrm,
...
// compute golden hmac
void hmacSHA384(const unsigned char* key,
                unsigned int keyLen,
    ...
    if (keyLen > BLOCK_SIZE) {
        SHA384((const unsigned char*)key, keyLen, (unsigned char*)h1);
    ...
    SHA384((const unsigned char*)kmsg, BLOCK_SIZE + msgLen, (unsigned char*)h2);
    ...
    SHA384((const unsigned char*)khsh, BLOCK_SIZE + HASH_SIZE, (unsigned char*)h);
}
...
int main() {
    ...
    for (unsigned int i = 0; i < NUM_TESTS; i++) {
        ...
        Test t(k, m, h);
        hmacSHA384((const unsigned char*)k, t.key.length(), (const unsigned char*)m, t.msg.length(), (unsigned char*)h);
...
#pragma HLS stream variable = msgStrm depth = 128
    hls::stream<ap_uint<LENW> > lenMsgStrm("lenMsgStrm");
...
#pragma HLS stream variable = endHshStrm depth = 128
    ...
    // call fpga module
    test_hmac_sha384(keyStrm, lenKeyStrm, msgStrm, lenMsgStrm, endLenStrm, hshStrm, endHshStrm);
