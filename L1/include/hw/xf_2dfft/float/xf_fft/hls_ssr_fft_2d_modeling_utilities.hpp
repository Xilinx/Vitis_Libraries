...
void genStimulusfor2DFFT(T_data p_data[t_rows][t_cols], bool p_print = false, StimDataType p_dataType = IMPULSE) {
    ...
    if (p_print) {
        ...
        std::cout << "The Stimulus Data generate ..." << std::endl;
        int nl = 0;
        for (int r = 0; r < t_rows; ++r) {
            for (int c = 0; c < t_cols; ++c) {
...
template <unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth>
void printMatStream(MemWideIFStreamTypeIn& p_stream, std::string p_msg) {
    std::cout << "===========================================================" << std::endl;
    std::cout << "===========================================================" << std::endl;
    std::cout << "===========================================================" << std::endl;
    std::cout << p_msg << std::endl;
    MemWideIFStreamTypeIn buffStream;
    unsigned int l_streamSize = p_stream.size();
    std::cout << "Stream Size :" << p_stream.size() << std::endl;
    for (int r = 0; r < t_rows / t_memWidth; ++r) {
        for (int c = 0; c < t_cols; ++c) {
            MemWideIFTypeIn sample = p_stream.read();
    ...
    for (int r = 0; r < t_rows / t_memWidth; ++r) {
        for (int c = 0; c < t_cols; ++c) {
            MemWideIFTypeIn sample = buffStream.read();
...
template <unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth, typename T_elemType>
void stream2DMatrix(T_elemType p_matrix[t_rows][t_cols], MemWideIFStreamTypeIn& p_stream) {
...
template <unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth, typename T_elemType>
void streamToMatrix(MemWideIFStreamTypeIn& p_inStream, T_elemType p_matrixOut[t_rows][t_cols]) {
    MemWideIFTypeIn l_inMemWideSample1;
    for (int r = 0; r < t_rows; ++r) {
        for (int c = 0; c < t_cols / t_memWidth; ++c) {
...
template <unsigned int t_rows,
          ...
          typename T_elemType>
void doBlkTransposeAndStream(MemWideIFStreamTypeIn& p_inStream, MemWideIFStreamTypeIn& p_stream) {
    MemWideIFTypeIn l_inMemWideSample1;
    ...
    MemWideIFTypeIn inMemWideSample;
...
template <unsigned int t_rows,
          ...
          unsigned int t_kernels,
          typename T_elemType>
void doBlkInverseTranspose(MemWideIFStreamTypeIn& p_stream, MemWideIFStreamTypeIn& p_outStream) {
    T_elemType p_matrix[t_rows][t_cols];
    MemWideIFTypeIn inMemWideSample;
    for (int blk = 0; blk < t_rows / t_kernels; ++blk) // blocks
    {
...
template <unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth, typename T_elemType>
void matrixTransElmntWise(MemWideIFStreamTypeIn& p_inStream, MemWideIFStreamTypeIn& p_outStream) {
    ...
    MemWideIFTypeIn l_inMemWideSample2;
    for (int r = 0; r < t_rows; ++r) {
        for (int c = 0; c < t_cols / t_memWidth; ++c) {
            for (int w = 0; w < t_memWidth; ++w) {
...
/*template< unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth, unsigned int t_kernels>
void doBlkTransposeAndStream(MemWideIFTypeOut p_matrix[][],MemWideIFStreamTypeOut &p_stream)
{
  MemWideIFTypeIn inMemWideSample;
  for (int blk = 0; blk < t_rows/t_kernels; ++blk) //blocks
  {
      for (int c = 0; c < t_cols/k_memWidth; ++c) // wide columns
      {
        for (int r = 0; r < t_kernels; ++r)    // rows in block
        {
          for (int  elemIndex=0; elemIndex< t_memWidth; elemIndex++)
          {
            ...
            int col_index = c*t_memWidth + elemIndex;
            inMemWideSample[elemIndex] = p_matrix[row_index][col_index];
            std::cout<<"Row:Col"<<row_index<<":"<<col_index<<std::endl;
            std::cout<<inMemWideSample[elemIndex]<<std::endl;
          }
          p_stream.write(inMemWideSample);
...
template< unsigned int t_rows, unsigned int t_cols, unsigned int t_memWidth, unsigned int t_kernels>
void doBlkInverseTransposeAndStream(MemWideIFStreamTypeOut &p_stream,MemWideIFTypeOut p_matrix[][])
{
  MemWideIFTypeIn inMemWideSample;
  for (int blk = 0; blk < t_rows/t_kernels; ++blk) //blocks
  {
