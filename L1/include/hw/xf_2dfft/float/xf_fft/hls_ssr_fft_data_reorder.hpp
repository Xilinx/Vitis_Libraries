...
writeBackCacheDataDR_LOverRLoop:
    for (int r = 0; r < (t_L / t_R); r++) {
#pragma HLS PIPELINE II = 1 rewind // 2dfft debug // This loop has rewind issue : VERIFIED
...
writeBackCacheDataDR_LOverRLoop:
    for (int r = 0; r < (t_L / t_R); r++) {
#pragma HLS PIPELINE II = 1 rewind // 2dfft debug // This loop has rewind issue
...
#pragma HLS DATA_PACK variable = temp
        ...
        int offset = (out_c + (out_r >> (ssrFFTLog2<t_L / (t_R * t_R)>::val))) &
                     (ssrFFTLog2BitwiseAndModMask<t_R>::val); // int out_cDash = (out_c  +  (out_r/t_R) ) %t_R; //
                                                              // ((r>>log2_radix) + c)%t_R;     //
        // MemReadBarrelShifter<t_R> readBarrelShifterObj;
        // readBarrelShifterObj.template readMemAndBarrelShift< t_R,t_L,complex_wrapper<T_in>
        // >(r,offset,p_digitReseversedOutputBuff, temp );
        for (int c = 0; c < t_R; c++) {
#pragma HLS UNROLL
...
//===========================================================================================================================
  ...
  void reOrderDataDynamic(complex_wrapper<T_dtype> p_inData[t_R][t_L/t_R], complex_wrapper<T_dtype> p_outData[t_R][t_L/t_R], int p_stage)
  { 
  ...
  template <typename T_dtype, int t_L, int t_R>
  void reverseReOrderDataDynamic(complex_wrapper<T_dtype> p_inData[t_R][t_L/t_R], complex_wrapper<T_dtype> p_outData[t_R][t_L/t_R], int p_stage)
  { 
    int no_of_fft_in_stage = (int)pow((double)(t_R), double(p_stage));
    int fft_len_in_stage = t_L / no_of_fft_in_stage;
    int no_bflys_per_fft = fft_len_in_stage / t_R;
  ...
  template <typename T_dtype, int t_L, int t_R>
  int verifyDataReversal(complex_wrapper<T_dtype> p_Data[t_R][t_L/t_R])
  { 
    int flag = 0;
