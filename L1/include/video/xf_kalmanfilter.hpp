...
void MeasUpdate_1x(float Uin_matrix[PROC_MU][UMAT_DEPTH],
    ...
    //  ---------------------------
    //					<a0,a1>			|	Dbar(1) = D(1)*a0/a1			<a1,a2>		|	Dbar(2) = D(2)*a1/a2
    //<a2,a3>
    //|..|
    ...
    // g(n+1)U(n+1)_sv
    //	f2 = U2*h						|	f3 = U3*h									|	f4 = U4*h									|..|
    //f(n+2)
    //=
    // mulAcc
    //	g2 = f2*D(2)					|	g3 = f3*D(3)								|	g4 = f4*D(4)								|..|
    //g(n+2)
    //=
    // f(n+2)*g(n+2)
    //	a1 = a0 + f1g1					|	a2 = a1 + f2g2								|	a3 = a2 + f3g3								|..|	a(n+1) =
    //a(n)
    //+
    // f(n+1)g(n+1)
    //##############################################################################################################
    ...
    //  |   a3 compute/pass								|..|
    //###############################################################################################################
    // a_prev = a0,a_up = a1 <a_prev,a_up>	|	Dbar(1)=D(1)*a_prev/a_up	<a_prev,a_up>	|
    // Dbar(2)=D(2)*a_prev/a_up
    // |..|
    ...
    //	k=k1			<K,g>			|	K= K + g*U1					<k,g>			|	K= K + g*U2									|..|	K=
    //K
    //+
    // g*U2
    //	g=g1							|	/*a1*/a_prev = a_up							|	/*a2*/a_prev = a_up							|..|
    ///*a(n)*/a_prev
    //=
