...
int WORDWIDTH_DST,
int TC>
void AWBChannelGain(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src1,
		xf::cv::Mat<DST_T, ROWS, COLS, NPC>& dst,
		float thresh,
		uint16_t height,
		uint16_t width,int i_gain[3]) {
...
void AWBNormalizationkernel(xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src,xf::cv::Mat<DST_T, ROWS, COLS, NPC> &dst,uint32_t hist[3][256],float p,
		float inputMin,
		float inputMax,
		float outputMin,
		float outputMax)
...
#pragma HLS INLINE OFF
		...
		// searching for s1 and s2
	{
			...
			// searching for s1 and s2
		{
			...
			while (n1 + hist[j][p1] < rval && p1<256)
			{
				// clang-format off
#pragma HLS PIPELINE
...
template< int SRC_T,int DST_T, int ROWS, int COLS, int NPC = 1,int WB_TYPE>
void AWBhistogramkernel(xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src1,xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src2,uint32_t hist[3][256],float p,
		float inputMin,
		float inputMax,
		float outputMin,
		float outputMax)
...
#pragma HLS UNROLL
	...
	uint32_t tmp_hist[XF_NPIXPERCYCLE(NPC)*XF_CHANNELS(SRC_T,NPC)][256]={0};
	uint32_t tmp_hist1[XF_NPIXPERCYCLE(NPC)*XF_CHANNELS(SRC_T,NPC)][256]={0};
	// clang-format off
...
#pragma HLS LOOP_TRIPCOUNT min=256 max=256
	...
	ROW_LOOP:for (int row = 0; row != (height); row++)// histogram filling
	{
		// clang-format off
#pragma HLS LOOP_TRIPCOUNT min=1 max=ROWS
		// clang-format on
		COL_LOOP:for (int col=0; col < (width ) ; col = col+2)// histogram filling
		{
			// clang-format off
...
#pragma HLS LOOP_TRIPCOUNT min=1 max=COLS/4
			...
			PLANES_LOOP:for(ap_uint<9> j=0; j < XF_NPIXPERCYCLE(NPC)*XF_CHANNELS(SRC_T,NPC);j++)
			{
				// clang-format off
...
#pragma HLS UNROLL
	...
	MERGE_HIST_LOOP: for(ap_uint<10> i = 0; i < 256; i++) {
		// clang-format off
#pragma HLS pipeline
		// clang-format on

		MERGE_HIST_CH_UNROLL: for(ap_uint<5> ch = 0; ch < num_ch; ch++) {
			// clang-format off
#pragma HLS UNROLL
			// clang-format on
			...
			MERGE_HIST_NPPC_UNROLL: for(ap_uint<5> p = 0; p < XF_NPIXPERCYCLE(NPC); p++) {
				// clang-format off
#pragma HLS UNROLL
				// clang-format on
...
template< int SRC_T,int DST_T, int ROWS, int COLS, int NPC = 1,int WB_TYPE>
void AWBhistogram(xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src1,xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src2,uint32_t histogram[3][256],float thresh,
		float inputMin,
		float inputMax,
		float outputMin,
		float outputMax)
...
#pragma HLS INLINE OFF
	...
	AWBhistogramkernel<XF_8UC3,XF_8UC3,ROWS, COLS, NPC,1>(src1,src2,histogram,thresh,inputMin,inputMax,outputMin,outputMax);

}

template< int SRC_T,int DST_T, int ROWS, int COLS, int NPC = 1,int WB_TYPE>
void AWBNormalization(xf::cv::Mat<SRC_T, ROWS, COLS, NPC> & src,xf::cv::Mat<DST_T, ROWS, COLS, NPC> &dst,uint32_t histogram[3][256],float thresh,
		float inputMin,
		float inputMax,
		float outputMin,
		float outputMax)
...
#pragma HLS INLINE OFF
	...
	AWBNormalizationkernel<XF_8UC3,XF_8UC3,ROWS, COLS, NPC,1>(src,dst,histogram,thresh,inputMin,inputMax,outputMin,outputMax);
...
template <int SRC_T,
          ...
          int WORDWIDTH_DST,
          int TC>
void balanceWhiteKernel_simple(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src1,
...
template <int SRC_T,
          ...
          int WORDWIDTH_DST,
          int TC>
void balanceWhiteKernel(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src1,
                       xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src2,
                       xf::cv::Mat<DST_T, ROWS, COLS, NPC>& dst,
                       float thresh,
                       uint16_t height,
...
void balanceWhite(xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src1,xf::cv::Mat<SRC_T, ROWS, COLS, NPC>& src2,
		xf::cv::Mat<DST_T, ROWS, COLS, NPC>& dst,
		float thresh,
		float inputMin,
		float inputMax,
