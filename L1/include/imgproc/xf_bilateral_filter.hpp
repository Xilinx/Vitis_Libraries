...
template <int TYPE,
          ...
          int NUM_DIST,
          int FPRES_SC>
static void ProcessBilateralNXN(
    xf::cv::Mat<TYPE, ROWS, COLS, NPC>& _src_mat,
    ...
    ap_uint<13> row,
    ap_uint<8> win_size,
    ap_ufixed<FPRES_SC, 1> exp_lut_sigma_color[NPC][WIN_SZ * WIN_SZ][NUM_DIST][256 * PLANES],
    ap_int<8> distances_array_revmap[(WIN_SZ >> 1) * (WIN_SZ >> 1) + 1],
    int& rd_ind,
    int& wr_ind) {
// clang-format off
...
#pragma HLS UNROLL
                ...
                xFBilateralProc<NPC, DEPTH, PLANES, WIN_SZ, WIN_SZ_SQ, NUM_DIST, FPRES_SC>(
                    OutputValues_percycle, src_buf_temp_med_apply, WIN_SZ, exp_lut_sigma_color[applymedian],
                    distances_array_revmap);
...
#pragma HLS UNROLL
            ...
            xFBilateralProc<NPC, DEPTH, PLANES, WIN_SZ, WIN_SZ_SQ, NUM_DIST, FPRES_SC>(
                OutputValues, src_buf, win_size, exp_lut_sigma_color[0], distances_array_revmap);
            if (col >= (WIN_SZ >> 1)) {
...
template <int TYPE,
          ...
          int NUM_DIST,
          int FPRES_SC>
static void xFBilateralFilterNXN(
    xf::cv::Mat<TYPE, ROWS, COLS, NPC>& _src_mat,
    xf::cv::Mat<TYPE, ROWS, COLS, NPC>& _dst_mat,
    ap_uint<8> win_size,
    uint16_t img_height,
...
#pragma HLS UNROLL
                // clang-format on
                for (unsigned short np = 0; np < NPC; np++) {
#pragma HLS UNROLL
                    exp_lut_sigma_color[np][k][i][m] = xf::cv::xFBilateralFloatMul(
