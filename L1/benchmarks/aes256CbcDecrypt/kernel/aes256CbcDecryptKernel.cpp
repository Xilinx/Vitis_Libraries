...
LOOP_MULTI_TASK:
    ...
    LOOP_SPLIT_TEXT:
        for (ap_uint<64> i = 0; i < msgNum * _channelNumber / 4; i++) {
#pragma HLS pipeline II = 1
...
#pragma HLS unroll
        ...
        xf::security::details::aesCbcDecrypt<_keyWidth>(textInStrm[m], endTextInStrm[m], cipherkeyStrm[m], IVStrm[m],
...
// @brief top of kernel
extern "C" void aes256CbcDecryptKernel(ap_uint<512> inputData[(1 << 30) + 100], ap_uint<512> outputData[1 << 30]) {
#pragma HLS dataflow

    const unsigned int _channelNumber = 4;
