...
XF FFT Library is an implementation of Discrete Fourier Transform using Fast 
...
that can be directly used for FPGA acceleration of different application in integration with
Xilinx Runtime (XRT). The L3 provides software APIs in C, C++ and Python which 
allows software developers  to offload FFT calculation to FPGA for acceleration. Before
FGPA can perform FFT computation FPGA need to be configured with particular image
called Overlay.
...
**Note**: Current release of the XF FFT only provides L1 primitives. L2 and L3 kernels and applications APIs are planned to be provided in next releases. 
