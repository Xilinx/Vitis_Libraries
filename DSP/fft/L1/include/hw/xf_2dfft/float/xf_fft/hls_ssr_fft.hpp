...
#include "xf_fft/hls_ssr_fft_fork_merge_utils.hpp"
#include "xf_fft/hls_ssr_fft_pragma_controls.hpp"
#include "xf_fft/hls_ssr_fft_enums.hpp"
#include "xf_fft/hls_ssr_fft_data_reorder.hpp"
#include "xf_fft/hls_ssr_fft_traits.hpp"
#include "xf_fft/hls_ssr_fft_twiddle_table.hpp"
#include "xf_fft/hls_ssr_fft_complex_exp_table.hpp"
#include "xf_fft/hls_ssr_fft_complex_exp_table_last_stage.hpp"
#include "xf_fft/hls_ssr_fft_streaming_transposer.hpp"
#include "xf_fft/hls_ssr_fft_streaming_data_commutor.hpp"
#include "xf_fft/hls_ssr_fft_adder_tree.hpp"
#include "xf_fft/hls_ssr_fft_input_traits.hpp"
#include "xf_fft/hls_ssr_fft_parallel_fft_kernel.hpp"
#include "xf_fft/hls_ssr_fft_multiplication_traits.hpp"
#include "xf_fft/fft_complex.hpp"
#include "xf_fft/hls_ssr_fft_types.hpp"
...
#pragma HLS PIPELINE II = 1 rewind // 2dfft debug
...
#pragma HLS UNROLL /// Unroll Twiddle Factor Multiplication loop, required for proper implementation of SSR FFT
        ...
        //=================================================================================================
        /*typedef typename FFTMultiplicationTraits<T_exp,T_twiddleInType>::T_productOpType t_product_op;
         T_twiddleInType  in_sample;
         T_exp  exp_factor;
...
#pragma HLS STREAM variable = fftOutData_local2 depth = 8
        ...
        fftStageKernelS2S<t_L, t_R, t_scalingMode, transform_direction, butterfly_rnd_mode, stage, T_stageOutType>(
            p_complexExpTable,
            twiddleObj.twiddleTable, // p_twiddleTable,
            p_fftInData, fftOutData_local);
        ...
        FFTStageClassS2SWithTable<t_L, t_R, t_instanceID, t_scalingMode, transform_direction, butterfly_rnd_mode,
                                  ...
                                  t_complexExpTableType, // T2_expTabType,
                                  T_twiddleType,         // T2_twiddleType,
                                  T_stageOutType, T_fftOut>::fftStage(p_complexExpTable,
                                                                      // p_twiddleTable,
...
#pragma HLS DATA_PACK variable = fftOutData_local
        ...
        fftStageKernelFullForForkingS2S<t_L, t_R, t_scalingMode, transform_direction, butterfly_rnd_mode, stage,
                                        T_stageOutType>(p_complexExpTable,
                                                        twiddleObj.twiddleTable, // p_twiddleTable,
                                                        p_fftInData, fftOutData_local);
    //**RW Info :: Read and Written in order and declared
    // as stream, p_fftInData declared as stream in top level
...
L_FFTs_LOOP: // fft_length/(radix^(stage+1)) L=64,R=4, S=0 the FFTs=16
    for (int f = 0; f < no_of_ffts_in_stage; f++) {
#pragma HLS PIPELINE II = 1 rewind // 2dfft debug    // This rewind created apparent deadlock that are not detected.
...
#endif
        // T_fftTwiddleType twiddle k,Table[TwiddleTableLENTraits< t_L,t_R> ::TWIDDLE_TALBE_LENGTH];
        // T_fftTwiddleType twiddleTable[TwiddleTableLENTraits<t_L, t_R>::EXTENDED_TWIDDLE_TALBE_LENGTH];
        // static TwiddleTableWrapper<t_L,t_R,t_instanceID,T_fftTwiddleType> twiddleObj;
        //#pragma HLS RESOURCE variable=twiddleObj.twiddleTable core=ROM_nP_LUTRAM
        // T_complexExpTableType complexExpTable[ComplexExpTableLENTraits<0,t_L,t_R>::EXP_TALBE_LENGTH];
        T_complexExpTableType complexExpTable[ComplexExpTableLENTraits<0, t_L, t_R>::EXTENDED_EXP_TALBE_LENGTH];
// TwiddleTable<t_L, t_R, iid, T_fftTwiddleType>::initTwiddleTable(twiddleTable);
...
#pragma HLS RESOURCE variable = casted_output core = FIFO_LUTRAM
        ...
        FFTStageClassS2SWithTable<
            ...
            NO_OF_FFT_STAGES,
            T_complexExpTableType, // typename
                                   // FFTTraits<t_scalingMode,t_L,t_R,NO_OF_FFT_STAGES,T_fftTwiddleType,T_complexExpTableType,casted_type,T_out>::T_expTabType,
            T_fftTwiddleType,      // typename
                                   // FFTTraits<t_scalingMode,t_L,t_R,NO_OF_FFT_STAGES,T_fftTwiddleType,T_complexExpTableType,casted_type,T_out>::T_twiddleType,
...
#pragma HLS RESOURCE variable = p_fftOutData core = FIFO_LUTRAM
        ...
        // T_fftTwiddleType twiddleTable[TwiddleTableLENTraits< t_L,t_R> ::TWIDDLE_TALBE_LENGTH];
        ////T_fftTwiddleType twiddleTable[TwiddleTableLENTraits<t_L, t_R>::EXTENDED_TWIDDLE_TALBE_LENGTH];
        //#pragma HLS RESOURCE variable=twiddleTable core=ROM_nP_LUTRAM
        // T_complexExpTableType complexExpTable[ComplexExpTableLENTraits<0,t_L,t_R>::EXP_TALBE_LENGTH];
        T_complexExpTableType complexExpTable[ComplexExpTableLENTraits<0, t_L, t_R>::EXTENDED_EXP_TALBE_LENGTH];
...
#pragma HLS ARRAY_PARTITION variable = complexExpTable complete dim = 1
        // T_complexExpTableType complexExpTable_forkingStage[t_R];
...
#pragma HLS ARRAY_PARTITION variable = complexExpTable_forkingStage complete dim = 1
        ...
        const int NO_OF_FFT_STAGES = (ssrFFTLog2<t_L>::val / ssrFFTLog2<t_R>::val) + 1;
        typedef typename FFTInputTraits<T_in>::T_castedType casted_type;
        // casted_type p_fftInData_reOrdered[t_R][t_L/t_R];
        hls::stream<SuperSampleContainer<t_R, casted_type> > p_fftInData_reOrdered;
#pragma HLS STREAM variable = p_fftInData_reOrdered depth = 8 dim = 2
#pragma HLS DATA_PACK variable = p_fftInData_reOrdered
#pragma HLS RESOURCE variable = p_fftInData_reOrdered core = FIFO_LUTRAM
...
#pragma HLS RESOURCE variable = casted_output core = FIFO_LUTRAM
        /*#ifdef SSR_FFT_PARTITION_INTERFACE_ARRAYS
         ...
         #endif*/
        castArrayS2Streaming<t_L, t_R, T_in, casted_type>(p_fftInData, casted_output);
        // InputTransposeChainStreamingS2S<NO_OF_FFT_STAGES-1,tp_outputForkingFactor,iid> swapObj;
        // swapObj.template swap<t_L,t_R,iid,1>(casted_output, p_fftInData_reOrdered);
...
#pragma HLS RESOURCE variable = p_fftInData_reOrdered core = FIFO_LUTRAM
        ...
        // casted_type casted_output[t_R][t_L/t_R];
        hls::stream<SuperSampleContainer<t_R, casted_type> > casted_output;
#pragma HLS DATA_PACK variable = casted_output
#pragma HLS STREAM variable = casted_output depth = 8 dim = 2
#pragma HLS RESOURCE variable = casted_output core = FIFO_LUTRAM
        ...
        // swapObj.template swap<t_L,t_R,iid,1>(casted_output, p_fftInData_reOrdered);
        // printComplexNumArray<double, t_L / t_R, t_R>(p_fftInData_reOrdered);
...
void fft(
         T_in p_fftInData[ssr_fft_param_struct::R][ssr_fft_param_struct::N/ ssr_fft_param_struct::R],
         typename FFTOutputTraits<ssr_fft_param_struct::N,
             ssr_fft_param_struct::R, ssr_fft_param_struct::scaling_mode,
             ssr_fft_param_struct::transform_direction,
             ssr_fft_param_struct::butterfly_rnd_mode,
             typename FFTInputTraits<T_in>::T_castedType>::T_FFTOutType
p_fftOutData[ssr_fft_param_struct::R][ssr_fft_param_struct::N / ssr_fft_param_struct::R])*/
