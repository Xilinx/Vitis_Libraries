...
#include "xf_fft/hls_ssr_fft_data_commute_barrel_shifter.hpp"
...
#pragma HLS DATA_PACK variable = temp
            ...
            // barrelShiftWrapperFunctionpPF1wrapper_obj.template
            // barrelShiftWrapperFunctionpf1<t_XF,t_L,t_R,t_PF,T_dtype>(p_buff,temp);
            for (int c = 0; c < t_R; c++) {
#pragma HLS UNROLL
...
#pragma HLS INLINE off
    ...
    //	assert( (t_L%(t_PF*t_R*t_R)) == 0 ); /// Length of FFT and R not selected properly
...
#pragma HLS DATAFLOW
#pragma HLS INLINE off
//	assert( (t_L%(t_R*t_R)) == 0 ); /// Length of FFT and R not selected properly
// Length should be power of Radix
#ifndef __SYNTHESIS__
    assert((t_L) == (ssrFFTPow<2, ssrFFTLog2<t_L>::val>::val)); // Length should be power of 2 always
#endif
    // assert((ssrFFTLog2<t_L>::val % ssrFFTLog2<t_R>::val)==0); // length should be power of radix also
    // Length should be multiple of Radix
...
#pragma HLS DATA_PACK variable = buff
        ...
        // writeCachedData<t_L,t_R,1,T_dtype>(buff,p_out);
    }
}
...
#pragma HLS ARRAY_RESHAPE variable = localFactoredOutputBuff complete dim = 1
    ...
    /* Function Signature:
     * template <int t_L, int t_R,int t_forkNumber, int t_forkingFactor, typename T_dtype>
     *				void copyFromLocalBuffToOuput(T_dtype p_in[t_L/t_R][t_R/t_forkingFactor],T_dtype
     *p_out[t_R][t_L/t_R])
     */
    ...
    /* Function Signature:
     * 	template<int t_forkNumber, int t_forkingFactor>
     template<int t_PF,int t_L, int t_R,typename T_dtype>
     void DataCommutorFork<t_forkNumber,t_forkingFactor>::copyForkCommuteAndMerge(T_dtype p_in[t_R][t_L/t_R],T_dtype
     p_out[t_R][t_L/t_R])
     */
