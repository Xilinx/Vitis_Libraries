...
#include "xf_fft/hls_ssr_fft_utility_traits.hpp"
...
template <int t_L, int t_R, int t_instanceID, typename T_dtype>
struct TwiddleTable<t_L, t_R, t_instanceID, complex_wrapper<T_dtype> > {
    static void initTwiddleTable(complex_wrapper<T_dtype> p_table[]) {
#pragma HLS INLINE off
        typedef typename TwiddleTypeCastingTraits<complex_wrapper<T_dtype> >::T_roundingBasedCastType casting_type;
...
template <int t_L, int t_R, int t_instanceID, typename T_dtype>
struct TwiddleTableWrapper<t_L, t_R, t_instanceID, complex_wrapper<T_dtype> > {
    complex_wrapper<T_dtype> twiddleTable[TwiddleTableLENTraits<t_L, t_R>::EXTENDED_TWIDDLE_TALBE_LENGTH];
    TwiddleTableWrapper() {
        typedef typename TwiddleTypeCastingTraits<complex_wrapper<T_dtype> >::T_roundingBasedCastType casting_type;
        static const int EXT_LEN = TwiddleTableLENTraits<t_L, t_R>::EXTENDED_TWIDDLE_TALBE_LENGTH;
        for (int i = 0; i < EXT_LEN; i++) {
