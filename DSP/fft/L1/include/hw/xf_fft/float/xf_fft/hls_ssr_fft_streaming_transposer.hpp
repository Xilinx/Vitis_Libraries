...
//#include "xf_fft/hls_ssr_fft_data_commutor.hpp"
#include "xf_fft/hls_ssr_fft_streaming_data_commutor.hpp"
#include "hls_ssr_fft_streaming_forking_data_commutor.hpp"
#include "xf_fft/hls_ssr_fft_pragma_controls.hpp"
...
#pragma HLS INLINE
        ...
        // streamingDataCommutations<10000,1,iid,t_PF,t_isLargeMemFlag> commutor;
        // template <int t_instanceID, int t_stage, int t_subStage, int t_forkNumber,int t_L, int t_R, int t_PF, int
        // t_isLargeMem>
...
// template<int tp_stages, int t_instanceID>
template <int t_instanceID, int t_stage, int t_subStage, int t_forkNumber, int t_L, int t_R, int t_PF>
// template <int t_L, int t_R, int t_PF,int iid, typename T_T>
template <typename T_dtype>
...
#pragma HLS ARRAY_RESHAPE variable = temp complete dim = 1
    ...
    static const int t_isLargeMemFlag = (((t_PF * t_R) > SSR_FFT_URAM_SELECTION_THRESHHOLD) && SSR_FFT_USE_URAMS);
    // template <int t_instanceID, int t_stage, int t_subStage, int t_forkNumber,int t_L, int t_R, int t_PF, int
    // t_isLargeMem>  streamingDataCommutations<10000+tp_stages,0,iid,t_PF,t_isLargeMemFlag> commutor;
    ...
    // template <int t_instanceID, int t_stage,int t_L, int t_R, int t_PF, int t_outputForkingFactor>
...
// template<int tp_stages,int t_outputForkingFactor, int t_instanceID>
template <int t_instanceID,
          int t_stage,
          int t_subStage,
          int t_forkNumber,
...
#pragma HLS DATA_PACK variable = temp
    ...
    // streamingDataCommutations<10000+tp_stages,0,iid,t_PF,t_isLargeMemFlag> commutor;
    // template <int t_instanceID, int t_stage, int t_subStage, int t_forkNumber,int t_L, int t_R, int t_PF, int
    // t_isLargeMem>
    streamingDataCommutations<t_instanceID, t_stage, t_subStage, t_forkNumber, t_L, t_R, t_PF, t_isLargeMemFlag>
        commutor;
    commutor.template streamingDataCommutor<T_dtype>(p_in, temp);
    // Instantiate Next State....Recursion
    // InputTransposeChainStreaming<tp_stages-1,t_outputForkingFactor,iid> nextStage;
...
#pragma HLS INLINE
    ...
    // StreamingDataCommutorFork<10000+1,iid,t_outputForkingFactor,t_outputForkingFactor >
    // StreamingDataCommutorFork_obj;  StreamingDataCommutorFork_obj.template
    // copyForkCommuteAndMerge<t_PF,t_L,t_R,iid,T_dtype>(p_in,p_out);
...
struct InputTransposeChainStreamingS2S {
    // template <int t_L, int t_R, int iid, int t_PF, typename T_T>
    template <typename T_dtype>
    // void swap(T_T p_in[t_R][t_L/t_R],T_T p_out[t_R][t_L/t_R]);
...
#pragma HLS INLINE
        static const int t_isLargeMemFlag = (((t_PF * t_R) > SSR_FFT_URAM_SELECTION_THRESHHOLD) && SSR_FFT_USE_URAMS);
        // DataCommutationsS2Streaming<10000,iid,1,t_PF,t_isLargeMemFlag> commutor;
        // commutor.template streamingDataCommutor<t_L,t_R,iid,T_T>(p_in,p_out);
...
// template<int tp_stages, int t_instanceID>
// template <int t_instanceID, int t_stage,int t_L, int t_R, int t_PF>
template <int t_instanceID, int t_stage, int t_subStage, int t_forkNumber, int t_L, int t_R, int t_PF>
template <typename T_dtype>
...
#pragma HLS STREAM variable = temp depth = 8
    ...
    // DataCommutationsS2Streaming<10000+tp_stages,0, iid, t_PF,t_isLargeMemFlag> commutor;
    // commutor.template streamingDataCommutor<t_L,t_R,iid,T_T>(p_in,temp);
    DataCommutationsS2Streaming<t_instanceID, t_stage, t_subStage, t_forkNumber, t_L, t_R, t_PF, t_isLargeMemFlag>
        commutor;
    commutor.template streamingDataCommutor<T_dtype>(p_in, temp);
    // Instantiate Next State....Recursion
    // InputTransposeChainStreamingS2S<tp_stages-1, 1, t_instanceID> nextStage;
    // nextStage.template swap<t_L,t_R, iid, (t_PF*t_R),T_T>(temp, p_out);  // Supply Next t_stage PF = PF*t_R
...
#pragma HLS STREAM variable = temp depth = 8
    ...
    // DataCommutationsS2Streaming<10000+tp_stages,tp_stages+t_PF,iid,t_PF,t_isLargeMemFlag> commutor;
    // commutor.template streamingDataCommutor<t_L,t_R,iid,T_T>(p_in,temp);
    DataCommutationsS2Streaming<t_instanceID, t_stage, t_subStage, t_forkNumber, t_L, t_R, t_PF, t_isLargeMemFlag>
        commutor;
    commutor.template streamingDataCommutor<T_dtype>(p_in, temp);
    // Instantiate Next State....Recursion
    // InputTransposeChainStreamingS2S<tp_stages-1,t_outputForkingFactor,t_instanceID> nextStage;
    // nextStage.template swap<t_L,t_R,iid,(t_PF*t_R),T_T>(temp, p_out);  // Supply Next t_stage PF
...
#pragma HLS STREAM variable = temp2 depth = 8
    ...
    // StreamingDataCommutorForkS2S<10000+t_outputForkingFactor,iid,t_outputForkingFactor,t_outputForkingFactor >
    // StreamingDataCommutorFork_obj;  StreamingDataCommutorFork_obj.template
    // forkedCompute<t_PF,t_L,t_R,iid,T_T>(temp,temp2);  template<int t_instanceID,int t_stage,int t_subStage,int
    // t_forkNumber,int t_L, int t_R, int t_PF, int t_forkingFactor>
    ...
    mergeSuperSampleStream<t_L, t_R, t_forkNumber, t_outputForkingFactor, T_dtype>(
        temp2, p_out); //(hls::stream< SuperSampleContainer< t_R/t_forkingFactor , T_T > > p_in[t_forkingFactor],
                       // hls::stream< SuperSampleContainer< t_R, T_T > > &outSuperStream)
}
