...
struct PrecisionMetrics {
     ...
     */
    void compute_snr(const std::vector<T>& golden_values, const std::vector<T>& dut_values) {
     ...
     */
    bool all_passed() const { return num_passed == num_samples; }

    /**
     * @brief Get pass rate percentage
     */
    float pass_rate() const { return num_samples > 0 ? (100.0f * num_passed / num_samples) : 0.0f; }
};
...
struct MultiOutputMetrics {
    ...
    void print_summary() const {
        ...
        std::cout << "  Pass rate:  " << std::fixed << std::setprecision(2) << (100.0f * total_passed / total_samples)
                  << "%\n";
