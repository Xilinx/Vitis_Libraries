...
AMD Vitis |trade| Security Library is an open-sourced Vitis library written in C++ for accelerating security applications in a variety of use cases.
...
Such a Symmetric Block Cipher, Symmetric Stream Cipher, Asymmetric Cryptography, Cipher Modes of Operations, Message Authentication Code, and Hash Function.
...
Benchmarks of four different acceleration applications are also provided with the library for easy on-boarding and comparison.
...
The recommended flow to evaluate and test L1 components is described as follows using AMD Vivado |trade| HLS tool.
...
As well as verifying functional correctness, the reports generated from this flow give an indication of logic utilization, timing performance, latency, and throughput. The output files of interest can be located at the location of the test project where the path name is correlated with the algorithm, that is, the callable function within the design under test.
