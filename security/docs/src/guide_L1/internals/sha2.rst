...
This library supports all algorithms mentioned above.
...
The internal structure of SHA-2 algorithms is seen in the following figure:
...
As seen from the figure, the SHA-2 hash calculation can be partitioned into two main parts.

* The pre-processing part pads or splits the input message into fixed sized blocks,
  and informs the down-stream parts about how many blocks you have in this message.
  The message word size is 32-bit for SHA-224/SHA-256, 64-bit for the rest four algorithms,
...
and generateMsgSchedule module is responsible to generate the message word stream in sequence.
...
the same internal structure, as illustrated in the preceding figure.
...
The hardware resource utilizations of SHA-224 are listed in :numref:`tab1SHA224` below:
...
The hardware resource utilizations of SHA-384 are listed in :numref:`tab1SHA384` below:
...
The hardware resource utilizations of SHA-512 are listed in :numref:`tab1SHA512` below:
...
The hardware resource utilizations of SHA-512/224 are listed in :numref:`tab1SHA512224` below:
...
The hardware resource utilizations of SHA-512/256 are listed in :numref:`tab1SHA512256` below:
...
To boost the throughput of SHA-2 primitives, multiple instances can be organized into a cluster,
