...
int main(int argc, const char* argv[]) {
    ...
    ap_uint<32>* adler = aligned_alloc<ap_uint<32> >(num);
    ap_uint<32>* adler32_out = aligned_alloc<ap_uint<32> >(num);
    ...
    mext_o[j++] = {3, adler, kernel()};
    mext_o[j++] = {4, data, kernel()};
    mext_o[j++] = {5, adler32_out, kernel()};
    ...
    cl::Buffer adler_buf = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                      sizeof(ap_uint<32>) * num, &mext_o[j++]);
    ...
    cl::Buffer adler32_buf = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                        sizeof(ap_uint<32>) * num, &mext_o[j++]);
    ...
    ob_in.push_back(adler_buf);
    ob_in.push_back(data_buf);
    ob_out.push_back(adler32_buf);
    ...
    kernel.setArg(j++, adler_buf);
    kernel.setArg(j++, data_buf);
    kernel.setArg(j++, adler32_buf);
    ...
    for (int i = 0; i < num; i++) {
        ap_uint<32> crc_out = adler32_out[i];
        if (golden != crc_out) {
