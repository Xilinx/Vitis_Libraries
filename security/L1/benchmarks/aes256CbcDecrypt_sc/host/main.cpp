...
uint64_t check(unsigned char* res, unsigned char* gld, uint64_t len) {
    ...
    for (uint64_t i = 0; i < len; i++) {
        // std::cout << "res[i]=" << res[i] << ", gld[i]=" << gld[i] << std::endl;
        if (res[i] != gld[i]) {
            num++;
...
bool aes256CbcDecrypt_execute_fpag(
    ...
    int si = 0;
    int ri = 0;
    auto hbInBP = aes256CbcDecrypt::create_bufpool(vpp::input);
    auto hbOutBP = aes256CbcDecrypt::create_bufpool(vpp::output);

    aes256CbcDecrypt::send_while([&]() -> bool {
        // std::cout << "---->> sending input" << std::endl;
        aes256CbcDecrypt::set_handle(si);

        ap_uint<128>* hbIn = (ap_uint<128>*)aes256CbcDecrypt::alloc_buf(hbInBP, sizeof(ap_uint<128>) * hb_in_size);
        ap_uint<128>* hbOut = (ap_uint<128>*)aes256CbcDecrypt::alloc_buf(hbOutBP, sizeof(ap_uint<128>) * hb_out_size);

        memcpy(hbIn, hb_in, sizeof(ap_uint<128>) * hb_in_size);

        aes256CbcDecrypt::compute(hb_in_size, hb_out_size, hbIn, hbOut);
        return (++si < num_rep);
    });

    aes256CbcDecrypt::receive_all_in_order([&]() {
        ap_uint<128>* hbOut = (ap_uint<128>*)aes256CbcDecrypt::get_buf(hbOutBP);
        memcpy(hb_outs[ri++], hbOut, sizeof(ap_uint<128>) * hb_out_size);

    });
    aes256CbcDecrypt::join();
...
int main(int argc, char* argv[]) {
    ...
    bool checked =
        aes256CbcDecrypt_execute_fpag(num_rep, in_pack_size / 16, out_pack_size / 16, (ap_uint<128>*)inputData, msg);
