...
int main() {
    std::cout << "**************************************************" << std::endl;
    std::cout << "   Testing GCM mode with AES-256 on HLS project   " << std::endl;
    std::cout << "**************************************************" << std::endl;
...
cout << "tag_golden : " << printr((unsigned char*)tag, BLK_SIZE) << endl;
        */
        ifile.read((char*)ciphertext, plaintext_len);
        ifile.read((char*)tag, BLK_SIZE);
    ...
    for (vector<Test>::const_iterator singletest = tests.begin(); singletest != tests.end(); singletest++) {
        ...
        lenCphStrm.write((*singletest).length * 8);
        lenAADStrm.write((*singletest).lenAAD * 8);
        endLenStrm.write(false);

        ap_uint<8 * BLK_SIZE> ciphertextReg;
        unsigned int n = 0;
        ...
        for (unsigned int i = 0; i < (*singletest).length; i++) {
            ...
            ciphertextReg.range(7 + 8 * n, 8 * n) = (unsigned char)((*singletest).result[i]);
            n++;
            if (n == BLK_SIZE) {
                ciphertextStrm.write(ciphertextReg);
                n = 0;
            }
        }
    ...
    // call fpga module
    test(ciphertextStrm, cipherkeyStrm, IVStrm, AADStrm, lenAADStrm, lenCphStrm, endLenStrm, plaintextStrm, lenPldStrm,
         tagStrm, endTagStrm);

    // check result
    ap_uint<8 * BLK_SIZE> plaintext_fpga;
    ap_uint<8 * BLK_SIZE> plaintext_golden;
    ...
    for (std::vector<Test>::const_iterator singletest = tests.begin(); singletest != tests.end(); ++singletest) {
        ...
        lenPlain = lenPldStrm.read();

        bool checked = true;
        int index = 0;
        for (unsigned int i = 0; i < (*singletest).length / BLK_SIZE; i++) {
            plaintext_fpga = plaintextStrm.read();
            for (unsigned int j = 0; j < BLK_SIZE; j++) {
                plaintext_golden.range(7 + 8 * j, 8 * j) = (unsigned char)((*singletest).data[j + index]);
            }
            index += BLK_SIZE;
            if (plaintext_golden != plaintext_fpga) {
                checked = false;
                cout << "fpga	: " << hex << plaintext_fpga << endl;
                cout << "golden : " << hex << plaintext_golden << endl;
            }
        }
        if ((*singletest).length % 16 > 0) {
            ...
            if (plaintext_golden.range((*singletest).length % BLK_SIZE * 8 - 1, 0) !=
                plaintext_fpga.range((*singletest).length % BLK_SIZE * 8 - 1, 0)) {
                checked = false;
                cout << "fpga	: " << hex << plaintext_fpga << endl;
                cout << "golden : " << hex << plaintext_golden << endl;
            }
