...
void injectData(std::string data, hls::stream<ap_uint<64> >& data_strm, hls::stream<ap_uint<5> >& len_strm) {
    ...
    if (total_len == 0) {
        ...
        for (size_t i = 0; i < total_len; i += 8) {
            if (i + 8 >= total_len) { // last pack
                tmp_len = total_len - i;
                tmp_len[4] = 1; // mark last pack
            } else {            // not last pack
                tmp_len = 8;
            }

            tmp_data = 0;
            for (int j = 0; j < tmp_len.range(3, 0); j++) { // fill in pack
...
int main() {
    ...
    hls::stream<ap_uint<5> > len_strm("len_strm");
