...
struct md4_wrapper {
    static void hash(hls::stream<ap_uint<32> >& msgStrm,
        ...
        xf::security::md4(msgStrm, lenStrm, eLenStrm, hshStrm, eHshStrm);
    }
};
...
void hmacMD4(const unsigned char* key,
    ...
    if (keyLen > BLOCK_SIZE) {
        MD4((const unsigned char*)key, keyLen, (unsigned char*)h1);
    ...
    MD4((const unsigned char*)kmsg, BLOCK_SIZE + msgLen, (unsigned char*)h2);
...
int main() {
    std::cout << "********************************" << std::endl;
    std::cout << "   Testing hmac+MD4 on HLS project   " << std::endl;
    std::cout << "********************************" << std::endl;
