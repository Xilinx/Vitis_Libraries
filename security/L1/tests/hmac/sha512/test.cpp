...
struct sha512_wrapper {
    // lW=128 bits, BLOCK_SIZE=128 bytes, hshW = 512 bits
    static void hash(hls::stream<ap_uint<msgW> >& msgStrm,
        ...
        xf::security::sha512<msgW>(msgStrm, lenStrm, eLenStrm, hshStrm, eHshStrm);
    }
};
...
// compute golden hmac
void hmacSHA512(const unsigned char* key,
                unsigned int keyLen,
    ...
    if (keyLen > BLOCK_SIZE) {
        SHA512((const unsigned char*)key, keyLen, (unsigned char*)h1);
    ...
    SHA512((const unsigned char*)kmsg, BLOCK_SIZE + msgLen, (unsigned char*)h2);
    ...
    SHA512((const unsigned char*)khsh, BLOCK_SIZE + HASH_SIZE, (unsigned char*)h);
}

int main() {
    std::cout << "********************************" << std::endl;
    std::cout << "   Testing hmac+SHA512 on HLS project   " << std::endl;
    std::cout << "********************************" << std::endl;
    ...
    for (unsigned int i = 0; i < NUM_TESTS; i++) {
        ...
        Test t(k, m, h);
        hmacSHA512((const unsigned char*)k, t.key.length(), (const unsigned char*)m, t.msg.length(), (unsigned char*)h);
