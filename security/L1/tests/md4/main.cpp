...
int main() {
    std::cout << "********************************" << std::endl;
    std::cout << "   Testing MD4 on HLS project   " << std::endl;
    std::cout << "********************************" << std::endl;
    ...
    for (unsigned int i = 0; i < NUM_TESTS; i++) {
        ...
        // call OpenSSL API to get the MD4 hash value of each message
        // MD4((const unsigned char*)message, len, (unsigned char*)h);
    ...
    hls::stream<ap_uint<128> > digest_strm("digest_strm");
    ...
    // check result
    for (vector<Test>::const_iterator test = tests.begin(); test != tests.end(); test++) {
        ap_uint<128> digest = digest_strm.read();
