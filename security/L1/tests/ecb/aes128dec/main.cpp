...
int main() {
    std::cout << "**************************************************" << std::endl;
    std::cout << "   Testing ECB mode with AES-128 on HLS project   " << std::endl;
    std::cout << "**************************************************" << std::endl;
    ...
    for (unsigned int i = 1; i < NUM_TESTS + 1; i++) {
        ...
        /*
        EVP_CIPHER_CTX ctx;
        EVP_DecryptInit(&ctx, EVP_aes_128_ecb(), key, ivec);
    ...
    for (vector<Test>::const_iterator singletest = tests.begin(); singletest != tests.end(); singletest++) {
        ...
        // call fpga module
        test(ciphertextStrm, endCiphertextStrm, cipherkeyStrm, plaintextStrm, endPlaintextStrm);
