...
int main() {
    std::cout << "**************************************************" << std::endl;
    std::cout << "   Testing XTS mode with AES-256 on HLS project   " << std::endl;
    std::cout << "**************************************************" << std::endl;
    ...
    const unsigned char key[] =
        ...
        "abcdefghijklmnopqrstuvexyzabcdef";
    ...
    for (unsigned int i = 1; i < NUM_TESTS + 1; i++) {
        ...
        EVP_EncryptInit_ex(ctx, EVP_aes_256_xts(), NULL, key, ivec);
    ...
    for (vector<Test>::const_iterator singletest = tests.begin(); singletest != tests.end(); singletest++) {
        ...
        for (unsigned int i = 0; i < (*singletest).length; i++) {
            ...
            datainReg.range(7 + 8 * n, 8 * n) = (unsigned char)((*singletest).result[i]);
            n++;
            if (n == BLK_SIZE) {
        ...
        // call fpga module
        test(ciphertextStrm, endCiphertextStrm, lenStrm, cipherkeyStrm, IVStrm, plaintextStrm, endPlaintextStrm);
        ...
        while (!end) {
            ...
            for (unsigned int i = 0; i < BLK_SIZE; i++) {
                golden.range(7 + 8 * i, 8 * i) = (unsigned char)((*singletest).data[i + index]);
            }
            if (golden != plaintext) {
                checked = false;
            }
