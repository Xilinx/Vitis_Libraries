...
void test(ap_uint<2048> plaintext,
          ap_uint<20> exponent,
          ap_uint<2048> modulus,
          ap_uint<2048> rMod,
          ap_uint<2048>& ciphertext) {
    ciphertext = xf::security::internal::modularExp<2048, 20>(plaintext, exponent, modulus, rMod);
}

ap_uint<2048> mont(ap_uint<2048> a, ap_uint<2048> modulus) {
    ap_uint<4096> tmp = a;
    tmp <<= 2048;
    tmp %= modulus;
    ap_uint<2048> res = tmp;
    return res;
}
...
int main() {
    ...
    unsigned char rawm[256] =
        ...
        "ijklmnopqrstuvwxyz.";
    for (int i = 0; i < 256; i++) {
        plaintext.range(i * 8 + 7, i * 8) = (unsigned int)rawm[255 - i];
    }
    plaintext.range(7, 0) = ap_uint<8>("0x0a");
    ...
    tmp <<= 4096;
    tmp %= modulus;
    ap_uint<2048> rMod = tmp;

    ap_uint<2048> result;
    test(plaintext, exponent, modulus, rMod, result);
