...
#pragma HLS PIPELINE II = 1
    inputWindow.range((i + 1) * c_parallelBit - 1, i * c_parallelBit) =
        inStream.read();
...
#pragma HLS PIPELINE II = 1
  ...
  for (; (processedBytes + inputIdx) < inputSize;) {
    ...
    for (uint32_t blockData = 0; blockData < compLen;
         blockData += PARALLEL_BYTES) {
#pragma HLS PIPELINE II = 1
      if ((inputIdx >= PARALLEL_BYTES)) {
        inputWindow >>= c_parallelBit;
        if (readBytes < inputSize) {
...
inline void lz4DecompressSimple(hls::stream<ap_uint<8>> &inStream,
                                hls::stream<ap_uint<32>> &outStream,
                                uint32_t input_size, bool uncomp_flag) {
  enum lz4DecompressStates {
    READ_TOKEN,
    READ_LIT_LEN,
    READ_LITERAL,
...
inline void
lz4MultiByteDecompress(hls::stream<ap_uint<PARALLEL_BYTES * 8>> &inStream,
  ...
  for (dt_lz4BlockInfo bInfo = blockInfoStream.read();
       bInfo.compressedSize != 0; bInfo = blockInfoStream.read()) {
...
#pragma HLS PIPELINE II = 1
      ...
      input_window.range(((i + 1) * c_parallelBit) - 1, i * c_parallelBit) =
          inValue;
    }
...
#pragma HLS PIPELINE II = 1
  ...
  if (!blockInfoStream.empty())
    dt_lz4BlockInfo bInfo = blockInfoStream.read();
...
#pragma HLS dataflow
  lz4MultiByteDecompress<PARALLEL_BYTES>(inStream, litlenStream, litStream,
                                         offsetStream, matchlenStream,
                                         blockInfoStream);
...
#pragma HLS dataflow
  lz4HeaderProcessing<PARALLEL_BYTES>(inStream, headerStream, blockInfoStream,
                                      input_size1);
  lz4MultiByteDecompress<PARALLEL_BYTES, uint32_t>(
      headerStream, litlenStream, litStream, offsetStream, matchlenStream,
      blockInfoStream);
  details::lzPreProcessingUnitLL<uint32_t, PARALLEL_BYTES>(
      litlenStream, matchlenStream, offsetStream, lzInfoStream);
  lzMultiByteDecompressLL<PARALLEL_BYTES, HISTORY_SIZE, 16, ap_uint<17>>(
...
template <int PARALLEL_BYTES, class SIZE_DT = uint32_t>
inline void
lz4MultiByteDecompress_opt1(hls::stream<ap_uint<PARALLEL_BYTES * 8>> &inStream,
                            hls::stream<SIZE_DT> &litlenStream,
                            hls::stream<ap_uint<PARALLEL_BYTES * 8>> &litStream,
                            hls::stream<ap_uint<16>> &offsetStream,
                            hls::stream<SIZE_DT> &matchlenStream,
  ...
  for (bInfo = blockInfoStream.read(); bInfo.compressedSize != 0;
       bInfo = blockInfoStream.read()) {
    uint32_t input_size = bInfo.compressedSize;
      ...
      if (next_state == READ_LITLEN_LIT_OFFSET) {
        ...
        if (c0) {
          ...
          outFlag = false;
                   ...
                   ((token_match_len != 0xF))) { //(token_match_len !=0xF)
...
void lz4DecompressEngine_NinMout(
    hls::stream<ap_uint<PARALLEL_BYTES * 8>> &inStream,
    hls::stream<ap_uint<(PARALLEL_OUT_BYTES * 8) + PARALLEL_OUT_BYTES>>
        &outStream, // opt2
    const uint32_t _input_size) {
...
#pragma HLS dataflow
  ...
  details::lzPreProcessingUnitLL_opt2<uint32_t, PARALLEL_BYTES,
                                      PARALLEL_OUT_BYTES>(
      litlenStream, matchlenStream, offsetStream, lzInfoStream);

  lzMultiByteDecompressLL_opt2<PARALLEL_BYTES, PARALLEL_OUT_BYTES, HISTORY_SIZE,
                               16, ap_uint<17>>(litStream, lzInfoStream,
                                                outStream);
}
