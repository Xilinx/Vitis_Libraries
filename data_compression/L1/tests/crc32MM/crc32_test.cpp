...
// DUT
void hls_crc32MM(const ap_uint<PARALLEL_BYTE * 8>* in, ap_uint<32>* crcData, ap_uint<32> inputSize) {
#pragma HLS INTERFACE m_axi port = in offset = slave bundle = gmem depth = c_size
#pragma HLS INTERFACE m_axi port = crcData offset = slave bundle = gmem depth = 2
...
#pragma HLS INTERFACE s_axilite port = return bundle = control
    ...
    xf::compression::crc32_mm<PARALLEL_BYTE>(in, crcData, inputSize);
}
...
int main(int argc, char* argv[]) {
    ...
    auto golden = crc32(crcTmp, reinterpret_cast<const unsigned char*>(in), size);
    ...
    for (auto t = 0; t < no_blks; t++) {
        ...
        hls_crc32MM(&in[offset], &crcData, inSize);
        crcData = ~crcData;
    }
