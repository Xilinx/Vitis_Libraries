...
void validate(std::string& original, std::string& generated) {
    std::fstream orig;
    orig.open(original.c_str(), std::fstream::binary | std::fstream::in);
    if (!orig.is_open()) {
        std::cout << "Cannot open the input file!!" << original << std::endl;
        std::cout << "TEST FAILED" << std::endl;
        exit(0);
    }

    std::fstream gen;
    gen.open(generated.c_str(), std::fstream::binary | std::fstream::in);
    if (!gen.is_open()) {
        std::cout << "Cannot open the generated file!!" << generated << std::endl;
        std::cout << "TEST FAILED" << std::endl;
        exit(0);
    }

    orig.seekg(0, std::ios::end); // reaching to end of file
    gen.seekg(0, std::ios::end);  // reaching to end of file
    const uint32_t origSize = (uint32_t)orig.tellg();
    const uint32_t genFileSize = (uint32_t)gen.tellg();
    orig.seekg(0, std::ios::beg); // reaching to end of file
    gen.seekg(0, std::ios::beg);  // reaching to end of file

    if (origSize != genFileSize) {
        std::cout << "Original File size mismatch" << std::endl;
        std::cout << "TEST FAILED" << std::endl;
        return;
    }
    for (uint32_t i = 0; i < origSize; i++) {
        ...
        gen.read((char*)&g, 1);
        if (o != g) {
            std::cout << "Expected=" << (char)o << " Got=" << (char)g << std::endl;
            std::cout << "TEST FAILED" << std::endl;
            return;
        }
    }
