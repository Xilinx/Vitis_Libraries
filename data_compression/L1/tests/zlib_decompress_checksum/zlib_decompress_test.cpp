...
void zlibMultiByteDecompressEngineRun(hls::stream<in_t>& inStream, hls::stream<out_t>& outStream)
...
#pragma HLS DATAFLOW
    ...
    xf::compression::inflate<c_decoderType, MULTIPLE_BYTES, xf::compression::FileFormat::BOTH, HISTORY_SIZE,
                             TUSER_WIDTH>(inStream, outStream);
}
...
void validateFile(std::string& fileName, std::string& originalFileName) {
    ...
    // DECOMPRESSION CALL
    zlibMultiByteDecompressEngineRun(inStream, outStream);
    ...
    do {
        val = outStream.read();
        ap_uint<MULTIPLE_BYTES* 8> o = val.data;
        auto w_size = MULTIPLE_BYTES;
        ...
        if (!pass) break;
        errorCheck = val.user.range(0, 0);
        sizeReceived = val.user.range(TUSER_WIDTH - 1, 1);
    } while (!val.last);

    assert(outCnt == sizeReceived);
    std::cout << "Decompressed size: " << std::dec << outCnt << " ";
    assert(errorCheck == false);
    outFile.close();
    if (pass) {
        std::cout << "\nTEST PASSED\n" << std::endl;
    } else {
...
int main(int argc, char* argv[]) {
    ...
    if (!listFileName.empty()) {
        ...
        while (std::getline(infilelist, curFileName)) {
            ...
            auto dcmpFileName = currentPath + "/" + curFileName + ".zlib"; // compressed file path
