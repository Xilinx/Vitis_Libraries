...
class xil_zlib {
   public:
    ...
    int release();
    uint32_t compress(uint8_t* in, uint8_t* out, uint32_t input_size);
   ...
   private:
    ...
    cl::Kernel* decompress_kernel[T_COMPUTE_UNIT];
    ...
    cl_mem_ext_ptr_t inExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT], outExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT],
        gzoutExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT], cssizeExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT],
        insizeExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT];
    cl_mem_ext_ptr_t dltreeFreqExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT],
        ddtreeFreqExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT], dbltreeFreqExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT];
    cl_mem_ext_ptr_t dltreeBlenExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT],
        ddtreeBlenExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT], dbltreeBlenExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT];
    cl_mem_ext_ptr_t dltreeCodesExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT],
        ddtreeCodesExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT], dbltreeCodesExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT];
    cl_mem_ext_ptr_t maxCodeExt[MAX_CCOMP_UNITS][OVERLAP_BUF_COUNT];

    // Kernel names
    std::vector<std::string> compress_kernel_names = {
        "xilLz77_cu1", "xilLz77_cu2", "xilLz77_cu3", "xilLz77_cu4",
    };

    std::vector<std::string> huffman_kernel_names = {
        "xilHuffman_cu1", "xilHuffman_cu2", "xilHuffman_cu3", "xilHuffman_cu4",
    };

    std::vector<std::string> treegen_kernel_names = {
        "xilTreegen_cu1", "xilTreegen_cu2", "xilTreegen_cu3", "xilTreegen_cu4",
    };

    std::vector<std::string> decompress_kernel_names = {
        "xilDecompress_cu1", "xilDecompress_cu2", "xilDecompress_cu3", "xilDecompress_cu4",
    };

    // DDR numbers
    std::vector<uint32_t> comp_ddr_nums = {
        XCL_MEM_DDR_BANK0, XCL_MEM_DDR_BANK3, XCL_MEM_DDR_BANK2, XCL_MEM_DDR_BANK3,
    };

    std::vector<uint32_t> decomp_ddr_nums = {
