...
#include "zlibd_mc_s2mm_tester.hpp"
...
void streamDataK2dmSync(hls::stream<ap_uint<STREAMDWIDTH> >& out,
                        ...
                        hls::stream<ap_axiu<STREAMDWIDTH, TUSER_WIDTH, 0, 0> >& dmOutStream,
    ...
    for (auto z = 0; z < numItr; z++) {
        ap_axiu<STREAMDWIDTH, TUSER_WIDTH, 0, 0> dataout;
        uint32_t strb;
        uint32_t outSize = 0;
        bool last = false;
        last_block = already_done;
        if (!already_done) {
        streamDataK2dm:
...
void gzipS2MM(uintMemWidth_t* out,
              ...
              uint32_t readBlockSize,
              uint32_t numItr,
              hls::stream<ap_axiu<OUTPUT_BYTES * 8, TUSER_WIDTH, 0, 0> >& outAxiStream,
              hls::stream<bool>& dcStatusStream) {
    hls::stream<uintMemWidth_t> outHlsStream("outputStream");
    hls::stream<bool> outHlsStream_eos("outputEosStream");

#pragma HLS STREAM variable = outHlsStream depth = 8
#pragma HLS STREAM variable = outHlsStream_eos depth = 8
...
#pragma HLS STREAM variable = outsize_val depth = 8

#pragma HLS dataflow
    streamDataK2dmSync<OUTPUT_BYTES * 8>(outHlsStream, outHlsStream_eos, readBlockSize, numItr, outsize_val,
                                         outAxiStream, cur_status, dcStatusStream);

    xf::compression::details::s2mmEosSimple<OUTPUT_BYTES * 8, kGMemBurstSize>(out, outHlsStream, outHlsStream_eos,
                                                                              numItr, outsize_val, encoded_size);
}
...
void xilGzipS2MM(uintMemWidth_t* out,
                 ...
                 uint32_t numItr,
                 hls::stream<ap_axiu<OUTPUT_BYTES * 8, TUSER_WIDTH, 0, 0> >& inStream) {
...
#pragma HLS STREAM variable = dcStatusStream depth = 8

    uint32_t st_flg = status_flag[0];

    gzipS2MM(out, encoded_size, st_flg, read_block_size, numItr, inStream, dcStatusStream);
