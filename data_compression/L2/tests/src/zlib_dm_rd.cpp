...
#include "zlib_dm_rd.hpp"

const int kGMemBurstSize = 64;
...
#pragma HLS PIPELINE II = 1
    ...
    bytEos << 1;
    out << 0;
    dataSize << outSize; // read encoded size from decompression kernel for this transfer
}
...
#pragma HLS dataflow
    streamDataK2dmSync<kGMemDWidth>(outHlsStream, outHlsStream_eos, readBlockSize, outsize_val, outAxiStream,
                                    sizestreamk, cur_status, dcStatusStream);

    xf::compression::details::s2mmEosSimple<kGMemDWidth, kGMemBurstSize>(out, outHlsStream, outHlsStream_eos,
