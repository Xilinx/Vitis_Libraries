...
void xilChecksumTop(std::string& compress_mod, bool checksum_type, std::string& compress_bin) {
    ...
    std::vector<ap_uint<8> > in(size);
    ifs.read(reinterpret_cast<char*>(in.data()), size);

    uint32_t golden = 0;
    if (checksum_type == false) {
        unsigned long checksumTmp = 1;
        golden = adler32(checksumTmp, reinterpret_cast<const unsigned char*>(in.data()), size);
    } else {
        unsigned long checksumTmp = 0;
        golden = crc32(checksumTmp, reinterpret_cast<const unsigned char*>(in.data()), size);
    }
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err, krnl_checksum = cl::Kernel(program, "xilChecksum32", &err));
            valid_device++;
    ...
    std::vector<uint32_t, aligned_allocator<uint32_t> > buf_in_data(1);
    std::vector<uint8_t, aligned_allocator<uint8_t> > buf_in(HOST_BUFFER_SIZE);
    buf_in_data[0] = 1;
    if (checksum_type == true) buf_in_data[0] = ~0;
    ...
    cl::Buffer buffer_init_data(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(uint32_t), buf_in_data.data());
    ...
    for (uint32_t i = 0; i < no_blocks; i++) {
        ...
        krnl_checksum.setArg(narg++, buffer_init_data);
        krnl_checksum.setArg(narg++, host_buffer_size);
        krnl_checksum.setArg(narg++, checksum_type);

        // Copy input data to device global memory
        q.enqueueMigrateMemObjects({buffer_input, buffer_init_data}, 0 /* 0 means from host*/);
        // Launch the Kernel
        ...
        q.enqueueMigrateMemObjects({buffer_init_data}, CL_MIGRATE_MEM_OBJECT_HOST);
        q.finish();

        result = buf_in_data[0];
    }

    if (checksum_type == true) result = ~result;
...
int main(int argc, char* argv[]) {
    ...
    parser.addSwitch("--checksum_type", "-t", "Checksum Type [0: Adler32; 1: CRC32]", "0");
    parser.parse(argc, argv);

    std::string compress_bin = parser.value("xclbin");
    std::string compress_mod = parser.value("compress");
    std::string checksum_type = parser.value("checksum_type");

    bool checksumType = (checksum_type == "1");

    if (!compress_mod.empty()) xilChecksumTop(compress_mod, checksumType, compress_bin);
}
