...
int main(int argc, char* argv[]) {
    ...
    bool free_running_kernel = true;

    // Driver class object
    gzipApp d(argc, argv);

    // Design class object creating and constructor invocation
    std::unique_ptr<gzipXrtHost> gzip(new gzipXrtHost(flow, d.getXclbin(), d.getInFileName(), d.getDeviceId(),
                                                      decKernelType, d.getDesignFlow(), free_running_kernel));
