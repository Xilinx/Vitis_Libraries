...
void xilCompressDecompressList(
    ...
    do {
        ...
        uint64_t enbytes = xzl.compressFileFull(lz_in, lz_out, input_size);
...
void xilBatchVerify(std::string& file_list, uint32_t block_size, std::string& compress_bin) {
    // Start of Flow : Xilinx ZLIB Compress with Standard GZIP Decompress
...
#endif
    // Call Zlib compression
    uint64_t enbytes = xzl.compressFileFull(zlib_compress_in, zlib_compress_out, input_size);
#ifdef EVENT_PROFILE
