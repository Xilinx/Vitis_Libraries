...
bool Pre_Check(const char* input, Token& Formula) {
    bool check_result = 1;
    int outLen = 0;
    int i = 0;
...
// Judge op priority
int priority(std::string Operator) {
    int result = 0;
...
void Reverse_Polish(Token& Input_Formula, Token& Output_Formula) {
...
bool is_mux(std::string op) {
    return (op == ":" || op == "?" || op == "[" || op == "]");
}

// build parse tree
Node* build_parse_tree(Token& Input_Formula, int target_tree_height) {
    // stack is empty when top==0
...
bool check_node(Node* tree, int target_height) {
    bool success = 1;
...
// tree
bool Parser(Node* tree, int target_height) {
    bool success = 1;
...
int Operator_Look_Up_Table(std::string Operator) {
    int result;
