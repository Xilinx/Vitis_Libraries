...
class XHost {
  protected:
    ...
    unordered_map<void*, cl::Buffer> m_devHandle;
  ...
  public:
    ...
    XHost ( const string & p_xclbin, const string & p_kernelName, xfblasStatus_t* p_status) {
      ...
      l_err = xclGetMemObjDeviceAddress(this->m_clInstrBuf.get(), m_fpga->m_Device.get(),
    ...
    bool addMat(void* p_handle, void * p_matPtr, unsigned long long p_bufSize) {
    ...
    xfblasStatus_t allocMat(void* p_handle, void * p_matPtr, unsigned long long p_bufSize){
    ...
    xfblasStatus_t setMatToFPGA(void* p_handle){
      auto &l_devPtr = m_devHandle;
      if (l_devPtr.find(p_handle) != l_devPtr.end()) {
    ...
    xfblasStatus_t getMat(void* p_handle, bool p_syncGet = true) {
      auto &l_hostPtr = m_hostMat;
      auto &l_devPtr = m_devHandle;
      if (l_hostPtr.find(p_handle) != l_hostPtr.end()) {
    ...
    xfblasStatus_t freeMat(void* p_handle) {
      auto &l_hostPtr = m_hostMat;
      if (l_hostPtr.find(p_handle) == l_hostPtr.end()) {
        return XFBLAS_STATUS_ALLOC_FAILED;
      } else {
...
class BLASHost : public XHost {
  ...
  public:
    ...
    BLASHost(const string & p_xclbin, const string & p_kernelName, xfblasStatus_t* p_status) : XHost ( p_xclbin, p_kernelName, p_status) {}
