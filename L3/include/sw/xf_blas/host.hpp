...
public:
  ...
  XFpga(const char *p_xclbin, const char *p_logFile, int *p_err) {
    ...
    struct mem_topology *l_topology =
        (mem_topology *)(l_header + l_topo->m_sectionOffset);
  ...
  bool copyToFpga(unsigned int p_bufHandle, size_t p_szBytes) {
    if (xclSyncBO(m_handle, p_bufHandle, XCL_BO_SYNC_BO_TO_DEVICE, p_szBytes,
                  0)) {
      return false;
    }
    return true;
  ...
  bool copyFromFpga(unsigned int p_bufHandle, size_t p_szBytes) {
    while (xclExecWait(m_handle, 1) == 0)
      ;
    if (xclSyncBO(m_handle, p_bufHandle, XCL_BO_SYNC_BO_FROM_DEVICE, p_szBytes,
                  0)) {
      return false;
    }
    return true;
  ...
  bool execKernel(unsigned int p_kernelIndex) {
    ...
    auto rsz = XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_P_DDRWR_M_VAL_DATA / 4 +
               2; // regmap array size
    ...
    ecmd->data[XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_AP_CTRL] =
        0x0; // ap_start
    ecmd->data[XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_P_DDRRD_M_VAL_DATA / 4] =
        m_baseAddress[p_kernelIndex];
    ecmd->data[XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_P_DDRWR_M_VAL_DATA / 4] =
        m_baseAddress[p_kernelIndex];
    ecmd->data[XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_P_DDRRD_M_VAL_DATA / 4 +
               1] = m_baseAddress[p_kernelIndex] >> 32;
    ecmd->data[XGEMXKERNEL_0_GEMXKERNEL_0_CONTROL_ADDR_P_DDRWR_M_VAL_DATA / 4 +
               1] = m_baseAddress[p_kernelIndex] >> 32;
...
public:
  XHost() = delete;
  XHost(const char *p_xclbin, const char *p_logFile, xfblasStatus_t *p_status,
        unsigned int p_kernelIndex) {
    ...
    int l_memAllocStatus =
        posix_memalign(&l_alignedMem, PAGE_SIZE, INSTR_BUF_SIZE);
    if (l_memAllocStatus) {
    ...
    m_instrOffset = 0;
    m_instrBufHandle = m_fpga->createBuf(
        m_instrBuf, INSTR_BUF_SIZE + KERN_DBG_BUF_SIZE, m_cuIndex);
  }

  bool addMatRestricted(void *p_hostHandle, void *p_matPtr,
  ...
  xfblasStatus_t allocMatRestricted(void *p_hostHandle, void *p_matPtr,
                                    unsigned long long p_bufSize) {
    ...
    if (l_devPtr.find(p_hostHandle) != l_devPtr.end()) {
      ...
      l_devPtr[p_hostHandle] = m_fpga->createBuf(
          l_hostPtr[p_hostHandle], l_hostSzPtr[p_hostHandle], m_cuIndex);
  ...
  xfblasStatus_t allocMat(t_dataType *p_devPtr, size_t p_bufSize) {
    ...
    if (l_devPtr.find(*p_devPtr) != l_devPtr.end()) {
      ...
      unsigned int l_deviceHandle =
          xclAllocBO(m_fpga->m_handle, p_bufSize, XCL_BO_DEVICE_RAM,
                     m_fpga->m_mem[m_cuIndex]);
  ...
  template <typename t_dataType>
  xfblasStatus_t setMatToFPGA(void *p_hostHandle, int p_rows, int p_lda,
                              int p_paddedLda, t_dataType &p_hostPtr,
                              t_dataType &p_devPtr) {
    auto &l_devPtr = m_bufHandle;
    auto &l_hostSzPtr = m_hostMatSz;
    if (l_devPtr.find(p_hostHandle) != l_devPtr.end()) {
      ...
      if (!m_fpga->copyToFpga(l_devPtr[p_hostHandle],
                              l_hostSzPtr[p_hostHandle])) {
        return XFBLAS_STATUS_ALLOC_FAILED;
      }
    } else {
      return XFBLAS_STATUS_ALLOC_FAILED;
  ...
  template <typename t_dataType>
  xfblasStatus_t getMat(void *p_hostHandle, int p_rows, int p_lda,
                        int p_paddedLda, t_dataType &p_hostPtr,
                        t_dataType &p_devPtr) {
    auto &l_hostSzPtr = m_hostMatSz;
    auto &l_devPtr = m_bufHandle;
    if (l_devPtr.find(p_hostHandle) != l_devPtr.end()) {
      if (!m_fpga->copyFromFpga(l_devPtr[p_hostHandle],
                                l_hostSzPtr[p_hostHandle])) {
        return XFBLAS_STATUS_ALLOC_FAILED;
      }
      for (int i = 0; i < p_rows; i++) {
...
public:
  ...
  BLASHost(const char *p_xclbin, const char *p_logFile,
           xfblasStatus_t *p_status, unsigned int p_kernelIndex)
      : XHost(p_xclbin, p_logFile, p_status, p_kernelIndex) {}

  xfblasStatus_t execute() {
    xfblasStatus_t l_status = XFBLAS_STATUS_SUCCESS;
    if (m_execControl) {
      if (!this->m_fpga->copyToFpga(this->m_instrBufHandle,
                                    this->INSTR_BUF_SIZE +
                                        this->KERN_DBG_BUF_SIZE)) {
