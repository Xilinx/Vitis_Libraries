...
public:
  ...
  GemmArgs(unsigned int p_aOffset, unsigned int p_bOffset,
           unsigned int p_cOffset, unsigned int p_xOffset, unsigned int p_m,
           unsigned int p_k, unsigned int p_n, unsigned int p_lda,
           unsigned int p_ldb, unsigned int p_ldc, unsigned int p_ldx,
           int p_postScale, int p_postShift)
      : m_GemmArgs({int(OpGemm), p_aOffset, p_bOffset, p_cOffset, p_xOffset,
                    p_m, p_k, p_n, p_lda, p_ldb, p_ldc, p_ldx, 0, 0, 0, 0}) {
...
public:
  ...
  GEMMHost(const char *p_xclbin, const char *p_logFile,
           xfblasStatus_t *p_status, unsigned int p_kernelIndex)
      : BLASHost(p_xclbin, p_logFile, p_status, p_kernelIndex) {}

  virtual xfblasStatus_t addGEMMOp(void *p_a, void *p_b, void *p_c,
                                   void *p_bias, unsigned int p_m,
                                   unsigned int p_k, unsigned int p_n,
                                   unsigned int p_lda, unsigned int p_ldb,
                                   unsigned int p_ldc, unsigned int p_ldx,
                                   int p_postScale, int p_postShift) {
    ...
    xclBOProperties p;
    uint64_t address_A =
        !xclGetBOProperties(this->m_fpga->m_handle, handle_A, &p) ? p.paddr
                                                                  : -1;
    uint64_t address_B =
        !xclGetBOProperties(this->m_fpga->m_handle, handle_B, &p) ? p.paddr
                                                                  : -1;
    uint64_t address_C =
        !xclGetBOProperties(this->m_fpga->m_handle, handle_C, &p) ? p.paddr
                                                                  : -1;
    uint64_t address_bias =
        !xclGetBOProperties(this->m_fpga->m_handle, handle_bias, &p) ? p.paddr
                                                                     : -1;
    ...
    GemmArgs l_gargs(l_aOff, l_bOff, l_cOff, l_xOff, p_m, p_k, p_n, p_lda,
                     p_ldb, p_ldc, p_ldx, p_postScale, p_postShift);
