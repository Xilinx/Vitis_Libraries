...
const char* CFBlackScholes::getKernelName() {
    return KERNEL_NAME;
}
...
std::string CFBlackScholes::getXCLBINName(Device* device) {
...
int CFBlackScholes::createOCLObjects(Device* device) {
    int retval = XLNX_OK;
    ...
    /////////////////////////
    if (cl_retval == CL_SUCCESS) {
        m_pKernel = new cl::Kernel(*m_pProgram, getKernelName(), &cl_retval);
    }
    ...
    if (cl_retval == CL_SUCCESS) {
        m_pStockPriceHWBuffer =
            new cl::Buffer(*m_pContext, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY,
                           m_numPaddedBufferElements * sizeof(KDataType), this->stockPrice, &cl_retval);
    }
...
int CFBlackScholes::releaseOCLObjects(void) {
    int retval = XLNX_OK;
    unsigned int i;

    if (m_pStockPriceHWBuffer != nullptr) {
        delete (m_pStockPriceHWBuffer);
        m_pStockPriceHWBuffer = nullptr;
    }
...
int CFBlackScholes::run(OptionType optionType, unsigned int numAssets) {
    int retval = XLNX_OK;
