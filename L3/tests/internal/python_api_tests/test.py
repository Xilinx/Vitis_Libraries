...
class Test:
  ...
  def test_basic_gemm(self, m, k, n, xclbin_opts, idxKernel=0, idxDevice=0, minRange=-16384, maxRange=16384):
    if xclbin_opts['GEMX_dataType'] == 'short':
        dtype = np.int16
    elif xclbin_opts['GEMX_dataType'] == 'float':
        dtype = np.float32
    else:
  ...
  def test_basic_fcn(self, m, k, n, xclbin_opts, post_val=[1,0], pRelu_val=[1,0], idxKernel=0, idxDevice=0, minRange=-16384, maxRange=16384):
    if xclbin_opts['GEMX_dataType'] == 'short':
        dtype = np.int16
    elif xclbin_opts['GEMX_dataType'] == 'float':
        dtype = np.float32
        ...
        raise TypeError("type", xclbin_opts["GEMX_dataType"], "not supported") 
    if xclbin_opts['GEMX_XdataType'] == 'short':
        Xdtype = np.int16
    elif xclbin_opts['GEMX_XdataType'] == 'float':
        Xdtype = np.float32
    elif xclbin_opts['GEMX_XdataType'] == 'int32_t':
        Xdtype = np.int32
    else:
        raise TypeError("type", xclbin_opts["GEMX_dataType"], "not supported") 
    ddrWidth = int(xclbin_opts["GEMX_ddrWidth"])
    ...
    if dtype != np.float32:
        golden_C = ((golden_C * post_val[0]).astype(dtype) // (2 ** post_val[1])).astype(dtype)
        if pRelu_val != [1,0]:
            golden_C = np.where(golden_C>0,golden_C,golden_C * pRelu_val[0] // (2 ** pRelu_val[1]))
    else:
        if pRelu_val != [1,0]:
            golden_C = np.where(golden_C>0,golden_C,golden_C * pRelu_val[0])
    xfblas.sendMat(A,idxKernel, idxDevice)
    xfblas.sendMat(B,idxKernel, idxDevice)
    xfblas.sendMat(C,idxKernel, idxDevice)
    xfblas.sendMat(X,idxKernel, idxDevice)
    xfblas.fcnOp(A,B,C,X,post_val[0],post_val[1],pRelu_val[0],pRelu_val[1],idxKernel, idxDevice)
    xfblas.getMat(C,idxKernel, idxDevice)
    if dtype == np.int16:
        self.cmp(C,golden_C)
    else:
