...
void xil_compress_decompress_list(std::string& file_list,
    ...
    xfZlib xlz(single_bin, max_cr, BOTH, device_id, 0, FULL, XILINX_ZLIB);
...
void xil_decompress_top(
    ...
    // Xilinx ZLIB object
    xfZlib xlz(single_bin, max_cr, DECOMP_ONLY, device_id, 0, FULL, XILINX_ZLIB);
    ERROR_STATUS(xlz.error_code());
...
void xil_compress_top(std::string& compress_mod, std::string& single_bin, uint8_t device_id, uint8_t max_cr) {
    // Xilinx ZLIB object
    xfZlib xlz(single_bin, max_cr, COMP_ONLY, device_id, 0, FULL, XILINX_ZLIB);
    ERROR_STATUS(xlz.error_code());
...
void xilCompressDecompressTop(std::string& compress_decompress_mod,
                              ...
                              uint8_t max_cr_val) {
    // Create xfZlib object
    xfZlib xlz(single_bin, max_cr_val, BOTH, device_id, 0, FULL, XILINX_ZLIB);
    ERROR_STATUS(xlz.error_code());
