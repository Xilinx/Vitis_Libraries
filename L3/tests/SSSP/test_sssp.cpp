...
int main(int argc, const char* argv[]) {
    ...
    std::cout << filenameOffset << std::endl;
    std::cout << filenameIndice << std::endl;
    std::cout << filenameCOO << std::endl;
    ...
    while (goldenfstream.getline(line, sizeof(line))) {
        ...
        if (pred_golden - 1 != pred[0][vertex - 1]) {
            unsigned int tmp_fromID = pred[0][vertex - 1];
    ...
    for (int i = 0; i < numVertices; i++) {
        if (connect[i] == false && result[0][i] != std::numeric_limits<float>::infinity()) {
            std::cout << "Error : distance " << i << " " << std::numeric_limits<float>::infinity() << " "
                      << result[0][i] << std::endl;
            err++;
        }
        if (connect[i] == false && pred[0][i] != std::numeric_limits<unsigned int>::max()) {
            std::cout << "Error : predecessor " << i << " not connected " << pred[0][i] << std::endl;
            err++;
        }
    }
    if (err == 0) {
