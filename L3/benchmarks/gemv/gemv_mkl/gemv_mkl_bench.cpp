...
int main(int argc, char **argv) {
  ...
  cout << std::string("DATA_CSV:,Type,Thread,Func,M,N,")
    + "TimeApiMs,"
    + "EffApiPct,PerfApiTops\n";
  cout << "DATA_CSV:,"<<"Cold Start,"<<getenv("OMP_NUM_THREADS")<<","<<DISPLAY_GEMV_FUNC<<","<<m<<","<<n<<","
	<< ((double)l_durationSec_cold.count() * 1e3) << ","
	<< "N/A," << (flops / (double)l_durationSec_cold.count() * 1.e-12) <<endl;
  cout << "DATA_CSV:,"<<"Benchmark,"<<getenv("OMP_NUM_THREADS")<<","<<DISPLAY_GEMV_FUNC<<","<<m<<","<<n<<","
