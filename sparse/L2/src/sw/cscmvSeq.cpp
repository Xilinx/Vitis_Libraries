...
int main(int argc, char** argv) {
    ...
    cl::Kernel l_cscmvSeqKernel;

    OCL_CHECK(l_err, l_cscmvSeqKernel = cl::Kernel(l_clProgram, "cscmvSeqKernel:{cscmvSeqKernel_0}", &l_err));
    ...
    OCL_CHECK(l_err, l_err = l_cscmvSeqKernel.setArg(0, l_loadColValBuf));
    ...
    OCL_CHECK(l_err, l_err = l_cscmvSeqKernel.setArg(1, l_loadColPtrBuf));
    OCL_CHECK(l_err, l_err = l_cmdQueue.enqueueMigrateMemObjects({l_loadColValBuf}, 0));
    ...
    for (unsigned int ch = 0; ch < SPARSE_hbmChannels; ++ch) {
        ...
        OCL_CHECK(l_err, l_err = l_cscmvSeqKernel.setArg(ch * 2 + 2, l_readHbmBuf[ch]));
        ...
        if (l_rowResSz[ch] != 0) {
            ...
            OCL_CHECK(l_err, l_err = l_cscmvSeqKernel.setArg(ch * 2 + 3, l_rowResBuf[ch]));
        } else {
            OCL_CHECK(l_err, l_err = l_cscmvSeqKernel.setArg(ch * 2 + 3, l_readHbmBuf[ch]));
        }
    }
    ...
    OCL_CHECK(l_err, l_err = l_cmdQueue.enqueueTask(l_cscmvSeqKernel));
    ...
    double l_krnPerf = (l_matNnzs * 2) / l_krnApiTime / 1e6;
    // double l_krnApiPerf = (l_krnApiTime == 0)? 0:  l_matNnzs * 2 / l_krnApiTime / 1e6;
    double l_apiPerf = (l_totalTime == 0) ? 0 : l_matNnzs * 2 / l_totalTime / 1e6;
