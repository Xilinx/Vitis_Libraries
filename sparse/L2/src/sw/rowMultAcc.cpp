...
int main(int argc, char** argv) {
    if (argc < 5) {
        ...
        cout << "  Usage: rowMultAcc.exe rowMultAcc.xclbin M N NNZs [mtxFile]" << endl;
        return EXIT_FAILURE;
    }
    ...
    // Creating CUs
    OCL_CHECK(l_err, l_krnlLoad = cl::Kernel(l_clProgram, "loadDatKernel", &l_err));
    ...
    OCL_CHECK(l_err, l_err = l_krnlLoad.setArg(1, l_colVecMemBlocks));

    OCL_CHECK(l_err, l_err = l_krnlCscRow.setArg(0, l_aDevBuf));
