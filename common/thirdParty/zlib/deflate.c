...
#endif
{
    ...
    if (memLevel < 1 || memLevel > MAX_MEM_LEVEL || method != Z_DEFLATED || windowBits < 8 || windowBits > 15 ||
        level < 0 || level > 9 || strategy < 0 || strategy > Z_FIXED || (windowBits == 8 && wrap != 1)) {
        return Z_STREAM_ERROR;
    }
...
#endif
{
    ...
    if (s == Z_NULL || s->strm != strm ||
        (s->status != INIT_STATE &&
         ...
         s->status != GZIP_STATE &&
#endif
         s->status != EXTRA_STATE && s->status != NAME_STATE && s->status != COMMENT_STATE && s->status != HCRC_STATE &&
...
#endif
{
    ...
    if (dictionary != Z_NULL && len) zmemcpy(dictionary, s->window + s->strstart + s->lookahead - len, len);
...
// printf("flush %d \n", flush);
    ...
    if (s->status == INIT_STATE) {
        ////printf("INIT_STATE \n");
        // printf("s->w_bits %d s->level %d s->strategy %d\n",s->w_bits, s->level,s->strategy);
        s->level = 1;
        /* zlib header */
...
#endif
{
    ...
    do {
        ...
        if (len > (ulg)left + s->strm->avail_in) len = left + s->strm->avail_in; /* limit len to the input */
