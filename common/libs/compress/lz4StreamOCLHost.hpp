...
/**
 *  lz4StreamOCLHost class. Class containing methods for LZ4
 * compression and decompression to be executed on host side.
 */
class lz4StreamOCLHost : public lz4Base {
   public:
    /**
     ...
     *
     */
    lz4StreamOCLHost(enum State flow,
     ...
     * @param actual_size input size
     */
    uint64_t compressEngine(uint8_t* in, uint8_t* out, size_t input_size);
     ...
     * @param actual_size input size
     */
    uint64_t decompressEngine(uint8_t* in, uint8_t* out, size_t input_size);
    /**
   ...
   private:
    ...
    std::vector<uint16_t, aligned_allocator<uint8_t> > h_blksize;
...
#endif // _XFCOMPRESSION_LZ4_STREAM_OCL_HPP_
