...
Using Monte Carlo Simulation to estimate the value of digital option. Here, we assume the process of asset price applies to Black-Scholes process. 
...
Let :math:`T` be the maturity time of option. The maturity time :math:`T` is discreted by time steps :math:`N`. 
...
Calculate the payoff and discount it to time 0 for option price :math:`P_i`. if :math:`M > K`,
