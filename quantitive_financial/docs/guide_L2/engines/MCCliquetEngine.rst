...
Using the Monte Carlo Simulation to estimate the value of Cliquet Option. Here, we assume the process of asset price applies to Black-Scholes process. 
...
The option price calculated as:
...
.. NOTE::
   ...
   the calendar dates, so the customer needs input them through the resetDate array.
...
2. simulate the log of asset price :math:`\ln S^i_j`;

3. calculate the payoff based on above formula and discount to time zero for option price.

Sum together all the price for mean value.
