...
int MCDemoRunEuropeanMultiple1(Device* pChosenDevice, MCEuropean* pMCEuropean) {
    ...
    if (retval == XLNX_OK) {
        printf(
            ...
            "-------+---------------------+\n");
        printf(
            "[XLNX] | Iteration | K1 Option | K2 Option | K3 Option | K4 Option | "
            "Execution Time | Avg. Time Per Asset |\n");
        printf(
            "[XLNX] "
            "+-----------+-----------+-----------+-----------+-----------+---------"
            "-------+---------------------+\n");
        ...
        for (i = 0; i < NUM_ITERATIONS; i++) {
            ...
            printf(
                "[XLNX] | %9d | %9.4f | %9.4f | %9.4f | %9.4f | %11lld us | %16.3f "
                "us |\n",
                i, optionPrice[0], optionPrice[1], optionPrice[2], optionPrice[3], duration, durationPerAsset);
        }
