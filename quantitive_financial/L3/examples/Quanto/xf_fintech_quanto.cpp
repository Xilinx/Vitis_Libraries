    ...
    if (retval == XLNX_OK) {
        ...
        for (unsigned int i = 0; i < numAssets; i++) {
            printf("[XLNX] | %5u | %8.5f | %8.5f | %8.5f | %8.5f | %8.5f | %8.5f |\n", i,
                   cfQuanto.optionPrice[i], cfQuanto.delta[i], cfQuanto.gamma[i],
                   cfQuanto.vega[i], cfQuanto.theta[i], cfQuanto.rho[i]);

            if (!check_result(cfQuanto.optionPrice[i], test_data[i].exp, tolerance)) {
                printf("[XLNX] | FAIL: s(%8.5f), k(%8.5f) rd(%8.5f), rf(%8.5f), v(%8.5f), q(%8.5f), t(%8.5f), E(%8.5f), fxv(%8.5f), corr(%8.5f)\n",
                        test_data[i].s,
                        test_data[i].k,
                        test_data[i].rd,
                        test_data[i].rf,
