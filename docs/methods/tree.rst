...
2. The Rollback module using the same structure of the tree and discount from the Rate Model to obtain the values of each tree node from N to 0 time point by time point. When the time point is 0, the value is NPV. The implementation is shown in Figure 2, where arrows indicate data flow.
