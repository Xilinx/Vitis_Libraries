...
void printG(graphNew* G, long* C, long* M, long star, long end, bool isCid, bool isDir, ParLV* p_par, int idx) {
    ...
    for (long v = star; v < end; v++) {
        ...
        if (m < 0) {
            ...
            printf(" \033[1;31;40mc=%-5ld (%-5ld)   v=%-5ld m=%-5ld c(m)=%-5ld\033[0m", c, c_final, v, m,
                   p_par->FindC_nhop(m));
        } else
...
void GLV::ResetColor() {
    double totTimeColoring;
    numColors = Phaseloop_InitColorBuff(G, colors, numThreads, totTimeColoring);
}
void GLV::ResetC() {
    this->com_list.clear();
    InitC(); /*
...
void GLV::printSimple() {
    ...
    if (NV < (1)) {
        if (NV == NVl)
            printf(" \033[1;37;40m%-3ld\033[0m(%-3ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
        else
            printf(" \033[1;37;40m%-3ld\033[0m(%-3ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
    } else if (NV < (1000000)) {
        if (NV == NVl)
            printf(" \033[1;37;40m%-6ld\033[0m(%-5ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
        else
            printf(" \033[1;37;40m%-6ld\033[0m(%-5ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
        ...
        if (NV == NVl)
            printf(" \033[1;37;40m%-6ld\033[0m(%-8ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
        else
            printf(" \033[1;37;40m%-6ld\033[0m(%-8ld/%2d%%)", NV, (NV - NVl),
                   (int)(100 * (float)(NV - NVl) / (float)NV));
...
GLV* SttGPar::ParNewGlv_Prun(long start_tg,
                             long* offsets_tg,
                             edge* edgelist_tg,
                             long* dgrlist_tg,
                             long start_par,
    ...
    long NE_par =
        offsets_tg[end - start_tg] - offsets_tg[start - start_tg]; // number of edges connected to local vertices
