...
//
//    // declare cl::buffers
//    int edgeAlign8 = ((g.edgeNum + 7) / 8) * 8;
//    for (unsigned int i = 0; i < splitNm; i++) {
//        int sizeW = (g.numVerticesPU[i] + 3) / 4 * edgeAlign8;
//
//        hds[0].buffer[2 + 4 * i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR |
//        CL_MEM_READ_WRITE,
...
void loadGraphCoreSimDenseInt(clHandle* hds, int nrows, int nnz, xf::graph::Graph<int32_t, int32_t> g) {
    cl::Device device = hds[0].device;
    cl::Context context = hds[0].context;
    cl::CommandQueue q = hds[0].q;
    uint32_t splitNm = g.splitNum;
...
void opSimilarityDense::loadGraphMultiCardBlocking(int deviceID, xf::graph::Graph<int32_t, int32_t> g) {
    int nnz = g.edgeNum;
    int nrows = g.nodeNum;
    int cnt = 0;

    for (unsigned int j = 0; j < maxCU; ++j) {
        if ((handles[j].deviceID == (unsigned int)deviceID) && (handles[j].cuID == 0) && (handles[j].dupID == 0)) {
            cnt = j;
            loadGraphCoreSimDenseInt(&handles[j], nrows, nnz, g);
        }
    }
    cnt = 0;
...
void opSimilarityDense::loadGraphMultiCardNonBlocking(int deviceID, xf::graph::Graph<int32_t, int32_t> g) {
    int nnz = g.edgeNum;
    int nrows = g.nodeNum;
    bool freed[maxCU];
    ...
    for (unsigned int j = 0; j < maxCU; ++j) {
        if ((handles[j].deviceID == (unsigned int)deviceID) && (handles[j].cuID == 0) && (handles[j].dupID == 0)) {
            cnt = j;
            std::packaged_task<void(clHandle*, int, int, xf::graph::Graph<int32_t, int32_t>)> t(
...
//    uint32_t CHANNEL_NUMBER = 8;
//    uint32_t startID[splitNm];
//    uint32_t tmp = g.refID;
//    for (unsigned int i = 0; i < splitNm - 1; i++) { // calculate multi PU start address
//        startID[i] = tmp;
...
//
//    int edgeAlign8 = ((g.edgeNum + 7) / 8) * 8;
//    for (unsigned int j = 0; j < splitNm; j++) {
//        config[4 + j] = startID[j];
...
//    kernel0.setArg(0, hds[0].buffer[0]); // config
//    kernel0.setArg(1, hds[0].buffer[1]); // source weight
//    for (unsigned int k = 0; k < 4 * splitNm; k++) {
//        kernel0.setArg(2 + k, hds[0].buffer[2 + k]); // weights
//    }
//    kernel0.setArg(18, hds[0].buffer[18]); // resultID
//    kernel0.setArg(19, hds[0].buffer[19]); // similarity
...
int opSimilarityDense::computeInt(unsigned int deviceID,
    ...
    bufferInitInt(hds, instanceName, g, similarityType, dataType, topK, sourceNUM, sourceWeight, config, resultID,
                  similarity, kernel0, ob_in, ob_out);
