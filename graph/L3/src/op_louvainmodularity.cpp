...
void createHandleLouvainModularity(class openXRM* xrm,
                                   clHandle& handle,
...
void opLouvainModularity::setHWInfo(uint32_t numDevices, uint32_t maxCU) {
    ...
    buff_hosts_prune = new KMemorys_host_prune[maxCU_]; //[numDevices_];
};
...
void opLouvainModularity::init(class openXRM* xrm,
    ...
    createHandleLouvainModularity(xrm, handles[cnt], kernelName, kernelAlias, xclbinFile, deviceIDs[cnt], requestLoad);
    ...
    for (int i = 1; i < maxCU_; ++i) {
        ...
        createHandleLouvainModularity(xrm, handles[i], kernelName, kernelAlias, xclbinFile, deviceIDs[i], requestLoad);
