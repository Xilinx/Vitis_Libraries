...
#ifndef _XF_GRAPH_L3_OP_LABELPROPAGATION_HPP_
#define _XF_GRAPH_L3_OP_LABELPROPAGATION_HPP_
...
class opLabelPropagation : public opBase {
   public:
    ...
    opLabelPropagation() : opBase(){};

    void setHWInfo(uint32_t numDev, uint32_t CUmax);

    void freeLabelPropagation(xrmContext* ctx);
    ...
    event<int> addwork(uint32_t maxIter, xf::graph::Graph<uint32_t, uint32_t> g, uint32_t* labels);
