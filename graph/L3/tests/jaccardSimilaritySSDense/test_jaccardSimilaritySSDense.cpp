...
int main(int argc, const char* argv[]) {
    ...
    if (!parser.getCmdOption("-weight", tmpStr)) { // weight
        filenameWeight = "./data/jaccard_dense_weight.csr";
    ...
    if (!parser.getCmdOption("-golden", tmpStr)) { // golden
        goldenFile = "./data/jaccard_dense.mtx";
    ...
    generateSourceParams<splitNm>(numVerticesPU, numEdges, dataType, sourceID, g.weightsDense, sourceNUM,
                                  &sourceWeight);
    ...
    //---------------- Run L3 API -----------------------------------
    auto ev = xf::graph::L3::jaccardSimilaritySSDense(handle0, sourceNUM, sourceWeight, topK, g, resultID, similarity);
    int ret = ev.wait();
