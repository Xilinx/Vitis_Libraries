...
template <int PUNUM>
void generateSourceParams(int dataType,
                          int sourceID,
                          uint32_t* offset32[PUNUM],
                          uint32_t* indice32[PUNUM],
                          float* weightSparse[PUNUM],
                          uint32_t* numVerticesPU,
                          uint32_t* numEdgesPU,
    ...
    for (int i = 0; i < PUNUM; ++i) {
        cur = prev + numVerticesPU[i];
        if ((sourceID < cur) && (sourceID >= prev)) {
            uint32_t start = offset32[i][sourceID - prev];
            uint32_t end = offset32[i][sourceID - prev + 1];
            sourceNUM = end - start;
            *sourceIndice = aligned_alloc<uint32_t>(sourceNUM);
            *sourceWeight = aligned_alloc<uint32_t>(sourceNUM);
            for (int k = 0; k < (end - start); ++k) {
                sourceIndice[0][k] = indice32[i][start - offset + k];
                tmp0.f = weightSparse[i][start - offset + k];
                sourceWeight[0][k] = tmp0.i;
            }
        }
        prev = cur;
        offset += numEdgesPU[i];
