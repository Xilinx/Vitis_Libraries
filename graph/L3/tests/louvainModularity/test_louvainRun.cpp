...
int main(int argc, char** argv) {
    ...
    Options options;
    LouvainRun::ComputeOptions computeOpts;
    ...
    switch (toolOptions.mode_alveo) {
        ...
        case ALVEOAPI_LOAD: // 3
            ...
            finalQ = louvainRun.loadAlveoAndComputeLouvain(computeOpts);
            if (finalQ < -1) {
    ...
    if (finalQ < 0.951202) {
        std::cout << "ERROR: finalQ of the ref_example has fall from original 0.951203 " << std::endl;
    } else {
