...
#pragma HLS array_partition variable = e1_strm_arry complete
    ...
    //---------------------------------dispatch PU-------------------------------
    dispatch_wrapper<_HashMode, _WKey, _KeyNM, _WPay, _PayNM, _WHashHigh, _WHashLow, PU, _WBuffer, _BurstLenR>(
    ...
    for (int p = 0; p < PU; ++p) {
#pragma HLS unroll
        merge1_1<_WKey, _KeyNM, _WPay, _PayNM, _WHashLow>(k1_strm_arry_c0[p], p1_strm_arry_c0[p], hash_strm_arry_c0[p],
                                                          e1_strm_arry_c0[p], k1_strm_arry[p], p1_strm_arry[p],
                                                          hash1_strm_arry[p], e1_strm_arry[p]);
    }

    //---------------------------------aggregate PU---------------------------------
    // hash aggregate processing unit
    hash_aggr_pu_wrapper<_HashMode, _WHashLow, _WKey, _KeyNM, _WPay, _PayNM, _Wcnt, _WBuffer, _BurstLenW>(
...
#pragma HLS loop_tripcount min = 1 max = 1
        if (round[0] == 0) {
            hash_aggr_top<_WDT, 8, _WDT, 8, 1, 0, _WHashLow, 1, 8, _WBuffer, _BurstLenW, _BurstLenR>(
                strm_key_in, strm_rng_in, strm_e_in, max_count, max_key, max_rng, op_type, key_column, pld_column,
                round, unhandle_cnt_r, unhandle_cnt_w, aggr_num,
