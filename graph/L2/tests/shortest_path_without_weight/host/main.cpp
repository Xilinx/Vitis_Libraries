...
int main(int argc, const char* argv[]) {
    ...
    for (int i = 0; i < repInt; i++) {
        config[i] = aligned_alloc<ap_uint<32> >(4);
    ...
    cl::Buffer offset_buf, column_buf, info_buf, ddrQue_buf;
    ...
    for (int i = 0; i < repInt; i++) {
        config_buf[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                   sizeof(ap_uint<32>) * 4, &mext_o[5 + repInt + i]);
    }
...
#else
    shortestPath_top(config[0], offset512, column512, ddrQue512, ddrQue, result512[0], result[0], info);
#endif
    std::cout << "============================================================" << std::endl;
    ...
    while (goldenfstream.getline(line, sizeof(line))) {
        ...
        if (tmp.compare("Infinity")) {
            tmpd = std::stof(tmp);
            if (std::abs(result[0][index] - tmpd) > 0.00000001) {
