...
int computeSimilarity(std::string xclbinPath,
    ...
    for (int i = 0; i < repInt; i++) {
        ...
        for (int j = 0; j < PUNUM; j++) {
            ...
            config[i][5 + PUNUM + j] = numVerticesPU[j];
            config[i][5 + 2 * PUNUM + j] = numEdgesPU[j];
        }
    }
///////////////////////////////////////////////////////////////////////
    ...
    for (int i = 0; i < repInt; i++) {
        similarity_kernel[i] = cl::Kernel(program, "generalSimilarityKernel");
    }
    ...
    for (int i = 0; i < PUNUM; i++) {
        mext_o[4 * i + 0] = {XCL_BANK(8 * i), offset32[i], 0};
        mext_o[4 * i + 1] = {XCL_BANK(8 * i + 1), indice32[i], 0};
        mext_o[4 * i + 2] = {XCL_BANK(8 * i + 2), weightSparse[i], 0};
        mext_o[4 * i + 3] = {XCL_BANK(8 * i + 3), nullptr, 0};
    }

    mext_o[4 * PUNUM] = {XCL_BANK28, sourceIndice, 0};
    mext_o[4 * PUNUM + 1] = {XCL_BANK28, sourceWeight, 0};
    ...
    for (int i = 0; i < PUNUM; i++) {
        ...
        offset_buf[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                   sizeof(ap_uint<32>) * (numVerticesPU[i] + CHANNEL_NUMBER), &mext_o[4 * i + 0]);
        indice_buf[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                   sizeof(ap_uint<32>) * (numEdgesPU[i] + CHANNEL_NUMBER), &mext_o[4 * i + 1]);
        weight_buf[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                                   sizeof(ap_uint<32>) * (sizeW + CHANNEL_NUMBER), &mext_o[4 * i + 2]);
        empty_buf[i] = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_HOST_NO_ACCESS | CL_MEM_READ_WRITE,
                                  sizeof(ap_uint<32>) * (sizeW + CHANNEL_NUMBER), &mext_o[4 * i + 3]);
    }
    ...
    for (int i = 0; i < repInt; i++) {
        ...
        for (int k = 0; k < PUNUM; k++) {
            ...
            similarity_kernel[i].setArg(j++, empty_buf[k]);
        }
...
#else
    generalSimilarityKernel(config[0], sourceIndice, sourceWeight, offsetDDR[0], indiceDDR[0], weightDDR[0], nullptr,
                            offsetDDR[1], indiceDDR[1], weightDDR[1], nullptr, offsetDDR[2], indiceDDR[2], weightDDR[2],
                            nullptr, offsetDDR[3], indiceDDR[3], weightDDR[3], nullptr, offsetDDR[4], indiceDDR[4],
                            weightDDR[4], nullptr, offsetDDR[5], indiceDDR[5], weightDDR[5], nullptr, offsetDDR[6],
                            indiceDDR[6], weightDDR[6], nullptr, offsetDDR[7], indiceDDR[7], weightDDR[7], nullptr,
...
///////////////////////////////////////////////////////////////////////
    ...
    mext_o[4 * PUNUM] = {XCL_BANK28, nullptr, 0};
    mext_o[4 * PUNUM + 1] = {XCL_BANK28, sourceWeight, 0};
    ...
    source_indice_buf = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_HOST_NO_ACCESS | CL_MEM_READ_WRITE,
                                   sizeof(ap_uint<32>) * (sourceNUM + CHANNEL_NUMBER), &mext_o[4 * PUNUM]);
...
#else
    generalSimilarityKernel(config[0], nullptr, sourceWeight, weightDense[0], weightDense[1], weightDense[2],
                            weightDense[3], weightDense[4], weightDense[5], weightDense[6], weightDense[7],
                            weightDense[8], weightDense[9], weightDense[10], weightDense[11], weightDense[12],
                            weightDense[13], weightDense[14], weightDense[15], result_id[0], similarity[0]);
#endif
