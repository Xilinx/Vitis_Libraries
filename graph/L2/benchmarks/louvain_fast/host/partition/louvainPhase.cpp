...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
// There are 3 flows in this codes
// 1. no ghost flow, for the no partition, no ghost test, designed for 20.2 kernels, rarely used
// 2. support the partition and ghost flow, designed for 20.2 kernels
// 3. support the partition and ghost flow, designed for the kernel after 21.2, faster than the origin kernels (_par
// _prune)
...
void PhaseLoop_UsingFPGA_1_KernelSetup_prune(bool isLargeEdge,
    ...
    kernel_louvain.setArg(14, buff_cl.db_totUpdate);     // totCurr
    kernel_louvain.setArg(15, buff_cl.db_cWeight);       // cWeight
    kernel_louvain.setArg(16, buff_cl.db_offsets);       // offsets
    kernel_louvain.setArg(17, buff_cl.db_indices);       // indices
...
void runLouvainWithFPGA_demo_par_core_prune(bool hasGhost,
    ...
    // select device id with multi board
    ...
    uint32_t totalXilinxDevices = devices.size();
    uint32_t totalSupportedDevices_ = 0;
    cl_device_id* c_devices = (cl_device_id*)malloc(sizeof(cl_device_id) * totalXilinxDevices);
    clGetDeviceIDs(platforms[platformID], CL_DEVICE_TYPE_ALL, totalXilinxDevices, c_devices, NULL);
    size_t valueSize;
    char* value;
    std::vector<std::string> supportedDeviceNames_; // Supported device names
    int XF_GRAPH_L3_MAX_DEVICES_PER_NODE = 4;
    uint32_t supportedDeviceIds_[XF_GRAPH_L3_MAX_DEVICES_PER_NODE];
    const std::string token = "xilinx_u55c_gen3x16_xdma_base_2";
    supportedDeviceNames_.push_back(token);

    for (uint32_t i = 0; i < totalXilinxDevices; ++i) {
        // print device name
        clGetDeviceInfo(c_devices[i], CL_DEVICE_NAME, 0, NULL, &valueSize);
        value = new char[valueSize];
        clGetDeviceInfo(c_devices[i], CL_DEVICE_NAME, valueSize, value, NULL);
        if (std::find(supportedDeviceNames_.begin(), supportedDeviceNames_.end(), value) !=
            supportedDeviceNames_.end()) {
...
#pragma omp parallel for
    ...
    cl::Device device = devices[dev_id];
    cl::Context context(device);
    std::string devName = device.getInfo<CL_DEVICE_NAME>();
    printf("INFO: Found Device=%s\n", devName.c_str());
    cl::Program::Binaries xclBins = xcl::import_binary_file(opts_xclbinPath);
...
//         } else {
//             PhaseLoop_UsingCPU(opts_threshold, numThreads, currMod, G, C, C_orig, totItr, nonColor,
//             totTimeClustering);
//         }
//         /* General post-processing for both FPGA and CPU */
//         isItrStop = PhaseLoop_CommPostProcessing(NV, numThreads, opts_threshold, opts_coloring, prevMod, currMod, G,
//         C,
//                                                  C_orig, nonColor, phase, totItr, numClusters, totTimeBuildingPhase);
//     } // End of while(1) = End of Louvain
