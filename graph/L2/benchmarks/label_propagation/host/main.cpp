...
int main(int argc, const char* argv[]) {
    ...
    DT* offsetsCSR = aligned_alloc<DT>(V);
    DT* columnsCSR = aligned_alloc<DT>(E);
    DT* offsetsCSC = aligned_alloc<DT>(V);
    DT* rowsCSC = aligned_alloc<DT>(E);

    DT* labelPing = aligned_alloc<DT>(V);
    DT* labelPong = aligned_alloc<DT>(V);
    DT* bufPing = aligned_alloc<DT>(V);
    DT* bufPong = aligned_alloc<DT>(V);
    DT* labelGolden = aligned_alloc<DT>(V);
    ...
    cl::Kernel LPKernel(program, "LPKernel");
