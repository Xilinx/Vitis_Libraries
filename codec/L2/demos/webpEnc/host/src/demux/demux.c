...
static void SetFrameInfo(size_t start_offset,
                         size_t size,
                         int frame_num,
                         int complete,
...
static ParseStatus StoreFrame(int frame_num, uint32_t min_size, MemBuffer* const mem, Frame* const frame) {
    ...
    do {
        ...
        const size_t payload_available =
            (payload_size_padded > MemDataSize(mem)) ? MemDataSize(mem) : payload_size_padded;
            ...
            case MKFOURCC('V', 'P', '8', ' '):
                if (image_chunks == 0) {
                    ...
                    const VP8StatusCode vp8_status =
                        WebPGetFeatures(mem->buf_ + chunk_start_offset, chunk_size, &features);
    ...
    while (f != NULL) {
        ...
        for (; f != NULL && f->frame_num_ == cur_frame_set; f = f->next_) {
            ...
            if (f->width_ > 0 && f->height_ > 0 &&
                !CheckFrameBounds(f, !is_animation, dmux->canvas_width_, dmux->canvas_height_)) {
                return 0;
            }
