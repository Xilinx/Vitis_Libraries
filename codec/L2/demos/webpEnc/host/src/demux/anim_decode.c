...
WebPAnimDecoder* WebPAnimDecoderNewInternal(const WebPData* webp_data,
    ...
    {
        const int canvas_bytes = dec->info_.canvas_width * NUM_CHANNELS * dec->info_.canvas_height;
...
static void ZeroFillCanvas(uint8_t* buf, uint32_t canvas_width, uint32_t canvas_height) {
...
static uint8_t BlendChannelNonPremult(
    uint32_t src, uint8_t src_a, uint32_t dst, uint8_t dst_a, uint32_t scale, int shift) {
...
static uint32_t BlendPixelNonPremult(uint32_t src, uint32_t dst) {
    ...
    if (src_a == 0) {
        ...
        const uint8_t blend_r = BlendChannelNonPremult(src, src_a, dst, dst_factor_a, scale, 0);
        const uint8_t blend_g = BlendChannelNonPremult(src, src_a, dst, dst_factor_a, scale, 8);
        const uint8_t blend_b = BlendChannelNonPremult(src, src_a, dst, dst_factor_a, scale, 16);
...
int WebPAnimDecoderGetNext(WebPAnimDecoder* dec, uint8_t** buf_ptr, int* timestamp_ptr) {
    ...
    // Decode.
    {
        ...
        const size_t out_offset = (iter.y_offset * width + iter.x_offset) * NUM_CHANNELS;
    ...
    if (iter.frame_num > 1 && iter.blend_method == WEBP_MUX_BLEND && !is_key_frame) {
        if (dec->prev_iter_.dispose_method == WEBP_MUX_DISPOSE_NONE) {
            ...
            for (y = 0; y < iter.height; ++y) {
                ...
                blend_row((uint32_t*)dec->curr_frame_ + offset, (uint32_t*)dec->prev_frame_disposed_ + offset,
                          iter.width);
            }
        } else {
            int y;
            ...
            for (y = 0; y < iter.height; ++y) {
                ...
                if (width1 > 0) {
                    ...
                    blend_row((uint32_t*)dec->curr_frame_ + offset1, (uint32_t*)dec->prev_frame_disposed_ + offset1,
                              width1);
                }
                if (width2 > 0) {
                    const size_t offset2 = canvas_y * width + left2;
                    blend_row((uint32_t*)dec->curr_frame_ + offset2, (uint32_t*)dec->prev_frame_disposed_ + offset2,
                              width2);
                }
            }
        }
    ...
    if (dec->prev_iter_.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
        ZeroFillFrameRect(dec->prev_frame_disposed_, width * NUM_CHANNELS, dec->prev_iter_.x_offset,
