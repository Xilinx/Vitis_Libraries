...
// ------------------------------------------------------------
/**
 * @brief convert strm to Aligned_block, for other integration ,not just JPEG
 *
 * @param block_strm the stream of coefficients in block,23:is_rst, 22:is_endblock,21~16:bpos,15~0:block val
 ...
 */
inline void hls_next_mcupos2(hls::stream<ap_uint<24> >& block_strm,
...
// ------------------------------------------------------------
inline void pick_huff_data(hls::stream<CHType>& image_strm,
                           hls::stream<bool>& eof_strm,
                           uint32_t& cnt_rst,
...
#pragma HLS PIPELINE II = 1
        ...
        // decode one huffman code from 16b
...
#pragma HLS UNROLL
            ...
            if (ac) {
                run_vlen = ac_value_buckets[hls_cmp[0]][val_addr_ac];
            } else {
                run_vlen = dc_value_buckets[hls_cmp[0]][val_addr_dc];
            }
        ...
        if (next_block) {
            ...
            if (((test / cpmall) % hls_mcuh == 31) && (test / (cpmall*hls_mcuh) == 31) && (test % cpmall == 2) ) {
...
#pragma HLS DATAFLOW
    ...
    xf::codec::details::Huffman_decoder(huff_sos_strm, sign_no_huff, dht_tbl1, ac_value_buckets, ac_huff_start_code,
