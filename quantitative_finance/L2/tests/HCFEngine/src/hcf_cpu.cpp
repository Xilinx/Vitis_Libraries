...
std::complex<TEST_DT> charFunc(struct xf::fintech::hcfEngineInputDataType<TEST_DT>* p, std::complex<TEST_DT> w) {
    ...
    TEST_DT gamma = vv / 2;
    std::complex<TEST_DT> alpha = (std::complex<TEST_DT>)-0.5 * ((w * w) + (w * (std::complex<TEST_DT>)1i));
    std::complex<TEST_DT> beta = p->kappa - (p->rho * p->vvol * w) * (std::complex<TEST_DT>)1i;
    ...
    cf = cf + (((std::complex<TEST_DT>)1i * w) * (std::complex<TEST_DT>)log(p->s0 * exp(p->r * p->T)));
    cf = exp(cf);
    return cf;
}
