...
void writecoef(int steps, ap_uint<4 * 64>* coefdata, std::string& file_name) {
    std::ofstream outfile(file_name);
    for (int i = 0; i < steps; i++) {
        for (int j = 0; j < 4; j++) {
            double tmp = xf::fintech::internal::bitsToDouble(coefdata[i](64 * j + 63, 64 * j));
            outfile << tmp << std::endl;
        }
    }
    outfile.close();
}
void readcoef(int steps, ap_uint<4 * 64>* coefdata, std::string& file_name) {
    std::ifstream infile;
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        for (int j = 0; j < 4; j++) {
            infile >> tmp;
            coefdata[i](64 * j + 63, 64 * j) = xf::fintech::internal::doubleToBits(tmp);
        }
        std::cout << "coefdata[i]=  " << coefdata[i] << std::endl;
    }
    infile.close();
}
...
int main(int argc, char* argv[]) {
    ...
    if (!run_csim) {
        timeSteps = 10;
        calibSamples = UN_K0 * 1024;
        requiredSamples = UN_K2 * 1024;
    }

    int iter = calibSamples / UN_K0 / 1024;
    ap_uint<UN_K0 * sizeof(double) * 8> pOut[102400 * iter];
    ap_uint<sizeof(double) * 8> mOut[900 * iter];
    ...
    kernel_2_top(underlying, volatility, dividendYield, riskFreeRate, timeLength, strike, optionType, seed2, coefOut,
                 outputs, requiredTolerance, requiredSamples, timeSteps);
