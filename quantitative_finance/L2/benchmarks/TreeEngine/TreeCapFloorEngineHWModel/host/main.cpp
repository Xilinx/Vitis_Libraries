...
int main(int argc, const char* argv[]) {
    ...
    scanTreeKernel3(len, inputParam0_alloc, inputParam1_alloc, output[2]);
    scanTreeKernel4(len, inputParam0_alloc, inputParam1_alloc, output[3]);
#endif
    for (int i = 0; i < KN; i++) {
        for (int j = 0; j < len; j++) {
            DT out = output[i][j];
