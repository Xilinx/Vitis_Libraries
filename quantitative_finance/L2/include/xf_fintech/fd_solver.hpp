...
#pragma HLS ARRAY_PARTITION variable = v_out cyclic factor = MEM_WIDTH dim = 1 partition
...
#pragma HLS ARRAY_PARTITION variable = u_r0 cyclic factor = MEM_WIDTH dim = 1 partition
#pragma HLS ARRAY_PARTITION variable = u_r1 cyclic factor = MEM_WIDTH dim = 1 partition
...
#pragma HLS ARRAY_PARTITION variable = val complete
    ...
    void streamSparseMultAdd(DT A[A_SIZE],
                             ...
                             unsigned int M,
                             DT y0[M_SIZE]) {
#pragma HLS ARRAY_PARTITION variable = y0 cyclic factor = MEM_WIDTH dim = 1 partition
        DT u[M_SIZE];
#pragma HLS ARRAY_PARTITION variable = u cyclic factor = MEM_WIDTH dim = 1 partition
        DT y0_tmp1[M_SIZE];
#pragma HLS ARRAY_PARTITION variable = y0_tmp1 cyclic factor = MEM_WIDTH dim = 1 partition
...
#pragma HLS ARRAY_PARTITION variable = y0 cyclic factor = MEM_WIDTH dim = 1 partition
    ...
    internal::Solver<DT, M_SIZE, LOG2_M_SIZE, A_SIZE, MEM_WIDTH> solver;
