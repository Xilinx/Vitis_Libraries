 ...
 */
#ifndef XF_DATA_ANALYTICS_L1_DATAFRAME_PARSE_DOUBLE_HPP
#define XF_DATA_ANALYTICS_L1_DATAFRAME_PARSE_DOUBLE_HPP
#include "strtod.hpp"
#include "hls_stream.h"
#include "ap_int.h"
#include "utils.hpp"
#include <limits>
#include "parse_double_v2.hpp"
...
static void parseStr2Dec(hls::stream<ap_uint<8> >& i_strm,
                         ...
                         hls::stream<ap_uint<64> >& o_s_strm,
                         hls::stream<int>& o_p_strm,
                         hls::stream<bool>& o_nf_strm,
                         hls::stream<bool>& o_sg_strm,
...
#pragma HLS pipeline II = 1
    ...
    o_nf_strm.write(false);
    o_sg_strm.write(false);
    o_s_strm.write(0);
    o_p_strm.write(0);
    o_e_strm.write(true);
};
...
/**
 * @brief implementation of fast-path decimal to floating-point conversion
 *
 **/
static void fastPath(hls::stream<ap_uint<64> >& s_strm,
                     hls::stream<int>& p_strm,
                     hls::stream<bool>& nf_strm,
                     hls::stream<bool>& sg_strm,
                     hls::stream<bool>& i_e_strm,
                     hls::stream<ap_uint<64> >& o_strm) {
    static const double pow10_tb[309] = {
...
#pragma HLS pipeline II = 1
        if (nb_1 && nb_2 && nb_3 && nb_4 && nb_5) {
            ...
            std::cout << d << ", " << bitsToDouble(d) << std::endl;
#endif
        }
        ap_uint<64> s;
        int p;
        bool isNanOrInf;
        nb_1 = i_e_strm.read_nb(e);
        nb_2 = s_strm.read_nb(s);
        nb_3 = p_strm.read_nb(p);
        nb_4 = nf_strm.read_nb(isNanOrInf);
        nb_5 = sg_strm.read_nb(isNeg);
        if (nb_1 && nb_2 && nb_3 && nb_4 && nb_5) {
            // NaN
            if (isNanOrInf) {
                d = s;
...
/**
 *
 * @brief parse the string into double data type.
 * This is a implementation of "Fast Path Decimal to Floating-Point Conversion"
 *
 * @param i_strm input stream of string.
 * @param i_e_strm input end flag of i_strm.
 * @param vld_strm input valid flag of i_strm.
 * @param o_strm double output stream.
 *
 */
static void parseDouble(hls::stream<ap_uint<8> >& i_strm,
                        hls::stream<bool>& i_e_strm,
                        hls::stream<bool>& vld_strm,
                        hls::stream<ap_uint<64> >& o_strm) {
#pragma HLS dataflow
    hls::stream<ap_uint<64> > s_strm;
#pragma HLS stream variable = s_strm depth = 8
    hls::stream<int> p_strm;
#pragma HLS stream variable = p_strm depth = 8
    hls::stream<bool> nf_strm;
#pragma HLS stream variable = nf_strm depth = 8
    hls::stream<bool> sg_strm;
#pragma HLS stream variable = sg_strm depth = 8
    hls::stream<bool> e_strm;
#pragma HLS stream variable = e_strm depth = 8
    parseStr2Dec(i_strm, i_e_strm, vld_strm, s_strm, p_strm, nf_strm, sg_strm, e_strm);
    fastPath(s_strm, p_strm, nf_strm, sg_strm, e_strm, o_strm);
}
#else
/**
 *
 * @brief parse the string into double data type.
 * This is a implementation of Grisu2 algorithm from the publication: Loitsch, Florian. "Printing floating-point numbers
 * quickly and accurately with Integers." ACM Sigplan Notices 45.6 (2010): 233-243.
 *
 * @param i_strm input stream of string.
 * @param i_e_strm input end flag of i_strm.
 * @param vld_strm input valid flag of i_strm.
 * @param o_strm double output stream.
 * @param o_strm float output stream.
 *
 */
static void parseDouble(hls::stream<ap_uint<8> >& i_strm,
                        hls::stream<bool>& i_e_strm,
                        hls::stream<bool>& vld_strm,
                        hls::stream<ap_uint<64> >& o_strm,
                        hls::stream<ap_uint<32> >& o_strm_1) {
#pragma HLS dataflow
    hls::stream<ap_uint<64> > s_strm;
#pragma HLS stream variable = s_strm depth = 8
    hls::stream<ap_int<16> > ex_strm;
#pragma HLS stream variable = ex_strm depth = 8
    hls::stream<ap_uint<16> > ln_strm;
#pragma HLS stream variable = ln_strm depth = 8
    hls::stream<ap_uint<16> > rm_strm;
#pragma HLS stream variable = rm_strm depth = 8
    hls::stream<bool> nf_strm;
#pragma HLS stream variable = nf_strm depth = 8
    hls::stream<bool> sg_strm;
#pragma HLS stream variable = sg_strm depth = 8
    hls::stream<bool> e_strm;
#pragma HLS stream variable = e_strm depth = 8
    hls::stream<bool> e_strm_1;
#pragma HLS stream variable = e_strm_1 depth = 8
    hls::stream<ap_uint<65> > d_strm;
#pragma HLS stream variable = d_strm depth = 8

    hls::stream<nb_dp> pk_strm;
#pragma HLS stream variable = pk_strm depth = 8
    parseStr2Dec(i_strm, i_e_strm, vld_strm, pk_strm);
    strtodDiyFp(pk_strm, d_strm);
    addFloat(d_strm, o_strm, o_strm_1);
}
#endif // end FAST_PATH
