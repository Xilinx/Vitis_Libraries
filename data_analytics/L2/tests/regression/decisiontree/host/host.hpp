...
#ifndef _XF_DATA_ANALYTICS_L1_DECISIONTREE_TRAIN_HOST_HPP_
#define _XF_DATA_ANALYTICS_L1_DECISIONTREE_TRAIN_HOST_HPP_
...
struct Node_H {
    ...
    ap_uint<8> featureId;
    MType regValue;  // regression value for a leaf node
...
int precisonAndRecall(DataType* testsamples,
    ...
    for (int i = 0; i < samples_num; i++) {
        ...
        // std::cout << "sample " << i << "'s real value is " << realValue << std::endl;
    }
...
template <typename MType, unsigned WD>
void DataPreocess(std::string path,
                  MType* samples,
                  ...
                  ap_uint<512>* configs,
                  ap_uint<512>* data) {
    std::ifstream fin_config(path + "/config.txt");
    std::string line;
    int read_int[128];
...
template <typename T>
void writeTreeToBF(std::string& path, std::string& name, int n, T* tree) {
    std::string dir = path + name;
    FILE* fp = fopen(dir.c_str(), "wb");
    if (!fp) {
        std::cout << "Error to open file: " << dir << std::endl;
    } else {
        size_t cnt = fwrite(tree, sizeof(T), n, fp);
        if (cnt != n)
            std::cerr << "ERROR: " << cnt << " entries read from " << dir << ", " << n << " entries required."
                      << std::endl;
    }
