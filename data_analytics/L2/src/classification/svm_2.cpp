...
#include "xf_DataAnalytics/classification/svm_train_2.hpp"
...
void SVM_flow(internal::tagTableRandomLoader<512, WD, BURST_LEN, MType, ap_uint<WD> >& loader,
              internal::s_aggr<MType, StreamN, SampleDepth, &funcB2, 7, internal::URAM>& processor_agg,
    ...
    static const int LatencyT = internal::sl2<MType, StreamN, SampleDepth, 1, 1, &funcA2, &funcB2, &funcC2, 6,
                                              internal::URAM, internal::URAM>::LatencyT;
    static const int x2_fifo_depth = axi_fifo_depth * 2 + LatencyT + 128 + 1024 + 512;
...
#pragma HLS dataflow
    ...
    internal::sl2<MType, StreamN, SampleDepth, 1, 1, &funcA2, &funcB2, &funcC2, 6, internal::URAM, internal::URAM>
        processor;

    loader.sample(ddr, offset, rows, cols, fraction, ifJump, bucketSize, x_data, e_x_data_strm, x_data_1, e_data_strm,
                  //    loader.sample(ddr, offset, rows, feature_num, fraction, ifJump, bucketSize, x_data,
                  //    e_x_data_strm, x_data_1, e_data_strm,
                  y_label_1, e_y_strm);
#ifndef __SYNTHESIS__
    std::cout << "x_data " << x_data[0].size() << std::endl;
#endif
...
#ifndef __SYNTHESIS__
    std::cout << "x_data_1 " << x_data_1[0].size() << "  y_label_1 " << y_label_1.size() << "  retStrm "
              << retStrm[0].size() << std::endl;
#endif
...
extern "C" void SVM_B(ap_uint<512>* ddr, ap_uint<512>* weight) {
...
#pragma HLS unroll
    ...
    xf::data_analytics::common::internal::s_aggr<DATA_TYPE, STREAM_NUM, SAMPLE_DEP, &funcB2, 7,
                                                 xf::data_analytics::common::internal::URAM>
        processor_agg;
    int L = xf::data_analytics::common::internal::s_aggr<DATA_TYPE, STREAM_NUM, SAMPLE_DEP, &funcB2, 7,
                                                         xf::data_analytics::common::internal::URAM>::L;
mainloop:
    while (i < MAX_ITER && !flag_tol) {
        DATA_TYPE sum_grediant[MAX_NUM_FEATURE];
        xf::data_analytics::classification::svm2::SVM_flow<DATA_TYPE, DATA_WIDTH, STREAM_NUM, SAMPLE_DEP>(
