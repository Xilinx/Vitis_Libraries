 ...
 * t_aColMemWords defines number of memwords in the columns of one row of
 buffer_A. Due to the reusability, the
 height of buffer_A is only one memwords. For buffer_B, t_aColMemWords defines
 number of memwords in the rows of one
 column in buffer_B, t_bColMemWords defines number of memwords in the cols of
 one row in buffer_B. t_aRowMemWords and
 ...
 * @tparam t_aColMemWords  number of memory words in one row of the matrix A
 buffer
 * @tparam t_aRowMemWords  number of memory words in one column of the matrix A
 buffer
 * @tparam t_bColMemWords   number of memory words in one row of the matrix B
 buffer
 *
 */
template <
    ...
    unsigned int t_aColMemWords =
        1, // number of memory words in one row of the matrix A buffer
    unsigned int t_aRowMemWords =
        1, // number of memory words in one column of the matrix A buffer
    unsigned int t_bColMemWords =
        1 // number of memory words in one row of the matrix B buffer
    >
class GemmKernel {
public:
  ...
  typedef hls::stream<
      typename TaggedWideType<t_FloatType, t_DdrWidth>::t_TypeInt>
      EdgeStream;

  typedef WideType<t_XDataType, t_XDdrWidth> XDdrWideType;
  typedef hls::stream<typename WideType<t_XDataType, t_XDdrWidth>::t_TypeInt>
      XDdrStream;
  typedef WideType<t_XDataType, t_DdrWidth> DdrWideTypeForX;

  // type definitions for enhanced MAC implementation, using 48-bits to store
...
public:
   ...
   *
   */
  void GemmReadMatA(DdrIntType *l_aAddr, unsigned int l_aColBlocks,
                    unsigned int l_aRowBlocks, unsigned int l_bColBlocks,
                    unsigned int l_aWordLd, DdrStream &p_As) {
...
#pragma HLS PIPELINE
              unsigned int l_aSrcOffset = l_aWordLd * t_aMH * l_aRowBlock +
                                          l_aColBlock * t_aColMemWords +
                                          i * l_aWordLd + j;
   ...
   *
   */
  void GemmReadMatB(DdrIntType *l_bAddr, unsigned int l_aColBlocks,
                    unsigned int l_aRowBlocks, unsigned int l_bColBlocks,
                    unsigned int l_bWordLd, DdrStream &p_Bs) {
    assert(t_DdrOverXDdr != 0);
    assert(t_DdrOverXDdr * t_XDdrWidth == t_DdrWidth);
...
#pragma HLS PIPELINE II = t_bColMemWords
            for (int j = 0; j < t_bColMemWords; ++j) {
              unsigned int l_bSrcOffset = i * l_bWordLd +
                                          l_bWordLd * t_bKD * l_aColBlock +
                                          l_bColBlock * t_bColMemWords + j;
   ...
   *
   */
  void GemmReadMatX(DdrIntType *l_xAddr, unsigned int l_aColBlocks,
                    unsigned int l_aRowBlocks, unsigned int l_bColBlocks,
                    unsigned int l_xWordLd, XDdrStream &p_Xs) {
    assert(t_DdrOverXDdr != 0);
    assert(t_DdrOverXDdr * t_XDdrWidth == t_DdrWidth);
...
#pragma HLS PIPELINE
          DdrIntType l_word =
              l_xAddr[l_bColBlock * t_bColMemWords * t_DdrOverXDdr + j];
...
#pragma HLS PIPELINE II = t_xColMemWords
          for (int j = 0; j < t_xColMemWords; ++j) {
            unsigned int l_xSrcOffset =
                l_xWordLd * t_aMH * l_aRowBlock +
                l_bColBlock * t_bColMemWords * t_DdrOverXDdr + l_xWordLd * i +
                j;
   ...
   *
   */
  void GemmReadABX(DdrIntType *l_aAddr, DdrIntType *l_bAddr,
                   DdrIntType *l_xAddr, unsigned int l_aColBlocks,
                   unsigned int l_aRowBlocks, unsigned int l_bColBlocks,
                   unsigned int l_aWordLd, unsigned int l_bWordLd,
                   unsigned int l_xWordLd, DdrStream &p_As, DdrStream &p_Bs,
                   XDdrStream &p_Xs) {
...
#pragma HLS PIPELINE
          unsigned int l_xSrcOffset =
              l_xWordLd * t_aMH * l_aRowBlock +
              l_bColBlock * t_bColMemWords * t_DdrOverXDdr + j;
          DdrIntType l_word = l_xAddr[l_xSrcOffset];
          XDdrWideType l_wordx = l_conv.convert(l_word);
          p_Xs.write(l_wordx);
        }
...
#pragma HLS PIPELINE II = t_xColMemWords
  ...
  //
  ///////////////////////////////////////////////////////////////////////////
  void GemmCBuffer(WideMacBitStream &p_Cs, unsigned int p_aColBlocks,
                   unsigned int p_cBlocks, WideMacBitStream &p_Cout) {
    WideMacBitType l_bufferC[t_aMH * t_bColMemWords];

    for (int i = 0; i < t_aMH * t_bColMemWords; i++)
#pragma HLS PIPELINE
      for (int j = 0; j < t_DdrWidth; j++)
        l_bufferC[i][j] = 0;
...
#pragma HLS PIPELINE
  ...
  //
  ///////////////////////////////////////////////////////////////////////////
  void GemmAddX(WideMacBitStream &p_Cs, XDdrStream &p_Xs,
                unsigned int p_cBlocks, int32_t p_postScale,
                DdrStream &p_Cout) {
#if BLAS_XVEC
...
#pragma HLS PIPELINE style = stp
  ...
  void GemmBlockStream(DdrStream &p_As, DdrStream &p_Bs, XDdrStream &p_Xs,
                       DdrStream &p_Cs, unsigned int p_aColBlocks,
                       unsigned int p_aRowBlocks, unsigned int p_bColBlocks,
                       unsigned int p_transpBlocks, int32_t p_postScale) {
...
#pragma HLS STREAM variable = p_CEdgeS depth =                                 \
    t_DdrWidth * t_aRowMemWords * t_bColMemWords
#pragma HLS RESOURCE variable = p_CEdgeS core = fifo_uram
    ...
    Transpose<t_FloatType, t_aColMemWords, t_DdrWidth> l_transp(p_transpBlocks,
                                                                t_bColMemWords);
    l_transp.process(p_As, p_AoutS);

    MatrixBuffer<typename DdrWideType::t_TypeInt, t_DdrWidth * t_aColMemWords,
                 t_bColMemWords, true, false>()
        .process(p_Bs, p_Bs1, l_abBlocks, t_aRowMemWords);

    Gemm<t_FloatType, t_bKD, t_DdrWidth>::gemm(
        p_AoutS, p_Bs1, p_CEdgeS, l_abBlocks * t_aRowMemWords * t_bColMemWords);

    GemmCBuffer(p_CEdgeS, p_aColBlocks, l_cBlocks, p_COutS);
    GemmAddX(p_COutS, p_Xs, l_cBlocks, p_postScale, p_Cs);
  }
  void GemmReadAndMult(DdrIntType *p_aAddr, DdrIntType *p_bAddr,
                       DdrIntType *p_xAddr, unsigned int p_aColBlocks,
                       unsigned int p_aRowBlocks, unsigned int p_bColBlocks,
                       unsigned int p_aLd, unsigned int p_bLd,
                       unsigned int p_xLd, unsigned int p_transpBlocks,
                       int32_t p_postScale, DdrStream &p_Cs) {
#pragma HLS DATAFLOW
...
#if BLAS_CACHE
    GemmReadMatA(p_aAddr, p_aColBlocks, p_aRowBlocks, p_bColBlocks, p_aLd,
                 l_As);
    GemmReadMatB(p_bAddr, p_aColBlocks, p_aRowBlocks, p_bColBlocks, p_bLd,
                 l_Bs);
    GemmReadMatX(p_xAddr, p_aColBlocks, p_aRowBlocks, p_bColBlocks, p_xLd,
                 l_Xs);
#else
#pragma HLS STREAM variable = l_As depth =                                     \
    t_aColMemWords * t_DdrWidth * t_aRowMemWords
#pragma HLS RESOURCE variable = l_As core = fifo_uram
    GemmReadABX(p_aAddr, p_bAddr, p_xAddr, p_aColBlocks, p_aRowBlocks,
                p_bColBlocks, p_aLd, p_bLd, p_xLd, l_As, l_Bs, l_Xs);
#endif
    GemmBlockStream(l_As, l_Bs, l_Xs, p_Cs, p_aColBlocks, p_aRowBlocks,
                    p_bColBlocks, p_transpBlocks, p_postScale);
  }
  // load A and B in t_DdrWidth x t_DdrWidth size blocks, multiply blocks and
  // write results back to memory
  void GemmBlocks(DdrIntType *p_aAddr, DdrIntType *p_bAddr, DdrIntType *p_cAddr,
                  DdrIntType *p_xAddr, unsigned int p_aColBlocks,
                  unsigned int p_aRowBlocks, unsigned int p_bColBlocks,
                  unsigned int p_aLd, unsigned int p_bLd, unsigned int p_cLd,
                  unsigned int p_xLd, unsigned int p_transpBlocks,
                  int32_t p_postScale) {
#pragma HLS DATAFLOW
    ...
    XDdrStream l_Xs;
    DdrStream l_Cs;
#pragma HLS STREAM variable = l_Cs depth =                                     \
    t_DdrWidth * t_aRowMemWords * t_bColMemWords
...
#pragma HLS RESOURCE variable = l_As core = fifo_uram
    ...
    GemmReadABX(p_aAddr, p_bAddr, p_xAddr, p_aColBlocks, p_aRowBlocks,
                p_bColBlocks, p_aLd, p_bLd, p_xLd, l_As, l_Bs, l_Xs);
    GemmBlockStream(l_As, l_Bs, l_Xs, l_Cs, p_aColBlocks, p_aRowBlocks,
                    p_bColBlocks, p_transpBlocks, p_postScale);
   ...
   */
  void
  runGemm(DdrIntType *p_DdrRd, // base DDR/memory address for matrix A and B
          DdrIntType *p_DdrWr, // base DDR/memory address for matrix C
          GemmArgsType &p_Args // GEMM argument that stores the address offset
                               // of matrix A, B and C, sizes of
          // matrix dimensions (M, K and N) and lead dimension sizes for matrix
          // A, B and C
          ) {
    ...
    const unsigned int l_aColBlocks =
        p_Args.m_K / (t_DdrWidth * t_aColMemWords);
    const unsigned int l_aRowBlocks =
        p_Args.m_M / (t_DdrWidth * t_aRowMemWords);
    const unsigned int l_bColBlocks =
        p_Args.m_N / (t_DdrWidth * t_bColMemWords);
    ...
    unsigned int l_transpBlocks =
        l_aColBlocks * l_aRowBlocks * l_bColBlocks * t_aRowMemWords;
    GemmBlocks(l_aAddr, l_bAddr, l_cAddr, l_xAddr, l_aColBlocks, l_aRowBlocks,
               l_bColBlocks, l_aLd, l_bLd, l_cLd, l_xLd, l_transpBlocks,
               l_postScale);
  }
   ...
   *
   */
  void GemmWriteDdrStream(DdrIntType *l_cAddr, DdrStream &p_Cs,
                          unsigned int l_aRowBlocks, unsigned int l_bColBlocks,
...
#pragma HLS PIPELINE II = t_bColMemWords
          for (int j = 0; j < t_bColMemWords; j++) {
            unsigned int l_dstOffset =
                i * l_cWordLd +
                l_cWordLd * t_DdrWidth * t_aRowMemWords * rowBlock +
