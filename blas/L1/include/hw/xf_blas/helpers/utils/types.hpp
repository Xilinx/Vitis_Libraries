...
template <typename T, unsigned int t_Width,
          unsigned int t_DataWidth = sizeof(T) * 8, typename Enable = void>
class WideType {
private:
  T m_Val[t_Width];
...
#pragma HLS UNROLL
      ap_uint<t_DataWidth> l_val =
          p_val.range(t_DataWidth * (1 + i) - 1, t_DataWidth * i);
...
#pragma HLS UNROLL
      T l_v = m_Val[i];
      ap_uint<t_DataWidth> l_val =
          *reinterpret_cast<ap_uint<t_DataWidth> *>(&l_v);
...
class WideType<T, 1, t_DataWidth, typename std::enable_if<std::is_same<
                                      ap_uint<t_DataWidth>, T>::value>::type> {
private:
  T m_Val;
  static const unsigned int t_4k = 4096;
  static const unsigned int FLOAT_WIDTH = 7;
...
template <typename t_FloatType,
          ...
          unsigned int
              t_MemWidthBits // In bits; both must be defined and be consistent
          >
class MemUtil {
private:
public:
...
template <class T, uint8_t t_NumCycles> T hlsReg(T p_In) {
#pragma HLS INLINE
...
template <class T> T hlsReg(T p_In) { return hlsReg<T, 1>(p_In); }

template <unsigned int W> class BoolArr {
private:
  bool m_Val[W];
...
template <class S, int W> bool streamsAreEmpty(S p_Sin[W]) {
#pragma HLS inline
...
//  Bit converter
template <typename T> class BitConv {
public:
...
#pragma HLS UNROLL
    BitConv<t_DataType> l_bitConv;
    ap_uint<t_InEntryBits> l_inDatBits =
        p_bits.range((i + 1) * t_InEntryBits - 1, i * t_InEntryBits);
...
template <typename TS, typename TD> class WideConv {
private:
...
template <typename t_FloatType,
          unsigned int t_DataWidth = sizeof(t_FloatType) * 8>
class TaggedFloat {
private:
  t_FloatType m_Val;
  bool m_Flush;
...
public:
  ...
  operator const t_TypeInt() {
    t_TypeInt l_val;
    l_val[0] = m_Flush;
    l_val.range(t_TypeWidth - 1, 1) =
        *reinterpret_cast<ap_uint<t_DataWidth> *>(&m_Val);
    return l_val;
  }
  TaggedFloat() {}
  TaggedFloat(t_FloatType p_Val, bool p_Flush)
      : m_Val(p_Val), m_Flush(p_Flush) {}
...
#pragma HLS INLINE
  ...
  void print(std::ostream &os) {
    os << std::setw(BLAS_FLOAT_WIDTH) << m_Val << "f" << m_Flush;
  }
...
template <typename T, unsigned int t_Width> class TaggedWideType {
private:
...
public:
  ...
  TaggedWideType(const t_TypeInt &p_val) {
    m_Val = (typename WideType<T, t_Width>::t_TypeInt)p_val.range(
        t_TypeWidth - 1, 2);
    m_Flush = p_val[1];
    m_Exit = p_val[0];
  }
  operator const t_TypeInt() {
    ...
    l_val[0] = m_Exit;
    l_val[1] = m_Flush;
    l_val.range(t_TypeWidth - 1, 2) =
        (typename WideType<T, t_Width>::t_TypeInt)m_Val;
    return l_val;
  }
...
template <typename T, unsigned int t_Width> class TriangSrl {
private:
...
// Row-major window
template <typename T, unsigned int t_Rows, unsigned int t_Cols> class WindowRm {
private:
...
template <typename t_DataType,
          unsigned int t_DataWidth = sizeof(t_DataType) * 8>
