...
#pragma HLS UNROLL
    ...
    macs(l_dataA[t_ParEntriesN - 1], l_dataB[t_ParEntriesN - 1],
         l_sum[t_ParEntriesN - 1]);
...
#pragma HLS PIPELINE style = stp
  ...
  void muls(DualTaggedStream &p_aIn, EdgeStreamN &p_bIn, EdgeStreamN &p_bOut,
            EdgeMacStreamN &p_mul) {
...
#pragma HLS PIPELINE style = stp
  ...
  void muls(DualTaggedStream &p_aIn, EdgeStreamN &p_bIn,
            EdgeMacStreamN &p_mul) {
    ap_shift_reg<bool, t_ParEntriesN> l_flush;
    for (int i = 0; i < t_ParEntriesN; i++)
#pragma HLS PIPELINE style = stp
...
#pragma HLS PIPELINE style = stp
      ...
      for (int i = 0; i < t_ParEntriesN; i++) {
        l_cVec[t_ParEntriesN - i - 1] =
            l_aSlr[i] * l_bWideVal[i]; // l_c.m_a[i] * l_c.m_b[i];
      }
...
#pragma HLS PIPELINE II = t_FloatDelay style = stp
        if (k == 0)
          l_sum = 0;
...
#pragma HLS ARRAY_PARTITION variable = l_pSum complete dim = 1
  ...
  void macs(DualTaggedStream &p_A, EdgeStreamM &p_Bin, EdgeStreamM &p_Bout,
            EdgeMacStreamN &p_sum) {
#pragma HLS DATAFLOW
...
#pragma HLS DATAFLOW
  ...
  void systolicArray(TaggedWideStreamM &p_As, EdgeStreamN &p_Bs,
...
#pragma HLS UNROLL
  ...
  void tagAB(EdgeStreamM &p_a, EdgeStreamN &p_b, TaggedWideStreamM &l_aOut,
             EdgeStreamN &l_b) {
    for (unsigned int i = 0; i < m_blocks; i++)
