...
template <typename t_DataType, unsigned int t_KBufferDim,
...
class Gemm {
  ...
  typedef WideType<typename TaggedMacType::t_TypeInt, t_ParEntriesN>
      WideTaggedMacType;
  typedef hls::stream<typename WideTaggedMacType::t_TypeInt>
      TaggedMacTypeStream;

public:
  static void gemm(EdgeStreamM &p_As, EdgeStreamN &p_Bs, EdgeMacStreamN &p_sum,
...
#pragma HLS PIPELINE
        ...
        for (int i = 0; i < t_ParEntriesM; i++)
          l_avec[i] = TaggedDataType(l_A[i], k == 0);
        TaggedArrayN l_bvec;
        for (int i = 0; i < t_ParEntriesN; i++)
          l_bvec[i] = TaggedDataType(l_B[i], k == 0);
        ...
        if (l > 0 && k >= t_ParEntriesN + 1 &&
            k <= t_ParEntriesM + t_ParEntriesN) {
...
template <unsigned int t_KBufferDim, unsigned int t_ParEntriesM,
...
#endif
#pragma HLS DATAFLOW
    SystolicArray<t_DataType, t_KBufferDim, t_ParEntriesM, t_ParEntriesN,
                  t_MacDataType>
        l_sysArr(p_blocks);
...
/**
 * @brief gemm function that returns the result vector of the multiplication of
 * two matrix
 *
 ...
 * @param p_n the number of cols of input matrix p_M, as well as the number of
 * entries in the input vector p_x, p_n %
 * l_ParEntries == 0
...
void gemm(
    const unsigned int p_m, const unsigned int p_n, const unsigned int p_k,
...
template <typename t_DataType, unsigned int t_KBufferDim,
          unsigned int t_ParEntries, unsigned int t_MaxSizeC,
...
void gemm(
  ...
  hls::stream<typename WideType<t_DataType, t_ParEntries>::t_TypeInt> l_sum,
      l_mat, l_betaC;

#pragma HLS DATAFLOW

  Gemm<t_DataType, t_KBufferDim, t_ParEntries, t_ParEntries>::gemm(
      p_A, p_B, l_sum, l_blocks);
  gemmBufferC<t_DataType, t_ParEntries, t_ParEntries, t_MaxSizeC>(p_m, p_n,
                                                                  l_sum, l_mat);
  scal<t_DataType, t_ParEntries, t_IndexType>(l_size, p_beta, p_C, l_betaC);
  axpy<t_DataType, t_ParEntries, t_IndexType>(l_size, p_alpha, l_mat, l_betaC,
                                              p_R);
}
