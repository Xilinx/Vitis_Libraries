...
/**
 * @file xBarCol.cpp
 * @brief main function for xBarCol kernel host code
 *
...
int main(int argc, char** argv) {
    if (argc < 5) {
        ...
        cout << "  Usage: xBarCol.exe xBarCol.xclbin Rows Cols NNZs [mtxFile]" << endl;
        return EXIT_FAILURE;
    }
    ...
    OCL_CHECK(l_err, l_err = l_krnlStore.setArg(2, l_nnzMemBlocks));

    // Copy input data to device memory
    OCL_CHECK(l_err,
              l_err = l_cmdQueue.enqueueMigrateMemObjects({l_aColPtrDevBuf, l_xDevBuf}, 0 /* 0 means from host*/));
    ...
    for (unsigned int i = 0; i < l_cols; ++i) {
        ...
        for (unsigned int j = 0; j < l_colPtr; ++j) {
            l_yVecRef.push_back(l_xVec[i]);
        }
    }
    assert(l_yVecRef.size() == l_nnzs);
