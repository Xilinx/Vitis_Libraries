...
int main(int argc, char** argv)
{
    ...
    */
    ...
    //Open CL nench start [[
    (void)cl_kernel_mgr::registerKernel("mergedebevec_accel_4",
                                        "krnl_mergedebevec",
                                        XCLIN(images[0]),
                                        XCLIN(images[1]),
                                        XCLIN(images[2]),
                                        XCLIN(images[3]),
                                        XCLOUT(hdrDebevecHLS),
                                        XCLIN(times_ln,SRC_NUM*sizeof(float)),
                                        XCLIN(input_response_vec_ln, cv::LDR_SIZE*images[0].channels()*sizeof(float)),
                                        XCLIN(images[0].rows),
                                        XCLIN(images[0].cols)
                                       );
