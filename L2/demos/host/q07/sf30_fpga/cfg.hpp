...
static void gen_fcfg_1(uint32_t cfg[]) {
    ...
    cfg[n++] = (uint32_t)19961231UL;
    cfg[n++] = 0UL | (FOP_GEU << FilterOpWidth) | (FOP_LEU);
    // cond_3
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = 0UL | (FOP_DC << FilterOpWidth) | (FOP_DC);
...
static void gen_fcfg_2(uint32_t cfg[]) {
    using namespace xf::database;
    int n = 0;
...
void get_cfg_dat_1(ap_uint<512>* hbuf) {
    ...
    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 * 0 + 16 * 0 + 8 * 0 + 4 + 2 + 1);
...
void get_cfg_dat_2(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_a[] = {2, 0, 1, -1, -1, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }

    signed char id_b[] = {1, 2, 3, 4, 0, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }

    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 * 0 + 16 + 8 + 4 + 2 + 1);
    ...
    shuffle2_cfg(7, 0) = 1;
    shuffle2_cfg(15, 8) = 2;
    shuffle2_cfg(23, 16) = 6;
    shuffle2_cfg(31, 24) = 7;
    shuffle2_cfg(39, 32) = 3;
    shuffle2_cfg(47, 40) = 0;
    shuffle2_cfg(55, 48) = -1;
    shuffle2_cfg(63, 56) = -1;
    ...
    shuffle3_cfg(7, 0) = 2;
    shuffle3_cfg(15, 8) = 3;
    shuffle3_cfg(23, 16) = 4;
    shuffle3_cfg(31, 24) = 5;
...
void get_cfg_dat_3j(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_a[] = {0, 1, 2, 3, 4, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }
    ...
    shuffle2_cfg(15, 8) = 7;
    shuffle2_cfg(23, 16) = 0;
    shuffle2_cfg(31, 24) = 8;
    shuffle2_cfg(39, 32) = 9;
    shuffle2_cfg(47, 40) = -1;
    shuffle2_cfg(55, 48) = -1;
    shuffle2_cfg(63, 56) = -1;
...
void get_cfg_dat_3p(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_a[] = {2, 0, 1, 3, 4, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }
...
void get_cfg_dat_4(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
...
void get_cfg_dat_5(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_b[] = {2, 0, 1, 3, 4, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }

    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 * 0 + 16 * 0 + 8 + 4 + 2 + 1);
