...
static void gen_fcfg_1(uint32_t cfg[]) {
    ...
    cfg[n++] = (uint32_t)20UL;
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = 0UL | (FOP_EQ << FilterOpWidth) | (FOP_DC);
    // cond_2
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = (uint32_t)0UL;
    cfg[n++] = 0UL | (FOP_DC << FilterOpWidth) | (FOP_DC);
...
static void gen_fcfg_3(uint32_t cfg[]) {
    using namespace xf::database;
    int n = 0;
...
void get_cfg_dat_1(ap_uint<512>* hbuf) {
    ...
    signed char id_b[] = {1, 0, -1, -1, -1, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }

    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 * 0 + 16 * 0 + 8 * 0 + 4 * 0 + 2 * 1 + 1 * 1);
...
void get_cfg_dat_2(ap_uint<512>* hbuf) {
    ...
    signed char id_b[] = {1, 0, 2, 3, -1, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }

    t.range(191, 184) = (128 * 0 + 64 * 0 + 32 * 0 + 16 * 0 + 8 * 0 + 4 + 2 * 1 + 1 * 1);
...
void get_cfg_dat_3(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_a[] = {0, 1, 2, -1, -1, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }

    signed char id_b[] = {0, 1, -1, -1, -1, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
...
void get_cfg_dat_4j(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
...
void get_cfg_dat_4p(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    signed char id_b[] = {0, 2, 1, -1, -1, -1, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
...
void get_cfg_dat_6(ap_uint<512>* hbuf) {
    ap_uint<512>* b = hbuf;
    memset(b, 0, sizeof(ap_uint<512>) * 9);
    ...
    ap_uint<512> t = 1; // join on
    t.set_bit(1, 0);    // aggr off
    t.set_bit(2, 1);    // dual_key on
    t.range(5, 3) = 2;  // hash join flag = 0 for normal, 1 for semi, 2 for anti

    signed char id_a[] = {0, 2, -1, -1, -1, -1, -1, -1}; // dup key to payload
    for (int c = 0; c < 8; ++c) {
        t.range(56 + 8 * c + 7, 56 + 8 * c) = id_a[c];
    }
