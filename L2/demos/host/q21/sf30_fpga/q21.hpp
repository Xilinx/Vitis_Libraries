...
void q21SemiJoin_l_t3(Table& tin1, Table* tin2, int hjTimes, Table& tout) {
    int nrow1 = tin1.getNumRow();
    std::unordered_multimap<Q21AntiKey, int32_t> ht1;
    for (int i = 0; i < nrow1; i++) {
        int32_t l_orderkey = tin1.getInt32(i, 0);
    ...
    for (int p = 0; p < hjTimes; p++) {
        int nrow2 = tin2[p].getNumRow();
        for (int i = 0; i < nrow2; i++) {
            int32_t l_orderkey = tin2[p].getInt32(i, 0);
            int32_t s_nationkey = tin2[p].getInt32(i, 1);
            int32_t s_suppkey = tin2[p].getInt32(i, 2);
            Q21AntiKey k = {l_orderkey, s_suppkey};
            auto it = ht1.find(k);
            if (it != ht1.end()) {
                tout.setInt32(r, 0, l_orderkey);
...
void q21AntiJoin_t4_t3(Table& tin1, Table* tin2, int hjTimes, Table& tout) {
    int nrow1 = tin1.getNumRow();
    std::unordered_multimap<KeyA, int32_t> ht1;
    for (int i = 0; i < nrow1; i++) {
        int32_t l_orderkey = tin1.getInt32(i, 0);
...
void q21Group_t4(Table* tin, int hjTimes, Table& tout) {
    std::unordered_map<int32_t, int32_t> ht1;
    for (int p = 0; p < hjTimes; p++) {
        int nrow = tin[p].getNumRow();
        for (int i = 0; i < nrow; i++) {
            int32_t s_suppkey = tin[p].getInt32(i, 0);
            auto it = ht1.find(s_suppkey);
            if (it != ht1.end()) {
                int32_t t = it->second + 1;
                ht1[s_suppkey] = t;
