...
// t4:150000
void combile_t4_t5(Table* tin1, Table& tin2, Table& tout, int hpTimes) {
    int shift = 0;
    for (int j = 0; j < hpTimes; j++) {
        int nrow1 = tin1[j].getNumRow();

        for (int i = 0; i < nrow1; i++) {
            tout.setInt32(i + shift, 1, tin1[j].getInt32(i, 0));
            tout.setInt32(i + shift, 0, tin1[j].getInt32(i, 8));
        }
        shift += nrow1;
    }

    int nrow2 = tin2.getNumRow();
    for (int i = shift; i < shift + nrow2; i++) {
        int j = i - shift;
        tout.setInt32(i, 1, tin2.getInt32(j, 0));
        tout.setInt32(i, 0, tin2.getInt32(j, 1));
    }
    shift += nrow2;

    tout.setNumRow(shift);
    std::cout << std::dec << shift << " after combile_t4_t5" << std::endl;
}
...
void q13Sort(Table& tin, Table& tout) {
    ...
    for (int i = 0; i < nrow; i++) {
        Q13SortRow t = {tin.getInt32(i, 0), tin.getInt32(i, 8)};
        rows.push_back(t);
    }
    ...
    for (auto& it : rows) {
        ...
        if (r < 50) std::cout << it.c_count << " " << it.custdist << std::endl;
        ++r;
    }
    tout.setNumRow(r);
