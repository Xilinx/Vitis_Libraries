...
int main(int argc, const char* argv[]) {
    ...
    tbs[3].addCol("s_address", TPCH_READ_S_ADDR_MAX + 1);
    tbs[3].addCol("s_name", TPCH_READ_S_NAME_LEN + 1);

    tbs[4] = Table("nation", nation_n, 2, in_dir);
    tbs[4].addCol("n_nationkey", 4);
    tbs[4].addCol("n_name", TPCH_READ_NATION_LEN + 1);
    ...
    // step2 : t1 part -> t2
    q20Join_t1_p(th0, tbs[1], tk0);

    gettimeofday(&tv_r_1, 0);
    // step3 :lineitem group->t3
    q20GroupBy(tbs[2], tk1);

    gettimeofday(&tv_r_2, 0);
    // step4 : t3 lineitem->t4
    q20Join_t2_t3(tk0, tk1, th0);

    gettimeofday(&tv_r_3, 0);
    // step5: t4,supplier->t5
    q20Join_t4_s(th0, tbs[3], tk0);
    // q20Join_s_t4(tbs[3],th0,tk0);
    ...
    std::cout << "Join CPU time of Host " << tvdiff(&tv_r_0, &tv_r_1) / 1000 << " ms" << std::endl;
    std::cout << "Groupby CPU time of Host " << tvdiff(&tv_r_1, &tv_r_2) / 1000 << " ms" << std::endl;
    std::cout << "Join CPU time of Host " << tvdiff(&tv_r_2, &tv_r_3) / 1000 << " ms" << std::endl;
    std::cout << "Join CPU time of Host " << tvdiff(&tv_r_3, &tv_r_4) / 1000 << " ms" << std::endl;
    std::cout << "Filter CPU time of Host " << tvdiff(&tv_r_4, &tv_r_5) / 1000 << " ms" << std::endl;
    std::cout << "Join time of Host " << tvdiff(&tv_r_5, &tv_r_6) / 1000 << " ms" << std::endl;
