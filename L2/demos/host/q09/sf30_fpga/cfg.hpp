...
void get_cfg_dat_3(ap_uint<512>* hbuf) {
    ...
    signed char id_b[] = {0, 1, 2, 3, 4, 5, -1, -1}; // not used.
    for (int c = 0; c < 8; ++c) {
        t.range(120 + 8 * c + 7, 120 + 8 * c) = id_b[c];
    }
    ...
    // alu:
    ap_uint<289> op = 0;
    xf::database::dynamicALUOPCompiler<uint32_t, uint32_t, uint32_t, uint32_t>("strm1*(-strm2+c2)-strm3*(strm4*c4)", 0,
                                                                               100, 0, 100, op);
    b[1] = op;
    b[2] = 0;
    ...
    shuffle2_cfg(7, 0) = 1;
    shuffle2_cfg(15, 8) = 2;
    shuffle2_cfg(23, 16) = 3;
    shuffle2_cfg(31, 24) = 7;
    shuffle2_cfg(39, 32) = 0;
    shuffle2_cfg(47, 40) = 6;
    shuffle2_cfg(55, 48) = -1;
    shuffle2_cfg(63, 56) = -1;
...
void get_cfg_dat_4(ap_uint<512>* hbuf) {
    ...
    ap_uint<512> t = 1; // join on
    t.set_bit(1, 0);    // aggr off
    t.set_bit(2, 0);    // dual-col key off
    t.set_bit(6, 1);    // concatenate tables
    t.range(5, 3) = 0;  // hash join flag = 0 for normal, 1 for semi, 2 for anti
