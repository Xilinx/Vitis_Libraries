...
void q4GroupBy(Table* tin, Table& origin, Table& tout, int hjTimes) {
    std::unordered_map<q4GroupKey, int64_t> ht1;
    /*    ap_uint<512>* data0 = tin.data + tin.size512[tin.ncol]*0;
        ap_uint<512>* data1 = tin.data + tin.size512[tin.ncol]*1;

        int size512=data0[0].range(63,32);
        int rowNum=data0[0].range(31,0);
        std::cout<<"!!! "<<rowNum<<std::endl;
        for(int i=0;i<rowNum;i++){
           int32_t o_rowid;// = tin.getInt32(i,0);
           memcpy(&o_rowid, (void*)(data0 + size512*0 + 1) + i * sizeof(int), sizeof(int));
    ...
    //       if(i<10) std::cout<<std::dec<<o_rowid<<" "<<orderpriority.data()<<std::endl;
        ...
        size512=data1[0].range(63,32);
        rowNum=data1[0].range(31,0);

        for(int i=0;i<rowNum;i++){
           int32_t o_rowid;// = tin.getInt32(i,0);
           memcpy(&o_rowid, (void*)(data1 + size512*0 + 1) + i * sizeof(int), sizeof(int));
           std::array<char,TPCH_READ_MAXAGG_LEN+1> orderpriority =
    origin.getcharN<char,TPCH_READ_MAXAGG_LEN+1>(o_rowid,0);
    //       if(i<10) std::cout<<std::dec<<o_rowid<<" "<<orderpriority.data()<<std::endl;
           q4GroupKey k{std::string(orderpriority.data())};
    ...
    for (int j = 0; j < hjTimes; j++) {
        for (int i = 0; i < tin[j].getNumRow(); i++) {
            int32_t o_rowid = tin[j].getInt32(i, 0);
            std::array<char, TPCH_READ_MAXAGG_LEN + 1> orderpriority =
                origin.getcharN<char, TPCH_READ_MAXAGG_LEN + 1>(o_rowid, 0);
            //            if(i<10) std::cout<<std::dec<<o_rowid<<" "<<orderpriority.data()<<std::endl;
            q4GroupKey k{std::string(orderpriority.data())};
