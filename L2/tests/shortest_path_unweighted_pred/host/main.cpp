...
int main(int argc, const char* argv[]) {
    ...
    int* result;
    result = aligned_alloc<int>(((numVertices + 1023) / 1024) * 1024);
    ...
    config[1] = std::numeric_limits<int>::max();
    config[2] = 0;
    config[3] = 10 * 300 * 4096;
    ap_uint<32> cmd;
    cmd.set_bit(0, 0); // enable weight?
    cmd.set_bit(1, 1); // enable predecessor?
    cmd.set_bit(2, 1); // float or fixed? 0 for float, 1 for fixed
    config[4] = cmd;
    config[5] = sourceID;
#ifndef HLS_TEST
    ...
    mext_o[1] = {(unsigned int)(3) | XCL_MEM_TOPOLOGY, column512, 0};
    mext_o[2] = {(unsigned int)(5) | XCL_MEM_TOPOLOGY, weight512, 0};
    mext_o[3] = {(unsigned int)(3) | XCL_MEM_TOPOLOGY, info, 0};
    mext_o[4] = {(unsigned int)(0) | XCL_MEM_TOPOLOGY, config, 0};
    mext_o[5] = {(unsigned int)(0) | XCL_MEM_TOPOLOGY, ddrQue, 0};
    mext_o[6] = {(unsigned int)(3) | XCL_MEM_TOPOLOGY, result, 0};
    mext_o[7] = {(unsigned int)(5) | XCL_MEM_TOPOLOGY, pred, 0};
#else
    ...
    result_buf = cl::Buffer(context, CL_MEM_EXT_PTR_XILINX | CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE,
                            sizeof(int) * ((numVertices + 1023) / 1024) * 1024, &mext_o[6]);
    ...
    shortestPath_top(config, offset512, column512, weight512, ddrQue512, ddrQue, result512, result32, pred512, pred,
                     info);
#endif
    std::cout << "============================================================" << std::endl;
    ...
    for (int i = 0; i < numVertices; i++) {
        if (connect[i] == false && result[i] != std::numeric_limits<int>::max()) {
            std::cout << "Err distance: " << i << " " << std::numeric_limits<int>::max() << " " << result[i]
                      << std::endl;
            err++;
        }
