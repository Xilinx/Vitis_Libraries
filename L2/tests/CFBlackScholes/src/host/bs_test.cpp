...
///
/// Usage: ./bs_test ./xclbin/<kernel_name> <number of prices>
///
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "************" << std::endl;
    std::cout << "BS Demo v1.0" << std::endl;
    std::cout << "************" << std::endl;
    std::cout << std::endl;
    ...
    for (unsigned int i = 0; i < num; i++) {
        ...
        // Use full Black-Scholes-Merton model with fixed q=0
        bsm_model(s_temp, v_temp, r_temp, t_temp, k_temp, 0, call, host_price[i], host_delta[i], host_gamma[i],
    ...
    OCL_CHECK(err, cl::Kernel krnl_cfBSEngine(program, "bs_kernel", &err));
    ...
    // Set the arguments
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(0, buffer_s));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(1, buffer_v));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(2, buffer_r));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(3, buffer_t));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(4, buffer_k));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(5, call));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(6, num));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(7, buffer_price));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(8, buffer_delta));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(9, buffer_gamma));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(10, buffer_vega));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(11, buffer_theta));
    OCL_CHECK(err, err = krnl_cfBSEngine.setArg(12, buffer_rho));

    // Copy input data to device global memory
    OCL_CHECK(err, err = q.enqueueMigrateMemObjects({buffer_s}, 0));
    ...
    cl::Event event;
    OCL_CHECK(err, err = q.enqueueTask(krnl_cfBSEngine, NULL, &event));
    ...
    if (std::abs(max_delta_diff) > 6.0e-7) {
        std::cout << "FAIL: max_delta_diff = " << max_delta_diff << std::endl;
        ret = 1;
    }
    if (std::abs(max_gamma_diff) > 7.0e-8) {
