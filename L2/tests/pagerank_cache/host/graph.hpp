...
template <class ID_T, class VALUE_T>
void readInWeightedDirectedGraphRow(std::string filename, CscMatrix<ID_T, VALUE_T>& cscMat, int nnz, int& nrows) {
...
template <class ID_T, class VALUE_T>
void readInWeightedDirectedGraphRow(std::string filename, CsrMatrix<ID_T, VALUE_T>& csrMat, int nnz, int& nrows) {
    std::fstream fin(filename.c_str(), std::ios::in);
    if (!fin) {
        std::cout << "Error : file doesn't exist !" << std::endl;
        exit(1);
    ...
    while (std::getline(fin, line)) {
        if (line[0] != 37) {
            ...
            for (int i = 0; i < nrows; i++) {
                ...
                VALUE_T tmp;
                istr >> tmp;
                csrMat.rowOffset.push_back(tmp);
            }
            csrMat.rowOffset.push_back(nnz);
            break;
        };
    }
    fin.close();
...
template <class ID_T, class VALUE_T>
void readInWeightedDirectedGraphCV(std::string filename, CsrMatrix<ID_T, VALUE_T>& csrMat, int& nnz) {
    std::fstream fin(filename.c_str(), std::ios::in);
    if (!fin) {
        std::cout << "Error : file doesn't exist !" << std::endl;
        exit(1);
    ...
    while (std::getline(fin, line)) {
        if (line[0] != 37) {
            ...
            for (int i = 0; i < nnz; i++) {
                ...
                csrMat.column.push_back(col);
                csrMat.value.push_back(tmp);
            }
            break;
        };
    }
...
template <class ID_T, class VALUE_T>
void writeOutWeightedDirectedGraphRow(std::string filename, CsrMatrix<ID_T, VALUE_T>& csrMat, int nrows) {
    std::fstream fin(filename.c_str(), std::ios::out);
    if (!fin) {
        std::cout << "Error : file doesn't exist !" << std::endl;
        exit(1);
...
template <class ID_T, class VALUE_T>
void writeOutWeightedDirectedGraphCV2(std::string filename, CsrMatrix<ID_T, VALUE_T>& csrMat, int nnz) {
    std::fstream fin(filename.c_str(), std::ios::out);
    if (!fin) {
        std::cout << "Error : file doesn't exist !" << std::endl;
        exit(1);
