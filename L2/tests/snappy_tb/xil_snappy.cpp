...
uint64_t xilSnappy::compress(uint8_t* in, uint8_t* out, uint64_t input_size, uint32_t host_buffer_size) {
    ...
    for (uint32_t brick = 0, itr = 0; brick < total_chunks; brick += 1, itr++, flag = !flag) {
        lcl_cu = 1;
        ...
        for (int cu = 0; cu < lcl_cu; cu++) {
            ...
            compress_kernel_snappy->setArg(narg++, m_block_size_in_kb);
            compress_kernel_snappy->setArg(narg++, sizeOfChunk[brick + cu]);

            // Transfer data from host to device
            m_q->enqueueMigrateMemObjects({*(buffer_input), *(buffer_block_size)}, 0);
            m_q->finish();
    ...
    int stride = total_chunks;

    // Handle leftover bricks
    for (int ovr_itr = 0, brick = stride - 1; ovr_itr < leftover; ovr_itr += 1, brick += 1) {
        lcl_cu = 1;
