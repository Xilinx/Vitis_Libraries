...
void readInIndiceWeight(std::fstream& fstream, // input: file stream
                        int dataType,
                        unsigned int length[PUNUM], // input: value numbers of each buffer
                        ap_int<32>* buffer1[PUNUM], // output: first array
...
void readInDataFile(std::string offsetFile,
    ...
    if (graphType == 0) {
        ...
        numVerticesPU[PUNUM - 1] += 1;
        for (int i = 0; i < PUNUM; i++) { // offset32 buffers allocation
            offset32[i] = aligned_alloc<ap_int<32> >(numVerticesPU[i] + 1);
        ...
        readInOffset<ap_int<32>, PUNUM>(offsetfstream, numVerticesPU, offset32);
        for (int i = 0; i < PUNUM - 1; ++i) { // add datas in order to meet kernel need
            offset32[i][numVerticesPU[i]] = offset32[i + 1][0];
        }
        numVerticesPU[PUNUM - 1] -= 1; // adjust last numVerticesPU in order to meet kernel need
        ...
        for (int i = 0; i < PUNUM; i++) {
            indice32[i] = aligned_alloc<ap_int<32> >(numEdgesPU[i]);
            weightSparse[i] = aligned_alloc<float>(numEdgesPU[i]);
        }
...
template <int PUNUM>
int checkData(std::string goldenFile, ap_int<32>* kernelID, float* kernelSimilarity) {
    int err = 0;
    char line[1024] = {0};
