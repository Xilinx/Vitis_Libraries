...
void xilSnappy::bufferExtensionAssignments(bool flow) {
...
uint64_t xilSnappy::decompressSequential(uint8_t* in, uint8_t* out, uint64_t input_size) {
    // Assignments to buffer extensions
    ...
    for (uint64_t inIdx = 0; inIdx < input_size; inIdx += HOST_BUFFER_SIZE * C_COMPUTE_UNIT) {
        ...
        for (int devbuf = 0; devbuf < compute_cu; devbuf++) {
            ...
            buffer_input[devbuf][0] =
                new cl::Buffer(*m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX,
                               bufSize_in_bytes_cu[devbuf], &inExt[devbuf]);

            buffer_output[devbuf][0] =
                new cl::Buffer(*m_context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX,
                               bufSize_in_bytes_cu[devbuf], &outExt[devbuf]);

            buffer_compressed_size[devbuf][0] =
                new cl::Buffer(*m_context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY | CL_MEM_EXT_PTR_XILINX,
                               sizeof(uint32_t) * total_blocks_cu[devbuf], &csExt[devbuf]);

            buffer_block_size[devbuf][0] =
                new cl::Buffer(*m_context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX,
                               sizeof(uint32_t) * total_blocks_cu[devbuf], &bsExt[devbuf]);
        }
