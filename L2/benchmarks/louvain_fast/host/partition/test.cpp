...
int LvTest_main(int argc, char** argv) {
    ...
    long* C_orig = (long*)malloc(NV_begin * sizeof(long));
    assert(C_orig != 0);
#pragma omp parallel for
    ...
    runLouvainWithFPGA_demo(G, C_orig, opts_xclbinPath, opts_coloring, opts_minGraphSize, opts_threshold, opts_C_thresh,
                            numThreads);
 ...
 */
    ...
    if (!usingPrune) {
        ...
        int id_dev = 0;
        int numPhase = 100;
        GLV* pglv_iter = LouvainGLV_general(false, MD_FAST, id_dev, pglv_src, opts_xclbinPath, numThreads, id_glv,
