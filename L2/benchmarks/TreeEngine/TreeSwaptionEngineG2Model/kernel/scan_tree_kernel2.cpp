...
static void treeKernel(hls::stream<int>& typeStrm,
    ...
    Model model;
    model.initialization(flatRate, a, sigma, 0.050052910248222851, 0.0094424313463861171, rho);
    DT process1[4] = {a, sigma, 0.0, 0.0};
    DT process2[4] = {0.050052910248222851, 0.0094424313463861171, 0.0, 0.0};

    treeSwaptionEngine<DT, Model, Process, DIM, LEN, LEN2>(model, process1, process2, type, fixedRate, timestep,
                                                           initTime, initSize, exerciseCnt, floatingCnt, fixedCnt,
...
static void scanDataDist(int len,
                         ...
                         hls::stream<int> typeStrm[K2],
                         hls::stream<DT> fixedRateStrm[K2],
                         hls::stream<int> timestepStrm[K2],
                         hls::stream<int> initSizeStrm[K2],
                         hls::stream<DT> aStrm[K2],
                         hls::stream<DT> sigmaStrm[K2],
                         hls::stream<DT> flatRateStrm[K2],
                         hls::stream<DT> x0Strm[K2],
                         hls::stream<DT> nominalStrm[K2],
                         hls::stream<DT> spreadStrm[K2],
                         hls::stream<DT> initTimeStrm[K2],
                         hls::stream<int> exerciseCntStrm[K2],
                         hls::stream<int> floatingCntStrm[K2],
                         hls::stream<int> fixedCntStrm[K2]) {
    for (int i = 0; i < len; i++) {
        typeStrm[i % K2].write(inputParam1[0].type);
        fixedRateStrm[i % K2].write(inputParam1[0].fixedRate);
        timestepStrm[i % K2].write(inputParam1[0].timestep);
        initSizeStrm[i % K2].write(inputParam1[0].initSize);
        aStrm[i % K2].write(inputParam1[0].a);
        sigmaStrm[i % K2].write(inputParam1[0].sigma);
        flatRateStrm[i % K2].write(inputParam1[0].flatRate);
        x0Strm[i % K2].write(inputParam0[0].x0);
        nominalStrm[i % K2].write(inputParam0[0].nominal);
        spreadStrm[i % K2].write(inputParam0[0].spread);
        for (int j = 0; j < inputParam1[0].initSize; j++) {
            initTimeStrm[i % K2].write(inputParam0[0].initTime[j]);
        }
        for (int j = 0; j < ExerciseLen; j++) {
            exerciseCntStrm[i % K2].write(inputParam1[0].exerciseCnt[j]);
        }
        for (int j = 0; j < FloatingLen; j++) {
            floatingCntStrm[i % K2].write(inputParam1[0].floatingCnt[j]);
        }
        for (int j = 0; j < FixedLen; j++) {
            fixedCntStrm[i % K2].write(inputParam1[0].fixedCnt[j]);
        }
    }
}

static void treeKernelWrapper(hls::stream<int> typeStrm[K2],
                              hls::stream<DT> fixedRateStrm[K2],
                              hls::stream<int> timestepStrm[K2],
                              hls::stream<int> initSizeStrm[K2],
                              hls::stream<DT> aStrm[K2],
