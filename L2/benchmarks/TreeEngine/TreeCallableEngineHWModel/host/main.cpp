...
int main(int argc, const char* argv[]) {
    ...
    for (int i = 0; i < 1; i++) {
        ...
        inputParam1_alloc[i].a = 0.03;
        inputParam1_alloc[i].sigma = 0.01;
        inputParam1_alloc[i].flatRate = 0.055;
        inputParam0_alloc[i].x0 = 0.0;
        inputParam0_alloc[i].nominal = 100.0;
    ...
    std::cout << "NPV= " << std::setprecision(15) << output[0][0] << " ,diff/NPV= " << (output[0][0] - golden) / golden
              << std::endl;
    return err;
}
