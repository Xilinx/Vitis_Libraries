...
#pragma HLS unroll
                    ...
                    singleSample.real(superSample.range(31 + 64 * w, 64 * w));
                    singleSample.imag(superSample.range(63 + 64 * w, 32 + 64 * w));
...
#pragma HLS pipeline II = 1
                ...
                for (int w = 0; w < t_memWidth; ++w) {
#pragma HLS unroll
                    singleSuperSample.range(31 + 64 * w, 64 * w) = l_inMemWideSample[w].real();
                    singleSuperSample.range(63 + 64 * w, 32 + 64 * w) = l_inMemWideSample[w].imag();
                }
...
#pragma HLS stream variable = fftInStrm depth = FIFO_SIZE
    hls::stream<SuperSampleContainer<t_memWidth, T_elemType> > fftOutStrm;
#pragma HLS stream variable = fftOutStrm depth = FIFO_SIZE

    readImages<t_memWidthBits, t_numRows, t_numCols, t_numImages, t_memWidth, T_elemType,
    ...
    writeImages<t_memWidthBits, t_numRows, t_numCols, t_numImages, t_memWidth, T_elemType,
                SuperSampleContainer<t_memWidth, T_elemType> >(fftOutStrm, p_fftOutData, n_images);
}
