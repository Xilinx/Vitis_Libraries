...
template <int COL_NM>
void Split_1D(/*bool& build_probe_flag,*/
...
//#if !defined __SYNTHESIS__ && XDEBUG == 1
    ...
    xf::database::hashMultiJoinBuildProbe<1, 64, 192, 192, 192, 3, 17, 24, 4>(
...
template <int COL_IN_NM, int CH_NM, int COL_OUT_NM, int ROUND_NM>
void hash_join_wrapper(bool build_probe_flag,
                       hls::stream<ap_uint<3> >& join_flag_strm,
...
#pragma HLS stream variable = pu_end_status_strm depth = 2
    ...
    if (jn_on) {
        ...
        hash_join_plus_adapter<COL_IN_NM, CH_NM, COL_OUT_NM, ROUND_NM>(
            build_probe_flag, jn_on, mk_on, join_flag_strm, in_strm, e_in_strm, out_strm, e_out_strm,
