...
static void usage(std::string exe)
{
    ...
    std::cout << "s=<>, k=<>, r_domestic=<>, r_foreign=<>, v=<>, t=<>, q=<>, E=<>, fxv=<>, corr=<> [exp=<>]" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    for (unsigned int i = 0; i < num; i++) {
        quanto_model(s[i], v[i], rd[i], t[i], k[i], rf[i], 
                q[i], E[i], fxv[i], corr[i],
                call, host_price[i], host_delta[i], host_gamma[i],
    ...
    OCL_CHECK(err, cl::Kernel krnl_cfGKEngine(program, "quanto_kernel", &err));

    // Allocate Buffer in Global Memory
    // Buffers are allocated using CL_MEM_USE_HOST_PTR for efficient memory and
    // Device-to-host communication
    std::cout << "Allocating buffers..." << std::endl;
    OCL_CHECK(err, cl::Buffer buffer_s(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       s.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_v(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       v.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_rd(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       rd.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_t(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       t.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_k(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       k.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_rf(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       rf.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_q(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       q.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_E(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       E.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_fxv(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       fxv.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_corr(context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_ONLY, modified_num * sizeof(KERNEL_DT),
                                       corr.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_price(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                           price.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_delta(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                           delta.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_gamma(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                           gamma.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_vega(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                          vega.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_theta(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                           theta.data(), &err));
    OCL_CHECK(err, cl::Buffer buffer_rho(context, CL_MEM_USE_HOST_PTR | CL_MEM_WRITE_ONLY, modified_num * sizeof(KERNEL_DT),
                                         rho.data(), &err));
    ...
    cq.finish();
    auto fpga_duration = std::chrono::duration_cast<std::chrono::microseconds>
        (std::chrono::high_resolution_clock::now() - t_start).count();
    ...
    std::cout << "FPGA time returned by profile API           = " << (duration_nanosec * (1.0e-6)) << " ms" << std::endl;
