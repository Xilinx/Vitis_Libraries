...
#define WIDTH_CH2 32
// width of reference channel, in bits
#define WIDTH_REF 32
...
int main(int argc, const char* argv[]) {
    ...
    const int nBlksRef = SIZE / (WIDTH_REF / 8);
    // Host buffers
    ap_uint<WIDTH_CH1>* hb_in1 = aligned_alloc<ap_uint<WIDTH_CH1> >(nBlks1);
    ap_uint<WIDTH_REF>* hb_in_ref1 = aligned_alloc<ap_uint<WIDTH_REF> >(nBlksRef);
    ...
    ap_uint<WIDTH_REF>* hb_in_ref2 = aligned_alloc<ap_uint<WIDTH_REF> >(nBlksRef);
    ...
    for (unsigned int j = 0; j < SIZE; j++) {
        ...
        hb_in_ref1[j / (WIDTH_REF / 8)].range((j % (WIDTH_REF / 8)) * 8 + 7, (j % (WIDTH_REF / 8)) * 8) = golden1[j];
        hb_out1[0] = 0;
        hb_in2[j / (WIDTH_CH2 / 8)].range((j % (WIDTH_CH2 / 8)) * 8 + 7, (j % (WIDTH_CH2 / 8)) * 8) = golden2[j];
        hb_in_ref2[j / (WIDTH_REF / 8)].range((j % (WIDTH_REF / 8)) * 8 + 7, (j % (WIDTH_REF / 8)) * 8) = golden2[j];
        hb_out2[0] = 0;
    }
    ...
    int mode = 0;
    kernel1.setArg(0, mode);
    kernel1.setArg(1, ch1_buff[1]);
    kernel1.setArg(3, ch1_buff[2]);
    kernel1.setArg(4, (uint64_t)SIZE);
    kernel1.setArg(5, ch2_buff[1]);
    kernel1.setArg(7, ch2_buff[2]);
    kernel1.setArg(8, (uint64_t)SIZE);
    ...
    q.enqueueTask(kernel1, &write_events[0], &kernel_events[0][0]);
