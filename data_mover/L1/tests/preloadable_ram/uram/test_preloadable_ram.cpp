...
void dut(hls::stream<xf::data_mover::ConstData::type>& is,
         hls::stream<ap_axiu<AXI_WIDTH, 0, 0, 0> >& os,
         hls::stream<ap_axiu<AXI_WIDTH, 0, 0, 0> >& cs,
         hls::stream<xf::data_mover::CheckResult::type>& rs,
         const uint64_t sz) {
#pragma HLS INTERFACE axis port = os
#pragma HLS INTERFACE axis port = cs
    xf::data_mover::PreloadableUram<AXI_WIDTH, RAM_DEPTH> puram;
    puram.preload(is, sz);
...
int main() {
    ...
    xf::data_mover::ConstData::type golden[AXI_WIDTH / xf::data_mover::ConstData::Port_Width * RAM_DEPTH];
    hls::stream<xf::data_mover::ConstData::type> is;
    hls::stream<ap_axiu<AXI_WIDTH, 0, 0, 0> > cs;
    hls::stream<ap_axiu<AXI_WIDTH, 0, 0, 0> > os;
    hls::stream<xf::data_mover::CheckResult::type> rs;
    ...
    srand(seed);
    const int bytePerInData = xf::data_mover::ConstData::Port_Width / 8;
    ...
    int error_cmp = 0;
    xf::data_mover::CheckResult::type r = rs.read();
    if (r != 1) {
        error_cmp++;
