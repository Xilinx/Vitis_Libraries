    ...
    for (int i = 0; i < numReadSeg; i++) {
        ...
        for (int j = 0; j < numHapSeg; j++) {
            ...
            if (violate) {
                ...
                usedFPGA = true;
                if (this->is_seq()) {
                    computePairhmmFPGASeq((char*)&(host_input[0]->dataPack), host_input[0]->numRead,
                                          host_input[0]->numHap, host_output[0], (char*)&(host_input[1]->dataPack),
                                          host_input[1]->numRead, host_input[1]->numHap, host_output[1],
                                          (char*)&(host_input[2]->dataPack), host_input[2]->numRead,
                                          host_input[2]->numHap, host_output[2]);
                } else {
                    computePairhmmFPGAOverlap((char*)&(host_input[0]->dataPack), host_input[0]->numRead,
                                              host_input[0]->numHap, host_output[0], (char*)&(host_input[1]->dataPack),
                                              host_input[1]->numRead, host_input[1]->numHap, host_output[1],
                                              (char*)&(host_input[2]->dataPack), host_input[2]->numRead,
                                              host_input[2]->numHap, host_output[2]);
                ...
                for (int k = 0; k < cur_numHap * cur_numRead; k++) {
                    ...
                    if (k < host_input[0]->numRead * cur_numHap)
                        ...
                        cur_float_result = host_output[2][k - host_input[0]->numRead * cur_numHap -
                                                          host_input[1]->numRead * cur_numHap];
...
void xilPairHMM::computePairhmm(pairhmmInput* input, pairhmmOutput* output, bool& usedFPGA) {
    if (useFPGA)
        computePairhmmxil(input, output, usedFPGA);
    else
        computePairhmmAVX(input, output, false);
}
