...
template <int SRC_T, int ROWS, int COLS, int DEPTH, int NPC, int XFCVDEPTH_IN, int COLS_TRIP>
void read_roi_row(xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_IN>& _src_mat,
                  int offset,
                  int col_loop_cnt,
                  int& cnt,
                  int end_loc,
 ...
 * Output      : _dst_mat
 */
template <int SRC_T, int ROWS, int COLS, int DEPTH, int NPC, int XFCVDEPTH_OUT, int COLS_TRIP>
...
template <int SRC_T,
          ...
          int WORDWIDTH_DST,
          int COLS_TRIP>
void xFcropkernel_memorymapped(xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_IN>& _src_mat,
                               xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_OUT>& _dst_mat,
...
// clang-format off
    #pragma HLS bind_storage variable=buf0 type=RAM_S2P impl=BRAM
    // clang-format on

    XF_TNAME(SRC_T, NPC) buf1[COLS >> XF_BITSHIFT(NPC)];
// clang-format off
    #pragma HLS bind_storage variable=buf1 type=RAM_S2P impl=BRAM
    // clang-format on
    ...
    read_roi_row<SRC_T, ROWS, COLS, DEPTH, NPC, XFCVDEPTH_IN, COLS_TRIP>(
...
// clang-format off
        ...
        if (toggle_flag) {
            write_roi_row<SRC_T, ROWS, COLS, DEPTH, NPC, XFCVDEPTH_OUT, COLS_TRIP>(cnt, buf0, write_offset, _dst_mat);
            write_offset += cnt;
            read_roi_row<SRC_T, ROWS, COLS, DEPTH, NPC, XFCVDEPTH_IN, COLS_TRIP>(
                _src_mat, offset, col_loop_cnt, cnt, end_loc, start_loc, pix_pos, left_over_pix, final_pix, buf1);
        } else {
            write_roi_row<SRC_T, ROWS, COLS, DEPTH, NPC, XFCVDEPTH_OUT, COLS_TRIP>(cnt, buf1, write_offset, _dst_mat);
            write_offset += cnt;
            read_roi_row<SRC_T, ROWS, COLS, DEPTH, NPC, XFCVDEPTH_IN, COLS_TRIP>(
                _src_mat, offset, col_loop_cnt, cnt, end_loc, start_loc, pix_pos, left_over_pix, final_pix, buf0);
        }
...
template <int SRC_T,
          ...
          int WORDWIDTH_DST,
          int COLS_TRIP>
void xFcropkernel_stream(xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_IN>& _src_mat,
                         xf::cv::Mat<SRC_T, ROWS, COLS, NPC, XFCVDEPTH_OUT>& _dst_mat,
                         xf::cv::Rect_<unsigned int>& roi,
                         unsigned short height,
                         unsigned short width) {
...
// clang-format off
    ...
    if (ARCH_TYPE == XF_MEMORYMAPPED) {
        xFcropkernel_memorymapped<SRC_T, ROWS, COLS, XF_DEPTH(SRC_T, NPC), NPC, XFCVDEPTH_IN, XFCVDEPTH_OUT,
                                  XF_WORDWIDTH(SRC_T, NPC), XF_WORDWIDTH(SRC_T, NPC), (COLS >> XF_BITSHIFT(NPC))>(
            _src_mat, _dst_mat, roi, height, width);
    } else {
        xFcropkernel_stream<SRC_T, ROWS, COLS, XF_DEPTH(SRC_T, NPC), NPC, XFCVDEPTH_IN, XFCVDEPTH_OUT,
                            XF_WORDWIDTH(SRC_T, NPC), XF_WORDWIDTH(SRC_T, NPC), (COLS >> XF_BITSHIFT(NPC))>(
            _src_mat, _dst_mat, roi, height, width);
    }
