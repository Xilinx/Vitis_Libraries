...
template <int DEPTH_SRC, int DEPTH_DST, int filter_height, int filter_width,
          int NPC, int PLANES, int buf_width, typename buf_type>
XF_PTNAME(DEPTH_DST)
xFApplyCustomFilter(buf_type _lbuf[][buf_width],
                    short int _kernel[][filter_width], int ind,
                    unsigned char shift) {
// clang-format off
...
#pragma HLS UNROLL
        // clang-format on
        conv_val[i][j][k] =
            ((_lbuf[i][arr_ind].range((k * 8) + 7, k * 8)) * _kernel[i][j]);
      }
      arr_ind++;
    }
  }
...
#pragma HLS UNROLL
      // clang-format on
      res.range((p * 16) + 15, p * 16) = tmp_res[p].range(15, 0);
    }
    //}
  }
  return res;
 ...
 *gain filter
 ****************************************************************************************/
template <int filter_height, int filter_width, int buf_width, int NPC,
          int DEPTH_SRC, int DEPTH_DST, int PLANES>
...
// clang-format off
    ...
    // clang-format on
    _mask_value[j] =
        xFApplyCustomFilter<DEPTH_SRC, DEPTH_DST, filter_height, filter_width,
                            NPC, PLANES>(_lbuf, _kernel, j, shift);
  }
}

template <int SRC_T, int DST_T, int ROWS, int COLS, int DEPTH_SRC,
          int DEPTH_DST, int NPC, int WORDWIDTH_SRC, int WORDWIDTH_DST, int TC,
          int FW, int filter_height, int filter_width, int F_COUNT, int PLANES>
void Convolution_Process(
    xf::cv::Mat<SRC_T, ROWS, COLS, NPC> &_src,
    ...
    short int _filter[][filter_width], uint16_t image_width, uchar row_ind,
    unsigned char shift, XF_SNAME(WORDWIDTH_DST) & P0,
    unsigned char index[filter_height], ap_uint<13> col_factor,
    uchar filter_width_factor, unsigned short image_height, ap_uint<13> row,
...
// clang-format off
    ...
    xFComputeCustomFilter<filter_height, filter_width,
                          (XF_NPIXPERCYCLE(NPC) + filter_width - 1), NPC,
                          DEPTH_SRC, DEPTH_DST, PLANES>(lbuf, _filter,
                                                        mask_value, shift);
...
// clang-format off
                ...
                #pragma HLS UNROLL
        // clang-format on
        P0.range((max_range_step + (step - 1)), (max_range_step)) =
            mask_value[l];
        max_range_step += step;
      }
 ...
 ************************************************************************************/
template <int SRC_T, int DST_T, int ROWS, int COLS, int DEPTH_SRC,
          int DEPTH_DST, int NPC, int WORDWIDTH_SRC, int WORDWIDTH_DST,
          int COLS_COUNT, int filter_height, int filter_width, int F_COUNT,
          int FW, int COL_FACTOR_COUNT, int PLANES>
void xFCustomConvolutionKernel(xf::cv::Mat<SRC_T, ROWS, COLS, NPC> &_src,
                               short int _filter[][filter_width],
                               xf::cv::Mat<DST_T, ROWS, COLS, NPC> &_dst,
                               unsigned char shift, unsigned short img_width,
...
mainRowLoop:
  for (ap_uint<13> row = (filter_height >> 1);
       row < (img_height + ((filter_height >> 1))); row++) {
// clang-format off
...
// clang-format off
    ...
    Convolution_Process<SRC_T, DST_T, ROWS, COLS, DEPTH_SRC, DEPTH_DST, NPC,
                        WORDWIDTH_SRC, WORDWIDTH_DST, COLS_COUNT, FW,
                        filter_height, filter_width, F_COUNT, PLANES>(
        _src, _dst, buf, lbuf, tmp_buf, mask_value, _filter, img_width, row_ind,
        shift, P0, index, col_factor, filter_width_factor, img_height, row,
        rd_ind, wr_ind);
...
// clang-format off
        ...
        // clang-format on
        col_border_mask[i] =
            xFApplyCustomFilter<DEPTH_SRC, DEPTH_DST, filter_height,
                                filter_width, NPC, PLANES>(lbuf, _filter, i,
                                                           shift);
      }
    }
...
template <int SRC_T, int DST_T, int ROWS, int COLS, int DEPTH_SRC,
          int DEPTH_DST, int NPC, int WORDWIDTH_SRC, int WORDWIDTH_DST, int TC,
          int K_HEIGHT, int K_WIDTH, int PLANES>
...
// Fetch the column from the line buffer to shift into the window.
      ...
      if ((ImagLocy < (-anchory)) ||
          (ImagLocy >= K_HEIGHT - 1 && ImagLocy < rows - 1)) {
        ...
        if (ImagLocx >= 0 && ImagLocx < cols) {
          ...
          for (ap_int<8> buf_row = K_HEIGHT - 1; buf_row >= 1; buf_row--) {
            ...
            if (ImagLocx >= cols - K_WIDTH) {
              right_border_buf[K_HEIGHT - buf_row]
                              [ImagLocx - (cols - K_WIDTH)] = temp;
            }
          }
      ...
      if (i >= (K_HEIGHT + K_HEIGHT - 1) && j >= (K_WIDTH - 1)) {
        ...
        xFApplyFilter2D<DEPTH_SRC, DEPTH_DST, K_HEIGHT, K_WIDTH, PLANES>(
            src_kernel_win, _filter_kernel, temp, shift);
...
template <int BORDER_TYPE, int FILTER_WIDTH, int FILTER_HEIGHT, int SRC_T,
          int DST_T, int ROWS, int COLS, int NPC>
...
// clang-format off
  ...
  if (NPC == XF_NPPC8) {
    xFCustomConvolutionKernel<
        SRC_T, DST_T, ROWS, COLS, XF_DEPTH(SRC_T, NPC), XF_DEPTH(DST_T, NPC),
        NPC, XF_WORDWIDTH(SRC_T, NPC), XF_WORDWIDTH(DST_T, NPC),
        ...
        (((FILTER_WIDTH >> 1) - 1) >> XF_BITSHIFT(NPC)),
        XF_CHANNELS(SRC_T, NPC)>(_src_mat, lfilter, _dst_mat, _shift, img_width,
                                 img_height);

  }

  else if (NPC == XF_NPPC1) {
    xFFilter2Dkernel<SRC_T, DST_T, ROWS, COLS, XF_DEPTH(SRC_T, NPC),
                     XF_DEPTH(DST_T, NPC), NPC, XF_WORDWIDTH(SRC_T, NPC),
                     XF_WORDWIDTH(DST_T, NPC), COLS, FILTER_HEIGHT,
                     FILTER_WIDTH, XF_CHANNELS(SRC_T, NPC)>(
