...
#ifndef _XF_TVL1_WRAPPER_HPP_
#define _XF_TVL1_WRAPPER_HPP_
...
class DualTVL1OpticalFlow {
   ...
   public:
    ...
    void init(int _height, int _width) {
        ...
        // Load binary file:
        // -------------------------
        std::string binaryFile = xcl::find_binary_file(device_name, "krnl_tvl1");
        ...
        // -------------------------
        OCL_CHECK(err, cl::CommandQueue _queue(
        ...
        cl::size_type _ParamSize = (cl::size_type)(
            ((4 * NScales) + 5) * sizeof(uint32_t)); // No.of Levels + ((row + col + offset) x (No.of levels))
        ...
        // .....................................................................
        uint32_t* AlgoParamData =
            (uint32_t*)queue.enqueueMapBuffer(AlgoParamBuff, CL_TRUE, CL_MAP_WRITE, 0, _AlgoParamSize);
    ...
    static ::cv::Ptr<DualTVL1OpticalFlow> create(double tau = 0.25,
                                                 double lambda = 0.15,
                                                 double theta = 0.3,
                                                 int nscales = 5,
                                                 int warps = 5,
...
void DualTVL1OpticalFlow::calc(::cv::Mat Frame, ::cv::Mat& Flow) {
    ...
    if (FrameCount > 2) OCL_CHECK(err, err = queue.enqueueTask(TVL1AlgoKernel, NULL, &AlgoKernelEvent));
