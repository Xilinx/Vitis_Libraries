...
class Mat {
   public:
    ...
    int rows, cols, size;        // actual image size
                                 //	int cols_align_npc;						// cols multiple of NPC
    ...
    DATATYPE* data __attribute((xcl_array_geometry(
        (ROWS) * ((COLS + NPC - 1) >> (XF_BITSHIFT(NPC)))))); // data[ ROWS * ( COLS >> ( XF_BITSHIFT ( NPC ) ) ) ];
#else
    DATATYPE* data;
#endif
...
// clang-format off
    ...
    size = _rows * ((_cols + NPPC - 1) >> XF_BITSHIFT(NPPC));
    // size = ( (ROWS * COLS) + (NPPC-1)) >> XF_BITSHIFT(NPPC);
    // cols_align_npc = ((_cols + (NPPC-1)) >> XF_BITSHIFT(NPPC) ) <<  XF_BITSHIFT(NPPC);
...
inline XF_CTUNAME(T, NPC) Mat<T, ROWS, COLS, NPC>::operator() (unsigned int r, unsigned int c, unsigned int ch) {
    ...
    pix_val = data[(r*npc_cols) + npc_c].range(pix_idx*bit_width + (ch+1)*pix_depth - 1, pix_idx*bit_width +
ch*pix_depth); #else unsigned int pix_depth = XF_DTPIXELDEPTH(T, NPC) / XF_CHANNELS(T, NPC);
...
// clang-format off
        ...
        for (int j = 0; j < XF_CHANNELS(T, NPPC); ++j) {
            pack_val.range((i * pixdepth + j * bitdepth) + bitdepth - 1, (i * pixdepth + j * bitdepth)) =
                tmp_val.chnl[i][j];
        }
    }
...
//
//                                    if (T == XF_32FC1) {
//                                            in_val = float2ap_uint< ap_uint<32> >(input[XF_CHANNELS(T,
// NPPC)*((r*packcols + c)*nppc + p) + ch]); } else { in_val = input[XF_CHANNELS(T, NPPC)*((r*packcols + c)*nppc + p) +
// ch];
//                                    }
//#ifdef __SDSVHLS__
...
//                                input[XF_CHANNELS(T, NPPC) * ((r * cols + c) * nppc + p) + ch]);
//                        } else {
//                            in_val = input[XF_CHANNELS(T, NPPC) * ((r * cols + c) * nppc + p) + ch];
//                        }
//                    } else {
//                        in_val = 0;
//                    }
//
//#ifdef __SDSVHLS__
//                    data[r * ncpr + c].range((p * pixdepth) + (ch + 1) * bitdepth - 1, (p * pixdepth) + ch * bitdepth)
//                    =
//                        in_val;
//#else
...
template <int T, int ROWS, int COLS, int NPPC>
template <int DST_T>
inline void Mat<T, ROWS, COLS, NPPC>::convertTo(Mat<DST_T, ROWS, COLS, NPPC>& dst,
                                                int otype,
                                                double alpha,
                                                double beta) {
...
/*
 * The purpose of this class is to provide an interator over ap_uint<PTR_WIDTH> object array such
 * that at each iter step one can get data bits corresponding to data to be processed per cycle.
 *
*/
template <int PTR_WIDTH, int T, int ROWS, int COLS, int NPC>
class MMIter : public Mat<T, ROWS, COLS, NPC> {
   public:
    using Mat<T, ROWS, COLS, NPC>::data;
    using Mat<T, ROWS, COLS, NPC>::rows;
    using Mat<T, ROWS, COLS, NPC>::cols;
    ...
    MMIter() : Mat<T, ROWS, COLS, NPC>() {}

    MMIter(int _rows, int _cols) : Mat<T, ROWS, COLS, NPC>(_rows, _cols) {}
};

template <int PTR_WIDTH, int T, int ROWS, int COLS, int NPC>
class MMIterIn : public MMIter<PTR_WIDTH, T, ROWS, COLS, NPC> {
   public:
    using Mat<T, ROWS, COLS, NPC>::data;
    using Mat<T, ROWS, COLS, NPC>::rows;
    using Mat<T, ROWS, COLS, NPC>::cols;

    using MMIter<PTR_WIDTH, T, ROWS, COLS, NPC>::XF_BITS_PER_CLOCK;
    using MMIter<PTR_WIDTH, T, ROWS, COLS, NPC>::ADDRBOUND;
    using MMIter<PTR_WIDTH, T, ROWS, COLS, NPC>::COLS_BOUND_PER_NPC;
    using MMIter<PTR_WIDTH, T, ROWS, COLS, NPC>::LAST_BLK_PXL_WIDTH;
    using MMIter<PTR_WIDTH, T, ROWS, COLS, NPC>::LOOPBOUND;
...
// clang-format off
    ...
    static void AxiStream2MatStream(hls::stream<ap_uint<PTR_WIDTH> >& din,
                                    hls::stream<ap_uint<XF_BITS_PER_CLOCK>>& dout,
                                    int rows,
...
// clang-format off
    ...
    static void MatStream2Mat(hls::stream<ap_uint<XF_BITS_PER_CLOCK>>& din,
...
// clang-format off
    ...
    static void MatStream2AxiStream(hls::stream<ap_uint<XF_BITS_PER_CLOCK>>& din,
                                    hls::stream<ap_uint<PTR_WIDTH> >& dout,
                                    int rows,
                                    int cols_bound_per_npc,
                                    int last_blk_width) {
...
// clang-format off
    ...
    static void Mat2AxiStream(hls::stream<ap_uint<XF_BITS_PER_CLOCK>>& din,
                              hls::stream<ap_uint<PTR_WIDTH> >& dout,
                              int rows = ROWS,
                              int cols = COLS) {
// clang-format off
...
// clang-format off
    ...
    static void Mat2Axi(hls::stream<ap_uint<XF_BITS_PER_CLOCK>>& din,
                        ap_uint<PTR_WIDTH>* dout,
                        int rows = ROWS,
                        int cols = COLS) {
// clang-format off
...
// clang-format off
   ...
   public:
    ...
    static void xfMat2Array(xf::cv::Mat<T, ROWS, COLS, NPC>& srcMat, ap_uint<PTR_WIDTH>* dstPtr) {
        Mat2Axi(srcMat.data, dstPtr, srcMat.rows, srcMat.cols);
    }

    static void xfMat2axiStrm(xf::cv::Mat<T, ROWS, COLS, NPC>& srcMat,
