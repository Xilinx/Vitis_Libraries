...
{
	//INSERT_VAL=128 corresponds to 0.5f
...
#else
	unsigned short in_height = 2;
	unsigned short in_width = 4;
	unsigned short out_height = 2;
	unsigned short out_width = 4;
#endif
...
#pragma HLS unroll
			read_word_init.range(bit1 + (bit2 + 7), bit1 + bit2) = INSERT_VAL;
		}
	}
...
//	fprintf(stderr,"\n**** bound row=%d col=%d", row_loop_bound, col_loop_bound);

	LOOP_ROW_COL:for (int row_col_index = 0; row_col_index < row_loop_bound*col_loop_bound; row_col_index++) {
#pragma HLS LOOP_TRIPCOUNT min=1 max=SRC_COLS
#pragma HLS pipeline II=1

		//fprintf(stderr,"\n**** rowcol=%d   row=%d col=%d",row_col_index, row_index, col_index);

			//			fprintf(stderr,"\n*** Loop row index: %d  Loop col index: %d",row_index, col_index);
			//			fprintf(stderr,"\n\t\t\tpad_col_count_left:%d input_col_index:%d",pad_col_count_left, input_col_index);
			//input_col_index?????
			bool input_read_en = pad_col_count_left<NPC && input_col_index<imgInput_ncpr && pad_row_count_top<=0 && (row_index-dy)<in_height;
			//if(pad_col_count_left<NPC && input_col_index<imgInput_ncpr && pad_row_count_top<=0 && (row_index-dy)<in_height)
			if(input_read_en==1)
			{
				//				fprintf(stderr,"\n\t\t\t DDr read index: %d",read_index);
...
#pragma HLS unroll
					...
					if((pixel>=non_padded_in_col_due_2_xfmat) && (input_col_index==(imgInput_ncpr-1)))
						read_word.range(bit1 + (bit2 + 7), bit1 + bit2) = INSERT_VAL;
					else
						read_word.range(bit1 + (bit2 + 7), bit1 + bit2) = read_word_tmp.range(bit1 + (bit2 + 7), bit1 + bit2);
				}
			}
			...
			//			fprintf(stderr,"\n\t\t\tprev_valid_elements:%d current_valid_elements:%d",prev_valid_elements, current_valid_elements);
...
#pragma HLS unroll
			...
			if(current_valid_elements < NPC && input_col_index_prev == (imgInput_ncpr-1))
				out_write_en = pad_col_count_left<NPC && input_col_index<(imgInput_ncpr+1) && pad_row_count_top<=0 && (row_index-dy)<in_height;
			else
				...
				for (int channel = 0; channel < PLANES; channel++)
				{
					...
					if(out_write_en==1)
					{
						if((pixel + current_valid_elements)<NPC)
							DDR_write_data[pixel][channel] = read_word_extract_prev[pixel + current_valid_elements][channel];
						else
							DDR_write_data[pixel][channel] = read_word_extract[pixel - prev_valid_elements][channel];
					}
					else
					{
						DDR_write_data[pixel][channel]=INSERT_VAL;
					}
...
/**
 * @tparam INTERPOLATION_TYPE interpolation type
 ...
 * @tparam MAX_DOWN_SCALE maximum downscale parameter
 * @tparam INSERT_VAL 
 * @param _src input image
 * @param _dst output image
 * @param rows_out_resize rows value after resize
 * @param cols_out_resize cols value after resize
 */
...
template <int INTERPOLATION_TYPE,
          ...
          int MAX_DOWN_SCALE,
		  int INSERT_VAL>
void letterbox(xf::cv::Mat<TYPE, SRC_ROWS, SRC_COLS, NPC>& _src, xf::cv::Mat<TYPE, DST_ROWS, DST_COLS, NPC>& _dst, int rows_out_resize, int cols_out_resize) 
{
		#pragma inline off
	...
	#pragma HLS DATAFLOW
		xf::cv::resize<INTERPOLATION_TYPE,TYPE,SRC_ROWS,SRC_COLS,DST_ROWS,DST_COLS,NPC,MAX_DOWN_SCALE> (_src, out_mat_resize);
