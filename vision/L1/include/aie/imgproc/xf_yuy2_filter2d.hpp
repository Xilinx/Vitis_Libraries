...
#include <algorithm>
#include <common/xf_aie_hw_utils.hpp>;
#include "xf_filter2d_16b_aie.hpp";

#ifndef _AIE_YUY2FILTER2D_H_
#define _AIE_YUY2FILTER2D_H_
...
__attribute__((noinline)) void yuy2_filter2D(adf::input_buffer<int16>& img_in,
                                             const int16 (&coeff)[16],
    ...
    const int16 image_width = xfGetTileWidth(img_in_ptr) * 2; // inclusive of channels
    const int16 image_height = xfGetTileHeight(img_in_ptr);

    xfCopyMetaData(img_in_ptr, img_out_ptr);

    int16* in_img_ptr = (int16*)xfGetImgDataPtr(img_in_ptr);
    int16* out_img_ptr = (int16*)xfGetImgDataPtr(img_out_ptr);

    alignas(::aie::vector_decl_align) int16 y_buff[image_width * image_height / 2];
    alignas(::aie::vector_decl_align) int16 uv_buff[image_width * image_height / 2];
    alignas(::aie::vector_decl_align) int16 y_filtered_buff[image_width * image_height / 2];

    int16* restrict y_filtered_ptr = (int16*)y_filtered_buff;
    int16* restrict y_ptr = (int16*)y_buff;
    int16* restrict uv_ptr = (int16*)uv_buff;
    ...
    // unzip
    {
        for (int i = 0; i < image_height; i++) chess_unroll_loop(4) {
                ...
                for (int j = 0; j < image_width; j += 32) chess_prepare_for_pipelining {
                        ...
                        ::aie::vector<int16, 16> y_temp, uv_temp;
    ...
    // filter
    filter2D_k3_border_impl((v16int16*)y_ptr, coeff, (v16int16*)y_filtered_ptr, image_width / 2, image_width / 2,
                            image_height);
    ...
    // zip
    {
        ...
        for (int i = 0; i < image_height; i++) chess_prepare_for_pipelining {
                for (int j = 0; j < image_width; j += 64) chess_prepare_for_pipelining {
                        ...
                        ::aie::vector<int16, 32> y_temp, uv_temp;
