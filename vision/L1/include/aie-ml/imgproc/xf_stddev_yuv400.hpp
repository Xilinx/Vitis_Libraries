...
#ifndef __XF_STDDEV_400
#define __XF_STDDEV_400
...
void Stddev::runImpl(adf::input_buffer<uint8_t>& input1,
    ...
    float* restrict Stddev_ptr = (float*)xfGetImgDataPtr(img_out_ptr_y);

    float mean_val_y = m_y;
    ::aie::vector<float, 2 * N> Mean_vec = ::aie::broadcast<float, 2 * N>(mean_val_y);
    ::aie::accum<accfloat, 2 * N> accf, accf_temp;
    ::aie::vector<uint8_t, 2 * N> in_vec = ::aie::load_v<2 * N>(Y_ptr);
    Y_ptr += 2 * N;
    ...
    SUM_ALL_TILES = SUM_ALL_TILES + ::aie::reduce_add(accf_00000.template to_vector<float>());
#else
    SUM_ALL_TILES = SUM_ALL_TILES + ::aie::reduce_add(accf_0000.template to_vector<float>());
#endif
    *Stddev_ptr = SUM_ALL_TILES;
}
