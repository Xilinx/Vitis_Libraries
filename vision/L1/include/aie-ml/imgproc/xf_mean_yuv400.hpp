...
#ifndef __XF_MEAN_400_
#define __XF_MEAN_400_
...
void Mean::runImpl(adf::input_buffer<uint8_t>& input1,
                   adf::output_buffer<uint8_t>& output1,
                   uint32_t reset,
                   int img_height_in,
                   int nChannels) {
    ...
    accf.template from_vector(in_vec_f);
    for (int i = 0; i < (tile_height_in * tile_width_in * nChannels) / (N * 2) - 1; i++)
        chess_prepare_for_pipelining chess_loop_range(14, ) {
            ...
            in_vec_f = ::aie::to_float<float>(in_vec);
            Y_ptr += 2 * N;
            accf = ::aie::add(accf, in_vec_f);
        }
