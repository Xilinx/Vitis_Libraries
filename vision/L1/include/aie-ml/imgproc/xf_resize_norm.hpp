...
class ResizeNorm {
   public:
    ...
    void xf_normalize(uint8_t* input, int8_t* output, int tile_width_out, const int16_t* coeff, uint8_t FBits[3]);
...
__attribute__((noinline)) void ResizeNorm::xf_normalize(
    uint8_t* input, int8_t* output, int tile_width_out, const int16_t* coeff, uint8_t FBits[3]) {
...
void ResizeNorm::runImpl(uint8_t* input,
    ...
    runImpl((uint8_t*)input, (uint8_t*)output, row, scale_x, scale_y, img_height_in, tile_width_in, line_stride_in,
            tile_width_out);
    set_rnd(rnd_conv_even);
    xf_normalize((uint8_t*)output, (int8_t*)output, tile_width_out, coeff, FBits);
    set_rnd(rnd_floor);
}
