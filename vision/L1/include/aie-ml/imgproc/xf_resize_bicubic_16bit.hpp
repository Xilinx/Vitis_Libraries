...
#ifndef __XF_RESIZE_BICUBIC_16BIT__
#define __XF_RESIZE_BICUBIC_16BIT__
...
// alignas(::aie::vector_decl_align) uint64_t wts[256];
...
class Resizebicubic {
   ...
   public:
    ...
    __attribute__((noinline)) void compute_wtsy_f(int row,
        ...
        ::aie::accum<accfloat, 16> mul_Acc = ::aie::mul(dist, ::aie::broadcast<float, 16>(4096.0f));
        ...
        uint32_t wtsy = weighty[idx[0] * 2];

        Wy1 = (int16_t)(wtsy >> 16);
        Wy2 = (int16_t)(wtsy);

        uint32_t wtsy34 = weighty[idx[0] * 2 + 1];

        Wy3 = (int16_t)(wtsy34 >> 16);
        Wy4 = (int16_t)(wtsy34);
        ...
        for (int j = 0; j < ((tile_width_out * channels) / N); j++)
            chess_prepare_for_pipelining chess_loop_range(32, ) {
                ...
                if
                    ...
                    set_sat();
                    ::aie::store_v(img_out_ptr, acc22.template to_vector<T>(15));
                }
                img_out_ptr += N;
            }
    }
