...
void Transpose1D::runImpl(adf::input_buffer<uint8_t>& input_metadata,
    ...
    int height = xfGetTileHeight(in_meta_ptr);
    int width = xfGetTileWidth(in_meta_ptr);

    xfCopyMetaData(in_meta_ptr, out_ptr);

    uint16_t mt_height = xfGetTileWidth(out_ptr);
    uint16_t mt_width = xfGetTileHeight(out_ptr);
    ...
    xfSetTileOutPosH(out_ptr, xfGetTilePosV(out_ptr));
    xfSetTileOutPosV(out_ptr, xfGetTilePosH(out_ptr));
