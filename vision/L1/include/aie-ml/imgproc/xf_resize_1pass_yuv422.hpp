...
template <int VEC_FACTOR>
__attribute__((noinline)) void Resize::xf_extract_yuv(
...
template <int VEC_FACTOR>
__attribute__((noinline)) void Resize::xf_merge_yuv(
    ...
    int rem_vals = (tile_width_out * tile_height_out) % (2 * VEC_FACTOR);
    if (rem_vals != 0) {
        auto y = ::aie::load_v<VEC_FACTOR>(ptr_y);
        ptr_y += VEC_FACTOR;
        auto y_even = ::aie::filter_even(y, 1);
        ...
        std::tie(y_odd, v) = ::aie::interleave_zip(y_odd, v, 1);
        auto[x1, y1] = ::aie::interleave_zip(::aie::concat(y_even, u), ::aie::concat(y_odd, v), 2);
        auto out_Vec = ::aie::concat(x1, y1);
        for (int i = 0; i < rem_vals; i++) {
            ptr_out[i] = out_Vec[i];
        }
    }
}
...
alignas(::aie::vector_decl_align) uint8_t Y_buffer[max_tile_width_in * max_tile_height_in] = {0};
...
__attribute__((noinline)) void Resize::runImpl(uint8_t* input,
    ...
    uint8_t* restrict Y_ptr = (uint8_t*)Y_buffer;
    ...
    printf("  \n YUYV in  \n ");
    uint8* ptr_in1 = input;
    for (int i = 0; i < 10; i++) { // tile_width_in * tile_height_in * 2
        printf(" %d ", *ptr_in1++);
    }
#endif

    xf_extract_yuv<2 * N>(img_in_ptr_1, Y_ptr, U_ptr, V_ptr, 2 * tile_width_in, tile_height_in);

#ifdef DEBUG12
    printf("tile_width_in = %d, tile_height_in = %d, tile_width_out = %d, tile_height_out = %d ", tile_width_in,
           tile_height_in, tile_width_out, tile_height_out);
    printf("  \n Y  \n ");
    uint8* Y_ptr1 = Y_ptr;
    for (int i = 0; i < tile_width_in * tile_height_in; i++) { //
        printf(" %d ", *Y_ptr1++);
    }
#endif
    ...
    xf_resize1DH1CH(Y_ptr, Y_ptr, row, scale_x, tile_width_in, tile_width_out);

    xf_resize1DV(U_ptr, U_ptr + tile_width_in / 2, U_ptr, row, scale_y, img_height_in, tile_width_in / 2, ch);
    xf_resize1DH1CH(U_ptr, U_ptr, row, scale_x, tile_width_in / 2, tile_width_out / 2);

    xf_resize1DV(V_ptr, V_ptr + tile_width_in / 2, V_ptr, row, scale_y, img_height_in, tile_width_in / 2, ch);
    xf_resize1DH1CH(V_ptr, V_ptr, row, scale_x, tile_width_in / 2, tile_width_out / 2);
    set_rnd(rnd_floor);

    xf_merge_yuv<2 * N>(output, Y_ptr, U_ptr, V_ptr, 2 * tile_width_out, tile_height_out);

#ifdef DEBUG11

    printf("  \n Y - V resize  \n ");
    uint8* Y_resize_ptr1 = Y_ptr;
    for (int i = 0; i < tile_width_out; i++) { // tile_width_in * tile_height_out
        printf(" %d ", *Y_resize_ptr1++);
    }
