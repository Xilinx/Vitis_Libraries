...
cv::Mat floyd_steinberg_dithering_updatedEquation(cv::Mat image, int scale) {
    ...
    for (int rowID = 0; rowID < image.rows; rowID++) {
        ...
        for (int colID = 0; colID < image.cols; colID++) {
            if (image.channels() == 1) {
                old_pix =
                    (float)new_image.at<float>(rowID, colID) + offset_buffer[0][colID] + floor(q_err_2nd[0] * 7 / 16);
                ...
                // new_image.at<unsigned char>(rowID, colID)   = (unsigned char)new_pix;
                ...
                if ((colID - 1 >= 0) && (rowID + 1 < image.rows))
                    offset = floor((q_err_1st[0] * 1 + q_err_2nd[0] * 5 + q_err_3rd[0] * 3) / 16);

                if ((colID - 1 >= 0)) {
                    offset_buffer[0][colID - 1] = offset;
                }

                if (colID == image.cols - 1) {
                    offset = (q_err_2nd[0] * 1 / 16) + (q_err_3rd[0] * 5 / 16);
                    offset_buffer[0][colID] = offset;
                }

                q_err_1st[0] = q_err_2nd[0];
                q_err_2nd[0] = q_err_3rd[0];
                q_err_3rd[0] = 0;
            } else if (image.channels() == 3) {
                for (int c = 0; c < 3; c++) {
                    old_pix = (float)new_image.at<cv::Vec3f>(rowID, colID)[c] + offset_buffer[c][colID] +
                              floor(q_err_2nd[c] * 7 / 16);
                    new_pix = round(old_pix * scale / max) * round(max / scale);
                    ...
                    if (new_pix >= max) new_pix = (scale - 1) * round(max / scale);
                    if (new_pix < 0) new_pix = 0;
                    // new_image.at<cv::Vec3b>(rowID, colID)[c]   = (unsigned char)new_pix;
                    ...
                    if ((colID - 1 >= 0)) {
                        offset_buffer[c][colID - 1] = offset;
                    }

                    if (colID == image.cols - 1) {
                        offset = floor((q_err_2nd[c] * 1 / 16) + (q_err_3rd[c] * 5 / 16));
                        offset_buffer[c][colID] = offset;
                    }

                    q_err_1st[c] = q_err_2nd[c];
                    q_err_2nd[c] = q_err_3rd[c];
                    q_err_3rd[c] = 0;
                }
            }
        }
