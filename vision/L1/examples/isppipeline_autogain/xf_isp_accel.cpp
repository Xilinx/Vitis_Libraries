...
uint16_t gain0_agc[3] = {MAX_REPRESENTED_VALUE, MAX_REPRESENTED_VALUE, MAX_REPRESENTED_VALUE};
uint16_t gain1_agc[3] = {MAX_REPRESENTED_VALUE, MAX_REPRESENTED_VALUE, MAX_REPRESENTED_VALUE};

// uint16_t rgain0_agc = 0;
// uint16_t ggain0_agc = 0;
// uint16_t bgain0_agc = 0;
...
#pragma HLS DATAFLOW
    ...
    if (WB_TYPE_V) {
        ...
        // FILE* fp = fopen("hist_host.txt", "a");

        // for (int j = 0; j < HIST_SIZE; j++) {
        //     for (int i = 0; i < 3; i++) {
        //         fprintf(fp, "%d  ", hist1[i][j]);
        //     }
        //     fprintf(fp ,"\n");
        // }
        // fclose(fp);
...
#endif
    // autogaincontrol functioncall

    xf::cv::autogain<OUT_TYPE, XF_NPPCX, HIST_SIZE>(hist1, gain0);
...
// gaincontrol functioncall
    ...
    function_gaincontrol<IN_TYPE, XF_HEIGHT, XF_WIDTH, XF_NPPCX, XF_CV_DEPTH_LSC_OUT, XF_CV_DEPTH_GAIN_OUT>(
        blackLevelCorrection_out_mat, gaincontrol_out_mat, gain1[0], gain1[2], gain1[1], bformat, height, width,
        funcs_bypass_config);
#else
    function_gaincontrol<IN_TYPE, XF_HEIGHT, XF_WIDTH, XF_NPPCX, XF_CV_DEPTH_LSC_OUT, XF_CV_DEPTH_GAIN_OUT>(
        in_img, gaincontrol_out_mat, gain1[0], gain1[2], gain1[1], bformat, height, width, funcs_bypass_config);
#endif
#endif
...
//  xf::cv::autogain<OUT_TYPE, XF_NPPCX, HIST_SIZE>(hist0, gain1);
...
#endif
#if XF_AWB_EN
                    hist0_awb, awb_config, inputMin, inputMax, outputMin, outputMax, hist1_awb, gain1_agc,
#endif
#if XF_GAMMA_EN
                    gamma_lut,
#endif
...
#endif
#if XF_AWB_EN
                    hist1_awb, awb_config, inputMin, inputMax, outputMin, outputMax, hist0_awb, gain0_agc,
#endif
#if XF_GAMMA_EN
                    gamma_lut,
#endif
