...
#endif
            unsigned int currentBin_r = ((in_r - minValue + 1e-4f) / interval);
            unsigned int currentBin_g = ((in_g - minValue + 1e-4f) / interval);
            unsigned int currentBin_b = ((in_b - minValue + 1e-4f) / interval);
...
int main(int argc, char** argv) {
    ...
    cv::Mat in_img, out_img, bayer_img, in_rgb;
...
// gain control
    ...
    unsigned int rgain = 0;
    unsigned int ggain = 0;
    ...
    ap_uint<XF_DTPIXELDEPTH(OUT_TYPE, XF_NPPCX)> gain[XF_CHANNELS(OUT_TYPE, XF_NPPCX)] = {0};

    // OpenCV Reference
    unsigned int histogram[XF_CHANNELS(OUT_TYPE, XF_NPPCX)][HIST_SIZE] = {0};
    ...
    // ggain = 32;
    // bgain = 96;
    std::cout << "rgain_tb is " << rgain_tb << std::endl;
    std::cout << "ggain_tb is " << ggain_tb << std::endl;
    std::cout << "bgain_tb is " << bgain_tb << std::endl;
    ...
    float inputMax1 = (1 << (XF_DTPIXELDEPTH(IN_TYPE, XF_NPPCX))) - 1; // 65535.0f;
    float mul_fact = (float)(inputMax1 / (inputMax1 - BLACK_LEVEL));
    ...
    float gamma_val_r = 0.6f, gamma_val_g = 1.2f, gamma_val_b = 1.5f;
