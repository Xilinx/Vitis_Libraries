...
#include "xf_lut_accel_config.h"

static constexpr int __XF_DEPTH = (HEIGHT * WIDTH * (XF_PIXELWIDTH(IN_TYPE, NPPCX)) / 8) / (INPUT_PTR_WIDTH / 8);
static constexpr int __XF_DEPTH_PTR = (XF_CHANNELS(IN_TYPE, NPPCX)) * 256;

void lut_accel(ap_uint<INPUT_PTR_WIDTH>* img_in,
...
// clang-format off
    ...
    // Run xfOpenCV kernel:
    xf::cv::LUT<IN_TYPE, HEIGHT, WIDTH, NPPCX, XF_CV_DEPTH_IN, XF_CV_DEPTH_OUT>(imgInput, imgOutput, lut_buf);
