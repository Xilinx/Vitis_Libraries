...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<uint8_t> demData;
        demData.assign(op_height * op_width * 4, 0);
        cv::Mat dstDem(op_height, op_width, CV_8UC4, (void*)demData.data());

        xF::xfcvDataMovers<xF::TILER, uint8_t, TILE_HEIGHT, TILE_WIDTH, VECTORIZATION_FACTOR, CORES, 0, true> tiler(2,
                                                                                                                    2);

        xF::xfcvDataMovers<xF::STITCHER, uint8_t, TILE_HEIGHT, (TILE_WIDTH << 2), VECTORIZATION_FACTOR, CORES, 0, true>
            stitcherDem;
        xF::xfcvDataMovers<xF::STITCHER, uint8_t, TILE_HEIGHT, (TILE_WIDTH << 2), VECTORIZATION_FACTOR, CORES, 0, true>
            stitcher;
        ...
        for (int i = 0; i < iterations; i++) {
            START_TIMER
            auto tiles_sz = tiler.host2aie_nb(srcData.data(), srcImageR.size(), {"TOP[0].in1[0]"});
            stitcherDem.aie2host_nb(demData.data(), cv::Size((srcImageR.cols << 2), srcImageR.rows), tiles_sz,
                                    {"TOP[0].out1[0]"});
            stitcher.aie2host_nb(dstData.data(), cv::Size((srcImageR.cols << 2), srcImageR.rows), tiles_sz,
                                 {"TOP[0].out2[0]"});

#if !__X86_DEVICE__
            std::cout << "Graph running for " << (tiles_sz[0] * tiles_sz[1]) << " iterations.\n";
            gHndl[0].run(tiles_sz[0] * tiles_sz[1]);
#endif
            tiler.wait({"TOP[0].in1[0]"});
            stitcherDem.start({"TOP[0].out1[0]"});
            stitcher.start({"TOP[0].out2[0]"});
            gHndl[0].wait();
            stitcherDem.wait({"TOP[0].out1[0]"});
            stitcher.wait({"TOP[0].out2[0]"});
            ...
            OCL_CHECK(err,
                      queue.enqueueWriteBuffer(buffer_inImage,           // buffer on the FPGA
                                               ...
                                               (void*)demData.data(),    // Pointer to the data to copy
                                               nullptr, &event));
            ...
            (void)cl_kernel_mgr::registerKernel("ISPPipeline_accel", "kernel",
                                                XCLIN((void*)dstData.data(), rgba_image_in_size_bytes),
