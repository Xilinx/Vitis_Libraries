/*
 * Copyright 2021 Xilinx, Inc.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#ifndef ADF_GRAPH_PRE_PROC_H
#define ADF_GRAPH_PRE_PROC_H

#include <adf.h>
#include <array>
#include <type_traits>
#include <common/xf_aie_const.hpp>

#include "kernels_resize_norm.h"
#include "kernels_yuv2rgba.h"

using namespace adf;

/*
 * ADF graph to compute weighted moving average of
 * the last 8 samples in a stream of numbers
 */

class preprocessGraph : public adf::graph {
   private:
    kernel k1;
    kernel k2;

   public:
    input_plio in1;
    input_plio in2;
    output_plio out1;

    preprocessGraph() {
        k1 = kernel::create(yuv2rgba_api);
        k2 = kernel::create(resize_norm_api);

        in1 = input_plio::create("DataIn1", adf::plio_128_bits, "data/input_y_1920x1.txt");
        in2 = input_plio::create("DataIn2", adf::plio_128_bits, "data/input_uv_1920x1.txt");
        out1 = output_plio::create("DataOut1", adf::plio_128_bits, "data/output.txt");

        connect<>(in1.out[0], k1.in[0]);
        connect<>(in2.out[0], k1.in[1]);
        connect<>(k1.out[0], k2.in[0]);
        connect<>(k2.out[0], out1.in[0]);

        adf::dimensions(k1.in[0]) = {ELEM_WITH_METADATA_Y};
        adf::dimensions(k1.in[1]) = {ELEM_WITH_METADATA_UV};
        adf::dimensions(k1.out[0]) = {ELEM_WITH_METADATA_OUT};
        adf::dimensions(k2.in[0]) = {ELEM_WITH_METADATA_OUT};
        adf::dimensions(k2.out[0]) = {ELEM_WITH_METADATA_RESIZED_OUT};

        // specify kernel sources
        source(k1) = "xf_yuv2rgba.cc";
        source(k2) = "xf_resize_normalize.cc";

        runtime<ratio>(k1) = 0.5;
        runtime<ratio>(k2) = 0.5;
    }
};

#endif
