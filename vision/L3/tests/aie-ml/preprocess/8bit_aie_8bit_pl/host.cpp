        ...
        for (int ii = 0; ii < dst.rows; ii++) {
            for (int jj = 0; jj < dst.cols; jj++) {
                for (int kk = 0; kk < dst.channels(); kk++) {
                    ref2 = dstRefImage.data[(ii * dstRefImage.cols * dstRefImage.channels()) +
                                            (jj * dstRefImage.channels()) + kk];
                    aie2 = dst.data[(ii * dst.cols * dst.channels()) + (jj * dst.channels()) + kk];
                    diff.at<cv::Vec4b>(ii, jj)[kk] = abs(ref2 - aie2);
                    /*if (diff.at<cv::Vec4b>(ii, jj)[kk] > 2) {
