...
/*  reading in the color image  */
    ...
    size_t vec_in_size_bytes = 256 * 1 * sizeof(unsigned char);
    ...
    OCL_CHECK(err, err = kernel.setArg(4, gain_lum));
    OCL_CHECK(err, err = kernel.setArg(5, buffer_inVec));
    OCL_CHECK(err, err = kernel.setArg(6, clip));
    OCL_CHECK(err, err = kernel.setArg(7, tilesY));
    OCL_CHECK(err, err = kernel.setArg(8, tilesX));
