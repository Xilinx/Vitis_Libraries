...
#include "xf_img_pyramid_accel_config.h"
...
#pragma HLS INLINE OFF
    ...
    xf::cv::Mat<IMG_TYPE_P, HEIGHT, WIDTH, NPC_PYRAMID, XF_CV_DEPTH_in_mat> in_mat(rows_i, cols_i);
    xf::cv::Mat<IMG_TYPE_P, NEWHEIGHT, NEWWIDTH, NPC_PYRAMID, XF_CV_DEPTH_out_mat> out_mat(rows_o, cols_o);
// clang-format off
#pragma HLS DATAFLOW
    // clang-format on
...
extern "C" {
void img_pyramid_accel(ap_uint<IMAGE_PTR_WIDTH_P>* img_inp, ap_uint<IMAGE_PTR_WIDTH_P>* img_out, uint32_t* params) {
// clang-format off
...
PARAMS_READ_LOOP:
    for (uint16_t i = 0; i <= pyr_levels; i++) {
// clang-format off
...
KERNEL_CALL_LOOP:
    for (uint8_t i = 0; i < pyr_levels; i++) {
// clang-format off
#pragma HLS LOOP_TRIPCOUNT min=TC_MAX_NUM_LEVELS max=TC_MAX_NUM_LEVELS
        // clang-format on

        xf::cv::ResizeStageN(img_inp + offsets[i], img_out + offsets[i + 1], img_rows[i], img_cols[i], img_rows[i + 1],
                             img_cols[i + 1]);
