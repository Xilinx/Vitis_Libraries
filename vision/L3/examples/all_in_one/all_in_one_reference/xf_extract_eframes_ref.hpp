...
void extractExposureFrames_ref(
    cv::Mat& interleaved_image, cv::Mat& lef_image, cv::Mat& sef_image, int N_ROWS_ref, int N_COLS_ref) {
#if T_8U
    int height, width;
...
#elif T_16U
    cv::imwrite("interleaved_image.png", interleaved_image);
    int height, width;
    height = lef_image.rows;
    width = lef_image.cols;
    int count = 0, count1 = 0;
...
void HDR_merge_ref(cv::Mat& _src1, cv::Mat& _src2, double wr[NO_EXPS][W_B_SIZE], cv::Mat& final_img) {
