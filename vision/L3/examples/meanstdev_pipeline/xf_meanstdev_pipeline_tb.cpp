...
#include "xf_meanstdev_pipeline_tb_config.h"
#include "xcl2.hpp"
...
int main(int argc, char** argv) {
    ...
    // mean and std reference
    int channels = refOutput0.channels();
    ...
    mean_c = xmean<xfcv_channels>(refOutput0);
    variance<xfcv_channels>(refOutput0, mean_c, var_c);
    ...
    // Load binary:
    std::string binaryFile = xcl::find_binary_file(device_name, "krnl_meanstdev_nv122rgb");
    cl::Program::Binaries bins = xcl::import_binary_file(binaryFile);
    devices.resize(1);
    OCL_CHECK(err, cl::Program program(context, devices, bins, NULL, &err));

    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "meanstdev_nv122rgb", &err));
    ...
    OCL_CHECK(err,
              cl::Buffer imageFromDevice(context, CL_MEM_WRITE_ONLY, (channels * sizeof(unsigned short)), NULL, &err));
    OCL_CHECK(err,
              cl::Buffer imageFromDevice1(context, CL_MEM_WRITE_ONLY, (channels * sizeof(unsigned short)), NULL, &err));
    ...
    OCL_CHECK(err, err = krnl.setArg(3, imageFromDevice1));
    ...
    OCL_CHECK(err, q.enqueueReadBuffer(imageFromDevice, // This buffers data will be read
                                       CL_TRUE,         // blocking call
                                       0,               // offset
                                       (channels * sizeof(unsigned short)),
                                       mean));           // Data will be stored here
    OCL_CHECK(err, q.enqueueReadBuffer(imageFromDevice1, // This buffers data will be read
                                       CL_TRUE,          // blocking call
                                       0,                // offset
                                       (channels * sizeof(unsigned short)),
                                       stddev)); // Data will be stored here
    ...
    // Load binary:
    std::string binaryFile = xcl::find_binary_file(device_name, "krnl_meanstdev_yuyv2rgb");
    cl::Program::Binaries bins = xcl::import_binary_file(binaryFile);
    devices.resize(1);
    OCL_CHECK(err, cl::Program program(context, devices, bins, NULL, &err));

    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "meanstdev_yuyv2rgb", &err));
    ...
    OCL_CHECK(err, err = krnl.setArg(2, imageFromDevice1));
    ...
    // Load binary:
    std::string binaryFile = xcl::find_binary_file(device_name, "krnl_meanstdev");
    cl::Program::Binaries bins = xcl::import_binary_file(binaryFile);
    devices.resize(1);
    OCL_CHECK(err, cl::Program program(context, devices, bins, NULL, &err));

    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "meanstdev_yuv400", &err));
    ...
    // cvtColor(refOutput0, refOutput0, cv::COLOR_BGR2RGB);
    mean_c = xmean<xfcv_channels>(refmat);
    variance<xfcv_channels>(refmat, mean_c, var_c);
    ...
    // Load binary:
    std::string binaryFile = xcl::find_binary_file(device_name, "krnl_meanstdev_pipeline");
    cl::Program::Binaries bins = xcl::import_binary_file(binaryFile);
    devices.resize(1);
    OCL_CHECK(err, cl::Program program(context, devices, bins, NULL, &err));

    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "meanstdev_yuv4442rgb", &err));
