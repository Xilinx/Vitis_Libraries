...
#include "xf_resize_pipeline_tb_config.h"
#include "xcl2.hpp"
...
int main(int argc, char** argv) {
    ...
    // resize reference
    cv::Mat img, out_img, result_ocv, diff;
    ...
    diff.create(cv::Size(NEWWIDTH, NEWHEIGHT), CV_8UC3);
#if INTERPOLATION == 0
    cv::resize(refOutput0, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_NEAREST);
#endif
#if INTERPOLATION == 1
    cv::resize(refOutput0, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_LINEAR);
#endif
#if INTERPOLATION == 2
    cv::resize(refOutput0, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_AREA);
#endif
    cl_int err;
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "resize_nv122rgb", &err));
    ...
    OCL_CHECK(err, cl::Buffer imageFromDevice(context, CL_MEM_WRITE_ONLY, (NEWWIDTH * NEWHEIGHT * 3), NULL, &err));

    // Set the kernel arguments
    OCL_CHECK(err, err = krnl.setArg(0, imageToDevice));
    OCL_CHECK(err, err = krnl.setArg(1, imageToDevice));
    ...
    /////////////////////////////////////// end of CL
    //////////////////////////////////////////
    cv::imwrite("hls.jpg", out_img);    // hls image
    cv::imwrite("ocv.jpg", result_ocv); // reference image
    cv::absdiff(result_ocv, out_img, diff);
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "resize_yuyv2rgb", &err));
    ...
    diff.create(cv::Size(NEWWIDTH, NEWHEIGHT), CV_8UC1);
#if INTERPOLATION == 0
    cv::resize(in_img, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_NEAREST);
#endif
#if INTERPOLATION == 1
    cv::resize(in_img, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_LINEAR);
#endif
#if INTERPOLATION == 2
    cv::resize(in_img, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_AREA);
#endif
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "resize_yuv400", &err));

    // Allocate the buffers:
    std::vector<cl::Memory> inBufVec, outBufVec;
    OCL_CHECK(err, cl::Buffer imageToDevice(context, CL_MEM_READ_ONLY, (in_img.rows * in_img.cols), NULL, &err));
    OCL_CHECK(err, cl::Buffer imageFromDevice(context, CL_MEM_WRITE_ONLY, (NEWWIDTH * NEWHEIGHT), NULL, &err));
    ...
    diff.create(cv::Size(NEWWIDTH, NEWHEIGHT), CV_8UC3);
#if INTERPOLATION == 0
    cv::resize(refmat, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_NEAREST);
#endif
#if INTERPOLATION == 1
    cv::resize(refmat, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_LINEAR);
#endif
#if INTERPOLATION == 2
    cv::resize(refmat, result_ocv, cv::Size(NEWWIDTH, NEWHEIGHT), 0, 0, cv::INTER_AREA);
#endif
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel krnl(program, "resize_yuv4442rgb", &err));
