...
#pragma HLS DATAFLOW
    // clang-format on
    xf::cv::Array2xfMat<INPUT_PTR_WIDTH, IN_TYPE, HEIGHT, WIDTH, NPC, XF_CV_DEPTH_IN>(img_inp, imgInput,
                                                                                      in_img_linestride);
#if BGR2RGB
    xf::cv::bgr2rgb<IN_TYPE, OUT_TYPE, HEIGHT, WIDTH, NPC, XF_CV_DEPTH_IN, XF_CV_DEPTH_CH_SWAP>(imgInput, ch_swap_mat);
    xf::cv::resize<INTERPOLATION, IN_TYPE, HEIGHT, WIDTH, NEWHEIGHT, NEWWIDTH, NPC, XF_CV_DEPTH_CH_SWAP,
                   XF_CV_DEPTH_RESIZE_OUT, MAXDOWNSCALE>(ch_swap_mat, resize_out_mat);
#else

    xf::cv::resize<INTERPOLATION, IN_TYPE, HEIGHT, WIDTH, NEWHEIGHT, NEWWIDTH, NPC, XF_CV_DEPTH_IN,
                   XF_CV_DEPTH_RESIZE_OUT, MAXDOWNSCALE>(imgInput, resize_out_mat);
#endif

#if CROP
    xf::cv::crop<OUT_TYPE, NEWHEIGHT, NEWWIDTH, 0, NPC, XF_CV_DEPTH_RESIZE_OUT, XF_CV_DEPTH_CROP>(resize_out_mat,
                                                                                                  crop_mat, roi);
    xf::cv::preProcess<IN_TYPE, OUT_TYPE, NEWHEIGHT, NEWWIDTH, NPC, XF_CV_DEPTH_CROP, XF_CV_DEPTH_OUT, WIDTH_A, IBITS_A,
                       WIDTH_B, IBITS_B, WIDTH_OUT, IBITS_OUT>(crop_mat, out_mat, params);
#else

    xf::cv::preProcess<IN_TYPE, OUT_TYPE, NEWHEIGHT, NEWWIDTH, NPC, XF_CV_DEPTH_RESIZE_OUT, XF_CV_DEPTH_OUT, WIDTH_A,
                       IBITS_A, WIDTH_B, IBITS_B, WIDTH_OUT, IBITS_OUT>(resize_out_mat, out_mat, params);

#endif
    xf::cv::xfMat2Array<OUTPUT_PTR_WIDTH, OUT_TYPE, NEWHEIGHT, NEWWIDTH, NPC, XF_CV_DEPTH_OUT>(out_mat, img_out,
                                                                                               out_img_linestride);
}
}
