...
#pragma HLS DATAFLOW
    ...
    if (WB_TYPE == 1) {
        xf::cv::AWBhistogram<IN_TYPE, IN_TYPE, HEIGHT, WIDTH, NPC1, XF_CV_DEPTH_IN_1, XF_CV_DEPTH_IN_2, 1>(
            in_mat, impop, hist0, thresh, inputMin, inputMax, outputMin, outputMax);
        xf::cv::AWBNormalization<IN_TYPE, IN_TYPE, HEIGHT, WIDTH, NPC1, XF_CV_DEPTH_IN_2, XF_CV_DEPTH_OUT_1, 1>(
            impop, out_mat, hist1, thresh, inputMin, inputMax, outputMin, outputMax);
    } else {
        xf::cv::AWBChannelGain<IN_TYPE, IN_TYPE, HEIGHT, WIDTH, NPC1, XF_CV_DEPTH_IN_1, XF_CV_DEPTH_IN_2, 0>(
            in_mat, impop, thresh, gain0);
        xf::cv::AWBGainUpdate<IN_TYPE, IN_TYPE, HEIGHT, WIDTH, NPC1, XF_CV_DEPTH_IN_2, XF_CV_DEPTH_OUT_1, 0>(
