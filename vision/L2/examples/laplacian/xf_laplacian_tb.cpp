...
#include "xcl2.hpp"
#include "xf_laplacian_tb_config.h"
int main(int argc, char** argv) {
    ...
    cv::Mat kx, ky, k1x, k1y;
    cv::getDerivKernels(kx, ky, 2, 0, FILTER_HEIGHT, false, CV_32F);

    cv::getDerivKernels(k1x, k1y, 0, 2, FILTER_WIDTH, false, CV_32F);

    filter = (kx * ky.t()) + (k1x * k1y.t());

    std::cout << "Filter Coefficients = " << std::endl << " " << filter << std::endl << std::endl;
...
/////////////////    OpenCV reference   /////////////////
...
#if OUT_8U
    cv::Laplacian(in_img, ocv_ref, CV_8U, FILTER_HEIGHT, 1, 0, cv::BORDER_CONSTANT);
#elif OUT_16S
    cv::Laplacian(in_img, ocv_ref, CV_16S, FILTER_HEIGHT, 1, 0, cv::BORDER_CONSTANT);
#endif
    ...
    for (int i = 0; i < FILTER_HEIGHT; i++) {
        for (int j = 0; j < FILTER_WIDTH; j++) {
            short int filter_coeff_fxp = (short int)filter.at<float>(i, j) * (1 << SHIFT);
            filter_vec[i * FILTER_WIDTH + j] = filter_coeff_fxp;
        }
    }
    ...
    std::string binaryFile = xcl::find_binary_file(device_name, "krnl_laplacian");
    ...
    // Create a kernel:
    OCL_CHECK(err, cl::Kernel kernel(program, "laplacian", &err));
