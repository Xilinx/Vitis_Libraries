...
int main(int argc, char** argv) {
    ...
    (void)cl_kernel_mgr::registerKernel("cca_custom_accel", "krnl_ccacustom", XCLIN(post_blur_threshold_binary_image),
                                        XCLIN(post_blur_threshold_binary_image), XCLIN(tmp_out_data1, height * width),
                                        XCLIN(tmp_out_data2, height * width), XCLOUT(out_img), XCLOUT(&obj_pix, 4),
                                        XCLOUT(&def_pix, 4), XCLIN(height), XCLIN(width));
