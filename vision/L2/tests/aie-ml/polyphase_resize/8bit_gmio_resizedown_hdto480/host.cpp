...
int main(int argc, char** argv) {
    try {
        ...
        // 1stpass
        cv::resize(srcImageR, srcImageRresize, cv::Size(op_width, op_height), 0, 0, cv::INTER_LANCZOS4);
        ...
        // 2ndpass
        cv::resize(resize_1DV_trans, resize_2ndpass, cv::Size(op_width2, op_height2), 0, 0, cv::INTER_LANCZOS4);
        ...
        // resize 2nd pass
        {
            ...
            for (int i = 0; i < iterations; i++) {
                //@{
                std::cout << "Iteration : " << (i + 1) << std::endl;
                auto tiles_sz2 = tiler2.host2aie_nb(
                    srcData2.data(), dst.size(), {"resize2[0].in1" /*, "resize2[1].in1", "resize2[2].in1"*/}, params2);
