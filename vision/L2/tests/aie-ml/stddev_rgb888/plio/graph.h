...
#ifndef ADF_GRAPH_STDDEV_RGB888_H
#define ADF_GRAPH_STDDEV_RGB888_H
...
class stddevGraph : public adf::graph {
   private:
   ...
   public:
    input_plio in1;
    output_plio out1;
    port<input> reset[NO_CORES_PER_COL];
    port<input> m_r[NO_CORES_PER_COL];
    port<input> m_g[NO_CORES_PER_COL];
    port<input> m_b[NO_CORES_PER_COL];

    shared_buffer<uint8_t> mtx_out, mtx_in;

    stddevGraph(int tile_col, int tile_row, int CORE_IDX) {
        int mt_col = tile_col;
        ...
        for (int i = 0; i < NO_CORES_PER_COL; i++) {
            k[i] = kernel::create_object<StddevRunner>();
            ...
            source(k[i]) = "xf_stddev_rgb888.cc";
            location<kernel>(k[i]) = tile(tile_col, i);
            runtime<ratio>(k[i]) = 1.0;

            connect<parameter>(reset[i], async(k[i].in[1]));
            connect<parameter>(m_r[i], async(k[i].in[2]));
            connect<parameter>(m_g[i], async(k[i].in[3]));
            connect<parameter>(m_b[i], async(k[i].in[4]));
        }
