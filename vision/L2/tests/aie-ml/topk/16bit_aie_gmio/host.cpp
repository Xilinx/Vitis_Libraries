...
int main(int argc, char** argv) {
    try {
        ...
        cv::Mat src(srcImageR.rows, srcImageR.cols, CV_16UC1, (void*)srcData.data());

        std::vector<uint16_t> dstData;
        dstData.assign(op_height * op_width, 0);
        cv::Mat dst1(op_height, op_width, CV_16UC1, (void*)dstData.data());

        std::vector<uint16_t> dstData2;
        dstData2.assign(op_height * op_width, 0);
        cv::Mat dst2(op_height, op_width, CV_16UC1, (void*)dstData2.data());

        xF::xfcvDataMovers<xF::TILER, uint16_t, TILE_HEIGHT, TILE_WIDTH_IN, 16, 1, 0, true> tiler(0, 0);
        xF::xfcvDataMovers<xF::STITCHER, uint16_t, TILE_HEIGHT, TILE_WIDTH_OUT, 16, 1, 0, true> stitcher1;
        xF::xfcvDataMovers<xF::STITCHER, uint16_t, TILE_HEIGHT, TILE_WIDTH_OUT, 16, 1, 0, true> stitcher2;
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            START_TIMER
            auto tiles_sz = tiler.host2aie_nb(srcData.data(), srcImageR.size(), {"topk.in1"});
            ...
            stitcher1.aie2host_nb(dstData.data(), dst1.size(), tiles_sz, {"topk.out1"});
            stitcher2.aie2host_nb(dstData2.data(), dst2.size(), tiles_sz, {"topk.out2"});

            tiler.wait({"topk.in1"});
#if !__X86__
            gHndl.wait();
#endif
            stitcher1.wait({"topk.out1"});
            stitcher2.wait({"topk.out2"});
