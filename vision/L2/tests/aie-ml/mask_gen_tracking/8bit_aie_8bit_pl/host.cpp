 ...
 ******************************************************************************
 */
void opencv_ref(cv::Mat srcPd, cv::Mat srcPs, cv::Mat dst, uint8_t fg_hold, uint8_t bg_hold, uint8_t depth_min,
               uint8_t depth_max, uint8_t fg_thresh_track_r, uint8_t bg_thresh_track_r, uint8_t pred_seg_thresh ) {
    ...
    maskgen_track_param_ref(inputDataPredDepth, inputDataPredSeg, (uint8)depth_min, (uint8)depth_max, (uint8)fg_hold, (uint8)bg_hold,
                            (uint8)pred_seg_thresh, (int)srcPd.rows, (int)srcPd.cols, depth_or_ref, sum_r,
    ...
    maskgen_ref(inputDataPredDepth, maskRef, (bool)MASKGEN_TRACKING, depth_min,  depth_max, fg_hold,
                bg_hold, fg_thresh_track_r, bg_thresh_track_r, (int)srcPd.rows, (int)srcPd.cols);
...
int main(int argc, char** argv) {
    try {
        if (argc < 3) {
            ...
            errorMessage << argv[0] << " <xclbin> <input Pred Depth Image> <input Pred Seg Image> [width] [height] [iterations]";
	    ...
	    xrt::bo *ptr_dstHndl_track = new xrt::bo(xF::gpDhdl, (op_height_track * op_width_track * srcImagePd.elemSize()),0,0 );
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            non_zero_count = dst_track.data[0] | (dst_track.data[1] << 8) | (dst_track.data[2] << 16) | (dst_track.data[3] << 24); 
