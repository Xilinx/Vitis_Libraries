...
int main(int argc, char** argv) {
    try {
        if (argc < 3) {
            ...
            errorMessage << argv[0] << " <xclbin> <input Pred Depth Image> <input "
                                       "Pred Seg Image> [width] [height] "
                                       "[iterations]";
        ...
        // Load image
        std::vector<uint8_t> srcDataPd;
        srcDataPd.assign(srcImagePd.data, (srcImagePd.data + srcImagePd.total()));
        cv::Mat srcPd(srcImagePd.rows, srcImagePd.cols, CV_8UC1, (void*)srcDataPd.data());

        std::vector<uint8_t> srcDataPs;
        srcDataPs.assign(srcImagePs.data, (srcImagePs.data + srcImagePs.total()));
        cv::Mat srcPs(srcImagePs.rows, srcImagePs.cols, CV_8UC1, (void*)srcDataPs.data());

        std::vector<uint8_t> dstDataTrack;
        dstDataTrack.assign(op_height_track * op_width_track * srcImagePd.elemSize(), 0);
        std::cout << op_height_track << " " << op_width_track << std::endl;
        cv::Mat dst_track(op_height_track, op_width_track, CV_8UC1, (void*)dstDataTrack.data());

        std::vector<uint8_t> dstData;
        dstData.assign(op_height * op_width * srcImagePd.elemSize(), 0);
        cv::Mat dst(op_height, op_width, CV_8UC1, (void*)dstData.data());

        xF::xfcvDataMovers<xF::TILER, uint8_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16, 1, 0, true> tiler1(0, 0);
        xF::xfcvDataMovers<xF::TILER, uint8_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16, 1, 0, true> tiler2(0, 0);
        xF::xfcvDataMovers<xF::STITCHER, uint8_t, TILE_HEIGHT_OUT, TILE_WIDTH_OUT, 16, 1, 0, true> stitcher1;

        xF::xfcvDataMovers<xF::TILER, uint8_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16, 1, 0, true> tiler3(0, 0);
        xF::xfcvDataMovers<xF::STITCHER, uint8_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16, 1, 0, true> stitcher2;
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            //@{
            START_TIMER
            auto tiles_sz1 = tiler1.host2aie_nb(srcDataPd.data(), srcImagePd.size(), {"maskGenTrack.in1"});
            tiler2.host2aie_nb(srcDataPs.data(), srcImagePs.size(), {"maskGenTrack.in2"});

#if !__X86_DEVICE__
            std::cout << "Graph running for " << (tiles_sz1[0] * tiles_sz1[1]) << " iterations.\n";
            gHndl1.run(tiles_sz1[0] * tiles_sz1[1]);
#endif
            stitcher1.aie2host_nb(dstDataTrack.data(), dst_track.size(), tiles_sz1, {"maskGenTrack.out1"});
            tiler1.wait({"maskGenTrack.in1"});
            tiler2.wait({"maskGenTrack.in2"});
#if !__X86_DEVICE__
            gHndl1.wait();
#endif
            stitcher1.wait({"maskGenTrack.out1"});
            ...
            START_TIMER
            auto tiles_sz2 = tiler3.host2aie_nb(srcDataPd.data(), srcImagePd.size(), {"maskGen.in1"});

#if !__X86_DEVICE__
            std::cout << "Graph running for " << (tiles_sz2[0] * tiles_sz2[1]) << " iterations.\n";
            gHndl2.run(tiles_sz2[0] * tiles_sz2[1]);
#endif
            stitcher2.aie2host_nb(dstData.data(), dst.size(), tiles_sz2, {"maskGen.out1"});
            tiler3.wait({"maskGen.in1"});
#if !__X86_DEVICE__
            gHndl2.wait();
#endif
            stitcher2.wait({"maskGen.out1"});
