...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            START_TIMER
            auto tiles_sz = tiler.host2aie_nb(srcData.data(), srcImageR.size(), {"maskGen.in1"});
#if !__X86_DEVICE__
            std::cout << "Graph running for " << (tiles_sz[0] * tiles_sz[1]) << " iterations.\n";
            gHndl.run(tiles_sz[0] * tiles_sz[1]);
#endif
            stitcher.aie2host_nb(dstData.data(), dst.size(), tiles_sz, {"maskGen.out1"});
            tiler.wait({"maskGen.in1"});
#if !__X86_DEVICE__
            gHndl.wait();
#endif
            stitcher.wait({"maskGen.out1"});
