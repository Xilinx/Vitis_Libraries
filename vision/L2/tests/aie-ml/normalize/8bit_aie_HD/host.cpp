    ...
    for (int i = 0; i < (input.rows); i++) {
        for (int j = 0; j < (input.cols); j++) {
            for (int k = 0; k < input.channels(); k++) {
                ...
                unsigned char x = input.at<cv::Vec<signed char, 3> >(i, j)[k];
                ...
                output.at<cv::Vec<signed char, 3> >(i, j)[k] = out_Q8p0;
            }
        }
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        cv::Mat dst(op_height, op_width, CV_8SC3, dstData);
        ...
        for (int ii = 0; ii < dst.rows; ii++) {
            for (int jj = 0; jj < dst.cols; jj++) {
                for (int kk = 0; kk < dst.channels(); kk++) {
                    ref = dstRefImage.at<cv::Vec<signed char, 3> >(ii, jj)[kk];
                    aie = dst.at<cv::Vec<signed char, 3> >(ii, jj)[kk];
