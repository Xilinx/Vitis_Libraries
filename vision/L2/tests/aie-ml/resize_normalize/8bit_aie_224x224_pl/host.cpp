...
int main(int argc, char** argv) {
    try {
        ...
        normalization_CRefImpl<0, 4, 0>(srcImageRresize, dstRefImage, alpha, beta);
...
#if !__X86_DEVICE__
        auto gHndl = xrt::graph(xF::gpDhdl, xF::xclbin_uuid, "resize_norm");
        std::cout << "XRT graph opened" << std::endl;
        gHndl.reset();
        std::cout << "Graph reset done" << std::endl;
        gHndl.update("resize_norm.k.in[1]", scale_x_fix);
        gHndl.update("resize_norm.k.in[2]", scale_y_fix);
        gHndl.update("resize_norm.k.in[3]", coeff);
#endif
        START_TIMER
