...
int main(int argc, char** argv) {
    try {
        ...
        cv::Mat image_yuv;
        cv::cvtColor(srcImageRGB, image_yuv,
                     cv::COLOR_BGR2YUV); // Converts to YUV format (with default 420, which we will expand to 444)

        // Extract the Y, U, and V channels from the YUV image
        cv::Mat y_channel, u_channel, v_channel;
        cv::Mat alpha_channel(srcImageRGB.size(), CV_8UC1, cv::Scalar(255)); // Fully opaque alpha channel

        cv::extractChannel(image_yuv, y_channel, 0); // Extract Y channel
        cv::extractChannel(image_yuv, u_channel, 1); // Extract U channel
        cv::extractChannel(image_yuv, v_channel, 2); // Extract V channel

        // Resize U and V channels to match the resolution of Y channel (YUV444 means all channels should have the same
        // resolution)
        cv::resize(u_channel, u_channel, srcImageRGB.size(), 0, 0, cv::INTER_LINEAR);
        cv::resize(v_channel, v_channel, srcImageRGB.size(), 0, 0, cv::INTER_LINEAR);

        // PL RGB2RGBA
        std::vector<cv::Mat> YUVA_resize_in_temp = {y_channel, u_channel, v_channel};
        cv::Mat YUVA_resize_in;
        cv::merge(YUVA_resize_in_temp, YUVA_resize_in);

        std::cout << "Image size of YUVA_resize_in : " << YUVA_resize_in.cols << " x " << YUVA_resize_in.rows << " x "
                  << YUVA_resize_in.channels() << " " << std::endl;

        //////////////////////////////////////////
        // Run opencv reference test (RESIZE YUYV)
        //////////////////////////////////////////
        int width_out = IMAGE_WIDTH_OUT;
        int height_out = IMAGE_HEIGHT_OUT;

        // Separate the planes
        cv::Mat Y = y_channel;
        cv::Mat U = u_channel;
        cv::Mat V = v_channel;
        cv::Mat A = cv::Mat::ones(height_out, width_out, CV_8UC1) * 255;

        // resize each plane
        cv::resize(Y, Y, cv::Size(width_out, height_out));
        cv::resize(U, U, cv::Size(width_out, height_out));
        cv::resize(V, V, cv::Size(width_out, height_out));

        std::cout << "Image size of resize Y : " << Y.cols << " x " << Y.rows << " x " << Y.channels() << " "
                  << std::endl;
        std::cout << "Image size of resize U : " << U.cols << " x " << U.rows << " x " << U.channels() << " "
                  << std::endl;
        std::cout << "Image size of resize V : " << V.cols << " x " << V.rows << " x " << V.channels() << " "
                  << std::endl;
        std::cout << "Image size of resize A : " << A.cols << " x " << A.rows << " x " << A.channels() << " "
                  << std::endl;

        // Merge the Y, U, V, and A channels into a single YUVA444 image
        std::vector<cv::Mat> YUVA_resize_out_temp = {Y, U, V};
        cv::Mat YUVA_resize_out;
        cv::merge(YUVA_resize_out_temp, YUVA_resize_out);

        cv::imwrite("./out/yuva444_resize_image.png", YUVA_resize_out);
        std::cout << "Image size of YUVA_resize_out : " << YUVA_resize_out.cols << " x " << YUVA_resize_out.rows
                  << " x " << YUVA_resize_out.channels() << " " << std::endl;

        //////////////////////////////////////////
        // Run AIE
        //////////////////////////////////////////

        cv::Mat srcImage = YUVA_resize_in;
        ...
        cv::imwrite("YUVA_resize_out.png", YUVA_resize_out);
        ...
        cv::Mat absDiffImage;
        cv::absdiff(YUVA_resize_out, dst, absDiffImage);
