...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            START_TIMER
            auto tiles_sz = tiler.host2aie_nb(srcData.data(), srcImageR.size(), {"demo[0].in1[0]"});
#if !__X86_DEVICE__

            for (int i = 0; i < CORES; i++) {
                std::cout << "Graph run(" << (tiles_sz[0] * tiles_sz[1]) << ")\n";
                gHndl[i].run(tiles_sz[0] * tiles_sz[1]);
            }

#endif
            stitcher.aie2host_nb(dstData.data(), cv::Size((srcImageR.cols << 2), srcImageR.rows), tiles_sz,
                                 {"demo[0].out1[0]"});
            tiler.wait({"demo[0].in1[0]"});
#if !__X86_DEVICE__
            for (int i = 0; i < CORES; i++) {
                // gHndl.wait();
                gHndl[i].wait();
