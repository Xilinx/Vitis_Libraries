...
int main(int argc, char** argv) {
    try {
        ...
        //////////////////////////////////////////
        cv::Mat dstRefImage(height, width, CV_8UC3);
        denormalization_CRefImpl<7, 8, 5>(srcImageR, dstRefImage, alpha, beta);
        cv::resize(dstRefImage, dstRefImage, cv::Size(op_width, op_height));
        ...
        xrt::bo* dst_hndl = new xrt::bo(xF::gpDhdl, (op_height * op_width * srcImageR.elemSize()), 0, 0);
        ...
        xF::xfcvDataMovers<xF::TILER, uint8_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16> tiler(0, 0, true, 4);
...
#if !__X86_DEVICE__
        auto gHndl = xrt::graph(xF::gpDhdl, xF::xclbin_uuid, "denorm_resize");
        std::cout << "XRT graph opened" << std::endl;
        gHndl.reset();
        std::cout << "Graph reset done" << std::endl;
        gHndl.update("denorm_resize.k.in[1]", scale_x_fix);
        gHndl.update("denorm_resize.k.in[2]", scale_y_fix);
        gHndl.update("denorm_resize.k.in[3]", coeff);
#endif
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            STOP_TIMER("denormalize resize function")
