...
int main(int argc, char** argv) {
    try {
        ...
        cv::Mat opencvHLS;
        cv::cvtColor(srcImageRGB, opencvHLS, cv::COLOR_RGB2HLS);
        // Convert OpenCV HLS to displayable format
        cv::Mat opencvHLSDisplay;
        opencvHLS.convertTo(opencvHLSDisplay, CV_8UC3);
        ...
        for (int k = 0; k < NO_COLS; k++) {
            std::string graph_name = "rgb2hls[" + std::to_string(k) + "]";
        ...
        std::cout << "Image size of dst : " << dst.cols << " x " << dst.rows << " x " << dst.channels() << " "
                  << std::endl;
        cv::imwrite("aieHLS.png", dst);
        cv::imwrite("opencvHLS.png", opencvHLS);
        ...
        findMaxDifference_hist_table(opencvHLSDisplay, dst);
