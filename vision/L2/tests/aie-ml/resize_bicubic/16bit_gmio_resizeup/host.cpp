...
int main(int argc, char** argv) {
    try {
        ...
        // cv::Mat dst(op_width, op_height, srcImageR.type(), (void*)dstData.data());
        // cv::Mat dst(op_height, op_width, dstRefImage.type(), dstData);
        cv::Mat dst(op_width, op_height, CV_16UC4);
        std::cout << "dst.rows=" << dst.rows << "dst.cols" << dst.cols << dst.size() << dst.elemSize() << std::endl;
        ...
        // resize 1st pass
        {
            ...
            // srcData.assign(reinterpret_cast<uint16_t*> (srcImageR.data), (reinterpret_cast<uint16_t*>
            // (srcImageR.data) + srcImageR.total() * srcImageR.channels()));
            xF::xfcvDataMoverParams params(srcImageR.size(), cv::Size(op_width, op_height));
            xF::xfcvDataMovers<xF::TILER, uint16_t, TILE_HEIGHT_IN, TILE_WIDTH_IN, 8, NO_INSTANCES, 0, true> tiler(
                0, 0, srcImageR.channels());
            xF::xfcvDataMovers<xF::STITCHER, uint16_t, TILE_WIDTH_OUT, TILE_HEIGHT_OUT, 8, NO_INSTANCES, 0, true>
            ...
            for (int i = 0; i < iterations; i++) {
                ...
                // auto tiles_sz = tiler.host2aie_nb(srcData.data(), srcImageR.size(), {"resize[0].in1",
                // "resize[1].in1", "resize[2].in1","resize[3].in1"}, params, grp_id);
                auto tiles_sz = tiler.host2aie_nb(srcImageR, {"resize[0].in1", "resize[1].in1", "resize[2].in1",
                                                              "resize[3].in1" /* , "resize[4].in1", "resize[5].in1" */},
                                                  params);
                stitcher.aie2host_nb(dst, tiles_sz, {"resize[0].out1", "resize[1].out1", "resize[2].out1",
                                                     "resize[3].out1" /* ,"resize[4].out1", "resize[5].out1" */});
#if !__X86_DEVICE__
...
#endif
                tiler.wait({"resize[0].in1", "resize[1].in1", "resize[2].in1", "resize[3].in1",
                            /* "resize[4].in1", "resize[5].in1" */});
...

        {
            ...
            // Allocate input buffer
            // std::vector<uint16_t> srcData2;
            // srcData2.assign(reinterpret_cast<uint16_t*>(dst.data), (reinterpret_cast<uint16_t*>(dst.data) +
            // dst.total() * dst.channels()));
            // std::cout << "srcData2.size()=" << srcData2.size() << std::endl;
            // Allocate output buffer
            /*         std::vector<uint16_t> dstData2;
                    dstData2.assign(op_height2 * op_width2 * dst.elemSize(), 0);
                    cv::Mat dst2(op_width2, op_height2, srcImageR.type(), (void*)dstData2.data()); */
            cv::Mat dst2(op_width2, op_height2, CV_16UC4); //, (void*)dstData2.data());
            // cv::Mat dst(op_height, op_width, dstRefImage.type(), dstData);
            // std::cout << "dstData2.size()=" << dstData2.size() << std::endl;
            ...
            xF::xfcvDataMovers<xF::TILER, DATA_TYPE, TILE_HEIGHT_IN2, TILE_WIDTH_IN2, 8, NO_INSTANCES, 0, true> tiler2(
                0, 0, dst.channels());
            xF::xfcvDataMovers<xF::STITCHER, DATA_TYPE, TILE_WIDTH_OUT2, TILE_HEIGHT_OUT2, 8, NO_INSTANCES, 0, true>
            ...
            for (int i = 0; i < iterations; i++) {
                //@{
                std::cout << "Iteration : " << (i + 1) << std::endl;
                //            auto tiles_sz2 = tiler2.host2aie_nb(srcData2.data(), dst.size(), {"resize2[0].in1",
                //            "resize2[1].in1", "resize2[2].in1", "resize2[3].in1"}, params2, grp_id1);
                //            stitcher2.aie2host_nb(dstData2.data(), dst2.size(), tiles_sz2, {"resize2[0].out1",
                //            "resize2[1].out1","resize2[2].out1", "resize2[3].out1"}, grp_id2);
                auto tiles_sz2 = tiler2.host2aie_nb(dst, {"resize2[0].in1", "resize2[1].in1", "resize2[2].in1",
                                                          "resize2[3].in1" /* ,"resize2[4].in1","resize2[5].in1" */},
                                                    params2, grp_id1);
                stitcher2.aie2host_nb(dst2, tiles_sz2, {"resize2[0].out1", "resize2[1].out1", "resize2[2].out1",
                                                        "resize2[3].out1" /* , "resize2[4].out1", "resize2[5].out1" */},
                                      grp_id2);
...
#endif
            tiler2.free_buffer();
            stitcher2.free_buffer();
            /*        std::cout<<"freed the tiler and stitcher2 obj"<<std::endl;
                    //////////////////////////////////////////
            ...
            //        cv::Mat dstRefImage = resize_1DV_trans;
                     std::cout<<"after 1st pass ref"<<std::endl;
                    //2ndpass
            ...
            //        cv::Mat dstRefImage2 = resize2_1DV_trans;
                    std::cout<<"after 2nd pass ref"<<std::endl;
            */
            // Analyze output {
            // std::cout << "Analyzing diff\n";
