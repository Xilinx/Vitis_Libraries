...
int run_opencv_ref(cv::Mat& srcImage1, cv::Mat& srcImage2, cv::Mat& dstRefImage, float alpha, float beta, float gamma) {
    dstRefImage.create(srcImage1.rows, srcImage1.cols, CV_8UC1);
    cv::addWeighted(srcImage1, alpha, srcImage2, beta, gamma, dstRefImage, -1);
    return 0;
}
...
int main(int argc, char** argv) {
    try {
        if (argc != 7) {
            ...
            errorMessage << argv[0] << " <xclbin> <inputImage1> <inputImage2> <alpha> <beta> <gamma>";
        ...
        //////////////////////////////////////////
        // Run opencv reference test (addweighted design)
        //////////////////////////////////////////
        cv::Mat dstRefImage;
        float alpha = atof(argv[4]);
        float beta = atof(argv[5]);
        float gamma = atof(argv[6]);
        run_opencv_ref(srcImage1, srcImage2, dstRefImage, alpha, beta, gamma);
        ...
        //@{
        START_TIMER
        auto tiles_sz = tiler1.host2aie_nb(srcData1.data(), srcImage1.size(), {"mygraph.in1"});
        tiler2.host2aie_nb(srcData2.data(), srcImage2.size(), {"mygraph.in2"});
