...
int sizeout = (TILE_WINDOW_SIZE_OUT * IMAGE_HEIGHT_OUT) / sizeof(int8_t);
...

{
    try {
        ...
        for (int j = 0; j < IMAGE_HEIGHT_OUT; j++) {
            int8_t* outp = (int8_t*)xf::cv::aie::xfGetImgDataPtr(bufferMapped[1] + (j * TILE_WINDOW_SIZE_OUT));
            int8_t* refp = (int8_t*)xf::cv::aie::xfGetImgDataPtr(golden + (j * TILE_WINDOW_SIZE_OUT));
            for (int i = 0; i < TILE_ELEMENTS_OUT; i++) {
                int8_t aie_out = outp[i];
                int8_t aie_Ref = refp[i];

                int diff = abs(int(aie_out - aie_Ref));
                if (errorthreshold < diff) {
                    printf("Error found @ %d %d, %d != %d\n", j, i, (int8_t)outp[i], (int8_t)refp[i]);
                    errorCount++;
                }
            }
        }
