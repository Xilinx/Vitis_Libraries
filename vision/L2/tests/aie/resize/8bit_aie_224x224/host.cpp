...
int sizein = (TILE_WINDOW_SIZE_IN * IMAGE_HEIGHT_OUT) / sizeof(uint8_t);
int sizeout = (TILE_WINDOW_SIZE_OUT * IMAGE_HEIGHT_OUT) / sizeof(uint8_t);
...
int data_buffer_init(xrtDeviceHandle& dhdl,
                     ...
                     int sizeIn,
                     int sizeOut,
                     std::vector<uint8_t*>& bufferMapped) {
    ...
    xrtBufferHandle in_bohdl = xrtBOAlloc(dhdl, sizeIn * sizeof(uint8_t), 0, 0);
    auto in_bomapped = reinterpret_cast<uint8_t*>(xrtBOMap(in_bohdl));
    memcpy(in_bomapped, imgBuffer, sizeIn * sizeof(uint8_t));
    ...
    xrtBufferHandle out_bohdl = xrtBOAlloc(dhdl, sizeOut * sizeof(uint8_t), 0, 0);
    auto out_bomapped = reinterpret_cast<uint8_t*>(xrtBOMap(out_bohdl));
    memset(out_bomapped, 0xABCDEF00, sizeOut * sizeof(uint8_t));
...

{
    try {
        ...
        //////////////////////////////////////////
        data_buffer_init(dhdl, bufferHandles, input_data, sizein, sizeout, bufferMapped);

        run_test(dhdl, top, bufferHandles);

        printf("verfifying the results\n");

        int errorCount = 0, errorthreshold = 6;

        for (int j = 0; j < IMAGE_HEIGHT_OUT; j++) {
            uint8_t* outp = (uint8_t*)xf::cv::aie::xfGetImgDataPtr(bufferMapped[1] + (j * TILE_WINDOW_SIZE_OUT));
            uint8_t* refp = (uint8_t*)xf::cv::aie::xfGetImgDataPtr(golden + (j * TILE_WINDOW_SIZE_OUT));
            for (int i = 0; i < TILE_ELEMENTS_OUT; i++) {
                uint8_t aie_out = outp[i];
                uint8_t aie_Ref = refp[i];
                int diff = abs(int(aie_out - aie_Ref));
                if (errorthreshold < diff) {
                    printf("Error found @ %d %d, %d != %d\n", j, i, (uint8_t)outp[i], refp[i]);
                    errorCount++;
                }
            }
        }
