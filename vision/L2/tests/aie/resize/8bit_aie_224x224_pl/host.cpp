...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << srcImageR.rows << std::endl << std::flush;
        std::cout << srcImageR.cols << std::endl << std::flush;
        std::cout << srcImageR.elemSize() << std::endl << std::flush;
        std::cout << "Image size (end)" << std::endl << std::flush;
        ...
        //////////////////////////////////////////
        cv::Mat dstRefImage;
        cv::resize(srcImageR, dstRefImage, cv::Size(op_height, op_width));
        // Initializa device
        ...
        xF::xfcvDataMovers<xF::TILER, char, TILE_HEIGHT_IN, TILE_WIDTH_IN, 16> tiler(0, 0, true);
        xF::xfcvDataMovers<xF::STITCHER, char, TILE_HEIGHT_OUT, TILE_WIDTH_OUT, 16> stitcher(true);
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            resize.run(tiles_sz[0] * tiles_sz[1]);
            resize.wait();
            tiler.wait();
            stitcher.wait();
