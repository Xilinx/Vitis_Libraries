...
int main(int argc, char** argv) {
    try {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            // std::cout << "\nTile Metadata: " << tileWidth << " " << tileHeight << " " << positionH << " " <<
            // positionV << " " << correctedTileWidth << " " << correctedTileHeight << " " << correctedPositionH << " "
            // << correctedPositionV << "\n" << std::endl;

            std::cout << "Graph run complete!\n";

            // p(dstData.data(), op_height, op_width*2);
            // p(dataRefOut, op_height, op_width*2);
