...
int sizein = (TILE_WINDOW_SIZE * NUM_TILES) / sizeof(int16_t);
 ...
 ******************************************************************************
 */
// int init_platform(xrtDeviceHandle &dhdl, const std::string& fnm, const axlf
// **top)
// int init_platform(xrtDeviceHandle &dhdl, const std::string& fnm, xuid_t
// &m_header_uuid)
...
int data_buffer_init(xrtDeviceHandle& dhdl,
    ...
    xrtBufferHandle out_bohdl_r = xrtBOAlloc(dhdl, sizeOut * sizeof(int16_t), 0, 0);
    auto out_bomapped_r = reinterpret_cast<uint16_t*>(xrtBOMap(out_bohdl_r));

    xrtBufferHandle out_bohdl_g = xrtBOAlloc(dhdl, sizeOut * sizeof(int16_t), 0, 0);
    auto out_bomapped_g = reinterpret_cast<uint16_t*>(xrtBOMap(out_bohdl_g));

    xrtBufferHandle out_bohdl_b = xrtBOAlloc(dhdl, sizeOut * sizeof(int16_t), 0, 0);
    auto out_bomapped_b = reinterpret_cast<uint16_t*>(xrtBOMap(out_bohdl_b));

    bufferHandles.push_back(in_bohdl);
    bufferHandles.push_back(out_bohdl_r);
    bufferHandles.push_back(out_bohdl_g);
    bufferHandles.push_back(out_bohdl_b);
    bufferMapped.push_back(in_bomapped);
    bufferMapped.push_back(out_bomapped_r);
    bufferMapped.push_back(out_bomapped_g);
    bufferMapped.push_back(out_bomapped_b);
...
int graph_run(xrtDeviceHandle& dhdl, const axlf* top, std::vector<xrtBufferHandle>& bufferHandles) {
    ...
    //////////////////////////////////////////
    printf(
        "graph init. This does nothing because CDO in boot PDI already "
        "configures AIE.\n");
    demo.init();
    ...
    xrtKernelHandle s2mm_khdl_r = xrtPLKernelOpen(dhdl, top->m_header.uuid, "s2mm:{s2mm_1}");
    xrtKernelHandle s2mm_khdl_g = xrtPLKernelOpen(dhdl, top->m_header.uuid, "s2mm:{s2mm_2}");
    xrtKernelHandle s2mm_khdl_b = xrtPLKernelOpen(dhdl, top->m_header.uuid, "s2mm:{s2mm_3}");

    xrtRunHandle mm2s_rhdl = dm_mm2s(mm2s_khdl, bufferHandles[0]);
    xrtRunHandle s2mm_rhdl_r = dm_s2mm(s2mm_khdl_r, bufferHandles[1]);
    xrtRunHandle s2mm_rhdl_g = dm_s2mm(s2mm_khdl_g, bufferHandles[2]);
    xrtRunHandle s2mm_rhdl_b = dm_s2mm(s2mm_khdl_b, bufferHandles[3]);

    printf("Enable datamovers\n");
    dm_run_once(mm2s_rhdl);
    dm_run_once(s2mm_rhdl_r);
    dm_run_once(s2mm_rhdl_g);
    dm_run_once(s2mm_rhdl_b);

    printf("Wait for datamovers to complete\n");
    dm_wait_done(mm2s_rhdl);
    dm_wait_done(s2mm_rhdl_r);
    dm_wait_done(s2mm_rhdl_g);
    dm_wait_done(s2mm_rhdl_b);
...
int main(int argc, char** argv) {
    try {
        ...
        //////////////////////////////////////////
        data_buffer_init(dhdl, bufferHandles, input_data, sizein, sizein, bufferMapped);
        ...
        for (int i = 0; i < NUM_TILES; i++) {
            int16_t* outp_r = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)bufferMapped[1]) + (i * TILE_WINDOW_SIZE));
            int16_t* outp_g = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)bufferMapped[2]) + (i * TILE_WINDOW_SIZE));
            int16_t* outp_b = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)bufferMapped[3]) + (i * TILE_WINDOW_SIZE));

            int16_t* refp_r = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)golden_r) + (i * TILE_WINDOW_SIZE));
            int16_t* refp_g = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)golden_g) + (i * TILE_WINDOW_SIZE));
            int16_t* refp_b = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)golden_b) + (i * TILE_WINDOW_SIZE));

            for (int j = 0; j < TILE_ELEMENTS; j++) {
                int16_t res = outp_r[j];
                int16_t res_g = refp_r[j];

                int diff = abs(int(res - res_g));
                if (diff > 1) {
                    printf("Red channel : Error found @ Tile %d, Loc %d, %d != %d\n", i, j, res, res_g);
                    errorCount++;
                }
                ...
                diff = abs(int(res - res_g));
                if (diff > 1) {
                    printf("Green channel : Error found @ Tile %d, Loc %d, %d != %d\n", i, j, res, res_g);
                    errorCount++;
                }
                ...
                diff = abs(int(res - res_g));
                if (diff > 1) {
                    printf("Blue channel : Error found @ Tile %d, Loc %d, %d != %d\n", i, j, res, res_g);
                    errorCount++;
                }
            }
        }
