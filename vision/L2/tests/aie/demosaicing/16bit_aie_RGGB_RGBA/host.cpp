...
int sizeout = (TILE_WINDOW_SIZE_RGBA * NUM_TILES) / sizeof(int16_t);
...
int main(int argc, char** argv) {
    try {
        ...
        //////////////////////////////////////////
        data_buffer_init(dhdl, bufferHandles, input_data, bufferMapped);
        ...
        for (int i = 0; i < NUM_TILES; i++) {
            int16_t* outp =
                (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)bufferMapped[1]) + (i * TILE_WINDOW_SIZE_RGBA));
            int16_t* refp = (int16_t*)xf::cv::aie::xfGetImgDataPtr(((char*)golden) + (i * TILE_WINDOW_SIZE_RGBA));

            for (int j = 0; j < TILE_ELEMENTS_RGBA; j++) {
                int16_t res = outp[j];
                int16_t res_g = refp[j];
