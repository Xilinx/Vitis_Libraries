...
int data_buffer_init(xrtDeviceHandle& dhdl,
    ...
    xrtBufferHandle in_bohdl = xrtBOAlloc(dhdl, sizeIn * sizeof(float), 0, 0);
    auto in_bomapped = reinterpret_cast<float*>(xrtBOMap(in_bohdl));
    memcpy(in_bomapped, imgBuffer, sizeIn * sizeof(float));
...

{
    try {
...

        {
            ...
            {
                if ((float)bufferMapped[1][i] != golden[i])

                {
                    printf("Error found @ %d, %d != %d	%d\n", i, (float)bufferMapped[1][i], golden[i],
                           (float)bufferMapped[0][i]);
                    errorCount++;
                }
            }
