...
int run_opencv_ref(cv::Mat& srcImageR, cv::Mat& dstRefImage) {
    dstRefImage = cv::Mat::zeros(srcImageR.size().height, srcImageR.size().width, srcImageR.type());
...
int main(int argc, char** argv) {
    try {
        ...
        std::vector<int16_t> dstData, dstRef;
        dstData.assign(op_height * op_width, 0);

        dstRef.assign(dstRefImage.data, (dstRefImage.data + dstRefImage.total()));
